...
# arg1: $sampleandaccessionlists the directory where the sample and accession lists are arg2: $results_dir the directory to which the end file is to be outputted arg3: $timepoint_name_list_BMonly the list of timepoints
...
sample_name_list_BMonly <- read.delim(paste(args[3], sep = ""), sep = "\t", stringsAsFactors = FALSE, header = FALSE)

print(sample_name_list_BMonly)

raw_comparison_table <- t(combn(sample_name_list_BMonly[, 1], 2))
...
list_of_timepoint_comparisons_final <- raw_comparison_table[grepl(x = raw_comparison_table[, 3], pattern = ".*ud.*")|grepl(x = raw_comparison_table[, 3], pattern = "(BM_MSC_to_AD).*(BM_MSC_to_AD)")|grepl(x = raw_comparison_table[, 3], pattern = "(BM_MSC_to_OB).*(BM_MSC_to_OB)")|(raw_comparison_table[, 3] == "BM_MSC_to_AD_14d_vs_BM_MSC_to_OB_14d"), 3]
