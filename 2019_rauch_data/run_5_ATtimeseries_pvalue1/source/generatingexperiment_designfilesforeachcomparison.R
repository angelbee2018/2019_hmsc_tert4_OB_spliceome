...
temp_exptdesign_table <- data.frame(matrix(nrow=4, ncol=2), row.names = NULL)
colnames(temp_exptdesign_table) <- c("replicatename", "condition")

temp_exptdesign_table[1:2, 2] <- gsub(x = comparison, pattern = "(.*)_vs_(.*)", replacement = "\\1")
...
temp_exptdesign_table[3:4, 2] <- gsub(x = comparison, pattern = "(.*)_vs_(.*)", replacement = "\\2")
temp_exptdesign_table[3, 1] <- gsub(x = comparison, pattern = "(.*)_vs_(.*)", replacement = "\\2_r1")
temp_exptdesign_table[4, 1] <- gsub(x = comparison, pattern = "(.*)_vs_(.*)", replacement = "\\2_r2")
