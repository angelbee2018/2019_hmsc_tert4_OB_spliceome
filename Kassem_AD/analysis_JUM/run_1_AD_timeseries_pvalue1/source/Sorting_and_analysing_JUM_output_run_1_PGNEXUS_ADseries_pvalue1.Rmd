...
all_domains_from_protein_tibble_filtered <- keep(all_domains_from_protein_tibble, ~(intersect(.x$range, r_ranges) %>% length) != 0)
...
# input tibble structure: MUST CONTAIN: gene "Gene", JUM isoform ID "AS_event_ID"
...
JUM_ASeventID_and_splicemode_to_chr_strand_start_end <- function(tibble_of_isoforms_to_be_consequence_assessed) {
  ...
  A3SS_subset <- A3SS_subset[, c("Gene", "AS_event_ID", "chr", "strand", "start", "end")] %>% as.tibble
  ...
  A5SS_subset <- A5SS_subset[, c("Gene", "AS_event_ID", "chr", "strand", "start", "end")] %>% as.tibble
  ...
  cassette_subset <- cassette_subset[, c("Gene", "AS_event_ID", "chr", "strand", "cassette_exon_start_coor", "cassette_exon_end_coor")] %>% as.tibble
  ...
  MXE_subset <- MXE_subset[, c("Gene", "isoform_ID", "chr", "strand", "start", "end")] %>% as.tibble
  ...
  IR_subset <- IR_subset[, c("Gene", "AS_event_ID", "chr", "strand", "retained_intron_start", "retained_intron_end")] %>% as.tibble
  ...
  list_of_composite_subset_coordinates <- purrr::map(.x = list_of_composite_subset_coordinates, .f = ~.x[1:(length(.x)-1)] %>% cbind(., end = .x[2:(length(.x))]) %>% as.tibble)
  ...
  composite_subset <- composite_subset[, c("Gene", "AS_event_ID", "chr", "strand", "start", "end")] %>% as.tibble
  
  ### CONCATENATE ALL THE SUBSET LISTS BACK INTO A SINGLE TABLE
  
  tibble_of_chr_strand_start_end <- bind_rows(A3SS_subset, A5SS_subset, cassette_subset, MXE_subset, IR_subset, composite_subset) %>% type.convert
...
MXE_events_summary_table_split <- split_JUM_output_table(MXE_events_summary_table, "MXE_exon_coordinates")

View(MXE_events_summary_table_split)
...
list_of_familyenrichment_by_SOM_cluster_ADseries <- purrr::map(.x = list_of_ADseries_gene_tables_by_SOM_cluster_flattened, .f = ~enrichment(genes = .x, reference = reference_geneset, genesets = list_of_pfam_gene_family_categories, adj = "BH", verbose = FALSE) %>% bc3net_benjamini_correction %>% cbind(., genes_contained = filtering_genehits_from_background_catalogue(list_of_pfam_gene_family_categories[.$TermID %>% as.character], .x) %>% lapply(toString) %>% paste(sep = ", ") %>% unlist) %>% type_convert %>% as.tibble)
...
DSG_A5SS <- gsub(x = wide_table_of_final_SOM_summary_5_by_5_ADseries[grep(x = wide_table_of_final_SOM_summary_5_by_5_ADseries$isoform_ID, pattern = "A5SS"), "isoform_ID"], pattern = "A5SS_", replacement = "")

chr_A5SS <- gsub(x = DSG_A5SS, pattern = "(.*)_(\\-|\\+|\\*)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10})_([0-9]{1,10})", replacement = "\\1")

strand_A5SS <- gsub(x = DSG_A5SS, pattern = "(.*)_(\\-|\\+|\\*)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10})_([0-9]{1,10})", replacement = "\\2")

start_A5SS <- gsub(x = DSG_A5SS, pattern = "(.*)_(\\-|\\+|\\*)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10})_([0-9]{1,10})", replacement = "\\3") %>% as.numeric

end_A5SS <- gsub(x = DSG_A5SS, pattern = "(.*)_(\\-|\\+|\\*)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10})_([0-9]{1,10})", replacement = "\\5") %>% as.numeric



testtable <- list_of_highest_PSI_isoforms_per_timepoint_ADseries_filtered_with_isoformID[[1]] %>% as.tibble


# HSA_enriched_isoforms
