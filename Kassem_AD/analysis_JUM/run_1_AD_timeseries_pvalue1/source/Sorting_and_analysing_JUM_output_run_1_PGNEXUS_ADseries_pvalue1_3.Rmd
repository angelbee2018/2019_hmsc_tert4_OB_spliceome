...
tibble_of_mapped_protein_positions_to_list_of_domains <- function(tibble_of_mapped_proteinID, ensemblmart) {
  
input_ranges <- purrr::map2(.x = tibble_of_mapped_proteinID$start, .y = tibble_of_mapped_proteinID$end, .f = ~.x:.y %>% unlist) %>% unlist %>% unique

# interpro
interpro_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("interpro", "interpro_start", "interpro_end"), mart = ensemblmart) %>% array_tree
...
# coiled coils
ncoils_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("ncoils_start", "ncoils_end"), mart = ensemblmart) %>% array_tree
...
# low complexity
seg_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("seg_start", "seg_end"), mart = ensemblmart) %>% array_tree
...
signalp_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("signalp_start", "signalp_end"), mart = ensemblmart) %>% array_tree
...
tmhmm_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("tmhmm_start", "tmhmm_end"), mart = ensemblmart) %>% array_tree 
...
#PDB-ENSP mappings
sifts_import_list <- getBM(filters = "ensembl_peptide_id", values = tibble_of_mapped_proteinID$names, attributes = c("sifts_import", "sifts_import_start", "sifts_import_end"), mart = ensemblmart) %>% array_tree 
...
# this will help us deal with ranges with partial overlap. I classify into 3 cases: 1. query start and end positions both lie in the CDS region 2. start is missing 3. end is missing.
...
conciding_domains <- map_depth(.x = mapped_aminoacid_positions, .depth = 2, .f = ~tibble_of_mapped_protein_positions_to_list_of_domains(.x, ensemblmart))

return(list(overlapping_annotated_regions = list_queryisoforms_overlapping_annotations_tibbles, conciding_domains = conciding_domains))
...
bind_rows(., list_of_highest_PSI_isoforms_per_timepoint_ADseries_filtered_with_isoformID[["9d"]] %>% .[.$Gene == "ATRX", ]) %>%
bind_rows(., list_of_highest_PSI_isoforms_per_timepoint_ADseries_filtered_with_isoformID[["9d"]] %>% .[.$Gene == "BTAF1", ])

SNF2_enriched_isoforms_chr_strand_start_end <- SNF2_enriched_isoforms %>% JUM_ASeventID_and_splicemode_to_chr_strand_start_end %>% enable_greedy_genome_coordinates

SNF2_enriched_isoforms_consequence <- tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain(HSA_enriched_isoforms_chr_strand_start_end, gtf_table, ensembl_mart)
...
ribonuclease_enriched_isoforms_chr_strand_start_end <- ribonuclease_enriched_isoforms %>% JUM_ASeventID_and_splicemode_to_chr_strand_start_end  
