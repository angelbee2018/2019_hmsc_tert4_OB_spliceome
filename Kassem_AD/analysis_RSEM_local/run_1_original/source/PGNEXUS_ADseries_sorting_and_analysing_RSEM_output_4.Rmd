...
nsYs <- RUVs(ADseries_gene_expectedcount_raw_highpass_upperQN %>% as.matrix, scIdx = replicate_rownumbers, k = 1)
 
ADseries_gene_expectedcount_RUVs <- bind_cols(ADseries_gene_expectedcount_raw_highpass[, 1], nsYs$normalizedCounts %>% as_tibble)
  
  pdf(paste(ADseries_qualitycheck_results_dir, "RLE_boxplot_RUVs_gene_expectedcount_originalconfig_k1.pdf", sep = "")) 
  
  par(mar = c(20, 4.1, 4.1, 2.1))
  
  plotRLE(ADseries_gene_expectedcount_RUVs[, 2:ncol(ADseries_gene_expectedcount_RUVs)] %>% as.matrix, outline = FALSE, ylim = c(-1.5, 1.5), col = timepoints_AD, las = 2, main = "RLE plot of RUVs expected count, k = 1", )
