...
tibble_phosphosite_allhuman_phosphosites <- read.delim("/media/Ubuntu/sharedfolder/phosphositeplus_phosphosites_human.tab", col.names = c("gene_name", "protein_name", "uniprotkb_entry", "chr_locus", "modified_residue_position", "SITE_GRP_ID", "organism", "MW.kD", "domain", "site_7AA_either.side", "LT_LIT", "MS_LIT", "MS_CST", "CST_CAT"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble
# tibble_phosphosite_allhuman_phosphosites <- read.delim("Z:/phosphositeplus_phosphosites_human.tab", col.names = c("gene_name", "protein_name", "uniprotkb_entry", "chr_locus", "modified_residue_position", "SITE_GRP_ID", "organism", "MW.kD", "domain", "site_7AA_either.side", "LT_LIT", "MS_LIT", "MS_CST", "CST_CAT"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

# change the residue position column
tibble_phosphosite_allhuman_phosphosites[, "modified_residue_position"] <- gsub(x = tibble_phosphosite_allhuman_phosphosites$modified_residue_position, pattern = "^([A-Z])([0-9]{1,5})(.*)", replacement = "\\2")

# remove rows where phosphosite is NA
row.indices_phosphosite.is.na <- which(is.na(tibble_phosphosite_allhuman_phosphosites$modified_residue_position))

tibble_phosphosite_allhuman_phosphosites <- tibble_phosphosite_allhuman_phosphosites[-row.indices_phosphosite.is.na, ]
...
list_ref_entries_matched_to_constituent_junctions <- list_constituent_junctions_for_query_sorted %>% (function(input_list) {
  ...
  output_list <- future_map(.x = input_list, 
                            .f = ~purrr::modify_at(.x = .x, 
                                                   ...
                                                   .f = ~purrr::map(.x = .x, .f = ~extract_junction.flanking.CDS_JUM(.x, ref_tibble)) %>% purrr::discard(.x = ., .p = ~length(.x) == 0)
                                                   ), 
                            .progress = TRUE, 
                            .options = future_options(globals = c("ref_tibble", "extract_junction.flanking.CDS_JUM"))) %>% purrr::discard(.x = ., .p = ~length(.x$contributing_junctions) <= 1)
...
list_transcript_and_protein_relative_differential_coords <- future_map(.x = list_genome_relative_differential_coords, .f = function(.x) {
  
  # DEBUG ###
  # .x <- list_genome_relative_differential_coords[[1]]
  ###########
  ...
  output_list <- purrr::splice(
    ...
    "transcript_relative_coords" = list(purrr::map(.x = input_list$genomic_coords_of_all_CDS_positions_per_protein_id, 
    ...
    purrr::splice(
      ...
      "protein_relative_coords" = list(.$transcript_relative_coords %>% 
