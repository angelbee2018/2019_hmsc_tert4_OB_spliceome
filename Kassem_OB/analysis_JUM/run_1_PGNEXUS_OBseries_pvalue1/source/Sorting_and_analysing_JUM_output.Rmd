...
# library(biomaRt)
ensembl_mart = useMart(biomart = "ensembl", dataset = "hsapiens_gene_ensembl", version = 98)
library(systemPipeR)
library(GOstats)
library(PFAM.db)
library(bc3net)
...
# R_processing_results_dir <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_JUM/run_1_PGNEXUS_OBseries_pvalue1/results/"
R_processing_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_JUM/run_2_PGNEXUS_OBseries_readlength100/results/"
...
list_of_detailed_tables <- future_map(.x = list_of_timepoint_comparisons_final, .f = ~purrr::map2(.x = .x, .y = list_of_AS_events, .f = ~read.delim(file = paste(R_processing_results_dir, "final_JUM_output_", .x, "/", list.files(path = paste(R_processing_results_dir, "final_JUM_output_", .x, "/", sep = ""), pattern = paste("(.)", .y, "(.*)detailed.txt", sep = "")), sep = ""), na.strings = c("NONE", "NA", "INF", "Inf") , sep = "\t", stringsAsFactors = FALSE) %>% as_tibble %>% add_column(., "splicemode" = .y)) %>% 
                                        set_names(list_of_AS_events), .progress = TRUE, .options = future_options(globals = c("list_of_AS_events", "R_processing_results_dir", "tibble"))) %>% set_names(list_of_timepoint_comparisons_final)
...
vector_of_timepoint_comparisons_final <- read.delim(paste(R_processing_results_dir, "list_of_timepoint_comparisons_final.txt", sep = ""), stringsAsFactors = FALSE, sep = "\t", header = FALSE, row.names = NULL)
...
for (comparison in vector_of_timepoint_comparisons_final[, 1])
{
  ...
  for (AS_event in c("A3SS_events", "A5SS_events", "cassette_exon_events", "composite_events", "intron_retention", "MXE_events"))
  {
    ...
    assign(x = paste(comparison, "_", AS_event, "_simplified", sep = ""), value = read.delim(file = paste(R_processing_results_dir, "final_JUM_output_", comparison, "/", list.files(path = paste(R_processing_results_dir, "final_JUM_output_", comparison, "/", sep = ""), pattern = paste("(.)", AS_event, "(.*)simplified.txt", sep = "")), sep = ""), sep = "\t", header = TRUE, stringsAsFactors = FALSE, row.names = NULL, na.strings = c("NONE", "NA", "INF", "Inf"), colClasses = "character"))
...
# write table 
write.table(x = wide_table_all_splicemodes_split_with_na, file = paste(R_processing_results_dir, "wide_table_of_all_JUM_results_", nrow(wide_table_all_splicemodes_split_with_na), "_entries_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# write all unique genes as background
vec_all_JUM_genes_genesymbol <- wide_table_all_splicemodes_split_with_na$Gene %>% unique
write.table(x = vec_all_JUM_genes_genesymbol, file = paste(R_processing_results_dir, "table_of_all_JUM_genes_genesymbol.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)
# convert to ENSG
...
write.table(x = vec_all_JUM_genes_ENSG, file = paste(R_processing_results_dir, "table_of_all_JUM_genes_ENSG.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)
...
# write table 
write.table(x = wide_table_all_splicemodes_split_with_na_with_ref_annot, file = paste(R_processing_results_dir, "wide_table_of_all_JUM_results_", nrow(wide_table_all_splicemodes_split_with_na_with_ref_annot), "_entries_with_na_annotated.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# also write chr start end strand

```

### Append the sub_junction_coords and save

```{r}

wide_table_all_splicemodes_split_with_na_final <- dplyr::left_join(wide_table_all_splicemodes_split_with_na_with_ref_annot, wide_tibble_AS.event.ID_mapping_to_composite_coords, by = c("AS_event_ID", "isoform_number", "splicemode"))

# write table 
write.table(x = wide_table_all_splicemodes_split_with_na_final, file = paste(R_processing_results_dir, "wide_table_all_splicemodes_split_with_na_final.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# wide_table_all_splicemodes_split_with_na_with_ref_annot <- read.delim(paste(R_processing_results_dir, "wide_table_of_all_JUM_results_", 37264, "_entries_with_na_annotated.txt", sep = ""), sep = "\t", row.names = NULL, header = TRUE, stringsAsFactors = FALSE)

wide_table_all_splicemodes_split_with_na_final <- read.delim(paste(R_processing_results_dir, "wide_table_all_splicemodes_split_with_na_final.txt", sep = ""), sep = "\t", row.names = NULL, header = TRUE, stringsAsFactors = FALSE)
...
write.table(x = PSI_levels_timeseries_OB_wide_with_na, file = paste(R_processing_results_dir, "wide_table_of_", length(PSI_levels_timeseries_OB_wide_with_na$AS_event_ID %>% unique), "_junctions_dPSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_dPSI_greaterthan_", dPSI_cutoff, "_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
# export associated gene set
write.table(x = PSI_levels_timeseries_OB_wide_with_na$Gene %>% unique %>% na.omit, file = paste(R_processing_results_dir, "geneset_", length( PSI_levels_timeseries_OB_wide_with_na$Gene %>% unique %>% na.omit), "_genes_dPSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_dPSI_greaterthan_", dPSI_cutoff, "_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)
...
write.table(x = PSI_levels_timeseries_OB_wide, file = paste(R_processing_results_dir, "wide_table_of_", length(PSI_levels_timeseries_OB_wide$AS_event_ID %>% unique), "_junctions_dPSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_dPSI_greaterthan_", dPSI_cutoff, "_no_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_cumulative_diff_changes <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h|12h"))] <= 0.01), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h|12h"))] >= 0.15)) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
                                             intersect(which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_table_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_table_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length)) %>% 
...
wide_tibble_all_splicemodes_differential_only <- dplyr::semi_join(wide_table_all_splicemodes_split_with_na_with_ref_annot, 
                                                                 PSI_levels_timeseries_OB_wide_with_na,
...
tibble_final_transcript_feature_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("AS_event_ID", "splicemode", "vector_matched_transcript_ids", "transcript_features")] %>% as_tibble) %>%
  ...
  ## add in gene symbols
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "splicemode", "Gene")] %>% unique %>% na.omit, ., by = c("AS_event_ID", "splicemode"))
# write table
...
tibble_final_annotation_supp <- list_tidied_annotation %>% 
  ...
  purrr::discard(.p = ~.x %>% nrow == 0) %>% rbindlist(use.names = TRUE) %>% as_tibble %>% 
  ## add in gene symbols
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "splicemode", "Gene")] %>% unique %>% na.omit, ., by = c("AS_event_ID", "splicemode"))
# write table
...
ggplot(data = subset_tibble_final_annotation_supp_with_frameshift_test) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "table_junction_ontology_supporting_info_frameshift_test_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 20, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "table_junction_ontology_supporting_info_frameshift_test_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 20, height = 15, units = "cm") 
...
ggplot(tibble_interpro_id_tally_per_ASeventID %>% dplyr::arrange(desc(tally)) %>% head(90), aes(x = reorder(interpro_identifier, -tally), y = tally)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 10, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 10, units = "cm") 

write.table(x = tibble_interpro_id_tally_per_ASeventID %>% dplyr::arrange(desc(tally)), file = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(tibble_PTM_tally, aes(x = reorder(PTM_type, -tally), y = tally)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_PTMs_differential_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 4, height = 8, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_PTMs_differential_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 4, height = 8, units = "cm") 
...
wide_table_all_splicemodes_split_with_na_with_ref_annot <- wide_table_all_splicemodes_split_with_na_with_ref_annot %>% 
  add_column("first_exon_reference" = grepl(x = wide_table_all_splicemodes_split_with_na_with_ref_annot$first_or_last_exon_reference, pattern = "first_exon"),
             "last_exon_reference" = grepl(x = wide_table_all_splicemodes_split_with_na_with_ref_annot$first_or_last_exon_reference, pattern = "last_exon"))

# ALL JUNCTIONS
# category means splicemode + NMD status + first/last junction
# also clean up the splicemode column
tibble_junctions_per_category_all <- wide_table_all_splicemodes_split_with_na_with_ref_annot %>% dplyr::distinct(., AS_event_ID, .keep_all = TRUE) %>% group_by(splicemode, first_exon_reference, last_exon_reference) %>% summarise(no_junctions_all = n())
