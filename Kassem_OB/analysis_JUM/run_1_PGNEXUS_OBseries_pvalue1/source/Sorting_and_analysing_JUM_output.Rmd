...
extract_junction.flanking.CDS_JUM <- function(spliceregion_list, tibble_gtf_table) {
...
all_plausible_diff_junctions_chr_strand_start_end <- wide_table_of_all_splicemodes_with_na_OBseries_filtered_qvalue_0.05_AS_plausible %>% JUM_ASeventID_and_splicemode_to_chr_strand_start_end
...
list_ref_entries_matched_to_constituent_junctions <- list_constituent_junctions_for_query_sorted %>% (function(input_list) {
  ...
  output_list <- future_map(.x = input_list, 
                            .f = ~purrr::modify_at(.x = .x, 
                                                   .at = "contributing_junctions", 
                                                   .f = ~purrr::map(.x = .x, .f = ~extract_junction.flanking.CDS_JUM(.x, ref_tibble_with_first.last)) %>% purrr::discard(.x = ., .p = ~length(.x) == 0)
                                                   ), 
                            .progress = TRUE, 
                            .options = future_options(globals = c("ref_tibble_with_first.last", "extract_junction.flanking.CDS_JUM"))) %>% purrr::discard(.x = ., .p = ~length(.x$contributing_junctions) <= 1)
...
list_genome_relative_differential_coords <- future_map(.x = list_ref_entries_matched_to_constituent_junctions, .f = function(.x) {
  ...
  # DEBUG ###
  # .x <- list_ref_entries_matched_to_constituent_junctions[[4]]
  ###########
  ...
  output_list <- purrr::splice(
    ...
    "vector_genomic_coords_of_all_differential_positions" = input_list[["contributing_junctions"]] %>% (function(.x) {
      
      # DEBUG ###
      # .x <- input_list$contributing_junctions
      ###########
      ...
      # take the highest and lowest exon entries and make unique
      tibble_left_exon_entries <- collapsed_tibble[seq(1, nrow(collapsed_tibble), by = 2), ] %>% dplyr::distinct(start, end, .keep_all = TRUE)
      tibble_right_exon_entries <- collapsed_tibble[seq(2, nrow(collapsed_tibble), by = 2), ] %>% dplyr::distinct(start, end, .keep_all = TRUE)
      
      # DEAL WITH THE LEFT EXON ENTRIES
      leftmost.end_of_left.exon <- min(tibble_left_exon_entries$end)
      number_of_start_coords_of_left_exon_overlapping <- length(which(tibble_left_exon_entries$start <= leftmost.end_of_left.exon))
      ...
      vector_genomic_coords_of_all_left_exon_positions <- list_genomic_coords_of_all_left_exon_positions %>% 
       ...
       dplyr::filter(tally < number_of_start_coords_of_left_exon_overlapping, .preserve = TRUE) %>% 
       .$genome_coord
      
      # add back in any first/last exon positions, left side
      list_genomic_coords_of_all_left_exon_positions_first.last <- purrr::map2(.x = tibble_left_exon_entries %>% dplyr::filter(is.na(first_or_last_exon) == FALSE) %>% .$start, 
                                                                   .y = tibble_left_exon_entries %>% dplyr::filter(is.na(first_or_last_exon) == FALSE) %>% .$end,
                                                                   .f = ~.x:.y)
      
      vector_genomic_coords_of_all_left_exon_positions_first.last <- list_genomic_coords_of_all_left_exon_positions_first.last %>% unlist
      ...
      tibble_left_exon_entries_middle.only <- tibble_left_exon_entries %>% 
      ...
      rightmost.start_of_right.exon <- max(tibble_right_exon_entries$start)
      number_of_end_coords_of_right_exon_overlapping <- length(which(tibble_right_exon_entries$end >= rightmost.start_of_right.exon))
      ...
      vector_genomic_coords_of_all_right_exon_positions <- list_genomic_coords_of_all_right_exon_positions %>% 
       ...
       dplyr::group_by(genome_coord) %>% 
       dplyr::summarise("tally" = n()) %>%
       dplyr::filter(tally < number_of_end_coords_of_right_exon_overlapping, .preserve = TRUE) %>% 
       .$genome_coord
      
      # add back in any first/last exon positions, right side
      list_genomic_coords_of_all_right_exon_positions_first.last <- purrr::map2(.x = tibble_right_exon_entries %>% dplyr::filter(is.na(first_or_last_exon) == FALSE) %>% .$start, 
                                                                   .y = tibble_right_exon_entries %>% dplyr::filter(is.na(first_or_last_exon) == FALSE) %>% .$end,
                                                                   .f = ~.x:.y)
      
      vector_genomic_coords_of_all_right_exon_positions_first.last <- list_genomic_coords_of_all_right_exon_positions_first.last %>% unlist
      ...
      tibble_right_exon_entries_middle.only <- tibble_right_exon_entries %>% 
      ...
      vector_genomic_coords_all_differential <- c(vector_genomic_coords_of_all_left_exon_positions,
                                                  vector_genomic_coords_of_all_left_exon_positions_first.last,
                                                  vector_genomic_coords_of_all_left.middle_exon_positions,
                                                  vector_genomic_coords_of_all_right_exon_positions,
                                                  vector_genomic_coords_of_all_right_exon_positions_first.last,
    ...
    "genomic_coords_of_all_CDS_positions_per_protein_id" = list(input_list[["contributing_junctions"]] %>% (function(.x) {
    ...
    "vector_matched_transcript_ids" = list(input_list[["contributing_junctions"]] %>% (function(.x) {
    ...
    }, .progress = TRUE, .options = future_options(globals = c("tibble_ref_gtf", "sort_tibble_according_to_strand", "as_tibble", "tibble::enframe")) ) %>%
    
    # drop AS junction structures that don't have any differential regions.
    purrr::discard(.x = ., .p = ~length(.x$vector_genomic_coords_of_all_differential_positions) == 0)

# MATCH DIFFERENTIAL GENOME-RELATIVE COORDS TO EACH TRANSCRIPT CORRESPONDING TO PROTEIN ID
...
list_transcript_and_protein_relative_differential_coords <- future_imap(.x = list_genome_relative_differential_coords, .f = function(.x, .y) {
  ...
  output_list <- purrr::splice(
    ...
    "CDS_relative_coords" = list(purrr::map(.x = input_list$genomic_coords_of_all_CDS_positions_per_protein_id, 
                                              .f = ~which(.x %in% input_list$vector_genomic_coords_of_all_differential_positions) %>% sort(decreasing = FALSE)
                                              ) %>% 
    ...
    purrr::splice(
      ...
      "protein_relative_coords" = list(.$CDS_relative_coords %>% 
                                         purrr::map(~ceiling(.x / 3) %>% unlist %>% unique))
      
    )
    
    }, .progress = TRUE, .options = future_options(globals = c("tibble_ENST_to_ENSP_mapping")))
...
list_matched_annotation <- purrr::map(.x = list_transcript_and_protein_relative_differential_coords, .f = function(.x) {
  ...
  tibble_phosphositeplus_allhuman_phosphosites <<- tibble_phosphositeplus_allhuman_phosphosites
  tibble_dbPTM_allhuman_annotations <<- tibble_dbPTM_allhuman_annotations
  tibble_ref_gtf_original <<- tibble_ref_gtf_original
  ############################################
  
  # DEBUG ###
  # .x <- list_transcript_and_protein_relative_differential_coords[[5]]
  ###########
  ...
  output_list <- purrr::splice(
    ...
    "overlapping_transcript_features" = purrr::map(.x = input_list$vector_matched_transcript_ids, .f = function(.x) {
      ...
      row.indices_of_overlaps_with_differential_positions <- which(purrr::map2(.x = tibble_ref_gtf_subset_by_transcript_id$start, 
                           ...
                           .f = ~any(.x < input_list$vector_genomic_coords_of_all_differential_positions &
                                       .y > input_list$vector_genomic_coords_of_all_differential_positions)) %>% unlist)
      ...
      rbindlist %>% .$type %>% unique,
    
    "overlapping_biomart_annotations" = list(purrr::map2(.x = input_list$CDS_relative_coords, .y = names(input_list$CDS_relative_coords), .f = function(.x, .y) {
      
      # DEBUG ###
      # .x <- input_list$CDS_relative_coords %>% .[[1]]
      # .y <- names(input_list$CDS_relative_coords) %>% .[1]
      ###########
      ...
      list_matched_biomart_annotations <- purrr::map(.x = list_of_tibbles_biomart_domain_annotation, 
                                                     .f = function(.x) {
                                                       # DEBUG ###
                                                       # .x <- list_of_tibbles_biomart_domain_annotation[[1]]
                                                       # .y <- names(list_of_tibbles_biomart_domain_annotation) %>% .[1]
                                                       ###########
                                                       
                                                       tibble_domain_subset <- .x
                                                       
                                                       # test for which differential positions lie between a domain
                                                       ...
                                                       # extract matched GTF entries
                                                       matched_domain_entries <- tibble_domain_subset[row.indices_of_overlaps_with_differential_positions, ] %>%
                                                         .[.$ensembl_peptide_id == protein.id, ]
                                                       
                                                       return(matched_domain_entries)
                                                       
                                                       } )
      
      return(list_matched_biomart_annotations)
      
      } ) )
    
  )
    
}, .progress = TRUE, .options = future_options(globals = c("rbindlist", "list_of_tibbles_biomart_domain_annotation", "tibble_ENSP_to_uniprotkb", "tibble_phosphositeplus_allhuman_phosphosites", "tibble_dbPTM_allhuman_annotations", "tibble_ref_gtf_original")))
