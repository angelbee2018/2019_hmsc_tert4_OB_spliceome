...
srsf2_clip_table <- read.delim("C:/Users/angel/Desktop/srsf2_clip_liang2018_mutant.txt", header = FALSE, stringsAsFactors = FALSE) %>% as_tibble %>% unique %>% setNames("Gene")

test <- dplyr::semi_join(wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible %>% as_tibble, srsf2_clip_table, by = "Gene")

targetsgenesinsom <- test$Gene %>% unique %>% sort

number_targets_enriched_per_cluster <- test$cluster %>% table %>% as_tibble %>% setNames(c("cluster", "frequency"))

wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible %>% group_by(cluster) %>% summarise %>% print

pvalues <- purrr::map(.x = number_targets_enriched_per_cluster$cluster %>% as.numeric, 
           .f = ~dhyper(x = 0:min(length(targetsgenesinsom), wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible[wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible$cluster == .x, "Gene"] %>% unique %>% length), 
           m = length(targetsgenesinsom), 
           n = length(wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible$Gene %>% unique), 
           k = wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible[wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible$cluster == .x, "Gene"] %>% unique %>% length, log = FALSE) %>% 
             .[(number_targets_enriched_per_cluster[number_targets_enriched_per_cluster$cluster == .x, "frequency"] %>% paste %>% as.numeric):(min(length(targetsgenesinsom), wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible[wide_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible$cluster == .x, "Gene"] %>% unique %>% length))] %>% sum)

names(pvalues) <- number_targets_enriched_per_cluster$cluster

pvalues <- plyr::ldply(pvalues, rbind) %>% as_tibble %>% setNames(c("cluster", "pvalue"))
...
list_of_RBP_target_gene_categories_raw <- list(
  SRSF2_CLIP_HeLa_WT_2018liang_0.01FDR = read.delim("C:/Users/angel/Desktop/srsf2_clip_liang2018_WT_0.01FDR.txt", header = FALSE, stringsAsFactors = FALSE) %>% unique %>% as.vector,
  SRSF2_CLIP_HeLa_WT_2018liang_0.05FDR = read.delim("C:/Users/angel/Desktop/srsf2_clip_liang2018_WT_0.05FDR.txt", header = FALSE, stringsAsFactors = FALSE) %>% unique %>% as.vector,
  SRSF2_literaturesummary_SRSFmutation_trait_loci_liang2018 = read.delim("C:/Users/angel/Desktop/SRSF2_literaturesummary_SRSFmutation_trait_loci_liang2018.txt", header = FALSE, stringsAsFactors = FALSE) %>% unique %>% as.vector,
  SRSF2_CLIP_HeLa_P95H_2018liang = read.delim("C:/Users/angel/Desktop/srsf2_clip_liang2018_P95H.txt", header = FALSE, stringsAsFactors = FALSE) %>% unique %>% as.vector
)
...
png(file = paste(qualitycheck_results_dir, "som_consensus_matrix_", nrow(wide_table_of_final_SOM_summary_5_by_5_OBseries), "_isoforms_OBseries_any_", p_or_q_value, qpvalue_cutoff, "_dPSI_greaterthan_", dPSI_cutoff, "_isoforms_sweep_from_", min(xdim_range), "x", min(ydim_range), "_to_", max(xdim_range), "x", max(ydim_range), ".png", sep = ""), 
...
# sd <- apply(wide_table_of_all_combined_count_files[, 2:ncol(wide_table_of_all_combined_count_files)], 1, FUN = function(X) {sd(X)})
