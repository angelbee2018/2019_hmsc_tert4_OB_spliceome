...
# FUNCTION TO EXTRACT IR TRANSCRIPTS WITH JUNCTION-FLANKING EXONS.
# NOTE: to be used with purrr
# input: spliceregion_list: a list containing details of ONE junction: $chr, $start, $end, $strand
# tibble_of_all_detailed_tables: the rbind result of all detailed tables
# tibble_gtf_table: rtracklayer::import(...) %>% as_tibble. can be any GTF. reconstructed or reference.
# index: loop progress marker to be used with imap

extract_IR_junction_associated_exons <- function(spliceregion_list, tibble_of_all_detailed_tables, tibble_gtf_table, index) {
  
  # DEBUG ###################

  index <- 1
  spliceregion_list <- wide_tibble_of_all_unique_exon_coords_array.tree[[3]]
  tibble_of_all_detailed_tables <- wide_tibble_of_all_detailed_tables
  tibble_gtf_table <- tibble_ref_gtf
...
list_of_tibblised_contributing_junctions <- purrr::map(.x = list_contributing_junctions[1:100], .f = ~tibble::enframe(.x, value = "junction_ID", name = NULL))
## do a dplyr left join on each disassembled junction with the annotation
list_of_contributing_junctions_joined_to_annotation <- purrr::map(.x = list_of_tibblised_contributing_junctions, .f = ~dplyr::left_join(.x, tibble_gtf_matching_exon_entries_extracted, by = "junction_ID"))
...
# write table 
write.table(x = wide_table_all_splicemodes_split_with_na, file = paste(results_directory_figures, "wide_table_of_", nrow(wide_table_all_splicemodes_split_with_na), "_junctions_dPSI_all.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
PSI_levels_timeseries_constitutive_OB_wide_with_na <- wide_table_all_splicemodes_split_with_na %>% as.data.frame
...
write.table(x = PSI_levels_timeseries_constitutive_OB_wide, file = paste(results_directory_figures, "wide_table_of_", nrow(PSI_levels_timeseries_OB_wide), "_constitutive_junctions_dPSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_dPSI_greaterthan_", dPSI_cutoff, "_no_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
