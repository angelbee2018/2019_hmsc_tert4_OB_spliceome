...
extract_junction.flanking.exons_JUM <- function(spliceregion_list, tibble_gtf_table) {
...
tibble_of_mapped_protein_positions_to_list_of_domains_PTMs <- function(list_of_mapped_proteinID, ensemblmart) {
  ...
  # search biomart
  list_biomart_result_per_attribute <- future_map(.x = list_of_attributes_to_retrieve, .f = ~getBM(filters = "ensembl_peptide_id", values =  vec_all_ENSP_ids %>% unique, attributes = .x, mart = ensemblmart), .progress = TRUE, .options = future_options(globals = c("vec_all_ENSP_ids")))
  ...
  table_ENSP_to_uniprot_entry_mapping <- getBM(filters = "ensembl_peptide_id", values = vec_all_ENSP_ids, attributes = c("ensembl_peptide_id", "uniprotsptrembl"), mart = ensembl_mart) %>% setNames(c("ensembl_peptide_id", "uniprotkb_entry")) %>% 
...
tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain <- function(tibble_queryisoforms_chr_strand_start_end, gtf_table, ensemblmart) {
  ...
  # gtf_table <- tibble_ref_gtf_simplifiedlist_queryisoforms_overlapping_annotations_tibbles
  # ensemblmart <- ensembl_mart
  ###########
  ...
  conciding_domains_PTMs <- tibble_of_mapped_protein_positions_to_list_of_domains_PTMs(mapped_aminoacid_positions, ensemblmart)
  ...
  list_of_tibbles_protein_level_results <- purrr::map2(.x = conciding_domains_PTMs, .y = names(conciding_domains_PTMs), .f = function(.x, .y) {
    
    # DEBUG ###
    # .x <- conciding_domains_PTMs[[1]]
    # .y <- names(conciding_domains_PTMs) %>% .[[1]]
    ###########
...
list_as.event.id_with_summarised_annotation_IR <- purrr::imap(.x = as.event.id_array.tree_IR, .f = function(.x, .y) {
  ...
  if (all(is.na(vec_ref_transcripts_matched_to_junction)) != TRUE) {
    ...
    tibble_all_confirmed_IR_entries_ref <- list(tibble_ref_gtf_subset_IR_overlap_start.matched, 
  ...
  if (all(is.na(vec_recon_transcripts_matched_to_junction)) != TRUE) {
    ...
    # find which IR matched entry has any same end coord
    tibble_recon_gtf_subset_IR_overlap_end.matched <- dplyr::semi_join(tibble_recon_gtf_subset_IR_overlap, vector_junction_matched_end_coords)
    
    tibble_all_confirmed_IR_entries_recon <- list(tibble_recon_gtf_subset_IR_overlap_start.matched, 
  ...
  summarised_tibble_annotation <- tibble("AS_event_ID" = .x, 
                                         ...
                                         "matched_ref_transcript_names" = subset_tibble_annotation$matched_ref_transcript_names %>% paste(collapse = ";"),
                                         "matched_recon_transcript_names" = subset_tibble_annotation$matched_recon_transcript_names %>% paste(collapse = ";"),
                                         "NMD_reference" = any(tibble_all_confirmed_IR_entries_ref$transcript_biotype == "nonsense_mediated_decay"),
                                         "NMD_recon" = all(tibble_all_confirmed_IR_entries_recon$NMD_candidate == TRUE),
                                         "first_or_last_exon_reference" = subset_tibble_annotation$first_or_last_exon_reference %>% paste(collapse = ";"),
                                         "first_or_last_exon_recon" = subset_tibble_annotation$first_or_last_exon_recon %>% paste(collapse = ";"),
                                         "any_first_exon" = any(c(grep(x = subset_tibble_annotation$first_or_last_exon_reference, pattern = "first_exon") %>% na.omit, 
                                                                  grep(x = subset_tibble_annotation$first_or_last_exon_recon, pattern = "first_exon") %>% na.omit)),
...
# sometimes, JUM seems to call novel splice junctions that are slightly off by at most 2 nt. If we expand the genomic coordinates by 2 on each side, we can correct for this.
...
all_plausible_diff_junctions_chr_strand_start_end <- wide_table_of_all_splicemodes_with_na_OBseries_filtered_qvalue_0.05_AS_plausible %>% JUM_ASeventID_and_splicemode_to_chr_strand_start_end %>% unique %>% enable_greedy_genome_coordinates

message("total number of unique junction structures queried: ", all_plausible_diff_junctions_chr_strand_start_end$AS_event_ID %>% unique %>% length)

all_plausible_diff_junctions_consequence <- tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain(all_plausible_diff_junctions_chr_strand_start_end, gtf_table = tibble_ref_gtf_simplified, ensembl_mart)

tibble_all_plausible_diff_transcript_level_results <- all_plausible_diff_junctions_consequence$transcript_level_results

tibble_transcript_level_summary_count <- tibble_all_plausible_diff_transcript_level_results %>% dplyr::group_by(AS_event_ID) %>% 
...
message("total number of unique differential junction structures with any sort of transcript-level annotation: ", nrow(tibble_transcript_level_summary_count))

message("percent of differential junction structures overlapping a protein coding transcript: ", length(which(tibble_transcript_level_summary_count$number_protein_coding == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential junction structures overlapping an lncRNA transcript: ", length(which(tibble_transcript_level_summary_count$number_lncrna == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential junction structures overlapping a miRNA transcript: ", length(which(tibble_transcript_level_summary_count$number_micrna == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)

message("percent of differential junction structures overlapping a 3' UTR region: ", length(which(tibble_transcript_level_summary_count$number_three_prime_utr == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential junction structures overlapping a 5' UTR region: ", length(which(tibble_transcript_level_summary_count$number_five_prime_utr == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential junction structures overlapping a CDS region: ", length(which(tibble_transcript_level_summary_count$number_CDS == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)

tibble_all_plausible_diff_protein_level_results <- all_plausible_diff_junctions_consequence$protein_level_results

unique_interpro_ids <- tibble_all_plausible_diff_protein_level_results$interpro %>% .[tibble_all_plausible_diff_protein_level_results$interpro != "FALSE"]  %>% strsplit(split = ";") %>% unlist %>% unique

tibble_protein_level_summary_count <- tibble_all_plausible_diff_protein_level_results %>% dplyr::group_by(AS_event_ID) %>% 
...
ggplot(tibble_interpro_id_tally_per_ASeventID %>% head(90), aes(x = reorder(interpro_identifier, -tally), y = tally)) +
  ...
  ggtitle(paste("Tally of the 90 most overlapping interpro identifiers of differential junctions", dPSI_cutoff, "and any sig", p_or_q_value, qpvalue_cutoff, "encompassing", nrow(tibble_protein_level_summary_count), "PLAUSIBLE junctions")) +
  xlab("Interpro identifier") +
  scale_x_discrete(labels = function(x) {str_wrap(x, width = 50)}) +
  ylab("Number of hits") +
  # coord_cartesian(ylim = c(0, 50)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.y = element_text(margin = margin(r = 90)), legend.title.align = 0.5, legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica-Normal")) +
 ggsave(filename = paste(results_directory_figures, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 15, units = "cm") +
 ggsave(filename = paste(results_directory_figures, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 15, units = "cm") 

write.table(x = tibble_interpro_id_tally_per_ASeventID, file = paste(results_directory_figures, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

message("total number of unique junction structures with any sort of protein-level annotation: ", nrow(tibble_protein_level_summary_count), " (", nrow(tibble_protein_level_summary_count)*100/nrow(tibble_transcript_level_summary_count), "% of those which have any transcript annotation)")

message("total percent of unique junction structures overlapping an interpro domain: ", length(which(tibble_protein_level_summary_count$overlaps_interpro_domain == TRUE))/nrow(tibble_protein_level_summary_count) * 100)

message("total percent of unique junction structures overlapping a low complexity region: ", length(which(tibble_protein_level_summary_count$overlaps_LCR == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique junction structures overlapping a disordered region: ", length(which(tibble_protein_level_summary_count$overlaps_disordered_region == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique junction structures overlapping a coiled-coiled domain: ", length(which(tibble_protein_level_summary_count$overlaps_ncoils == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique junction structures overlapping a signalp domain: ", length(which(tibble_protein_level_summary_count$overlaps_signalp == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique junction structures overlapping a tmhmm domain: ", length(which(tibble_protein_level_summary_count$overlaps_tmhmm == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
