...
# tibble_of_AS.event.ID_joined_to_annotation <- read.delim(file = paste(R_processing_results_dir, "tibble_of_AS.event.ID_joined_to_annotation.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL) %>% as_tibble
...
# read back in
# wide_tibble_all_splicemodes_split_with_na <- read.delim(file = paste(R_processing_results_dir, list.files(R_processing_results_dir, pattern = "wide_table_of_all_JUM_results_.*_VSRs_with_na.txt"), sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL) %>% as_tibble
# long_tibble_all_splicemodes_split_with_na <- read.delim(file = paste(R_processing_results_dir, list.files(R_processing_results_dir, pattern = "long_table_of_all_JUM_results_.*_VSRs_with_na.txt"), sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL) %>% as_tibble
...
tibble_junctions_per_category_all <- wide_tibble_all_splicemodes_split_with_na_with_ref_annot_2 %>% dplyr::distinct(., AS_event_ID, .keep_all = TRUE) %>% group_by(splicemode, NMD_flagged_recon, poison_exon_candidate_recon, first_exon_reference, last_exon_reference) %>% summarise(no_junctions_all = n())
