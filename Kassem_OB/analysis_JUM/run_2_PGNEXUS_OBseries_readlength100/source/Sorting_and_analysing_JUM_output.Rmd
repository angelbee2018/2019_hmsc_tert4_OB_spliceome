...
wide_tibble_AS.event.ID_mapping_to_composite_coords <- wide_tibble_of_all_detailed_tables_with_na %>% 
  ...
  dplyr::ungroup() %>%
  dplyr::select(AS_event_ID, sub_junction_chr, sub_junction_start_coor, sub_junction_end_coor, splicemode, isoform_number) %>%
  unique
...
tibble_cumulative_diff_changes <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             intersect(which(wide_tibble_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_tibble_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h|12h"))] <= 0.01), 
                                                       which(wide_tibble_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_tibble_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h|12h"))] >= 0.15)) %>% length,
                                             intersect(which(wide_tibble_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_tibble_all_splicemodes_split_with_na_with_ref_annot), "qvalue"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit <= 0.01)} )), 
                                                       which(wide_tibble_all_splicemodes_split_with_na_with_ref_annot[, which(str_detect(colnames(wide_tibble_all_splicemodes_split_with_na_with_ref_annot), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit >= 0.15)} ))) %>% length,
...
tibble_cumulative_diff_changes <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0,
                                             which(wide_tibble_all_splicemodes_differential_only[, which(str_detect(colnames(wide_tibble_all_splicemodes_differential_only), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h|12h"))] %>% unlist %>% abs >= 0.15) %>% length,
                                             which(wide_tibble_all_splicemodes_differential_only[, which(str_detect(colnames(wide_tibble_all_splicemodes_differential_only), "deltaPSI"))] %>% .[, -which(str_detect(colnames(.), "12d|9d|6d|3d|24h"))] %>% apply(X = ., MARGIN = 1, FUN = function(X) {any(X %>% as.numeric %>% na.omit %>% abs >= 0.15)} )) %>% length,
...
PSI_levels_timeseries_OB_wide_scaled[, OBseries_timepoints]  <- PSI_levels_timeseries_OB_wide_scaled[, OBseries_timepoints] %>% genescale(m = ., axis = 1, method = "Z") %>% as_tibble
...
wide_table_of_final_SOM_summary_5_by_5_OBseries <- cbind(PSI_levels_timeseries_OB_wide[, c("Gene", "AS_event_ID", "splicemode", "isoform_ID", "sub_junction_chr", "sub_junction_start_coor", "sub_junction_end_coor")], som_5_by_5_OBseries_table, cluster = somdata_5_by_5_OBseries[["unit.classif"]])
...
long_table_of_final_SOM_summary_5_by_5_OBseries <- reshape2::melt(wide_table_of_final_SOM_summary_5_by_5_OBseries, id.vars = c("Gene", "AS_event_ID", "splicemode", "isoform_ID", "sub_junction_chr", "sub_junction_start_coor", "sub_junction_end_coor", "cluster"), variable.name = "timepoint", value.name = "scaled_PSI_value")
...
ggplot() +
  geom_line(data = long_table_of_final_SOM_summary_5_by_5_OBseries, aes(x = timepoint, y = scaled_PSI_value, group = isoform_ID), alpha = 0.33) +
...
list_of_scaled_PSI_OBseries <- purrr::map(.x = vector_OBseries_timepoints, .f = ~PSI_levels_timeseries_OB_wide_scaled[, c("Gene", "AS_event_ID", "splicemode", "sub_LSV_ID", "sub_junction_start_coor", "sub_junction_end_coor", "NMD_flagged_ref", "NMD_recon", "any_NMD", "first_or_last_exon_reference", "first_or_last_exon_recon", "any_first_exon", "any_last_exon", .x)])
