...
# also write table of differential information for MSC, 6h, 1d only
PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only <- PSI_levels_timeseries_OB_wide_with_na[which((PSI_levels_timeseries_OB_wide_with_na$deltaPSI_BM_MSC_to_OB_6h.BM_MSC_to_OB_ud %>% as.numeric %>% abs >= dPSI_cutoff) |
                                                                                             (PSI_levels_timeseries_OB_wide_with_na$deltaPSI_BM_MSC_to_OB_1d.BM_MSC_to_OB_ud %>% as.numeric %>% abs >= dPSI_cutoff) |
                                                                                             (PSI_levels_timeseries_OB_wide_with_na$deltaPSI_BM_MSC_to_OB_1d.BM_MSC_to_OB_6h %>% as.numeric %>% abs >= dPSI_cutoff)), ]
# create time-series columns 
PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only <- PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only %>% dplyr::select(-contains("pvalue"), -contains("qvalue"), -contains("deltaPSI"), matches("deltaPSI.*ud$"))

colnames(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only) <- gsub(x = colnames(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only), pattern = "(deltaPSI_BM_MSC_to_)(OB_)([0-9]{1,2}d|[0-9]{1,2}h)(.BM_MSC_to_OB_ud)", replacement = "\\3")

PSI_levels_timeseries_OB_wide_not.all.na.rows_ud.6h.1d_only <- (apply(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only[, vector_OBseries_timepoints_edited[vector_OBseries_timepoints_edited != "MSC"]], 1, function(X) {all(is.na(X))}) == FALSE) %>% which

PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only <- cbind(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only, MSC = 0)

PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only <- PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only[PSI_levels_timeseries_OB_wide_not.all.na.rows_ud.6h.1d_only, ]

# reorder cols to match chronological order
PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only <- dplyr::bind_cols(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only %>% dplyr::select(-vector_OBseries_timepoints_edited), PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only %>% dplyr::select(vector_OBseries_timepoints_edited))
# write table of MSC, 6h, 1d only
write.table(x = PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only, file = paste(R_processing_results_dir, "wide_table_of_", length(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only$AS_event_ID %>% unique), "_differential_VSRs_", p_or_q_value, qpvalue_cutoff, "_dPSI", dPSI_cutoff, "_with_na_ud.6h.1d_only.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
# export associated gene set of MSC, 6h, 1d only
write.table(x = PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only$Gene %>% unique %>% na.omit, file = paste(R_processing_results_dir, "geneset_", length(PSI_levels_timeseries_OB_wide_with_na_ud.6h.1d_only$Gene %>% unique %>% na.omit), "_differential_genes_", p_or_q_value, qpvalue_cutoff, "_dPSI", dPSI_cutoff, "_with_na_ud.6h.1d_only.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)

# CONTINUE WITH THE MAIN TABLE ###
...
PSI_levels_timeseries_OB_wide_not.all.na.rows <- which(apply(PSI_levels_timeseries_OB_wide_with_na[, vector_OBseries_timepoints_edited[2:length(vector_OBseries_timepoints_edited)]], 1, function(X) {all(is.na(X))}) == FALSE)

PSI_levels_timeseries_OB_wide_with_na <- cbind(PSI_levels_timeseries_OB_wide_with_na, MSC = 0)

PSI_levels_timeseries_OB_wide_with_na <- PSI_levels_timeseries_OB_wide_with_na[PSI_levels_timeseries_OB_wide_not.all.na.rows, ]
...
ggplot() +
  geom_line(data = long_table_of_final_SOM_summary_5_by_5_OBseries, aes(x = timepoint, y = scaled_PSI_value, group = paste(AS_event_ID, sub_LSV_ID)), alpha = 0.33) +
...
### extract the splicing information according to gene enrichment to identify any trends

```{r}

# PROTEIN KINASES ###
vector_differential_protein_kinase_genes <- OB_anysig_GOtest_all_topten[OB_anysig_GOtest_all_topten$Term == "protein kinase activity", "SampleKeys"] %>% strsplit(split = " ") %>% unlist

tibble_differential_anysig_with_na_protein_kinases <- PSI_levels_timeseries_OB_wide_with_na[PSI_levels_timeseries_OB_wide_with_na$Gene %in% vector_differential_protein_kinase_genes, ] %>% as_tibble
...
message("percent of differential regions overlapping a start codon: ", 100 * grep(x = tibble_final_transcript_feature_annotation %>% dplyr::distinct(AS_event_ID, .keep_all = TRUE) %>% .$transcript_features, pattern = "start_codon") %>% length/(tibble_final_transcript_feature_annotation$AS_event_ID %>% unique %>% length))
message("percent of differential regions overlapping a stop codon: ", 100 * grep(x = tibble_final_transcript_feature_annotation %>% dplyr::distinct(AS_event_ID, .keep_all = TRUE) %>% .$transcript_features, pattern = "stop_codon") %>% length/(tibble_final_transcript_feature_annotation$AS_event_ID %>% unique %>% length))
...
tibble_protein_level_summary_count <- tibble_final_biomart_annotation %>% dplyr::group_by(AS_event_ID, splicemode) %>% 
...
message("total number of unique LSVs with any sort of protein-level annotation: ", tibble_final_biomart_annotation$AS_event_ID %>% unique %>% length, " (", tibble_final_biomart_annotation$AS_event_ID %>% unique %>% length *100/tibble_final_transcript_feature_annotation$AS_event_ID %>% unique %>% length, "% of those which have any transcript annotation)")
...
tibble_PTM_tally <- tibble_final_PTM_annotation %>% dplyr::distinct(AS_event_ID, splicemode, modified_residue_position, PTM_type) %>% dplyr::group_by(PTM_type) %>% dplyr::summarise("tally" = n())
