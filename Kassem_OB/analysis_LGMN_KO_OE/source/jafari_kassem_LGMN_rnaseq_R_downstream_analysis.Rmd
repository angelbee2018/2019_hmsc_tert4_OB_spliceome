---
title: "2020 RNA Atlas (total RNA, splicing): Downstream PSI-Sigma analysis"
author: "Angel Liang"
date: "09/02/2021"
...
source_dir <- "/mnt/LTS/projects/2020_RNA_atlas/source/"

manifest_dir <- "/mnt/LTS/data/2020_RNA_atlas/manifest/"

# psisigma_results_pooled_technical_replicates_dir <- "/mnt/helium_raid0/2020_RNA_atlas_results/3_PSISigma_universal_db/"
# psisigma_results_separated_technical_replicates_dir <- "/mnt/savvio_raid5/3_PSISigma_universal_db/"
psisigma_results_pooled_technical_replicates_dir <- "/mnt/scratch/2020_RNA_Atlas/total_RNA_splicing/3_PSISigma_universal_db_pooled_technical_replicates/"

R_processing_results_dir <- "/mnt/LTS/projects/2020_RNA_atlas/results/R_processing_results_PSISigma/"
...
vector_sample_names <- list.dirs(path = "/mnt/LTS/data/2020_RNA_atlas/fastq_files/Total_RNA/", recursive = FALSE, full.names = FALSE)

# generate folder -> subfolder mapping
tibble_folder_subfolder_mapping <- purrr::map(.x = list.dirs(path = "/mnt/LTS/data/2020_RNA_atlas/fastq_files/Total_RNA/", recursive = FALSE, full.names = TRUE), .f = ~list.dirs(path = .x, recursive = FALSE, full.names = FALSE)) %>% set_names(nm = list.dirs(path = "/mnt/LTS/data/2020_RNA_atlas/fastq_files/Total_RNA/", recursive = FALSE, full.names = FALSE)) %>%
  purrr::map2(.x = ., .y = names(.), .f = ~tibble("RNA_number" = .y, "condition_names" = .x)) %>%
  rbindlist %>% as_tibble

tibble_sample_info <- read.delim(paste(manifest_dir, "sample_info.txt", sep = ""), sep = "\t", row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble

tibble_combined_mapping_info <- dplyr::full_join(tibble_folder_subfolder_mapping, tibble_sample_info)

bam_bai_sj_sam_dir <- "/mnt/scratch/2020_RNA_Atlas/total_RNA_splicing/2_STAR_2_pass_mapping_pooled_technical_replicates/"

reference_data_dir <- "/mnt/LTS/reference_data/"

clusterone_path <- "/mnt/LTS/tools/clusterONE/cluster_one-1.2.jar"
...
test <- furrr::future_map2(
  .x = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = TRUE),
  .y = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = FALSE),
  .f = ~file.exists(paste(.x, "/psisigma_atlas_universal_r1_ir3.sorted.txt", sep = "")), .progress = TRUE) %>% set_names(nm = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = FALSE))

tibble_test <- test %>% unlist %>% tibble::enframe(name = "folder", value = "value")

tibble_test[which(tibble_test$value == FALSE), ] %>% .$folder %>% paste(collapse = " ")
...
list_sorted_joined_tibbles <- furrr::future_map2(
  .x = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = TRUE),
  .y = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = FALSE),
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a1 <- list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = TRUE)[[70]]
    # a2 <- list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = FALSE)[[70]]
    ###########
    
    cat(a2, "\n")
    
    # import the main result tibbles
    tibble_raw_sorted_out <- read.delim(file = paste(a1, "/psisigma_atlas_universal_r1_ir3.sorted.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE, na.strings = c("NA", "N/A", "na")) %>% as_tibble
    
    # extract info on the number of lanes
    number_of_lanes <- tibble_raw_sorted_out$`N Values` %>% .[1] %>% strsplit(split = "\\|") %>% unlist %>% length
    
    # post-import clean-up
    tibble_sorted_out_cleaned <- tibble_raw_sorted_out %>% 
      setNames(nm = c("VSR_coords", "psisigma_gene_symbol", "alternative_exon_coords", "splicemode", "no_replicates_comparison.1", paste(a2, "_no_replicates", sep = ""), "psisigma_nmd", "psisigma_reference_transcript", "dPSI", "t_test_pvalue", "t_test_FDR", "PSI_comparison.1", "PSI_comparison.2", "database_ID")) %>% 
      dplyr::relocate(database_ID, .before = 1) %>%
      tibble::add_column("LIV_ID" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\1"), 
                         "exon_number" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\2") %>% type.convert, .after = "database_ID")
    
    # import denominator tables
    tibble_denominator_table <- read.delim(file = paste(a1, "/psisigma_atlas_universal_r1_ir3.denominator.gct", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE, skip = 2, na.strings = c("NA", "N/A", "na")) %>% 
      as_tibble %>%
      tibble::add_column("LIV_ID" = gsub(x = .$Event, pattern = ".*\\:(.*ENST.*)_(.*)$", replacement = "\\1"), 
                         "exon_number" = gsub(x = .$Event, pattern = ".*\\:(.*ENST.*)_(.*)$", replacement = "\\2") %>% type.convert, .after = "Event") %>%
      dplyr::select(-5, -6, -8)

    tibble_joined <- dplyr::left_join(tibble_sorted_out_cleaned, tibble_denominator_table %>% dplyr::select(-Event, -Annotation), by = c("LIV_ID", "exon_number"))
    
    tibble_joined <- tibble_joined %>% 
      dplyr::select(-contains("comparison.1"), -contains("dPSI"), -contains("t_test"))
    
    # separate absolute PSI columns
    tibble_joined[, "PSI_comparison.2"] <- gsub(x = tibble_joined$PSI_comparison.2, pattern = "^(.*)\\|.*", replacement = "\\1") %>% type.convert
    
    colnames(tibble_joined)[colnames(tibble_joined) == "PSI_comparison.2"] <- paste(a2, "_absolute.psi", sep = "")
    colnames(tibble_joined)[ncol(tibble_joined)] <- paste(colnames(tibble_joined)[ncol(tibble_joined)], "_denominator", sep = "")
    
    # create numerator
    tibble_joined <- tibble_joined %>% 
      dplyr::mutate(!!(paste(a2, "_numerator", sep = "") %>% as.name) := (tibble_joined[, 11] %>% unlist) * (tibble_joined[, 12] %>% unlist) * 0.01)
    ...
    vector_logical_passes_read_threshold <- ((tibble_joined %>% dplyr::select(contains("numerator")) %>% unlist) > read_threshold)
    
    tibble_joined <- tibble::add_column(tibble_joined, !!(paste(a2, "_passes_read_threshold", sep = "") %>% as.name) := vector_logical_passes_read_threshold)
    
    return(tibble_joined)
    
  }, .progress = TRUE) %>% set_names(nm = list.dirs(path = psisigma_results_pooled_technical_replicates_dir, recursive = FALSE, full.names = FALSE))

# wide table join for master tibble
wide_tibble_processed_sorted_tibbles <- list_sorted_joined_tibbles %>% purrr::reduce(dplyr::full_join, by = c("database_ID", "LIV_ID", "exon_number", "VSR_coords", "psisigma_gene_symbol", "alternative_exon_coords", "splicemode", "psisigma_nmd", "psisigma_reference_transcript"))

tibble_universal_dbfile <- data.table::fread(file = paste(psisigma_results_pooled_technical_replicates_dir, "psisigma_atlas_universal.db", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = FALSE, check.names = FALSE, na.strings = c("NA", "N/A", "na"), fill = TRUE) %>% 
  as_tibble %>% 
  .[!is.na(.$V2), ] %>%
  ...
  tibble::add_column("LIV_ID" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\1"), 
                     "exon_number" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\2") %>% type.convert, 
                     "alternative_exon_coords" = paste(.$chr, ":", .$alternative_exon_start, "-", .$alternative_exon_end, sep = ""), .after = "database_ID")

# create mapping from LIV ID to concatenated alternative exon coords.
# this will account for the entirety of LIVs.
tibble_LIV_ID_to_LIV_exon_mapping <- tibble_universal_dbfile %>% 
  dplyr::arrange(alternative_exon_start) %>%
  dplyr::group_by(LIV_ID) %>%
  dplyr::summarise("LIV_exon_coords" = alternative_exon_coords %>% paste(collapse = ";"))

wide_tibble_processed_sorted_tibbles_with_LIV_info <- wide_tibble_processed_sorted_tibbles %>% 
      dplyr::left_join(., tibble_LIV_ID_to_LIV_exon_mapping, by = "LIV_ID") %>%
      dplyr::relocate(LIV_exon_coords, .after = "VSR_coords")

wide_tibble_processed_sorted_tibbles_with_LIV_info <- wide_tibble_processed_sorted_tibbles_with_LIV_info %>% 
...
# fix LIV exon coords for A3/5SS events. they're not meant to describe alternative exons to begin with.
wide_tibble_processed_sorted_tibbles_with_LIV_info[wide_tibble_processed_sorted_tibbles_with_LIV_info$splicemode %in% c("A3SS", "A5SS"), "LIV_exon_coords"] <- wide_tibble_processed_sorted_tibbles_with_LIV_info[wide_tibble_processed_sorted_tibbles_with_LIV_info$splicemode %in% c("A3SS", "A5SS"), "alternative_exon_coords"]

wide_tibble_processed_sorted_tibbles_with_LIV_info <- wide_tibble_processed_sorted_tibbles_with_LIV_info %>% 
  tibble::add_column("unprocessed_id" = 1:nrow(wide_tibble_processed_sorted_tibbles_with_LIV_info), .before = 1)

# save(wide_tibble_processed_sorted_tibbles_with_LIV_info, file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_wide_tibble_processed_sorted_tibbles_with_LIV_info_pooled_replicates.tibble", sep = ""), compress = FALSE)

data.table::fwrite(x = wide_tibble_processed_sorted_tibbles_with_LIV_info, file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_wide_tibble_processed_sorted_tibbles_with_LIV_info_pooled_replicates.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

# filter for read threshold
list_wide_tibble_processed_sorted_tibbles_with_LIV_info_read_threshold_cols_only_array_tree <- wide_tibble_processed_sorted_tibbles_with_LIV_info %>% dplyr::select(contains("_passes_read_threshold")) %>% array_tree(margin = 1)

options(mc.cores = 12)

row_indices_of_rows_passing_read_threshold <- furrr::future_map(.x = list_wide_tibble_processed_sorted_tibbles_with_LIV_info_read_threshold_cols_only_array_tree, .f = ~any(.x %>% na.omit == TRUE), .progress = TRUE) %>% unlist %>% which

# filter the raw table
wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any <- wide_tibble_processed_sorted_tibbles_with_LIV_info[row_indices_of_rows_passing_read_threshold, ] 

# add in group and feature IDs for DEXSeq anyways
wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids <- wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any %>% dplyr::group_by(VSR_coords) %>% dplyr::mutate("groupid" = which(group_data(.) %>% .$VSR_coords == `VSR_coords` %>% unique), "featureid" = 1:n()) %>% dplyr::ungroup()

data.table::fwrite(x = wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids, file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

  # write.table(x = long_tibble_processed_sorted_tibbles, file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_long_tibble_processed_sorted_tibbles_pooled_replicates.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
# long_tibble_processed_sorted_tibbles <- read.delim(file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_long_tibble_processed_sorted_tibbles_pooled_replicates.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE, na.strings = c("NA", "N/A", "na")) %>% as_tibble
...
tibble_VSR_alt_exon_info <- wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids %>% 
  dplyr::select(contains("VSR_coords"), contains("LIV_exon_coords"), contains("splicemode")) %>% 
  dplyr::mutate("chr" = `VSR_coords` %>% gsub(pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\1"),
                "VSR_start" = `VSR_coords` %>% gsub(pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\2"),
                "VSR_end" = `VSR_coords` %>% gsub(pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\3"),
                "id" = 1:nrow(.))
    ...
    vector_logical_splice_event_exists_in_SJ <- furrr::future_map2(
      ...
      .f = function(b1, b2) {
        ...
        vector_logical_splice_event_exists_in_SJ <- apply(X = b2, MARGIN = 1, FUN = function(X) {
          ...
          chr <- X[names(X) == "chr"]
          VSR_start <- X[names(X) == "VSR_start"] %>% type.convert %>% suppressMessages() %>% suppressWarnings()
          VSR_end <- X[names(X) == "VSR_end"] %>% type.convert %>% suppressMessages() %>% suppressWarnings()
          
          vector_LIV_exon_starts <- X[names(X) == "LIV_exon_coords"] %>% strsplit(split = "\\;") %>% unlist %>% gsub(pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\2") %>% type.convert %>% suppressMessages() %>% suppressWarnings()
          
          vector_LIV_exon_ends <- X[names(X) == "LIV_exon_coords"] %>% strsplit(split = "\\;") %>% unlist %>% gsub(pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\3") %>% type.convert %>% suppressMessages() %>% suppressWarnings()
...
tibble_unique_LIV_info <- wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids_filtered_SJ_dezeroed %>%
  dplyr::distinct(unprocessed_id, exon_number, VSR_coords, LIV_exon_coords, alternative_exon_coords, splicemode) %>%
  dplyr::mutate("chr" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\1"),
                
                "VSR_start" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\2"),
                "VSR_end" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\3"),
  ...
  .f = function(a1, a2, a3) {
    ...
    list_L1_results <- furrr::future_imap(
      ...
      .f = function(b1, b2) {
        ...
        vector_vertex_pool <- c(b1$VSR_start %>% type.convert, 
                                b1$VSR_end %>% type.convert, 
                                b1$LIV_exon_starts %>% strsplit(split = "\\;") %>% unlist %>% type.convert, 
                                b1$LIV_exon_ends %>% strsplit(split = "\\;") %>% unlist %>% type.convert) %>% sort
        ...
        vector_exon_starts <- c(intersect(b1$VSR_end %>% type.convert, vector_nonduplicated_vertices) + 1, 
                                intersect(b1$LIV_exon_starts %>% strsplit(split = "\\;") %>% unlist %>% type.convert, vector_nonduplicated_vertices))
        
        vector_exon_ends <- c(intersect(b1$VSR_start %>% type.convert, vector_nonduplicated_vertices) - 1, 
                              intersect(b1$LIV_exon_ends %>% strsplit(split = "\\;") %>% unlist %>% type.convert, vector_nonduplicated_vertices))
        ...
        list_ref_gtf_entries_matched_to_vertices <- list(
          "LIV_exon_starts" = purrr::map(.x = c(vector_exon_starts),
                         .f = ~L1_tibble_ref_gtf_exon_only[L1_tibble_ref_gtf_exon_only$start >= ((.x %>% type.convert) - 1) & L1_tibble_ref_gtf_exon_only$start <= ((.x %>% type.convert) + 1), ]),
          "LIV_exon_ends" = purrr::map(.x = vector_exon_ends,
                         .f = ~L1_tibble_ref_gtf_exon_only[L1_tibble_ref_gtf_exon_only$end >= ((.x %>% type.convert) - 1) & L1_tibble_ref_gtf_exon_only$end <= ((.x %>% type.convert) + 1), ])
        ) 
        ...
        if (flag_all_vertices_matched == TRUE & !b1$splicemode %in% c("A3SS", "A5SS")) {
          ...
          ## match VSR first
          list_ref_exons_matching_VSR <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$VSR_start %>% type.convert, query_end = b1$VSR_end %>% type.convert, query_strand = "*", tibble_gtf_table = L1_tibble_ref_gtf_exon_only, tolerance_left = 1, tolerance_right = 1, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = FALSE, return_type = "exon")
          
          ## construct exon start/ends
          list_battery_coords <- purrr::map2(.x = b1$LIV_exon_starts %>% strsplit(split = "\\;") %>% unlist, .y = b1$LIV_exon_ends %>% strsplit(split = "\\;") %>% unlist, .f = ~c(.x, .y) %>% type.convert)
          ...
          if (vector_ENST_ids_perfectly_matched %>% length > 0) {
            ...
            result_matched_transcript_biotype <- purrr::map(.x = vector_ENST_ids_perfectly_matched, .f = ~a2[a2$transcript_id == .x, ] %>% .$transcript_biotype %>% unique %>% sort %>% paste(collapse = ",")) %>% unlist %>% paste(collapse = ";") %>% type.convert
    ...
    .f = function(a1, a2, a3) {
      ...
      list_L1_results <- furrr::future_imap(
        ...
        .f = function(b1, b2) {
          ...
          vector_exon_starts <- c(b1$VSR_start %>% type.convert,
                                  b1$alternative_exon_end %>% type.convert + 1)
          
          vector_exon_ends <- c(b1$VSR_end %>% type.convert, 
                                b1$alternative_exon_start %>% type.convert - 1)
          
          ## match the IR junction
          list_ref_gtf_matched_to_IR_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$alternative_exon_start %>% type.convert, query_end = b1$alternative_exon_end %>% type.convert, query_strand = "*", tibble_gtf_table = a2, tolerance_left = 1, tolerance_right = 1, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
          ...
          if (list_ref_gtf_matched_to_IR_junction %>% length > 0) {
            ...
            tibble_reduced_gtf_exons_encompassing_IR_junction <- tibble_reduced_gtf[tibble_reduced_gtf$start <= (b1$alternative_exon_start %>% type.convert + 1) & tibble_reduced_gtf$end >= (b1$alternative_exon_end %>% type.convert - 1) & tibble_reduced_gtf$type == "exon", ]
            ...
            if (tibble_reduced_gtf_exons_encompassing_IR_junction %>% nrow > 0) {
              ...
              list_reduced_gtf_exons_encompassing_IR_junction_entries_matched_to_vertices <- list(
                "LIV_exon_starts" = purrr::map(.x = c(vector_exon_starts),
                                               .f = ~tibble_reduced_gtf_exons_encompassing_IR_junction[tibble_reduced_gtf_exons_encompassing_IR_junction$start >= ((.x %>% type.convert) - 1) & tibble_reduced_gtf_exons_encompassing_IR_junction$start <= ((.x %>% type.convert) + 1), ]),
                "LIV_exon_ends" = purrr::map(.x = vector_exon_ends,
                                             .f = ~tibble_reduced_gtf_exons_encompassing_IR_junction[tibble_reduced_gtf_exons_encompassing_IR_junction$end >= ((.x %>% type.convert) - 1) & tibble_reduced_gtf_exons_encompassing_IR_junction$end <= ((.x %>% type.convert) + 1), ])
              ) 
            ...
            list_ref_gtf_entries_matched_to_vertices <- list(
              "LIV_exon_starts" = purrr::map(.x = c(vector_exon_starts),
                                             .f = ~a2[a2$start >= ((.x %>% type.convert) - 1) & a2$start <= ((.x %>% type.convert) + 1), ]),
              "LIV_exon_ends" = purrr::map(.x = vector_exon_ends,
                                           .f = ~a2[a2$end >= ((.x %>% type.convert) - 1) & a2$end <= ((.x %>% type.convert) + 1), ])
            ) 
...
# export for 3FT - LIS
data.table::fwrite(x = wide_tibble_main_info_with_annotations %>% dplyr::select(VSR_coords, LIV_exon_coords, splicemode, matched_gene_names, unprocessed_id) %>% dplyr::rename("gene_name" = "matched_gene_names", "custom_identifier" = "unprocessed_id", "alternative_exon_coords" = "LIV_exon_coords") %>% dplyr::mutate("custom_identifier" = paste("atlas_totalrna_psisigma_", `custom_identifier`, sep = ""), "organism" = "homo_sapiens"), file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_LIS_export_for_3FT.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

# export for 3FT - VSR
data.table::fwrite(x = wide_tibble_main_info_with_annotations %>% dplyr::select(VSR_coords, splicemode, matched_gene_names, matched_strand, unprocessed_id) %>% dplyr::mutate("chr" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\1"), "start" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\2"), "end" = gsub(x = VSR_coords, pattern = "(.*)\\:(\\d+)\\-(.*)", replacement = "\\3")) %>% dplyr::rename("gene_name" = "matched_gene_names", "custom_identifier" = "unprocessed_id", "strand" = "matched_strand") %>% dplyr::select(-VSR_coords) %>% dplyr::mutate("custom_identifier" = paste("atlas_totalrna_psisigma_", `custom_identifier`, sep = ""), "organism" = "homo_sapiens") %>% split_delimited_column_in_table(input_table = ., target_colname = "strand", split = ";", columns_to_deduplicate = NULL), file = paste(R_processing_results_dir, "atlas_totalrna_psisigma_VSR_junctions_export_for_3FT.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
#   dplyr::select(VSR_coords, alternative_exon_coords, LIV_exon_coords, contains("combined_absolute.psi"), contains("sample_absolute.psi"), sample_name) %>%
...
#   purrr::reduce(dplyr::inner_join, by = c("VSR_coords", "alternative_exon_coords", "LIV_exon_coords", "combined_absolute.psi_1", "combined_absolute.psi_2", "combined_absolute.psi_3", "combined_absolute.psi_4", "combined_absolute.psi_5", "combined_absolute.psi_6"))
# 
# wide_tibble_matrix_processed_sorted_tibbles <- wide_tibble_processed_sorted_tibbles %>%
#   dplyr::select(-VSR_coords, -alternative_exon_coords, -LIV_exon_coords, -contains("combined"))
...
## a) Characteristic markers for each cluster (DExpSeq)
...
list_characteristic_markers_per_cluster_DExpSeq_results <- future_pmap(
  list(
    ...
    tibble_stdevs <- purrr::map2(
      .x = a3,
      .y = names(a3),
      .f = ~.x %>% apply(MARGIN = 1, FUN = function(X) {return(sd(X %>% type.convert()))} ) %>% tibble::enframe(name = NULL, value = paste("stdev_", .y, sep = ""))) %>% 
      purrr::reduce(dplyr::bind_cols) %>%
      suppressMessages() %>%
      suppressWarnings()
...
# fetch the LIS data
# tibble_LIS_info_filtered_for_at_least_5_in_any_with_dexseq_ids_for_clustering <- wide_tibble_processed_sorted_tibbles_with_LIV_info_filtered_for_at_least_5_in_any_with_dexseq_ids_for_clustering[, c("database_ID", "LIV_ID", "exon_number", "VSR_coords", "LIV_exon_coords", "psisigma_gene_symbol", "alternative_exon_coords", "splicemode", "psisigma_nmd", "psisigma_reference_transcript")]
...
tibble_characteristic_markers_for_each_som_cluster_annotated1 <- tibble_characteristic_markers_for_each_som_cluster_annotated0 %>% 
  dplyr::mutate("avg_psi" = tibble_characteristic_markers_for_each_som_cluster_annotated0 %>% dplyr::select(contains("absolute.psi")) %>% rowMeans(),
                "stdev" = tibble_characteristic_markers_for_each_som_cluster_annotated0 %>% dplyr::select(contains("absolute.psi")) %>% apply(MARGIN = 1, FUN = function(X) {return(sd(X %>% type.convert))} )) %>%
  suppressMessages() %>%
  suppressWarnings()
...
vector_chr <- gsub(x = tibble_alternative_exon_table$VSR_coords, pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\1")
vector_VSR_start <- purrr::map2(.x = gsub(x = tibble_alternative_exon_table$VSR_coords, pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\2") %>% type.convert,
                                .y = gsub(x = tibble_alternative_exon_table$VSR_coords, pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\3") %>% type.convert,
                                .f = ~min(.x, .y)) %>% unlist
vector_VSR_end <- purrr::map2(.x = gsub(x = tibble_alternative_exon_table$VSR_coords, pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\2") %>% type.convert,
                              .y = gsub(x = tibble_alternative_exon_table$VSR_coords, pattern = "^([^\\:]+)\\:([^\\-]+)\\-(\\d+)", replacement = "\\3") %>% type.convert,
                              .f = ~max(.x, .y)) %>% unlist
...
tibble_alternative_exons_chr_start_end_strand <- tibble(
  "VSR_coords" = tibble_alternative_exon_table$VSR_coords,
  ...
  .f = function(a1, a2) {
    ...
    list_per_chromosome <- future_imap(
      ...
      .f = function(b1, b2) {
        ...
        vector_VSR_exon_coords <- b1[c("vector_VSR_start", "vector_VSR_end", "vector_alt_segment_starts", "vector_alt_segment_ends")] %>% unlist %>% type.convert
        ...
        if (grepl(x = b1$splicemode, pattern = intron_retention_string) == TRUE) {
          ...
          list_matched_GTF_exon_entries <- extract_matching.exons(query_chr = b1$chr, query_start = b1$vector_VSR_start %>% type.convert, query_end = b1$vector_VSR_end %>% type.convert, query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") %>% dplyr::group_split(transcript_id) %>% set_names(nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
          ...
          if (list_matched_GTF_exon_entries %>% length == 0) {
            list_GTF_entries_matched_to_IR_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$vector_alt_segment_starts %>% type.convert, query_end = b1$vector_alt_segment_ends %>% type.convert, query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
            ...
            list_matched_GTF_exon_entries <- 
              tibble_all_candidate_overlapping_full_entries[tibble_all_candidate_overlapping_full_entries$type == "exon" &
                                                         tibble_all_candidate_overlapping_full_entries$start <= b1$vector_alt_segment_starts %>% type.convert &
                                                         tibble_all_candidate_overlapping_full_entries$end >= b1$vector_alt_segment_ends %>% type.convert, ] %>% 
          ...
          list_matched_GTF_exon_entries <- purrr::map2(.x = b1$vector_alt_segment_starts, .y = b1$vector_alt_segment_ends, .f = ~extract_matching.exons(query_chr = b1$chr, query_start = .x %>% type.convert, query_end = .y %>% type.convert, query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon"))
          ...
          common_coord <- intersect(c(b1$vector_VSR_start %>% type.convert, b1$vector_VSR_end %>% type.convert), 
                                    c(b1$vector_alt_segment_starts %>% type.convert, b1$vector_alt_segment_ends %>% type.convert))
          event_region_only_coord <- setdiff(c(b1$vector_VSR_start %>% type.convert, b1$vector_VSR_end %>% type.convert), common_coord)
          exon_only_coord <- setdiff(c(b1$vector_alt_segment_starts %>% type.convert, b1$vector_alt_segment_ends %>% type.convert), common_coord)
...
tibble_ref_entries_matched_to_exons_unmatched <- list_ref_entries_matched_to_exons %>% purrr::map(~.x$unmatched_list) %>% flatten %>% purrr::map(~.x[c("VSR_coords", "LIV_exon_coords", "splicemode", "chr", "strand", "vector_VSR_start", "vector_VSR_end", "vector_alt_segment_starts", "vector_alt_segment_ends")] %>% as_tibble) %>% rbindlist %>% as_tibble
# write
  ...
  .f = function(a1, a2) {
    ...
    future_imap(
      ...
      .f = function(b1, b2) {
        ...
        vector_genomic_coords_of_all_differential_exon_positions <- purrr::map2(.x = b1$vector_alt_segment_starts, .y = b1$vector_alt_segment_ends, .f = ~(.x %>% type.convert):(.y %>% type.convert)) %>% unlist %>% unique %>% sort
...
tibble_final_transcript_feature_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "LIV_exon_coords", "splicemode", "matched_gene_names", "vector_matched_transcript_ids", "tibble_overlapping_parent_transcript_features")] %>% as_tibble) %>%
  rbindlist %>% 
  as_tibble %>%
  setNames(c("VSR_coords", "LIV_exon_coords", "splicemode", "matched_gene_names", "vector_matched_transcript_ids", "tibble_overlapping_parent_transcript_features"))
# write table
...
tibble_final_biomart_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "LIV_exon_coords", "splicemode", "matched_gene_names", "biomart_annotations")] ) %>%
...
tibble_final_PTM_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "LIV_exon_coords", "splicemode", "matched_gene_names", "PTMs")] ) %>%
...
tibble_final_annotation_supp <- list_tidied_annotation %>% 
  furrr::future_map(~.x[c("VSR_coords", "LIV_exon_coords", "splicemode", "matched_gene_names", "vector_genomic_coords_of_all_differential_exon_positions", "list_differential_CDS_positions", "vector_matched_transcript_ids", "protein_relative_differential_coords")], .progress = TRUE) %>%
  ...
  .f = function(a1, a2) {
    ...
    furrr::future_map(
      ...
      .f = function(b1) {
        ...
        return(
          list(
            "transcript_ontology" = dplyr::left_join(b1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_transcript_feature_annotation) %>% unique,
            "biomart" = dplyr::left_join(b1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_biomart_annotation) %>% unique,
            "dbPTM" = dplyr::left_join(b1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_PTM_annotation) %>% unique
          )
        )
      ...
      list_positive_negative_markers_result <- purrr::map2(
        ...
        .f = function(b1, b2) {
          ...
          tibble_biomart_ontology_summary <- tibble(
            "frequency_interpro" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `interpro` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_mobidblite" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `mobidblite` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_seg" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `seg_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_ncoils" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `ncoils_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_tmhmm" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `tmhmm_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_signalp" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `signalp_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
            "frequency_sifts" = b1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `sifts_import` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
          ) %>%
            tibble::add_column(
              "cluster_name" = a2,
              "positive_or_negative_markers" = b2,
      ...
      .f = function(b1, b2) {
        ...
        furrr::future_map2(
          ...
          .f = function(c1, c2) {
            ...
            return(
              list(
                "transcript_ontology" = dplyr::left_join(c1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_transcript_feature_annotation) %>% unique,
                "biomart" = dplyr::left_join(c1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_biomart_annotation) %>% unique,
                "dbPTM" = dplyr::left_join(c1[, c("VSR_coords", "LIV_exon_coords", "splicemode")] %>% unique, tibble_final_PTM_annotation) %>% unique
              )
            )
          ...
          list_positive_negative_markers_result <- purrr::map2(
            ...
            .f = function(c1, c2) {
              ...
              if (nrow(c1$biomart) > 0) {
                tibble_biomart_ontology_summary <- tibble(
                  "frequency_interpro" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `interpro` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_mobidblite" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `mobidblite` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_seg" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `seg_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_ncoils" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `ncoils_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_tmhmm" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `tmhmm_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_signalp" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `signalp_percentage_overlap` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                  "frequency_sifts" = c1$biomart %>% dplyr::group_by(VSR_coords, LIV_exon_coords, splicemode) %>% dplyr::summarise("exists" = `sifts_import` %>% na.omit %>% length > 0) %>% .$exists %>% which %>% length,
                ) %>%
                  tibble::add_column(
                    "cluster_name" = a2,
                    "comparison_name" = b2,
