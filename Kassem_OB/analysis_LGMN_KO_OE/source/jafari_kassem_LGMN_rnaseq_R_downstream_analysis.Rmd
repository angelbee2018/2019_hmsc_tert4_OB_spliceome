...
list_sorted_joined_tibbles <- furrr::future_map2(
  .x = list.dirs(path = psisigma_results_dir, recursive = FALSE, full.names = TRUE),
  .y = list.dirs(path = psisigma_results_dir, recursive = FALSE, full.names = FALSE),
  .f = function(a1, a2) {
    
    # DEBUG ###
    a1 <- list.dirs(path = psisigma_results_dir, recursive = FALSE, full.names = TRUE) %>% .[[1]]
    a2 <- list.dirs(path = psisigma_results_dir, recursive = FALSE, full.names = FALSE) %>% .[[1]]
    ###########
    
    cat(a2, "\n")
    
    # import the main result tibbles
    tibble_raw_sorted_out <- read.delim(file = paste(a1, "/", a2, "_r1_ir3.sorted.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE, na.strings = c("NA", "N/A", "na")) %>% as_tibble
    ...
    tibble_denominator_table <- read.delim(file = paste(a1, "/", a2, "_r1_ir3.denominator.gct", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE, skip = 2, na.strings = c("NA", "N/A", "na")) %>% 
      as_tibble %>%
    ...
    tibble_joined <- tibble_joined %>% 
      tidyr::separate(col = "PSI_comparison.1", into = paste("absolute.psi_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\2"), "_", 1:3, sep = ""), sep = "\\|") %>% type.convert(na.strings = c("NA", "N/A", "na")) %>%
      tidyr::separate(col = "PSI_comparison.2", into = paste("absolute.psi_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\1"), "_", 1:3, sep = ""), sep = "\\|") %>% type.convert(na.strings = c("NA", "N/A", "na")) 
    
    colnames(tibble_joined)[21:23] <- paste("denominator_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\2"), "_", 1:3, sep = "")
    colnames(tibble_joined)[24:26] <- paste("denominator_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\1"), "_", 1:3, sep = "")
    
    # create numerator - DON'T NEED IT BECAUSE WE'RE NEVER DOING DEXSEQ EVER AGAIN. IT SUCKS.
    tibble_joined <- tibble_joined %>% 
      dplyr::mutate(!!(paste("numerator_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\1"), sep = "") %>% as.name) := type.convert(apply(X = tibble_joined[, 24:26], MARGIN = 1, FUN = function(X) {X %>% na.omit %>% mean} ) ) * type.convert(apply(X = tibble_joined[, 18:20], MARGIN = 1, FUN = function(X) {X %>% na.omit %>% mean} ) ) * 0.01) %>% 
      dplyr::mutate(!!(paste("numerator_", gsub(x = a2, pattern = "(.*)_minus_(.*)", replacement = "\\2"), sep = "") %>% as.name) := type.convert(apply(X = tibble_joined[, 21:23], MARGIN = 1, FUN = function(X) {X %>% na.omit %>% mean} ) ) * type.convert(apply(X = tibble_joined[, 15:17], MARGIN = 1, FUN = function(X) {X %>% na.omit %>% mean} ) ) * 0.01)
