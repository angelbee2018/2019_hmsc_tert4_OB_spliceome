...
list_ref_matching_exon_entries_not_IR <- future_map(.x = wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_not_IR, .f = function(a1, a2) {
  ...
  if (length(raw_reference_gtf_match) == 0) {
    ...
    tibble_matched_exons_ref <- purrr::map(.x = list_ref_parent_transcript_entries, 
                                        .f = function(b1) {
  ...
  # RECON. GTF ###
  # for the recon. GTF matching, match the VSR first then get the parent transcript entries then do exact exon matching.
  ...
  ## retrieve all entries from the parent transcript_id
  tibble_recon_parent_transcript_entries <- vector_recon_matched_parent_transcript_ids %>% purrr::map(.f = ~tibble_recon_gtf[which(tibble_recon_gtf$transcript_id == .x & tibble_recon_gtf$type == "exon"), ]) %>% rbindlist(use.names = TRUE) %>% as_tibble
  
  tibble_matched_exons_recon <- extract_overlapping.exons(query_chr = a1$chr %>% type.convert, query_start = a1$diff_exon_start %>% type.convert, query_end = a1$diff_exon_end %>% type.convert, query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon")
  
  return(splice(a1,
                "reference_gtf_match" = list(tibble_matched_exons_ref),
                "reconstructed_gtf_match" = list(tibble_matched_exons_recon)))
...
tibble_complete_grid <- tidyr::expand_grid("splicemode" = tibble_exons_per_category_summary$splicemode %>% unique, 
...
# melt the sample
long_tibble_exons_per_category_summary <- reshape2::melt(tibble_exons_per_category_summary_completed, id.vars = c("splicemode", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts", "first_exon_reference", "last_exon_reference"), variable.name = "sample", value.name = "number_of_exons_per_category") %>% as_tibble %>% 
...
list_exons_for_query_non.IR <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::filter(!str_detect(string = splicemode, pattern = "IR")) %>% (function(input_table) {
  ...
  output_list <- input_table[, c("diff_exon_coords", "chr", "diff_exon_start", "diff_exon_end", "matched_strand")] %>% 
    as_tibble %>% 
    unique %>%
    array_tree
...
## get chromosomes
vector_of_chromosomes_to_pluck <- list_genome_relative_differential_coords %>% purrr::map(~.x$chr) %>% unlist %>% unique
...
list_plucked_differential_exonic_positions_by_chromosome <- purrr::map(.x = vector_of_chromosomes_to_pluck, .f = function(a1) {
  ...
  vector_all_exonic_positions <- purrr::keep(.x = list_genome_relative_differential_coords, .p = ~.x$chr == a1) %>%
...
list_genome_relative_differential_coords <- purrr::map(.x = list_genome_relative_exonic_coords,
                                                       .f = function(a1) {
                                                         ...
                                                         list_a1_modified <- purrr::modify_at(.x = a1, 
                                                                                              ...
                                                                                              .f = ~intersect(.x, list_plucked_differential_exonic_positions_by_chromosome[[a1$chr]]))
