...
alltimepoints_reconstructed_gtf_path <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_strawberry/results_assemblyonly/merged/alltimepoints_denovo_reconstructed_stringtiemerged_exon.only_with_first.last.gtf"
...
no.exons_differentially.spliced_complete.timeseries_sum <- nrow(wide_tibble_of_absolute_PSI_averages_complete.timeseries_differential_sum)
...
list_of_vectors_genesets_per_SOM_cluster <- purrr::map_depth(.x = list_of_tibble_SOM.separate.PSI_subset_by_cluster, .depth = 2, .ragged = FALSE, .f = ~.x$matched_gene_names)
...
purrr::map2(.x = list_of_tibbles_SOM_hyperGOresult_clusterwise_top.hits, .y = list_of_combinations_of_SOM_genes_and_GOterms_names, .f = ~ggplot(.x, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[2], "_exons_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[1], "_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_", .y[[2]], "_GO.pdf", sep = ""), device = "pdf", dpi = 600, width = 50, height = 30, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[2], "_exons_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[1], "_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_", .y[[2]], "_GO.svg", sep = ""), device = "svg", dpi = 600, width = 50, height = 30, units = "cm"))

purrr::map2(.x = list_of_tibbles_SOM_hyperGOresult_clusterwise_top.hits, .y = list_of_combinations_of_SOM_genes_and_GOterms_names, .f = ~write.table(x = .x, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[2], "_exons_", .y[[1]] %>% strsplit(split = "_") %>% unlist %>% .[1], "_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_", .y[[2]], "_GO.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE))
...
# ENRICHMENT OF UPSTREAM SPLICING FACTORS
...
pdf(paste(qualitycheck_results_dir, "qualityplot_pvalues_density.distribution.pdf", sep = "")) 
  ...
  plot(density(wide_tibble_of_psisigma_results_allcomparisons_final[wide_tibble_of_psisigma_results_allcomparisons_final$comparison == "BM_MSC_to_OB_12d_vs_BM_MSC_to_OB_12h", "pvalue"] %>% na.omit %>% unlist))
  
  dev.off()
  
    # plot all the absolute PSI values
  plot(density(wide_tibble_of_psisigma_results_allcomparisons_final[wide_tibble_of_psisigma_results_allcomparisons_final$comparison == "BM_MSC_to_OB_12d_vs_BM_MSC_to_OB_12h" & wide_tibble_of_psisigma_results_allcomparisons_final$pvalue < 0.4 & wide_tibble_of_psisigma_results_allcomparisons_final$pvalue > 0.35, c(1, vector_col.index_absolute.psi.comparison.1, vector_col.index_absolute.psi.comparison.2)] %>% melt(id.vars = "event_region_coords", variable.name = "comparison.replicate", value.name = "absolute.psi") %>% .$absolute.psi %>% na.omit))
