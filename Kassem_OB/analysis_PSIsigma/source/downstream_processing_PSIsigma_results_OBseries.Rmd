...
tibble_recon_gtf <- rtracklayer::import("Z:/hg38_ensembl_reference/gtf/alltimepoints_denovo_reconstructed_stringtiemerged_NMD_first.last.gtf") %>% as_tibble
...
# tibble_recon_gtf <- rtracklayer::import("/media/Ubuntu/sharedfolder/hg38_ensembl_reference/gtf/alltimepoints_denovo_reconstructed_stringtiemerged_NMD_first.last.gtf") %>% as_tibble
...
annotate_differential_exon <- function(list_matched_ref_recon_gtf) {
  ...
  return(list("event_region_coords" = list_matched_ref_recon_gtf$event_region_coords,
              "diff_exon_coords" = list_matched_ref_recon_gtf$diff_exon_coords,
...
list_of_psisigma_result_tibbles_raw <- future_map(.x = list_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "/", .x, "_r5_ir3.sorted.txt", sep = "")) %>% as_tibble, .progress = TRUE)
...
wide_tibble_of_psisigma_results_allcomparisons_processed[, vector_col.index_absolute.psi.comparison.1] <- future_apply(X = wide_tibble_of_psisigma_results_allcomparisons_processed[, vector_col.index_absolute.psi.comparison.1], MARGIN = 1, FUN = function(X) {sort(X, na.last = TRUE)}) %>% t %>% as_tibble

wide_tibble_of_psisigma_results_allcomparisons_processed[, vector_col.index_absolute.psi.comparison.2] <- future_apply(X = wide_tibble_of_psisigma_results_allcomparisons_processed[, vector_col.index_absolute.psi.comparison.2], MARGIN = 1, FUN = function(X) {sort(X, na.last = TRUE)}) %>% t %>% as_tibble
...
wide_tibble_of_all_unique_exon_coords <- wide_tibble_of_psisigma_results_allcomparisons_processed[, c("event_region_coords", "splicemode", "diff_exon_coords", "diff_exon_chr", "diff_exon_start", "diff_exon_end")] %>% unique
...
list_ref_matching_exon_entries_IR <- future_imap(.x = wide_tibble_of_all_unique_exon_coords_array.tree_IR, .f = ~list(
  ...
  "event_region_coords" = .x$event_region_coords, 
  "diff_exon_coords" = .x$diff_exon_coords,
  "splicemode" = .x$splicemode), 
  .progress = TRUE, globals = c("tibble_ref_gtf", "tibble_recon_gtf", "extract_junction.flanking.exons"))

list_ref_matching_exon_entries_IR <- future_imap(.x = list_ref_matching_exon_entries_IR, .f = ~annotate_differential_exon(.x) %>% as_tibble)
...
list_ref_matching_exon_entries_not_IR <- future_imap(.x = wide_tibble_of_all_unique_exon_coords_array.tree_not_IR, .f = ~list(
  ...
  "diff_exon_coords" = .x$diff_exon_coords,
  "splicemode" = .x$splicemode), 
  .progress = TRUE, globals = c("tibble_ref_gtf", "tibble_recon_gtf", "extract_overlapping.exons"))

list_ref_matching_exon_entries_not_IR <- future_imap(.x = list_ref_matching_exon_entries_not_IR, .f = ~annotate_differential_exon(.x) %>% as_tibble)
...
### label the reference info. with the processed result tables.

```{r}

tibble_ref_matching_exon_entries_all <- read.delim(file = paste(R_processing_results_dir, "table_all_exons_reference_match_info.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

wide_tibble_of_psisigma_results_allcomparisons_final <- dplyr::left_join(wide_tibble_of_psisigma_results_allcomparisons_processed, tibble_ref_matching_exon_entries_all, by = c("event_region_coords", "diff_exon_coords", "splicemode"))

# create column of definitive NMD or not, and definitive first/last exon
wide_tibble_of_psisigma_results_allcomparisons_final <- wide_tibble_of_psisigma_results_allcomparisons_final %>% add_column(., "NMD_or_not" = FALSE, "first_exon" = FALSE, "last_exon" = FALSE)

wide_tibble_of_psisigma_results_allcomparisons_final[union(which(wide_tibble_of_psisigma_results_allcomparisons_final$NMD_reference == TRUE), 
                                                               which(wide_tibble_of_psisigma_results_allcomparisons_final$NMD_recon == TRUE)), "NMD_or_not"] <- TRUE
  
wide_tibble_of_psisigma_results_allcomparisons_final[union(grep(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_reference, pattern = "first_exon"), 
                                                           grep(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_recon, pattern = "first_exon")), "first_exon"] <- TRUE

wide_tibble_of_psisigma_results_allcomparisons_final[union(grep(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_reference, pattern = "last_exon"), 
                                                           grep(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_recon, pattern = "last_exon")), "last_exon"] <- TRUE

# reorder columns
wide_tibble_of_psisigma_results_allcomparisons_final <- wide_tibble_of_psisigma_results_allcomparisons_final[, c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode", "psisigma_gene.symbol", "psisigma_reference.transcript", "psisigma_exon_type", "diff_exon_chr", "diff_exon_start", "diff_exon_end", "matched_gene_names", "matched_recon_transcript_names", "matched_strand", "NMD_reference", "NMD_recon", "NMD_or_not", "first_or_last_exon_reference", "first_or_last_exon_recon", "first_exon", "last_exon", "num.replicates_comparison.1", "num.replicates_comparison.2", "pvalue", "FDR", "dpsi", "comparison.1_absolute.psi_1", "comparison.1_absolute.psi_2", "comparison.1_absolute.psi_3", "comparison.2_absolute.psi_1", "comparison.2_absolute.psi_2", "comparison.2_absolute.psi_3", "comparison")]
...
## ALERT! SUM USED AS FUN.AGGREGATE. it means we'll be getting PSI values for the whole exon, not LSV.
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only <- reshape2::dcast(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp %>% unique, formula =  database_ID + event_region_coords + diff_exon_coords + splicemode + psisigma_gene.symbol + diff_exon_chr + diff_exon_start + diff_exon_end + matched_gene_names + matched_recon_transcript_names + matched_strand + NMD_reference + NMD_recon + NMD_or_not + first_or_last_exon_reference + first_or_last_exon_recon + first_exon + last_exon + ud_absolute.psi_1 + ud_absolute.psi_2 + ud_absolute.psi_3 ~ timepoint + replicate, value.var = "absolute_psi") %>% as_tibble
...
tibble_exons_per_category_all <- wide_tibble_of_psisigma_results_allcomparisons_final %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2", perl = TRUE)} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, NMD_or_not, first_exon, last_exon) %>% summarise(no_exons_all = n())

all_exons_count <- tibble_exons_per_category_all$no_exons_all %>% sum(na.rm = TRUE)
number_NMD_all_exons <- tibble_exons_per_category_all[tibble_exons_per_category_all$NMD_or_not == TRUE, ] %>% .$no_exons_all %>% sum(na.rm = TRUE)
number_first_exon_all_exons <- tibble_exons_per_category_all[tibble_exons_per_category_all$first_exon == TRUE, ] %>% .$no_exons_all %>% sum(na.rm = TRUE)
number_last_exon_all_exons <- tibble_exons_per_category_all[tibble_exons_per_category_all$last_exon == TRUE, ] %>% .$no_exons_all %>% sum(na.rm = TRUE)
...
tibble_exons_per_category_constitutive <- tibble_constitutive_LSV_info %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, NMD_or_not, first_exon, last_exon) %>% summarise(no_exons_constitutive = n())

constitutive_exons_count <- tibble_exons_per_category_constitutive$no_exons_constitutive %>% sum(na.rm = TRUE)
number_NMD_constitutive_exons <- tibble_exons_per_category_constitutive[tibble_exons_per_category_constitutive$NMD_or_not == TRUE, ] %>% .$no_exons_constitutive %>% sum(na.rm = TRUE)
number_first_exon_constitutive_exons <- tibble_exons_per_category_constitutive[tibble_exons_per_category_constitutive$first_exon == TRUE, ] %>% .$no_exons_constitutive %>% sum(na.rm = TRUE)
number_last_exon_constitutive_exons <- tibble_exons_per_category_constitutive[tibble_exons_per_category_constitutive$last_exon == TRUE, ] %>% .$no_exons_constitutive %>% sum(na.rm = TRUE)
...
tibble_exons_per_category_differential <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, NMD_or_not, first_exon, last_exon) %>% summarise(no_exons_differential = n())

differential_exons_count <- tibble_exons_per_category_differential$no_exons_differential %>% sum(na.rm = TRUE)
number_NMD_differential_exons <- tibble_exons_per_category_differential[tibble_exons_per_category_differential$NMD_or_not == TRUE, ] %>% .$no_exons_differential %>% sum(na.rm = TRUE)
number_first_exon_differential_exons <- tibble_exons_per_category_differential[tibble_exons_per_category_differential$first_exon == TRUE, ] %>% .$no_exons_differential %>% sum(na.rm = TRUE)
number_last_exon_differential_exons <- tibble_exons_per_category_differential[tibble_exons_per_category_differential$last_exon == TRUE, ] %>% .$no_exons_differential %>% sum(na.rm = TRUE)
...
# melt the sample
long_tibble_exons_per_category_summary <- reshape2::melt(tibble_exons_per_category_summary, id.vars = c("splicemode", "NMD_or_not", "first_exon", "last_exon"), variable.name = "sample", value.name = "number_of_exons_per_category") %>% as_tibble %>% 
...
ggplot() + 
  ...
  scale_shape_manual(name = "Category", limits = c("NMD_or_not", "first_exon", "last_exon"), labels = c("NMD candidate", "First exon", "Last exon"), values = c(1, 4, 19)) + 
  theme_bw() +
  theme(text = element_text(family = "Helvetica")) +
...
ComplexHeatmap::UpSet(upset_combs, 
                      ...
                      left_annotation = rowAnnotation("Set Type" = c("NMD_or_not" = "Exon ontology",
                                                                     "first_exon" = "Exon ontology",
                                                                     "last_exon" = "Exon ontology",
                                                                     "A3SS" = "Splicemode",
                                                                     "A5SS" = "Splicemode",
                                                                     "IR" = "Splicemode",
                                                                     "MES" = "Splicemode",
...
# MAPPING/ANNOTATING DIFFERENTIALL EXONS WITH PROTEIN DOMAIN/MOTIF/STRUCTURE INFORMATION
...
tibble_ref_gtf <- rtracklayer::import("/media/Ubuntu/sharedfolder/hg38_ensembl_reference/gtf/Homo_sapiens.GRCh38.98.gtf") %>% as_tibble
# tibble_ref_gtf <- rtracklayer::import("Z:/hg38_ensembl_reference/gtf/Homo_sapiens.GRCh38.98.gtf") %>% as_tibble

tibble_ref_gtf_simplified <- tibble_ref_gtf %>% dplyr::select(-source, -score, -gene_version, -gene_source, -transcript_version, -transcript_source, -tag, -transcript_support_level, -exon_version, -protein_version, -ccds_id) %>% unique

colnames(tibble_ref_gtf_simplified) <- c("chr", "start", "end", "width", "strand", "type", "phase", "gene_id", "gene_name", "gene_biotype", "transcript_id", "transcript_name", "transcript_biotype", "exon_number", "exon_id", "protein_id") 

tibble_ref_gtf_simplified <- tibble_ref_gtf_simplified %>% mutate_if(is.factor, as.character)

# import dbPTM info
# Z:/dbPTM_download/extract/allPTM_human.txt
dbPTM_allhuman_annotations <- read.delim("/media/Ubuntu/sharedfolder/dbPTM_download/extract/allPTM_human.txt", col.names = c("uniprotkb_entryname", "uniprotkb_entry", "modified_residue_position", "PTM_type", "pubmed_accessions", "context_sequence"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble
...
phosphosite_allhuman_phosphosites <- read.delim("/media/Ubuntu/sharedfolder/phosphositeplus_phosphosites_human.tab", col.names = c("gene_name", "protein_name", "uniprotkb_entry", "chr_locus", "modified_residue_position", "SITE_GRP_ID", "organism", "MW.kD", "domain", "site_7AA_either.side", "LT_LIT", "MS_LIT", "MS_CST", "CST_CAT"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble
# phosphosite_allhuman_phosphosites <- read.delim("Z:/phosphositeplus_phosphosites_human.tab", col.names = c("gene_name", "protein_name", "uniprotkb_entry", "chr_locus", "modified_residue_position", "SITE_GRP_ID", "organism", "MW.kD", "domain", "site_7AA_either.side", "LT_LIT", "MS_LIT", "MS_CST", "CST_CAT"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

# change the residue position column
phosphosite_allhuman_phosphosites[, "modified_residue_position"] <- gsub(x = phosphosite_allhuman_phosphosites$modified_residue_position, pattern = "^([A-Z])([0-9]{1,5})(.*)", replacement = "\\2")

# remove rows where phosphosite is NA
row.indices_phosphosite.is.na <- which(is.na(phosphosite_allhuman_phosphosites$modified_residue_position))

phosphosite_allhuman_phosphosites <- phosphosite_allhuman_phosphosites[-row.indices_phosphosite.is.na, ]
...
# sometimes, PSI-Sigma seems to call novel splice junctions that are slightly off by at most 2 nt. If we expand the genomic coordinates by 2 on each side, we can correct for this.

enable_greedy_genome_coordinates <- function(tibble_chr_strand_start_end) {
  
  tibble_chr_strand_start_end[, "start"] <- tibble_chr_strand_start_end$start - 2
  tibble_chr_strand_start_end[, "end"] <- tibble_chr_strand_start_end$end + 2
  
  return(tibble_chr_strand_start_end)
  
}

# all differential exons (plausible)
# first process the table. it's not hard. just need to make into names: "Gene", <identifier>, "chr", "strand", "start", "end"
tibble_all_diff_exons_chr_strand_start_end <- wide_tibble_of_psisigma_results_allcomparisons_differential[, c("matched_gene_names", "diff_exon_coords", "splicemode", "diff_exon_chr", "matched_strand", "diff_exon_start", "diff_exon_end")] %>% 
  setNames(c("Gene", "diff_exon_coords", "splicemode", "chr", "strand", "start", "end")) %>% 
  dplyr::distinct(chr, strand, start, end, .keep_all = TRUE) %>% 
  add_column("width" = .$end - .$start + 1) %>% 
  dplyr::arrange(desc(width))
## PLAUSIBLE FILTERING
tibble_all_diff_exons_chr_strand_start_end_plausible <- tibble_all_diff_exons_chr_strand_start_end %>% filtering_chr_strand_start_end_tibble_for_plausible_AS_events

message("total number of unique exons queried: ", tibble_all_diff_exons_chr_strand_start_end_plausible$diff_exon_coords %>% unique %>% length)

all_diff_plausible_exons_consequence <- tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain(tibble_all_diff_exons_chr_strand_start_end_plausible, gtf_table = tibble_ref_gtf_simplified, ensembl_mart)

tibble_all_diff_plausible_exons_transcript_level_results <- all_diff_plausible_exons_consequence$transcript_level_results

tibble_transcript_level_summary_count <- tibble_all_diff_plausible_exons_transcript_level_results %>% dplyr::group_by(diff_exon_coords) %>% 
  dplyr::summarise(number_protein_coding = any(transcript_biotype == "protein_coding"),
                   number_lncrna = any(transcript_biotype == "lncRNA"),
                   number_micrna = any(transcript_biotype == "miRNA"),
                   number_three_prime_utr = any(type == "three_prime_utr"),
                   number_five_prime_utr = any(type == "five_prime_utr"),
                   number_CDS = any(type == "CDS"))

message("total number of unique differential plausible exons with any sort of transcript-level annotation: ", nrow(tibble_transcript_level_summary_count))

message("percent of differential plausible exons overlapping a protein coding transcript: ", length(which(tibble_transcript_level_summary_count$number_protein_coding == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential plausible exons overlapping an lncRNA transcript: ", length(which(tibble_transcript_level_summary_count$number_lncrna == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential plausible exons overlapping a miRNA transcript: ", length(which(tibble_transcript_level_summary_count$number_micrna == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)

message("percent of differential plausible exons overlapping a 3' UTR region: ", length(which(tibble_transcript_level_summary_count$number_three_prime_utr == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential plausible exons overlapping a 5' UTR region: ", length(which(tibble_transcript_level_summary_count$number_five_prime_utr == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)
message("percent of differential plausible exons overlapping a CDS region: ", length(which(tibble_transcript_level_summary_count$number_CDS == TRUE))/nrow(tibble_transcript_level_summary_count) * 100)

tibble_all_diff_plausible_exons_protein_level_results <- all_diff_plausible_exons_consequence$protein_level_results

unique_interpro_ids <- tibble_all_diff_plausible_exons_protein_level_results$interpro %>% .[tibble_all_diff_plausible_exons_protein_level_results$interpro != "FALSE"]  %>% strsplit(split = ";") %>% unlist %>% unique

tibble_protein_level_summary_count <- tibble_all_diff_plausible_exons_protein_level_results %>% dplyr::group_by(diff_exon_coords) %>% 
  dplyr::summarise(overlaps_interpro_domain = any(interpro != "FALSE"),
                   unique_interpro_ids = interpro %>% strsplit(split = ";") %>% unlist %>% unique %>% .[. != "FALSE"] %>% paste(collapse = ";"),
                   overlaps_LCR = any(seg != "FALSE"),
                   overlaps_disordered_region = any(mobidblite != "FALSE"),
                   overlaps_ncoils = any(ncoils != "FALSE"),
                   overlaps_signalp = any(signalp != "FALSE"),
                   overlaps_tmhmm = any(tmhmm != "FALSE"))
...
ggplot(tibble_interpro_id_tally_per_ASeventID %>% head(90), aes(x = reorder(interpro_identifier, -tally), y = tally)) +
  ...
  ggtitle(paste("Tally of the 90 most overlapping interpro identifiers of", nrow(tibble_protein_level_summary_count), "unique differential plausible exons", dpsi_cutoff, "and any sig", pvalue_cutoff, pvalue_or_FDR)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_exons_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_exons_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 15, units = "cm") 

write.table(x = tibble_interpro_id_tally_per_ASeventID, file = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_exons_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

message("total number of unique plausible exons structures with any sort of protein-level annotation: ", nrow(tibble_protein_level_summary_count), " (", nrow(tibble_protein_level_summary_count)*100/nrow(tibble_transcript_level_summary_count), "% of those which have any transcript annotation)")

message("total percent of unique plausible exons structures overlapping an interpro domain: ", length(which(tibble_protein_level_summary_count$overlaps_interpro_domain == TRUE))/nrow(tibble_protein_level_summary_count) * 100)

message("total percent of unique plausible exons structures overlapping a low complexity region: ", length(which(tibble_protein_level_summary_count$overlaps_LCR == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique plausible exons structures overlapping a disordered region: ", length(which(tibble_protein_level_summary_count$overlaps_disordered_region == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique plausible exons structures overlapping a coiled-coiled domain: ", length(which(tibble_protein_level_summary_count$overlaps_ncoils == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique plausible exons structures overlapping a signalp domain: ", length(which(tibble_protein_level_summary_count$overlaps_signalp == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
message("total number of unique plausible exons structures overlapping a tmhmm domain: ", length(which(tibble_protein_level_summary_count$overlaps_tmhmm == TRUE))/nrow(tibble_protein_level_summary_count) * 100)
