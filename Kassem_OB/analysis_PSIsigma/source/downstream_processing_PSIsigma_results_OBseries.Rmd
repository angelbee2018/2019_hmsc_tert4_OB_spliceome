...
alltimepoints_reconstructed_gtf_path <- "Z:/hg38_ensembl_reference/gtf/Homo_sapiens.GRCh38.98.gtf_exon.only_with_first.last.gtf"
...
# reorder columns
wide_tibble_of_psisigma_results_allcomparisons_final <- wide_tibble_of_psisigma_results_allcomparisons_final[, c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode", "psisigma_gene.symbol", "psisigma_reference.transcript", "psisigma_exon_type", "diff_exon_chr", "diff_exon_start", "diff_exon_end", "matched_gene_names", "matched_recon_transcript_names", "matched_strand", "NMD_or_not", "first_or_last_exon_reference", "first_or_last_exon_recon", "num.replicates_comparison.1", "num.replicates_comparison.2", "pvalue", "FDR", "dpsi", "comparison.1_absolute.psi_1", "comparison.1_absolute.psi_2", "comparison.1_absolute.psi_3", "comparison.2_absolute.psi_1", "comparison.2_absolute.psi_2", "comparison.2_absolute.psi_3", "comparison")]
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only <- reshape2::dcast(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp %>% unique, formula =  database_ID + event_region_coords + diff_exon_coords + splicemode + psisigma_gene.symbol + diff_exon_chr + diff_exon_start + diff_exon_end + matched_gene_names + matched_recon_transcript_names + matched_strand + NMD_or_not + first_or_last_exon_reference + first_or_last_exon_recon + ud_absolute.psi_1 + ud_absolute.psi_2 + ud_absolute.psi_3 ~ timepoint + replicate, value.var = "absolute_psi") %>% as_tibble
...
# category means splicemode + NMD status(reference only at this stage) + first/last exon(reference only at this stage)
# also clean up the splicemode column
tibble_exons_per_category_constitutive <- wide_tibble_of_psisigma_results_allcomparisons_constitutive %>% 
  add_column(., "splice_NMD_first.last" = paste(gsub(x = wide_tibble_of_psisigma_results_allcomparisons_constitutive$splicemode, pattern = "^([^\\||_]+).*", replacement = "\\1"), " ", "NMD_", wide_tibble_of_psisigma_results_allcomparisons_constitutive$NMD_or_not, " ", wide_tibble_of_psisigma_results_allcomparisons_constitutive$first_or_last_exon_reference, sep = "")) %>% 
  dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splice_NMD_first.last) %>% summarise(no_exons_constitutive = n())

str_split_fixed(before$type, "_and_", 2)

tibble_exons_per_category_differential <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% 
  add_column(., "splice_NMD_first.last" = paste(gsub(x = wide_tibble_of_psisigma_results_allcomparisons_differential$splicemode, pattern = "^([^\\||_]+).*", replacement = "\\1"), " ", "NMD_", wide_tibble_of_psisigma_results_allcomparisons_differential$NMD_or_not, " ", wide_tibble_of_psisigma_results_allcomparisons_differential$first_or_last_exon_reference, sep = "")) %>% 
  dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splice_NMD_first.last) %>% summarise(no_exons_differential = n())

tibble_exons_per_category_summary <- dplyr::full_join(tibble_exons_per_category_constitutive, tibble_exons_per_category_differential, by = "splice_NMD_first.last")

long_tibble_exons_per_category_summary <- reshape2::melt(tibble_exons_per_category_summary, id.vars = "splice_NMD_first.last") %>% as_tibble

# ggplot

ggplot(long_tibble_exons_per_category_summary) + 
geom_col(aes(y = value, x = splice_NMD_first.last)) +
facet_wrap(~variable, scales = "free") +
geom_label(data = long_tibble_exons_per_category_summary, aes(label = value, y = value, x = splice_NMD_first.last)) +
ggtitle(paste("frequency distribution of exons per category 
              ", pvalue_or_FDR, pvalue_cutoff, "_dpsi", dpsi_cutoff, sep = "")) +
xlab("Exon category (splicemode, NMD, first/last exon") +
...
ggsave(filename = paste(qualitycheck_results_dir, "scatterplot_no._exons_vs_max_transcript_length_", pvalue_or_FDR, pvalue_cutoff, "_dpsi", dpsi_cutoff, ".pdf", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm") 
