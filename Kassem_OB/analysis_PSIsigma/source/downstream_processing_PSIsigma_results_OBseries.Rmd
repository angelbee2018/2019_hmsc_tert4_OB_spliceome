...
# library(biomaRt)
# ensembl_mart = useMart(biomart = "ensembl", dataset = "hsapiens_gene_ensembl", host = "asia.ensembl.org")
# , version = 98
...
list_of_psisigma_result_tibbles_raw <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "_r5_ir3.sorted.txt", sep = "")) %>% as_tibble, .progress = TRUE, .options = future_options(globals = c("as_tibble")))
...
# write table of all differential exons info - for all timepoints and not just the significant timepoint.
## at the level of LSVs
wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV <- dplyr::semi_join(wide_tibble_of_psisigma_results_allcomparisons_final, wide_tibble_of_psisigma_results_allcomparisons_differential, by = c("database_ID", "event_region_coords", "diff_exon_coords"))

write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV, file = paste(R_processing_results_dir, "long_table_of_all_differential_exons_info_dpsi_", dpsi_cutoff,  "_any", pvalue_or_FDR, pvalue_cutoff, "_", number.of.exons_differential_anysig_with_na, "_exons_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# export associated gene set
write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV$matched_gene_names %>% unique %>% na.omit, file = paste(R_processing_results_dir, "geneset_of_differential_results_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV$matched_gene_names %>% unique %>% na.omit %>% length, "_genes_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)

## at the level of exons.
wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons <- dplyr::semi_join(wide_tibble_of_psisigma_results_allcomparisons_final, wide_tibble_of_psisigma_results_allcomparisons_differential, by = c("diff_exon_coords"))

write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons, file = paste(R_processing_results_dir, "long_table_of_all_differential_exons_info_dpsi_", dpsi_cutoff,  "_any", pvalue_or_FDR, pvalue_cutoff, "_", number.of.exons_differential_anysig_with_na, "_exons_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# export associated gene set
write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons$matched_gene_names %>% unique %>% na.omit, file = paste(R_processing_results_dir, "geneset_of_differential_results_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons$matched_gene_names %>% unique %>% na.omit %>% length, "_genes_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)
...
wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential <- dplyr::semi_join(wide_tibble_of_psisigma_results_allcomparisons_final,
                                                                                                   tibble_unique_differential_LSVs, by = c("database_ID", "event_region_coords", "diff_exon_coords")) 
# %>%
#   dplyr::filter(str_detect(string = `splicemode`, pattern = "SES"))

tibble_cumulative_diff_changes <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "24h") & !str_detect(string = comparison, pattern = "12h")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "24h")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_final_filtered_for_differential %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff) %>% .$diff_exon_coords %>% unique %>% length)) %>% 
...
# all (LSV PSI)
pdf(paste(R_processing_results_dir, "plot_of_absolute_dpsi_changes_LSV_all.pdf", sep = ""), width = 5, height = 5)
plot(density(wide_tibble_of_psisigma_results_allcomparisons_final$dpsi %>% abs, bw = 1), 
     main = "Distribution of absolute magnitude of change in dpsi, PSI-Sigma (LSVs)", 
     xlab = "Absolute dPSI in any direction (%)",
     ylab = "Frequency density",
     col = "firebrick4", 
     xaxt = "n",
     xlim = c(0, 100)) + 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_final$dpsi %>% abs, bw = 1),
...
# LSV PSI - differential
pdf(paste(R_processing_results_dir, "plot_of_absolute_dpsi_changes_LSV_diff.pdf", sep = ""), width = 5, height = 5)
plot(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV$dpsi %>% abs, bw = 1), 
     main = "Distribution of absolute magnitude of change in dpsi, PSI-Sigma (LSVs)", 
     xlab = "Absolute dPSI in any direction (%)",
     ylab = "Frequency density",
     col = "firebrick4", 
     xaxt = "n",
     xlim = c(0, 100)) + 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LSV$dpsi %>% abs, bw = 1),
          col = alpha("firebrick4", 0.33),
          border = FALSE,
          xaxt = "n",
          xlim = c(0, 100)) +
...
plot(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(diff_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(dpsi)) %>% .$exon_dpsi %>% abs), 
  ...
  # xaxt = "n",
  # xlim = c(0, 100)) 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(diff_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(dpsi)) %>% .$exon_dpsi %>% abs),
          col = alpha("firebrick4", 0.33),
          border = FALSE,
          xaxt = "n")
# ,
...
list_of_tables_absolute_PSI_average_subset_by_region_sum <- future_map(.x = list_of_tibbles_absolute_PSI_average_subset_by_region, 
...
wide_tibble_of_absolute_PSI_averages_sum <- list_of_tables_absolute_PSI_average_subset_by_region_sum %>% rbindlist %>% as_tibble
...
tibble_of_absolute_PSI_averages_exon.switched_differential_separate <- dplyr::semi_join(tibble_of_absolute_PSI_averages_exon.switched_separate, tibble_unique_differential_LSVs, by = c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode"))
...
wide_tibble_of_psisigma_results_allcomparisons_final <- wide_tibble_of_psisigma_results_allcomparisons_final %>% 
  add_column("first_exon_reference" = grepl(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_reference, pattern = "first_exon"),
             "last_exon_reference" = grepl(x = wide_tibble_of_psisigma_results_allcomparisons_final$first_or_last_exon_reference, pattern = "last_exon"))

# category means splicemode + NMD status + first/last exon
# also clean up the splicemode column
tibble_exons_per_category_all <- wide_tibble_of_psisigma_results_allcomparisons_final %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2", perl = TRUE)} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% 
  # , any_NMD
...
tibble_exons_per_category_constitutive <- tibble_constitutive_LSV_info %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, first_exon_reference, last_exon_reference) %>% summarise(no_exons_constitutive = n())
...
### import the .gct files

```{r}

list_of_psisigma_gct_tibbles <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "_r5_ir3.gct", sep = ""), sep = "\t", skip = 2, check.names = FALSE, na.strings = c("-")) %>% as_tibble, .progress = TRUE, .options = future_options(globals = c("as_tibble", "psisigma_results_dir"))) %>%
  # get rid of the stupid bracket in front of every colname
  purrr::map(.f = function(a1) {
    colnames(a1) <- gsub(x = colnames(a1), pattern = "\\(\\)", replacement = "")
    return(a1)
  } )

```

### process the .gct files

```{r}

list_of_psisigma_gct_tibbles_processed <- purrr::map(.x = list_of_psisigma_gct_tibbles,
                                                     .f = ~.x %>% 
                                                       dplyr::select(-Annotation) %>% 
                                                       # reshape2::melt(id.vars = c("Event ID"), value.name = "PSI", variable.name = "sample") %>% 
                                                       as_tibble)

# select only the ud comparisons, remove NA and reduce into a left join (we're not keeping the shit not in common anyways.)
wide_tibble_gct_tibbles_ud.no.na <- list_of_psisigma_gct_tibbles_processed %>% 
  purrr::map(~.x %>% na.omit) %>% 
  purrr::keep(.p = ~str_detect(string = colnames(.x), pattern = "ud") %>% any == TRUE) %>% 
  purrr::reduce(dplyr::left_join)

# rbind and tibblise - yeah... we're not going to do this.
# tibble_psisigma_gct_processed <- list_of_psisigma_gct_tibbles_processed %>% rbindlist %>% as_tibble %>% unique
...
wide_matrix.tibble_gct_tibbles_ud.no.na <- wide_tibble_gct_tibbles_ud.no.na %>% 
  dplyr::select(-`Event ID`)

colnames(wide_matrix.tibble_gct_tibbles_ud.no.na) <- paste(gsub(x = colnames(wide_matrix.tibble_gct_tibbles_ud.no.na), pattern = "BM_MSC_to_OB_(.*d|.*h)_(.*)", replacement = "\\1"), "|", gsub(x = colnames(wide_matrix.tibble_gct_tibbles_ud.no.na), pattern = "BM_MSC_to_OB_(.*d|.*h)_(.*)", replacement = "\\2"), sep = "")
# edit
colnames(wide_matrix.tibble_gct_tibbles_ud.no.na) <- gsub(x = colnames(wide_matrix.tibble_gct_tibbles_ud.no.na), pattern = "24h", replacement = "1d")
colnames(wide_matrix.tibble_gct_tibbles_ud.no.na) <- gsub(x = colnames(wide_matrix.tibble_gct_tibbles_ud.no.na), pattern = "ud", replacement = "MSC")

plot_PCA_for_timepoint_and_replicate(wide_matrix.tibble_gct_tibbles_ud.no.na %>% na.omit, 
...
list_genome_relative_exonic_coords <- future_map(.x = list_ref_entries_matched_to_constituent_junctions, .f = function(.x) {
