...
# psisigma_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/"
...
# R_processing_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/R_processing_results/"
...
extract_junction.flanking.exons <- function(spliceregion_list, tibble_gtf_table, index) {
  ...
  # index <- 1
  # spliceregion_list <- list_ref_matching_exon_entries_IR[[13]]
  ...
  # +/- 1 nt tolerance
    tibble_gtf_subset_flanking_exons <- tibble_gtf_table[which(tibble_gtf_table$seqnames == spliceregion_list$diff_exon_chr %>% trimws), ] %>% 
      .[which(.$start <= ((spliceregion_list$diff_exon_end %>% as.numeric) + 2) & .$end >= ((spliceregion_list$diff_exon_start %>% as.numeric) - 2)), ] %>% 
      .[which(!(.$start <= ((spliceregion_list$diff_exon_end %>% as.numeric) - 1) & .$end >= ((spliceregion_list$diff_exon_start %>% as.numeric) + 1))), ] %>% 
      .[which(.$type == "exon"), ]
...
list_of_psisigma_result_tibbles_processed <- future_map(.x = list_of_psisigma_result_tibbles_raw, .f = 
                                                          ...
                                                          add_column("diff_exon_chr" = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1"), 
                                                                     ...
                                                                     "diff_exon_end" = purrr::map2(.x = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert, .y = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert, .f = ~max(.x, .y)) %>% unlist), .progress = TRUE)
...
# save.image(paste(R_processing_results_dir, "01082020_psisigma_dexseq_processed.RData", sep = ""))
...
tibble_poison_exon_finder_results_processed0 <- tibble_poison_exon_finder_results %>%
  ...
  dplyr::select(-three_frame_translation_result, -vector_transcript_segment_forward_genome_relative_coords, -list_forward_nucleotides)
...
# write coords for poison exon finder
# write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons[, c("event_region_coords", "diff_exon_coords", "matched_strand", "splicemode")] %>% dplyr::rename("VSR_coords" = "event_region_coords", "alternative_exon_coords" = "diff_exon_coords"), file = paste(R_processing_results_dir, "long_tibble_of_psisigma_allcomparisons_differential_exons", number.of.exons_differential_anysig_with_na, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_anysig_with_na_poison_exon_finder.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# write coords for poison exon finder
# write.table(x = tibble_constitutive_LIV_info[, c("event_region_coords", "diff_exon_coords", "matched_strand", "splicemode")] %>% dplyr::rename("VSR_coords" = "event_region_coords", "alternative_exon_coords" = "diff_exon_coords"), file = paste(R_processing_results_dir, "long_table_of_all_constitutive_LIV", number.of.LIVs_constitutive, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "poison_exon_finder.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_exons_per_category_all <- long_tibble_of_psisigma_DEXSeq_results_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2", perl = TRUE)} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% 
  # , any_NMD
  group_by(splicemode, introduces_PTC_for_all_matched_transcripts, alters_frame_for_all_matched_transcripts, first_exon_reference, last_exon_reference) %>% summarise(no_exons_all = n())

all_exons_count <- tibble_exons_per_category_all$no_exons_all %>% sum(na.rm = TRUE)
# number_NMD_all_exons <- tibble_exons_per_category_all[tibble_exons_per_category_all$NMD_flagged_ref == TRUE, ] %>% .$no_exons_all %>% sum(na.rm = TRUE)
...
ggplot() + 
  ...
  geom_point(data = long_tibble_exons_per_category_summary_2[long_tibble_exons_per_category_summary_2$NMD_first_last_exon_value == TRUE, ], aes(x = splicemode, y = percentage_exons_per_category * max_number_of_exons_per_splicemode_for_plot / max_percentage_exons_per_category , shape = NMD_first_last_exon, colour = NMD_first_last_exon), width = 0.1, height = 0) + ggtitle(paste("Frequency distribution of exons per category 
  ...
  scale_shape_manual(name = "Category", limits = c("first_exon_reference", "last_exon_reference", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts"), labels = c("First exon", "Last exon", "Poison exon", "Alters frame"), values = c(15, 19, 8, 11)) +
  scale_colour_brewer(name = "Category", type = "seq", limits = c("first_exon_reference", "last_exon_reference", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts"), labels = c("First exon", "Last exon", "Poison exon", "Alters frame"), palette = "Set2") +
