...
# psisigma_results_dir <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9e_in_parallel_with_gct/"
psisigma_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator/"

# R_processing_results_dir <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9e_in_parallel_with_gct/R_processing_results/"
R_processing_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator/R_processing_results/"
...
extract_overlapping.exons <- function(spliceregion_list, tibble_gtf_table, stranded = FALSE) {
  ...
  # DEBUG ###################
  # index <- 1
  # spliceregion_list <- wide_tibble_of_all_unique_exon_coords_array.tree_not_IR[[index]]
  # # tibble_gtf_table <- tibble_ref_gtf
  ...
  if (stranded == FALSE) {
    ...
    tibble_gtf_subset_overlapping_exons <- tibble_gtf_table[tibble_gtf_table$seqnames == spliceregion_list$diff_exon_chr %>% trimws, ] %>% 
      .[which(.$start > ((spliceregion_list$diff_exon_start %>% as.numeric) - 2) & .$end < ((spliceregion_list$diff_exon_end %>% as.numeric) + 2)), ] %>% 
      .[which((.$start < ((spliceregion_list$diff_exon_start %>% as.numeric) + 2) & .$end > ((spliceregion_list$diff_exon_end %>% as.numeric) - 2))), ] %>% 
      .[which(.$type == "exon"), ]
...
list_of_psisigma_result_tibbles_raw <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "/", .x, "_r1_ir3.sorted.txt", sep = ""), sep = "\t") %>% as_tibble, .progress = TRUE, .options = future_options(globals = c("as_tibble")))
...
wide_tibble_of_psisigma_results_allcomparisons_processed[, colnames(wide_tibble_of_psisigma_results_allcomparisons_processed)[grep(x = colnames(wide_tibble_of_psisigma_results_allcomparisons_processed), pattern = "comparison.1_")]] <- future_map2(.x = wide_tibble_of_psisigma_results_allcomparisons_processed %>% dplyr::select(contains("comparison.1_")) %>% array_tree(margin = 1), .y = wide_tibble_of_psisigma_results_allcomparisons_processed %>% dplyr::select(contains("comparison.1_")) %>% rowMeans(na.rm = TRUE), .f = function(a1, a2) {a1[is.na(a1)] <- a2; return(a1 %>% t %>% as.data.frame)} ) %>% rbindlist(use.names = TRUE) %>% as_tibble

wide_tibble_of_psisigma_results_allcomparisons_processed[, colnames(wide_tibble_of_psisigma_results_allcomparisons_processed)[grep(x = colnames(wide_tibble_of_psisigma_results_allcomparisons_processed), pattern = "comparison.2_")]] <- future_map2(.x = wide_tibble_of_psisigma_results_allcomparisons_processed %>% dplyr::select(contains("comparison.2_")) %>% array_tree(margin = 1), .y = wide_tibble_of_psisigma_results_allcomparisons_processed %>% dplyr::select(contains("comparison.2_")) %>% rowMeans(na.rm = TRUE), .f = function(a1, a2) {a1[is.na(a1)] <- a2; return(a1 %>% t %>% as.data.frame)} ) %>% rbindlist(use.names = TRUE) %>% as_tibble
...
list_ref_matching_exon_entries_not_IR <- future_map(.x = wide_tibble_of_all_unique_exon_coords_array.tree_not_IR, .f = ~list(
  "reference_gtf_match" = extract_overlapping.exons(spliceregion_list = .x, tibble_gtf_table = tibble_ref_gtf), 
  "reconstructed_gtf_match" = extract_overlapping.exons(spliceregion_list = .x, tibble_gtf_table = tibble_recon_gtf), 
  "diff_exon_coords" = .x$diff_exon_coords,
  "diff_exon_chr" = .x$diff_exon_chr,
  "diff_exon_start" = .x$diff_exon_start,
  "diff_exon_end" = .x$diff_exon_end,
...
# tibblize
tibble_as.event.id_with_summarised_annotation_IR <- list_ref_matching_exon_entries_all_array.tree_IR_reannotated %>% rbindlist(fill = TRUE) %>% as_tibble
...
long_tibble_of_psisigma_DEXSeq_results <- dplyr::left_join(wide_tibble_of_psisigma_results_allcomparisons_processed, tibble_ref_matching_exon_entries_all_summarised, by = c("diff_exon_coords", "splicemode", "diff_exon_chr", "diff_exon_start", "diff_exon_end")) %>% dplyr::mutate_if(is.factor, as.character)
...
# save.image(paste(R_processing_results_dir, "15072020_psisigma_dexseq_processed.RData", sep = ""))
...
list_of_psisigma_denominator_gct_tibbles <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "/", .x, "_r1_ir3.denominator.gct", sep = ""), skip = 2, sep = "\t") %>% as_tibble, .progress = TRUE, .options = future_options(globals = c("as_tibble"))) %>% 
...
wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator <- wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator %>% as_tibble(rownames = "id")

wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator[, "id"] <- paste("id_", wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator$id, sep = "")
...
purrr::map2(.x = list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_comparison, 
            ...
            .f = function(a1, a2) {
              ...
              list_GFF_building_tibble <- future_map(.x = list_GFF_building_exons_info, 
                                                     .f = ~tibble("seqnames" = .x$diff_exon_chr,
...
future_map2(.x = list_DEXSeq_results,
           ...
           }, .progress = TRUE, .options = future_options(globals = c("plotDispEsts", "plotMA")))
...
list_DEXSeq_results_tibble.only_with_PSI <- purrr::map2(.x = list_DEXSeq_results_tibble.only,
                                                        ...
                                                        .f = function(a1, a2) {
                                                          ...
                                                          tibble <- a1 %>% 
                                                            ...
                                                            dplyr::mutate("DEXSeq_dpsi_first_minus_second" = DEXSeq_PSI_first - DEXSeq_PSI_second)
...
future_map2(.x = list_DEXSeq_results_tibble.only_with_PSI,
           ...
           .f = function(a1, a2) {
             ...
             plot(density(a1$DEXSeq_dpsi_first_minus_second %>% na.omit))
             dev.off()
             
             pdf(paste(DEXSeq_dir, "DEXSeq_dpsi_cdf_distributions_", a2, ".pdf", sep = ""))
             plot(ecdf(a1$DEXSeq_dpsi_first_minus_second %>% na.omit))
             dev.off()
...
# select only cols that are informative
  dplyr::select(id, DEXSeq_exonBaseMean, DEXSeq_dispersion, DEXSeq_stat, DEXSeq_pvalue, DEXSeq_padj, DEXSeq_log2FC_second_minus_first, comparison, DEXSeq_IR_first, DEXSeq_IR_second, DEXSeq_pseudotranscript_IR_plus_ER_first, DEXSeq_pseudotranscript_IR_plus_ER_second, DEXSeq_PSI_first, DEXSeq_PSI_second, DEXSeq_dpsi_first_minus_second)
...
wide_tibble_of_psisigma_results_allcomparisons_differential <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::filter(., !!as.name(pvalue_or_FDR) <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & num.replicates_comparison.1 >= 2 & num.replicates_comparison.2 >= 2)

number.of.LIVs_differential_anysig_with_na <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::distinct(database_ID, event_region_coords, diff_exon_coords, splicemode, diff_exon_chr, diff_exon_start, diff_exon_end, matched_strand) %>% nrow
...
tibble_differential_results_12d_vs_ud <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::filter(comparison == "BM_MSC_to_OB_12d_vs_BM_MSC_to_OB_ud" & !!as.name(pvalue_or_FDR) <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff)
# write table
...
tibble_cumulative_diff_changes_total <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "24h") & !str_detect(string = comparison, pattern = "12h")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "24h")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff) %>% .$diff_exon_coords %>% unique %>% length)) %>% 
...
tibble_cumulative_diff_changes_IR <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             ...
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff) %>% .$diff_exon_coords %>% unique %>% length)) %>% 
  add_column("cumulative_diff_changes_pct" = .$cumulative_diff_changes * 100 / (.$cumulative_diff_changes %>% max))
...
tibble_cumulative_diff_changes_non.IR <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             ...
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$diff_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(DEXSeq_dpsi_first_minus_second) >= dpsi_cutoff) %>% .$diff_exon_coords %>% unique %>% length)) %>% 
  add_column("cumulative_diff_changes_pct" = .$cumulative_diff_changes * 100 / (.$cumulative_diff_changes %>% max))
...
plot(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(diff_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(DEXSeq_dpsi_first_minus_second, na.rm = TRUE)) %>% .$exon_dpsi %>% abs), 
  ...
  # xaxt = "n",
  # xlim = c(0, 100)) 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(diff_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(DEXSeq_dpsi_first_minus_second, na.rm = TRUE)) %>% .$exon_dpsi %>% abs),
          col = alpha("firebrick4", 0.33),
          border = FALSE,
          xaxt = "n")
# ,
...
wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIV <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::group_by(database_ID, event_region_coords, diff_exon_coords, splicemode, diff_exon_chr, diff_exon_start, diff_exon_end, matched_strand)
...
tibble_unique_constitutive_LIVs <- wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIV_constitutive.only %>% dplyr::distinct(database_ID, event_region_coords, diff_exon_coords, splicemode, diff_exon_chr, diff_exon_start, diff_exon_end, matched_strand, .keep_all = TRUE)
...
write.table(x = tibble_constitutive_LIV_info, file = paste(R_processing_results_dir, "long_table_of_all_constitutive_LIV", number.of.LIVs_constitutive, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_LIVs.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 <- dplyr::filter(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV, comparison %in% vector_of_ud_comparisons) %>% dplyr::select(database_ID, event_region_coords, diff_exon_coords, splicemode, diff_exon_chr, diff_exon_start, diff_exon_end, matched_gene_names, matched_recon_transcript_names, matched_ref_transcript_names, matched_strand, NMD_reference, NMD_recon, any_NMD, first_or_last_exon_reference, first_or_last_exon_recon, any_first_exon, any_last_exon, comparison, DEXSeq_PSI_first, DEXSeq_PSI_second) %>% dplyr::rename("DEXSeq_PSI_ud" = "DEXSeq_PSI_second")
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp1 <- dplyr::bind_rows(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 %>% dplyr::select(-DEXSeq_PSI_ud) %>% dplyr::rename("PSI" = "DEXSeq_PSI_first"),
                                                                                       wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 %>% dplyr::select(-DEXSeq_PSI_first) %>% dplyr::rename("PSI" = "DEXSeq_PSI_ud") %>% dplyr::mutate_at(.vars = "timepoint", .funs = function(x) {"ud" %>% return}) %>% dplyr::group_by_at(.vars = setdiff(colnames(.), "PSI")) %>% dplyr::mutate_at(.vars = "PSI", .funs = mean )) %>% dplyr::distinct()
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only <- reshape2::dcast(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp1, formula = database_ID + event_region_coords + diff_exon_coords + splicemode + diff_exon_chr + diff_exon_start + diff_exon_end + matched_gene_names + matched_recon_transcript_names + matched_ref_transcript_names + matched_strand + NMD_reference + NMD_recon + any_NMD + first_or_last_exon_reference + first_or_last_exon_recon + any_first_exon + any_last_exon ~ timepoint, value.var = "PSI") %>% as_tibble

wide_tibble_of_psisigma_results_allcomparisons_final_ud.only <- wide_tibble_of_psisigma_results_allcomparisons_final_ud.only %>% dplyr::select(colnames(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only)[!colnames(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only) %in% vector_OBseries_timepoints], all_of(vector_OBseries_timepoints))

colnames(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only) <- c(colnames(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only)[!colnames(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only) %in% vector_OBseries_timepoints], vector_OBseries_timepoints_edited)
...
list_of_tables_absolute_PSI_average_subset_by_region_sum <- future_map(.x = list_of_tibbles_absolute_PSI_average_subset_by_exon, 
                                                                        .f = ~cbind(.x[, !colnames(.x) %in% vector_OBseries_timepoints_edited] %>% unique, 
                                                                                    apply(X = .x[, colnames(.x) %in% vector_OBseries_timepoints_edited], MARGIN = 2, FUN = function(X) {
                                                                                      ...
                                                                                      }) %>% t), .progress = TRUE, .options = future_options(globals = FALSE))
...
somdata_5_by_5_OBseries <- som(tibble_matrix_LIV_PSI_no_na_scaled, grid = somgrid(xdim = OB_xdim, ydim = OB_ydim, topo = "rectangular", toroidal = FALSE), rlen = 100, keep.data = TRUE)

wide_table_of_final_SOM_summary_5_by_5_OBseries_separate <- cbind(wide_tibble_of_LIV_PSI_no_na[, -grep(x = colnames(wide_tibble_of_LIV_PSI_no_na), pattern = "absolute.psi")], som_5_by_5_OBseries_table_scaled.PSI_separate, "cluster" = somdata_5_by_5_OBseries[["unit.classif"]])
...
ggplot() +
 ...
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_separate, "_exons_LIV.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_separate, "_exons_LIV.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".svg", sep = ""), device = "svg", dpi = 600, width = 33, height = 20, units = "cm")

write.table(x = long_table_of_final_SOM_summary_5_by_5_OBseries_separate, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_separate, "_exons_LIV.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_long.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

write.table(x = wide_table_of_final_SOM_summary_5_by_5_OBseries_separate, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_separate, "_exons_LIV.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_wide.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# get row indices which have any na.
row.indices_with_na <- apply(X = wide_tibble_of_psisigma_PSI_with_na_exon.sum[, colnames(wide_tibble_of_psisigma_PSI_with_na_exon.sum) %in% vector_OBseries_timepoints_edited], MARGIN = 1, FUN = function(X) {any(is.na(X))})

# remove those rows that are true (that have any na)
wide_tibble_of_exon_PSI_no_na <- wide_tibble_of_psisigma_PSI_with_na_exon.sum[which(row.indices_with_na == FALSE), ]
...
somdata_5_by_5_OBseries <- som(wide_tibble_of_exon_PSI_no_na_scaled, grid = somgrid(xdim = OB_xdim, ydim = OB_ydim, topo = "rectangular", toroidal = FALSE), rlen = 100, keep.data = TRUE)

wide_table_of_final_SOM_summary_5_by_5_OBseries_sum <- cbind(wide_tibble_of_absolute_PSI_averages_complete.timeseries_differential_sum[, -grep(x = colnames(wide_tibble_of_absolute_PSI_averages_complete.timeseries_differential_sum), pattern = "absolute.psi")], wide_tibble_of_exon_PSI_no_na_scaled, "cluster" = somdata_5_by_5_OBseries[["unit.classif"]])
...
write.table(x = long_table_of_final_SOM_summary_5_by_5_OBseries_sum, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_sum, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_long.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

write.table(x = wide_table_of_final_SOM_summary_5_by_5_OBseries_sum, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", no.exons_differentially.spliced_complete.timeseries_sum, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_wide.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
pdf(paste(qualitycheck_results_dir, "qualityplot_pvalues_density.distribution.pdf", sep = "")) 
  ...
  plot(density(long_tibble_of_psisigma_DEXSeq_results[, "DEXSeq_pvalue"] %>% na.omit %>% unlist), xlab = "pvalue", ylab = "frequency density", main = "PSI-Sigma p-value density distribution")
  
dev.off()

pdf(paste(qualitycheck_results_dir, "qualityplot_pvalues_CDF.pdf", sep = "")) 
  
  plot(ecdf(long_tibble_of_psisigma_DEXSeq_results[, "pvalue"] %>% na.omit %>% unlist), xlab = "pvalue", ylab = "cumulative frequency", main = "PSI-Sigma p-value CDF")
...
## NMD
row.indices_NMD_ref <- grep(x = long_tibble_of_psisigma_DEXSeq_results$NMD_reference, pattern = "TRUE")
row.indices_NMD_recon <- grep(x = long_tibble_of_psisigma_DEXSeq_results$NMD_recon, pattern = "TRUE")
...
ggplot(tibble_all_exons_per_gene_freq_2) + 
  ...
  ggtitle(paste("frequency distribution of exons per gene\n1:2 ratio = ", tibble_all_exons_per_gene_freq_2[1, 2]/tibble_all_exons_per_gene_freq_2[2, 2], sep = "")) +
...
ggplot(tibble_constitutive_exons_per_gene_freq_2) + 
  ...
  ggtitle(paste("frequency distribution of exons per gene\n1:2 ratio = ", tibble_constitutive_exons_per_gene_freq_2[1, 2]/tibble_constitutive_exons_per_gene_freq_2[2, 2], sep = "")) +
  # scale_x_continuous(trans = "log2") +
...
ggplot(tibble_differential_exons_per_gene_freq_2) + 
  ...
  ggtitle(paste("frequency distribution of exons per gene\n1:2 ratio = ", tibble_differential_exons_per_gene_freq_2[1, 2]/tibble_differential_exons_per_gene_freq_2[2, 2], sep = "")) +
  # scale_x_continuous(trans = "log2") +
...
tibble_exons_per_category_all <- long_tibble_of_psisigma_DEXSeq_results_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2", perl = TRUE)} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% 
  # , any_NMD
  group_by(splicemode, first_exon_reference, last_exon_reference) %>% summarise(no_exons_all = n())
...
tibble_exons_per_category_constitutive <- tibble_constitutive_LIV_info_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, first_exon_reference, last_exon_reference) %>% summarise(no_exons_constitutive = n())
...
tibble_exons_per_category_differential <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, first_exon_reference, last_exon_reference) %>% summarise(no_exons_differential = n())

differential_exons_count <- tibble_exons_per_category_differential$no_exons_differential %>% sum(na.rm = TRUE)
# number_NMD_differential_exons <- tibble_exons_per_category_differential[tibble_exons_per_wide_tibble_of_psisigma_results_allcomparisons_differentialcategory_differential$any_NMD == TRUE, ] %>% .$no_exons_differential %>% sum(na.rm = TRUE)
...
# melt the sample
long_tibble_exons_per_category_summary <- reshape2::melt(tibble_exons_per_category_summary, id.vars = c("splicemode", "first_exon_reference", "last_exon_reference"), variable.name = "sample", value.name = "number_of_exons_per_category") %>% as_tibble %>% 
...
ggplot() + 
  ...
  geom_jitter(data = long_tibble_exons_per_category_summary_2[long_tibble_exons_per_category_summary_2$NMD_first_last_exon_value == TRUE, ], aes(x = splicemode, y = percentage_exons_per_category * max_number_of_exons_per_splicemode_for_plot / max_percentage_exons_per_category , shape = NMD_first_last_exon), width = 0.1, height = 0) + ggtitle(paste("Frequency distribution of exons per category 
                ", pvalue_or_FDR, pvalue_cutoff, "_dpsi", dpsi_cutoff, sep = "")) +
  xlab("Splicemode") +
  scale_y_continuous(name = "Number of exons", limits = c(0, max_number_of_exons_per_splicemode_for_plot), sec.axis = sec_axis(trans = ~. * max_percentage_exons_per_category/max_number_of_exons_per_splicemode, (name = "Percentage first/last exon per splicemode"), breaks = seq(0, 100, 20))) +
...
# upset plot
message("all exons")
df <- long_tibble_of_psisigma_DEXSeq_results_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% .[, c("diff_exon_coords", "splicemode", "any_NMD", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = diff_exon_coords + any_NMD + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 
...
df <- tibble_constitutive_LIV_info_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% .[, c("diff_exon_coords", "splicemode", "any_NMD", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = diff_exon_coords + any_NMD + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 
...
df <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., diff_exon_coords, .keep_all = TRUE) %>% .[, c("diff_exon_coords", "splicemode", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = diff_exon_coords + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 
# , "any_NMD"
 # + any_NMD
...
pdf(file = paste(R_processing_results_dir, "upset_plot_of_intersections_exons_per_NMD.first.last.exon_per_splicemode_", pvalue_or_FDR, pvalue_cutoff, "_dpsi", dpsi_cutoff, "_differential.pdf", sep = ""), height = 3, width = 7)

  ht <- ComplexHeatmap::UpSet(upset_combs, 
                              comb_order = order(-comb_size(upset_combs)), 
                              left_annotation = rowAnnotation("Set Type" = c(
                                # "any_NMD" = "Exon ontology",
                                "first_exon_reference" = "Reference location",
                                "last_exon_reference" = "Reference location",
                                "A3SS" = "Splicemode",
                                "A5SS" = "Splicemode",
                                "IR" = "Splicemode",
                                "MES" = "Splicemode",
                                "MXS" = "Splicemode",
                                "SES" = "Splicemode")[set_name(upset_combs)], show_annotation_name = FALSE, 
                                col = list("Set Type" = c("Reference location" = "tomato1", "Splicemode" = "red4"))
                              )) %>% draw
  od <- column_order(ht)
  cs <- comb_size(upset_combs)
  decorate_annotation("Intersection\nsize", {
    grid.text(cs[od], x = seq_along(cs), y = unit(cs[od], "native") + unit(2, "pt"), 
              default.units = "native", just = c("left"), gp = gpar(fontsize = 10), rot = 90)
  })
...
list_of_tibble_raw_counts_PSI_for_PCA_by_comparison <- purrr::map2(.x = list_of_tibble_raw_counts_PSI_for_PCA_by_comparison,
                                                                   ...
                                                                   .f = ~.x %>% dplyr::select(-comparison) %>% setNames(c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode", paste(.y, c("|r1", "|r2", "|r3"), sep = ""), paste("ud", c("|r1", "|r2", "|r3"), sep = ""))) %>% na.omit %>% dplyr::distinct())

# reduce into a left join
wide_tibble_raw_counts_PSI_for_PCA <- list_of_tibble_raw_counts_PSI_for_PCA_by_comparison %>% purrr::reduce(.f = inner_join , by = c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode", paste("ud", c("|r1", "|r2", "|r3"), sep = ""))) %>% dplyr::distinct()
...
colnames(wide_tibble_raw_counts_PSI_for_PCA) <- gsub(x = colnames(wide_tibble_raw_counts_PSI_for_PCA), pattern = "24h", replacement = "1d")
colnames(wide_tibble_raw_counts_PSI_for_PCA) <- gsub(x = colnames(wide_tibble_raw_counts_PSI_for_PCA), pattern = "ud", replacement = "MSC")
...
# set up list of exon coords for looping
## split into IR and non-IR
list_exons_for_query_non.IR <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::filter(!str_detect(string = splicemode, pattern = "IR")) %>% (function(input_table) {
  
  output_list <- input_table[, c("diff_exon_coords", "diff_exon_chr", "diff_exon_start", "diff_exon_end", "matched_strand")] %>% 
    as_tibble %>% 
    unique %>%
    array_tree
...
list_exons_for_query_IR <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::filter(str_detect(string = splicemode, pattern = "IR")) %>% (function(input_table) {
...
## easy for non-IR
list_ref_entries_matched_to_exons_non.IR <- list_exons_for_query_non.IR %>% future_map(
  .f = ~purrr::splice(.x,
                      
                      # DEBUG ###
                      # .x <- list_constituent_junctions_for_query[[100]]
                      ###########
...
## hard for IR
### junction-flanking exon matches for IR events
list_ref_entries_matched_to_IR_junctions <- future_imap(.x = list_exons_for_query_IR, .f = ~list(
  "reference_gtf_match" = extract_junction.flanking.exons(.x, tibble_ref_gtf_original, .y), 
  "diff_exon_coords" = .x$diff_exon_coords,
  "diff_exon_chr" = .x$diff_exon_chr,
  "diff_exon_start" = .x$diff_exon_start,
  "diff_exon_end" = .x$diff_exon_end,
  "matched_strand" = .x$matched_strand), 
  .progress = TRUE, .options = future_options(globals = c("tibble_ref_gtf_original", "extract_junction.flanking.exons")))

### use the IR-junction-matched flanking exons to find the IR exon.
list_ref_entries_matched_to_exons_non.IR <- future_imap(.x = list_ref_entries_matched_to_IR_junctions, .f = function(.x, .y) {
...
}, .progress = TRUE, .options = future_options(globals = c("as_tibble", "tibble_ref_gtf")))

# add the matched CDS from the exon number and transcript ids of matched exons
list_ref_entries_matched_to_exons.and.CDS <- list_ref_entries_matched_to_exons %>% future_map(
  .f = ~purrr::splice(.x,
...
list_genome_relative_exonic.and.CDS_coords <- future_imap(.x = list_ref_entries_matched_to_exons.and.CDS, .f = function(.x, .y) {
  
  # DEBUG ###
  # .x <- list_ref_entries_matched_to_exons.and.CDS[[2]]
  ###########
  
  cat("\nprocessing entry number: ", .y)
  ...
  output_list <- purrr::splice(
    ...
    "vector_genomic_coords_of_all_differential_exon_positions" = if (nrow(input_list$ref_matched_exons) !=0) {(input_list$ref_matched_exons$start %>% min):(input_list$ref_matched_exons$end %>% max)} else {return(NA)},
    "vector_genomic_coords_of_all_differential_CDS_positions" = if (nrow(input_list$ref_matched_CDS) !=0) {(input_list$ref_matched_CDS$start %>% min):(input_list$ref_matched_CDS$end %>% max)} else {return(NA)}
  ...
  return(output_list)
    ...
    purrr::discard(.x = list_genome_relative_exonic.and.CDS_coords[1], .p = ~(length(.x$vector_genomic_coords_of_all_differential_exon_positions) == 0) & (length(.x$vector_genomic_coords_of_all_differential_CDS_positions) == 0))

# take only the differential exonic positions
# first pluck positions by chromosome
# to get differential positions, we reason that when we sort coordinates, the tally frequencies will show up as "islands" with a certain local maximum. For every island, simply remove the locally most frequent coordinate, and the resulting vector of genome-relative positions will only be differential, ready for filtering.
## get chromosomes
vector_of_chromosomes_to_pluck <- list_genome_relative_differential_coords %>% purrr::map(~.x$diff_exon_chr) %>% unlist %>% unique

## pluck exonic positions by chromosome and unlist
list_plucked_differential_exonic_positions_by_chromosome <- purrr::map(.x = vector_of_chromosomes_to_pluck, .f = function(a1) {
  
  # DEBUG ###
  # a1 <- 5
  ###########
  
  cat("\nnow processing chr:", a1, "\n")
  
  # pluck all the exonic positions
  vector_all_exonic_positions <- purrr::keep(.x = list_genome_relative_differential_coords, .p = ~.x$diff_exon_chr == a1) %>%
    purrr::map(~.x$vector_genomic_coords_of_all_differential_exon_positions) %>%
    unlist %>%
    mixedsort
  
  # tibblise and tally each of the positions
  # determine the boundaries between tallies of 1 and 2+ by adding a difference-to-the-next-row column and difference-from-previous
  tibble_all_exonic_positions_tally <- vector_all_exonic_positions %>% 
    tibble::enframe(name = NULL, value = "exonic_position") %>%
    dplyr::group_by(exonic_position) %>%
    dplyr::summarise("tally" = n()) %>%
    tibble::add_column("tally_difference_to_next" = c((.$tally %>% .[-1]), NA) - .$tally,
                       "tally_difference_from_previous" = .$tally - c(NA, (.$tally %>% .[-length(.)])))
  
  # these are the start row indices of the islands
  row.indices_island_starts <- which(tibble_all_exonic_positions_tally$tally == 1 & tibble_all_exonic_positions_tally$tally_difference_to_next > 0) + 1
  # these are the end row indices of the islands
  row.indices_island_ends <- which(tibble_all_exonic_positions_tally$tally == 1 & tibble_all_exonic_positions_tally$tally_difference_from_previous < 0) - 1
  
  # deal with the case if the middle of an island is either at the very start or end.
  # in those cases, we'll have to add in the start as the first row or the end as the last row.
  if (tibble_all_exonic_positions_tally[1, ] %>% .$tally > 1) {
    
    row.indices_island_starts <- c(1, row.indices_island_starts)
    
  } 
  
  if (tibble_all_exonic_positions_tally[nrow(tibble_all_exonic_positions_tally), ] %>% .$tally > 1) {
    
    row.indices_island_ends <- c(row.indices_island_ends, nrow(tibble_all_exonic_positions_tally))
    
  }
  
  # BEHAVIOUR: for tally == 1, we automatically keep it.
  ## next, we looop thru each island, take the max tally, remove all positions which have the max tally and keep the rest. 
  ## mix together and finished :)
  
  ### loop thru each island. remove positions with max local tally.
  vector_island_positions_to_keep <- purrr::map2(.x = row.indices_island_starts,
                                                 .y = row.indices_island_ends,
                                                 .f = function(a1, a2) {
                                                   
                                                   max_tally <- tibble_all_exonic_positions_tally[a1:a2, "tally"] %>% unlist %>% max
                                                   
                                                   vector_island_positions_which_are_not_max_tally <- tibble_all_exonic_positions_tally[a1:a2, ] %>% .[.$tally != max_tally, "exonic_position"] %>% unlist
                                                   
                                                   return(vector_island_positions_which_are_not_max_tally)
                                                   
                                                 } ) %>% unlist
  
  ### these are the tally 1 positions
  vector_tally_1_positions <- tibble_all_exonic_positions_tally[tibble_all_exonic_positions_tally$tally == 1, "exonic_position"] %>% unlist
  
  # return all the differential positions
  c(vector_tally_1_positions, vector_island_positions_to_keep) %>% as.vector %>% sort %>% return
  
} ) %>% set_names(vector_of_chromosomes_to_pluck)

# then filter each genome-relative exonic coordinate to give true differential coordinates.
list_genome_relative_differential_coords <- purrr::map(.x = list_genome_relative_exonic_coords,
                                                       .f = function(a1) {
                                                         
                                                         list_a1_modified <- purrr::modify_at(.x = a1, 
                                                                                              .at = "vector_genomic_coords_of_all_differential_exon_positions", 
                                                                                              .f = ~intersect(.x, list_plucked_differential_exonic_positions_by_chromosome[[a1$diff_exon_chr]]))
...
list_transcript_and_protein_relative_differential_coords <- future_imap(.x = list_genome_relative_differential_coords, .f = function(.x, .y) {
  ...
  output_list <- purrr::splice(
    ...
    "vector_genomic_coords_of_all_differential_CDS_positions" = list(purrr::map(.x = input_list$genomic_coords_of_all_CDS_positions_per_protein_id, 
                                                                                .f = ~intersect(.x, input_list$vector_genomic_coords_of_all_differential_exon_positions) %>% sort(decreasing = FALSE)
...
tibble_final_transcript_feature_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("diff_exon_coords", "matched_strand", "splicemode", "vector_matched_transcript_ids", "transcript_features")] %>% as_tibble) %>%
  rbindlist %>% 
  as_tibble %>%
  setNames(c("diff_exon_coords", "matched_strand", "splicemode", "transcript_ids_with_annotation", "transcript_features")) %>% 
  ## add in gene symbols
  dplyr::right_join(tibble_ref_matching_exon_entries_all[, c("diff_exon_coords", "matched_strand", "matched_gene_names")] %>% na.omit %>% unique, ., by = c("diff_exon_coords", "matched_strand"))
# write table
...
tibble_final_biomart_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("diff_exon_coords", "matched_strand", "splicemode", "biomart_annotations")] ) %>%
...
tibble_final_PTM_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("diff_exon_coords", "matched_strand", "splicemode", "PTMs")] ) %>%
...
tibble_final_annotation_supp <- list_tidied_annotation %>% 
  purrr::map(~.x[c("diff_exon_coords", "matched_strand", "splicemode", "vector_genomic_coords_of_all_differential_exon_positions", "vector_genomic_coords_of_all_differential_CDS_positions", "vector_matched_transcript_ids", "protein_relative_differential_coords")]) %>%
  purrr::map_if(.p = ~.x$protein_relative_differential_coords %>% length != 0, .f = ~.x %>% (function(x) {
    purrr::splice(x, 
                  "tibble" = x[c("protein_relative_differential_coords", "vector_genomic_coords_of_all_differential_CDS_positions")] %>% purrr::map_depth(.depth = 2, .f = ~paste(.x, collapse = ",")) %>% purrr::map2(.x = ., .y = names(.), .f = ~.x %>% as_tibble %>% t %>% as_tibble(rownames = "ensembl_peptide_id") %>% setNames(c("ensembl_peptide_id", .y))) %>% purrr::reduce(dplyr::full_join)) %>% return}) ) %>%
  ...
  purrr::map(.f = ~.x[-which(names(.x) %in% c("protein_relative_differential_coords", "vector_genomic_coords_of_all_differential_CDS_positions"))]) %>%
...
ggplot(tibble_interpro_id_tally_per_diff.exon %>% dplyr::arrange(desc(tally)) %>% head(90), aes(x = reorder(interpro_identifier, -tally), y = tally)) +
  ...
  ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_exons_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 10, units = "cm") 
...
tibble_PTM_tally <- tibble_final_PTM_annotation %>% dplyr::distinct(diff_exon_coords, splicemode, PTM_type) %>% na.omit %>% dplyr::group_by(PTM_type) %>% dplyr::summarise("tally" = n())
...
ggplot(tibble_PTM_tally, aes(x = reorder(PTM_type, -tally), y = tally)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_PTMs_differential_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 4, height = 8, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_PTMs_differential_", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_exons_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 4, height = 8, units = "cm") 
