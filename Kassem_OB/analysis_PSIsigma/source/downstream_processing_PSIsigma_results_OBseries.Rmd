...
list_matchtest_combinedexpr_som_sweep_byisoform <- purrr::map(.x = list_of_wide_df_combinedexpr_som_sweep_clusters, .f = function(.x) {
  ...
  list_of_clusters <- purrr::map(.x = df_of_membership %>% array_tree, .f = ~unlist(.x)) 
  
  list_of_correlation_2 <- future_map(.x = list_of_clusters, .f = ~apply(X = df_of_membership, MARGIN = 1, FUN = function(X){.x == X}), .progress = TRUE, .options = future_options(globals = c("list_of_clusters", "df_of_membership")))
