...
psisigma_results_dir <- "/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/"

R_processing_results_dir <- "/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/R_processing_results/"
...
extract_junction.flanking.exons <- function(query_chr, query_start, query_end, query_strand, tibble_gtf_table, tolerance_left = 1, tolerance_right = 1, tolerance_inside = 1, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") {
  ...
  # DEBUG ###
  # query_chr = b1$chr
  # query_start = b1$diff_exon_start
  # query_end = b1$diff_exon_end
  # query_strand = "*"
  # tibble_gtf_table = a2
  # tolerance_left = 0
  # tolerance_right = 0
...
tibble_of_mapped_protein_positions_to_list_of_domains_PTMs <- function(list_of_mapped_proteinID, ensemblmart) {
  ...
  # search biomart
  list_biomart_result_per_attribute <- future_map(.x = list_of_attributes_to_retrieve, .f = ~getBM(filters = "ensembl_peptide_id", values =  vec_all_ENSP_ids %>% unique, attributes = .x, mart = ensemblmart), .progress = TRUE)
...
tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain <- function(tibble_queryisoforms_chr_strand_start_end, gtf_table, ensemblmart) {
  ...
  list_overlapping_protein_CDS_exons_tibbles <- future_map(.x = list_queryisoforms_overlapping_annotations_tibbles, .f = ~.x$protein_id %>% na.omit %>% unique %>% as.list %>% purrr::map(.x = ., .f = ~gtf_table[gtf_table$protein_id == .x & gtf_table$type == "CDS", ] %>% .[apply(X = ., MARGIN = 1, FUN = function(X) {all(is.na(X)) == FALSE}), ]), .progress = TRUE)
  
  message("Extract the individual genome-relative CDS nucleotide positions")
  list_overlapping_protein_CDS_positions_tibbles <- future_map(.x = list_overlapping_protein_CDS_exons_tibbles, .f = ~purrr::map(.x = .x, .f = ~sort_tibble_according_to_strand(.x)), .progress = TRUE)
...
list_of_psisigma_result_tibbles_raw <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "/", .x, "_r1_ir3.sorted.txt", sep = ""), sep = "\t") %>% as_tibble, .progress = TRUE)
...
list_of_psisigma_result_tibbles_processed <- furrr::future_map(
  .x = list_of_psisigma_result_tibbles_raw, .f = 
    # rename all columns
    ...
    add_column("chr" = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1"), 
               "diff_exon_start" = purrr::map2(.x = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~min(.x, .y)) %>% unlist, 
               "diff_exon_end" = purrr::map2(.x = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~max(.x, .y)) %>% unlist,
               
               "event_region_start" = purrr::map2(.x = gsub(x = .$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~min(.x, .y)) %>% unlist,
               "event_region_end" = purrr::map2(.x = gsub(x = .$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~max(.x, .y)) %>% unlist,
               "LIS_ID" = gsub(x = .$database_ID, pattern = "(.*)_(\\d+)", replacement = "\\1"),
               "exon_number" = gsub(x = .$diff_exon_coords, pattern = "(.*)_(\\d+)", replacement = "\\2")), .progress = TRUE)
...
# append alternative_exon_starts and alternative_exon_ends
tibble_lis_information <- wide_tibble_of_psisigma_results_allcomparisons_processed %>% 
  dplyr::group_by(LIS_ID) %>%
  dplyr::summarise(
    "alternative_exon_coords" = `diff_exon_coords` %>% unique %>% paste(collapse = ";"),
    "alternative_exon_starts" = `diff_exon_coords` %>% unique %>% gsub(pattern = "^(.*)\\:(\\d+)\\-(\\d+)", replacement = "\\2") %>% paste(collapse = ";"),
    "alternative_exon_ends" = `diff_exon_coords` %>% unique %>% gsub(pattern = "^(.*)\\:(\\d+)\\-(\\d+)", replacement = "\\3") %>% paste(collapse = ";")
    )

wide_tibble_of_psisigma_results_allcomparisons_processed <- dplyr::left_join(wide_tibble_of_psisigma_results_allcomparisons_processed, tibble_lis_information, by = "LIS_ID")
...
list_ref_matching_exon_entries_IR <- furrr::future_imap(.x = wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_IR, .f = function(a1, a2) {
  ...
  reference_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = a1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") %>% (function(x) {if (length(x) != 0) {x %>% rbindlist(use.names = TRUE) %>% as_tibble} } )
  
  reconstructed_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = a1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") %>% (function(x) {if (length(x) != 0) {x %>% rbindlist(use.names = TRUE) %>% as_tibble} } )
  
  return(splice(a1,
                "reference_gtf_match" = list(reference_gtf_match),
                "reconstructed_gtf_match" = list(reconstructed_gtf_match)) %>% suppressWarnings() %>% suppressMessages())
  
}, .progress = TRUE)

list_ref_matching_exon_entries_IR_annotated <- furrr::future_imap(.x = list_ref_matching_exon_entries_IR, .f = function(a1, a2) {
  
  # DEBUG ###
  # a1 <- list_ref_matching_exon_entries_IR[[2]]
  ###########
  
  # cat(a2, "\n")
  
  annotate_differential_exon(a1) %>% tibble::as_tibble() %>% return
...
list_ref_matching_exon_entries_not_IR <- future_map(.x = wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_not_IR, .f = function(a1, a2) {
  ...
  raw_reference_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$event_region_start %>% type.convert(as.is = TRUE), query_end = a1$event_region_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = FALSE, return_type = "exon")
  
  raw_reconstructed_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$event_region_start %>% type.convert(as.is = TRUE), query_end = a1$event_region_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = FALSE, return_type = "exon")
  
  # go to matching by exons only if VSR match failed
  if (length(raw_reference_gtf_match) == 0) {
    
    list_matched_exon_ref <- extract_matching.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = a1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon")
    ...
    list_matched_exon_ref <- purrr::map(.x = list_ref_parent_transcript_entries, 
                                        .f = function(b1) {
                                          ...
                                          tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= a1$diff_exon_end %>% type.convert(as.is = TRUE) & b1$end >= a1$diff_exon_start %>% type.convert(as.is = TRUE)), ]
                                          # if that fails, then check for adjacency
                                          if (tibble_matched_overlapping_exon_entry %>% nrow == 0) {
                                            tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= (a1$diff_exon_start %>% type.convert(as.is = TRUE)) - 1 & b1$end >= (a1$diff_exon_start %>% type.convert(as.is = TRUE)) - 1),]
                                          }
                                          
                                          if (tibble_matched_overlapping_exon_entry %>% nrow == 0) {
                                            tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= (a1$diff_exon_end %>% type.convert(as.is = TRUE)) + 1 & b1$end >= (a1$diff_exon_end %>% type.convert(as.is = TRUE)) + 1),]
                                          }
  ...
  if (length(raw_reconstructed_gtf_match) == 0) {
    ...
    list_matched_exon_recon <- extract_matching.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = a1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon")
  ...
  return(splice(a1,
                ...
                "reconstructed_gtf_match" = list(list_matched_exon_recon)) %>% suppressWarnings() %>% suppressMessages())
  
}, .progress = TRUE)

list_ref_matching_exon_entries_not_IR_annotated <- furrr::future_imap(.x = list_ref_matching_exon_entries_not_IR, .f = ~annotate_differential_exon(.x) %>% as_tibble, .progress = TRUE)
...
list_ref_matching_exon_entries_all_array.tree_IR_reannotated <- furrr::future_imap(.x = list_ref_matching_exon_entries_all_array.tree_IR, .f = function(.x, .y) {
  ...
  start <- .x$diff_exon_start %>% paste %>% trimws %>% type.convert(as.is = TRUE)
  end <- .x$diff_exon_end %>% paste %>% trimws %>% type.convert(as.is = TRUE)
  ...
  if (all(is.na(vec_ref_transcripts_matched_to_junction)) != TRUE) {
    ...
    tibble_ref_gtf_subset_junction_matching_entries <- dplyr::semi_join(tibble_ref_gtf[tibble_ref_gtf$type == "exon", ], vec_ref_transcripts_matched_to_junction, by = "transcript_id") %>% suppressWarnings() %>% suppressMessages()
    ...
    tibble_ref_gtf_subset_IR_overlap_start.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_start_coords) %>% suppressWarnings() %>% suppressMessages()
    ...
    tibble_ref_gtf_subset_IR_overlap_end.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_end_coords) %>% suppressWarnings() %>% suppressMessages()
    ...
    # find which IR matched entry has any same end coord
    tibble_ref_gtf_subset_IR_overlap_gene.name.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_gene_names) %>% suppressWarnings() %>% suppressMessages()
    ...
    tibble_union_start.and.end.matched <- list(tibble_ref_gtf_subset_IR_overlap_start.matched, 
                                            ...
                                            tibble_ref_gtf_subset_IR_overlap_gene.name.matched) %>% purrr::reduce(dplyr::union) %>% suppressWarnings() %>% suppressMessages()
  ...
  if (all(is.na(vec_recon_transcripts_matched_to_junction)) != TRUE) {
    ...
    # get all the entries which matched to the junction
    tibble_recon_gtf_subset_junction_matching_entries <- dplyr::semi_join(tibble_recon_gtf[tibble_recon_gtf$type == "exon", ], vec_recon_transcripts_matched_to_junction, by = "transcript_id") %>% suppressWarnings() %>% suppressMessages()
    
    # extract all the start coords of the junction-matched entries
    vector_junction_matched_start_coords <- tibble_recon_gtf_subset_junction_matching_entries[, "start"]
    # find which IR matched entry has any same start coord
    tibble_recon_gtf_subset_IR_overlap_start.matched <- dplyr::semi_join(tibble_recon_gtf_subset_IR_overlap, vector_junction_matched_start_coords) %>% suppressWarnings() %>% suppressMessages()
    
    # extract all the end coords of the junction-matched entries
    vector_junction_matched_end_coords <- tibble_recon_gtf_subset_junction_matching_entries[, "end"]
    # find which IR matched entry has any same end coord
    tibble_recon_gtf_subset_IR_overlap_end.matched <- dplyr::semi_join(tibble_recon_gtf_subset_IR_overlap, vector_junction_matched_end_coords, by = "end") %>% suppressWarnings() %>% suppressMessages()
    
    tibble_union_start.and.end.matched <- list(tibble_recon_gtf_subset_IR_overlap_start.matched, 
                                            tibble_recon_gtf_subset_IR_overlap_end.matched) %>% purrr::reduce(dplyr::union) %>% suppressWarnings() %>% suppressMessages()
  ...
  global_summarised_tibble_annotation <<- summarised_tibble_annotation
  ...
  summarised_tibble_annotation[, "NMD_biotype_reference"] <- if (tibble_all_confirmed_IR_entries_ref[[1]][1] == "uninit") {
    FALSE
    } else {
      all(tibble_all_confirmed_IR_entries_ref$transcript_biotype == "nonsense_mediated_decay")
    }
  
  # summarised_tibble_annotation[, "NMD_flagged_ref"] <- if (tibble_all_confirmed_IR_entries_ref[[1]][1] == "uninit") {
  #   FALSE
  #   } else {
  #     any(tibble_all_confirmed_IR_entries_ref$NMD_flagged_ref == "nonsense_mediated_decay")
  #   }
  # 
  #  summarised_tibble_annotation[, "NMD_flagged_recon"] <- if (tibble_all_confirmed_IR_entries_recon[[1]][1] == "uninit") {
  #    FALSE
  #    } else {
  #      all(tibble_all_confirmed_IR_entries_recon$NMD_flagged_recon == TRUE)
  #    }
   
   summarised_tibble_annotation[, "first_or_last_exon_reference"] <- if (tibble_all_confirmed_IR_entries_ref[[1]][1] == "uninit") {
     .x$first_or_last_exon_reference %>% paste(collapse = ";")
     } else {
       tibble_all_confirmed_IR_entries_ref$first_or_last_exon %>% paste(collapse = ";")
     }
   
   summarised_tibble_annotation[, "first_or_last_exon_recon"] <- if (tibble_all_confirmed_IR_entries_recon[[1]][1] == "uninit") {
...
# reorder columns
long_tibble_of_psisigma_DEXSeq_results <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::relocate(dpsi, pvalue, FDR, contains("comparison"), .after = "any_NMD")
...
list_missing_strand_info <- future_map(.x = row.indices_na_matched_strand, .f = ~tibble_ref_gtf[tibble_ref_gtf$gene_name == long_tibble_of_psisigma_DEXSeq_results[.x, "psisigma_gene.symbol"] %>% paste, "strand"] %>% unlist %>% unique %>% paste, .progress = TRUE)
...
list_of_psisigma_denominator_gct_tibbles <- future_map(.x = vector_of_comparisons_final, .f = ~read.delim(paste(psisigma_results_dir, .x, "/", .x, "_r1_ir3.denominator.gct", sep = ""), skip = 2, sep = "\t") %>% as_tibble, .progress = TRUE) %>% 
...
list_DEXSeq_results_tibble.only_with_PSI <- purrr::map2(.x = list_DEXSeq_results_tibble.only,
                                                        ...
                                                        .f = function(a1, a2) {
                                                          ...
                                                          #   dplyr::mutate("DEXSeq_dpsi" = comparison.1_average.psi - comparison.2_average.psi)
...
# select only cols that are informative
  dplyr::select(id, DEXSeq_exonBaseMean, DEXSeq_dispersion, DEXSeq_stat, DEXSeq_pvalue, DEXSeq_padj, DEXSeq_log2FC_second_minus_first, comparison)
...
# save image (for katana use)
# save.image(paste(R_processing_results_dir, "04082020_psisigma_dexseq_processed.RData", sep = ""))
# load(paste(R_processing_results_dir, "04082020_psisigma_dexseq_processed.RData", sep = ""))
...
long_tibble_of_psisigma_DEXSeq_results <- read.delim(paste(R_processing_results_dir, "long_tibble_of_psisigma_DEXSeq_results.txt", sep = ""), sep = "\t", row.names = NULL, header = TRUE, stringsAsFactors = FALSE) %>% tibble::as_tibble()

# write coords for poison exon finder
write.table(x = long_tibble_of_psisigma_DEXSeq_results[, c("id", "chr", "event_region_start", "event_region_end", "alternative_exon_starts", "alternative_exon_ends", "matched_strand", "splicemode", "matched_gene_names")] %>% dplyr::rename("VSR_start" = "event_region_start", "VSR_end" = "event_region_end", "strand" = "matched_strand", "gene_name" = "matched_gene_names", "custom_identifier" = "id") %>% dplyr::mutate("organism" = NA), file = paste(R_processing_results_dir, "long_tibble_of_psisigma_DEXSeq_results_prepare_for_poison_exon_finder.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_poison_exon_finder_results_processed0 <- tibble_poison_exon_finder_results %>%
  ...
  dplyr::select(-list_three_frame_translation)
...
long_tibble_of_psisigma_DEXSeq_results <- dplyr::left_join(long_tibble_of_psisigma_DEXSeq_results, tibble_poison_exon_finder_results_processed1, by = c("chr", "event_region_coords", "alternative_exon_starts", "alternative_exon_ends", "splicemode"))
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 <- dplyr::filter(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIV, comparison %in% vector_of_ud_comparisons) %>% dplyr::select(-dpsi, -pvalue, -FDR, -contains("comparison"), -contains("DEXSeq"), comparison, contains("average.psi")) %>% dplyr::rename("comparison_ud" = "comparison.2_average.psi")
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only <- reshape2::dcast(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp1, formula = event_region_coords + psisigma_gene.symbol + diff_exon_coords + splicemode + psisigma_exon_type + psisigma_reference.transcript + database_ID + chr + diff_exon_start + diff_exon_end + event_region_start + event_region_end + LIS_ID + exon_number + alternative_exon_coords + alternative_exon_starts + alternative_exon_ends + matched_gene_names + matched_strand + matched_ref_transcript_names + NMD_biotype_reference + NMD_flagged_ref + NMD_flagged_recon + poison_exon_candidate_ref + poison_exon_candidate_recon + contains_PTC_ref + contains_PTC_recon + first_or_last_exon_reference + first_or_last_exon_recon + any_first_exon + any_last_exon + matched_recon_transcript_names + any_NMD + introduces_PTC_for_all_matched_transcripts + introduces_PTC_for_any_matched_transcripts + alters_frame_for_all_matched_transcripts + alters_frame_for_any_matched_transcripts ~ timepoint, value.var = "PSI", fun.aggregate = function(x) {return(x[1])} ) %>% tibble::as_tibble()
...
list_of_tibbles_absolute_PSI_average_subset_by_exon <- future_map(.x = list_unique_exon_regions, .f = ~wide_tibble_of_absolute_PSI_averages_sum_temp[wide_tibble_of_absolute_PSI_averages_sum_temp$event_region_coords == (.x[[1]] %>% paste) & wide_tibble_of_absolute_PSI_averages_sum_temp$diff_exon_coords == (.x[[2]] %>% paste), ], .progress = TRUE)
...
list_of_tibbles_absolute_PSI_levels_exon.switching <- future_map(.x = wide_tibble_of_psisigma_PSI_with_na_LIV$event_region_coords %>% unique, .f = ~wide_tibble_of_psisigma_PSI_with_na_LIV[wide_tibble_of_psisigma_PSI_with_na_LIV$event_region_coords == .x, ], .progress = TRUE)
...
list_of_tibbles_absolute_PSI_levels_exon.switching <- future_map(.x = wide_tibble_of_psisigma_PSI_with_na_exon.sum$event_region_coords %>% unique, .f = ~wide_tibble_of_psisigma_PSI_with_na_exon.sum[wide_tibble_of_psisigma_PSI_with_na_exon.sum$event_region_coords == .x, ], .progress = TRUE)
...
wide_tibble_of_LIV_PSI_no_na_scaled_MSC_6h_1d <- dplyr::semi_join(wide_tibble_of_LIV_PSI_no_na_scaled, tibble_differential_results_MSC_6h_1d, by = c("database_ID", "event_region_coords", "diff_exon_coords", "splicemode"))

write.table(x = wide_tibble_of_LIV_PSI_no_na_scaled_MSC_6h_1d, file = paste(R_processing_results_dir, "wide_table_of_",  nrow(wide_tibble_of_LIV_PSI_no_na_scaled_MSC_6h_1d), "LIVs_scaled.PSI_OB_diff_MSC_6h_1d.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
list_matchtest_combinedexpr_som_sweep_byisoform <- purrr::map(.x = list_of_wide_df_combinedexpr_som_sweep_clusters, .f = function(.x) {
  ...
  list_of_correlation_2 <- furrr::future_map(.x = list_of_clusters, .f = ~base::apply(X = df_of_membership, MARGIN = 1, FUN = function(X){.x == X}), .progress = TRUE)
...
list_percentage_consensus_byisoform_combinedexpr <- purrr::map(.x = list_matchtest_combinedexpr_som_sweep_byisoform, .f = function(.x) {
  ...
  list_percentage_consensus <- furrr::future_map(.x = list_logical_coclustering_test, .f = ~base::apply(X = .x, MARGIN = 2, FUN = function(X){sum(X)/length(X)}) %>% t(.) %>% as_tibble, .progress = TRUE)
...
list_of_consensus_matrices_combinedexpr_dist_result <- furrr::future_map(.x = list_of_consensus_matrices_combinedexpr, .f = ~.x %>% Rfast::Dist(method = "euclidean") %>% as.dist, .progress = TRUE)
...
list_genes_or_exons_from_combinedexpr_cooccuring <- purrr::map2(.x = list_genes_or_exons_from_combinedexpr, .y = list_tibble_consensus_matrix_combinedexpr_long, .f = function(.x, .y) {
  ...
  list_of_coclustered_IDs <- future_map(.x = gene_identifier_IDs, .f = ~consensus_scores[consensus_scores$variable == .x & consensus_scores$cocluster_percentage >= 0.5, "gene_isoform_ID"], .progress = TRUE)
...
upset_combs <- data.frame(df %>% dplyr::select(., -event_region_coords, -diff_exon_coords), row.names = paste(df$event_region_coords, df$diff_exon_coords, sep = "_")) %>% make_comb_mat(mode = "distinct")
...
# query biomart
# list_of_tibbles_biomart_domain_annotation <- future_map(.x = list_of_attributes_to_retrieve, .f = ~getBM(attributes = .x, mart = ensembl_mart, curl = RCurl::getCurlMultiHandle()) %>% as_tibble %>% na.omit, .progress = TRUE)

# write tables
# future_map2(.x = list_of_tibbles_biomart_domain_annotation, .y = names(list_of_tibbles_biomart_domain_annotation), .f = ~write.table(.x, file = paste(shared_dir, "table_biomart_ENSP_to_", .y, "_38.98.txt", sep = ""), row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t"), .progress = TRUE)
...
# read tables
list_of_tibbles_biomart_domain_annotation <- future_map(.x = names(list_of_attributes_to_retrieve), .f = ~read.delim(paste(shared_dir, "table_biomart_ENSP_to_", .x, "_38.98.txt", sep = ""), row.names = NULL, header = TRUE, sep = "\t", stringsAsFactors = FALSE) %>% as_tibble, .progress = TRUE) %>% 
...
vector_event_region_start <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                .y = gsub(x = tibble_alternative_exons_identifier$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                .f = ~min(.x, .y)) %>% unlist
vector_event_region_end <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                              .y = gsub(x = tibble_alternative_exons_identifier$event_region_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                              .f = ~max(.x, .y)) %>% unlist

vector_diff_exon_start <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                             .y = gsub(x = tibble_alternative_exons_identifier$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                             .f = ~min(.x, .y)) %>% unlist
vector_diff_exon_end <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                           .y = gsub(x = tibble_alternative_exons_identifier$diff_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                           .f = ~max(.x, .y)) %>% unlist
  ...
  .f = function(a1, a2) {
    ...
    list_per_chromosome <- future_imap(
      ...
      .f = function(b1, b2) {
        ...
        vector_VSR_exon_coords <- b1[c("event_region_start", "event_region_end", "diff_exon_start", "diff_exon_end")] %>% unlist %>% type.convert(as.is = TRUE)
        ...
        if (grepl(x = b1$splicemode, pattern = intron_retention_string) == TRUE) {
          ...
          list_matched_GTF_exon_entries <- extract_matching.exons(query_chr = b1$chr, query_start = b1$event_region_start %>% type.convert(as.is = TRUE), query_end = b1$event_region_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") %>% dplyr::group_split(transcript_id) %>% set_names(nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
          ...
          if (list_matched_GTF_exon_entries %>% length == 0) {
            list_GTF_entries_matched_to_IR_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = b1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
            ...
            list_matched_GTF_exon_entries <- 
              tibble_all_candidate_overlapping_full_entries[tibble_all_candidate_overlapping_full_entries$type == "exon" &
                                                         tibble_all_candidate_overlapping_full_entries$start <= b1$diff_exon_start %>% type.convert(as.is = TRUE) &
                                                         tibble_all_candidate_overlapping_full_entries$end >= b1$diff_exon_end %>% type.convert(as.is = TRUE), ] %>% 
          ...
          # exact exon match
          list_matched_GTF_exon_entries <- extract_matching.exons(query_chr = b1$chr, query_start = b1$diff_exon_start %>% type.convert(as.is = TRUE), query_end = b1$diff_exon_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") %>% dplyr::group_split(transcript_id) %>% set_names(nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
          # get parent transcript entries
          list_parent_GTF_transcript_entries <- purrr::map(.x = list_matched_GTF_exon_entries %>% names, .f = ~a2[which(a2$transcript_id == .x), ]) %>% set_names(list_matched_GTF_exon_entries %>% names)
          ...
          common_coord <- intersect(c(b1$event_region_start %>% type.convert(as.is = TRUE), b1$event_region_end %>% type.convert(as.is = TRUE)), 
                                    c(b1$diff_exon_start %>% type.convert(as.is = TRUE), b1$diff_exon_end %>% type.convert(as.is = TRUE)))
          event_region_only_coord <- setdiff(c(b1$event_region_start %>% type.convert(as.is = TRUE), b1$event_region_end %>% type.convert(as.is = TRUE)), common_coord)
          exon_only_coord <- setdiff(c(b1$diff_exon_start %>% type.convert(as.is = TRUE), b1$diff_exon_end %>% type.convert(as.is = TRUE)), common_coord)
...
list_genome_relative_exonic.and.CDS_coords <- future_imap(.x = list_ref_entries_matched_to_exons.and.CDS, .f = function(a1, a2) {
  ...
  # cat("\nprocessing entry number: ", a2)
    ...
    vector_genomic_coords_of_all_differential_exon_positions <- (a1$diff_exon_start %>% type.convert(as.is = TRUE)):(a1$diff_exon_end %>% type.convert(as.is = TRUE))
    ...
    tibble_overlapping_parent_transcript_features <- tibble_parent_transcript_entries[which(tibble_parent_transcript_entries$start <= a1$diff_exon_end %>% type.convert(as.is = TRUE) & tibble_parent_transcript_entries$end >= a1$diff_exon_start %>% type.convert(as.is = TRUE)), ]
...
plan(list(tweak(multicore, workers = 30),
          tweak(multicore, workers = 8)))

# get IR events only
tibble_differential_IR_events <- wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$splicemode, pattern = "IR"), ] %>% dplyr::select(event_region_coords, diff_exon_coords, splicemode, matched_gene_names) %>% setNames(c("event_region_coords", "diff_exon_coords", "splicemode", "matched_gene_names")) %>% unique

# extract chr start end strand
tibble_differential_IR_events <- tibble_differential_IR_events %>% 
  dplyr::mutate(
    "chr" = gsub(x = `event_region_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1"),
    "strand" = "*",
    "event_region_start" = purrr::map2(.x = gsub(x = `event_region_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                .y = gsub(x = `event_region_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                .f = ~min(.x, .y)) %>% unlist,
    "event_region_end" = purrr::map2(.x = gsub(x = `event_region_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                              .y = gsub(x = `event_region_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                              .f = ~max(.x, .y)) %>% unlist,
    "diff_exon_start" = purrr::map2(.x = gsub(x = `diff_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                             .y = gsub(x = `diff_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                             .f = ~min(.x, .y)) %>% unlist,
    "diff_exon_end" = purrr::map2(.x = gsub(x = `diff_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                           .y = gsub(x = `diff_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                           .f = ~max(.x, .y)) %>% unlist
  )
  ...
  .f = function(a1, a2, a3) {
    ...
    list_L2_matched_ref_tibble_entries <- furrr::future_imap(
      ...
      .f = function(b1, b2) {
        ...
        list_tibbles_ref_matched_to_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$diff_exon_start, query_end = b1$diff_exon_end, query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
        
        # get the ENST IDs which contain the IR event region
        list_ref_entries_containing_IR_event_region <- a2[a2$type == "exon", ] %>% .[which(.$start == b1$event_region_start %>% type.convert(as.is = TRUE) | .$end == b1$event_region_end %>% type.convert(as.is = TRUE)),] %>% dplyr::group_split(transcript_id) %>% set_names(x = ., nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
        ...
        tibble_overlapping_features_with_IR_region <- extract_overlapping_features(query_chr = b1$chr, query_start = b1$diff_exon_start, query_end = b1$diff_exon_end, query_strand = b1$strand, tibble_gtf_table = a2, left_query_shift = 0, right_query_shift = 0, left_tolerance = 1, right_tolerance = 1, return_type = ".*") 
...
upset_combs <- data.frame(tibble_transcript_level_summary_count, row.names = paste(1:nrow(tibble_transcript_level_summary_count), tibble_transcript_level_summary_count$event_region_coords, tibble_transcript_level_summary_count$diff_exon_coords, sep = "|")) %>% make_comb_mat(mode = "distinct")
