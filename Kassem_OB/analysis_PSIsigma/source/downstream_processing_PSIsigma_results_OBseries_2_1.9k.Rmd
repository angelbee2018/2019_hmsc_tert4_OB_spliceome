...
psisigma_results_dir <- "/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_PSIsigma/results/run_1.9k/"
...
R_processing_results_dir <- "/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_PSIsigma/results/run_1.9k/R_processing_results/"
...
tibble_ref_gtf <- rtracklayer::import("/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_NMD_classifier/results/Homo_sapiens.GRCh38.98_NMD_PTC_E4.gtf") %>% as_tibble
tibble_recon_gtf <- rtracklayer::import("/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/analysis_NMD_classifier/results/2023_hmsc_ob_ont_alltimepoints_NMD_PTC_E4.gtf") %>% as_tibble
...
extract_junction.flanking.exons <- function(query_chr, query_start, query_end, query_strand, tibble_gtf_table, tolerance_left = 1, tolerance_right = 1, tolerance_inside = 1, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") {
  ...
  # DEBUG ###
  # query_chr = b1$chr
  # query_start = b1$alternative_exon_start
  # query_end = b1$alternative_exon_end
  # query_strand = "*"
  # tibble_gtf_table = a2
  # tolerance_left = 0
  # tolerance_right = 0
  ...
  if (query_strand == "." | query_strand == 0 | query_strand == "*") {
    ...
    tibble_gtf_subset_flanking_exons <- tibble_gtf_table[tibble_gtf_table$seqnames == query_chr %>% trimws, ] %>% .[.$start <= ((query_end %>% type.convert(as.is = TRUE)) + 1 + tolerance_outside + tolerance_left) & .$end >= ((query_start %>% type.convert(as.is = TRUE)) - 1 - tolerance_outside - tolerance_left), ] %>% .[!(.$start <= ((query_end %>% type.convert(as.is = TRUE)) - tolerance_inside - tolerance_right) & .$end >= ((query_start %>% type.convert(as.is = TRUE)) + tolerance_inside + tolerance_left)), ]
    
  } else if (query_strand == "+" | query_strand == "-") {
    
    tibble_gtf_subset_flanking_exons <- tibble_gtf_table[tibble_gtf_table$seqnames == query_chr %>% trimws, ] %>% .[.$strand == query_strand %>% trimws, ] %>% .[.$start <= ((query_end %>% type.convert(as.is = TRUE)) + 1 + tolerance_right) & .$end >= ((query_start %>% type.convert(as.is = TRUE)) - 1 - tolerance_left), ] %>% .[!(.$start <= ((query_end %>% type.convert(as.is = TRUE)) - tolerance_inside - tolerance_right) & .$end >= ((query_start %>% type.convert(as.is = TRUE)) + tolerance_inside + tolerance_left)), ]
  ...
  if (match_consecutive == TRUE) {
    ...
    list_of_tibbles_flanking_exon_gtf.entries_per_transcript <- purrr::map(.x = list_of_junction_associated_transcripts, .f = ~tibble_gtf_subset_flanking_exons[tibble_gtf_subset_flanking_exons$transcript_id == .x, ] %>% dplyr::arrange(exon_number %>% type.convert(as.is = TRUE))) %>% set_names(list_of_junction_associated_transcripts) %>% keep(.x = ., .p = ~nrow(.x) == 2) %>% keep(.x = ., .p = ~abs((.x[2, "exon_number"] %>% paste %>% type.convert(as.is = TRUE)) - (.x[1, "exon_number"] %>% paste %>% type.convert(as.is = TRUE))) == 1)
    
  } else if (match_consecutive == FALSE) {
    
    list_of_tibbles_flanking_exon_gtf.entries_per_transcript <- purrr::map(.x = list_of_junction_associated_transcripts, .f = ~tibble_gtf_subset_flanking_exons[tibble_gtf_subset_flanking_exons$transcript_id == .x, ] %>% dplyr::arrange(exon_number %>% type.convert(as.is = TRUE))) %>% set_names(list_of_junction_associated_transcripts) %>% keep(.x = ., .p = ~nrow(.x) == 2)
...
# NOTE: to be used with purrr
# input: spliceregion_list: a list containing details of ONE junction: $chr, $alternative_exon_start, $alternative_exon_end
# tibble_gtf_table: rtracklayer::import(...) %>% as_tibble. can be any GTF. reconstructed or reference.
# index: loop progress marker to be used with imap
...
extract_matching.exons <- function(query_chr, query_start, query_end, query_strand, tibble_gtf_table, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") {
  ...
  if (query_strand == "." | query_strand == 0 | query_strand == "*") {
    ...
    tibble_gtf_subset_overlapping_exons <- tibble_gtf_table[tibble_gtf_table$seqnames == query_chr %>% trimws, ] %>% 
      .[which(.$start > ((query_start %>% type.convert(as.is = TRUE)) - 1 - tolerance_left - tolerance_outside) & .$end < ((query_end %>% type.convert(as.is = TRUE)) + 1 + tolerance_right + tolerance_outside)), ] %>% 
      .[which((.$start < ((query_start %>% type.convert(as.is = TRUE)) + 1 + tolerance_left + tolerance_inside) & .$end > ((query_end %>% type.convert(as.is = TRUE)) - 1 - tolerance_right - tolerance_inside))), ] %>% 
...
extract_overlapping_features <- function(query_chr, query_start, query_end, query_strand, tibble_gtf_table, left_query_shift = 0, right_query_shift = 0, left_tolerance = 0, right_tolerance = 0, return_type) {
  ...
  if (tibble_gtf_subset_matching_exons %>% nrow > 0) {
    
    tibble_gtf_subset_matching_exons <- tibble_gtf_subset_matching_exons %>% dplyr::filter(str_detect(string = `type`, pattern = return_type))
...
map_genome_coordinate_to_transcriptandprotein <- function(chr_strand_start_end_tibble, gtf_table, mode, targetlevel = NULL) {
  ...
  list_of_query_AS_ID <- chr_strand_start_end_tibble$alternative_exon_coords %>% array_tree
...
list_of_all_overlapping_annotations <- future_imap(.x = list_of_query_AS_ID, .f = function(.x, .y) {
  ...
  overlapping_gtf_entries <- gtf_table[(gtf_table$chr == chr_strand_start_end_tibble[chr_strand_start_end_tibble$alternative_exon_coords == .x, "chr"] %>% unique %>% paste %>% as.character) &
    (gtf_table$strand == chr_strand_start_end_tibble[chr_strand_start_end_tibble$alternative_exon_coords == .x, "strand"] %>% unique %>% paste %>% as.character), ] %>% 
    .[(.$start < chr_strand_start_end_tibble[chr_strand_start_end_tibble$alternative_exon_coords == .x, "end"] %>% unique %>% paste %>% as.numeric) & 
        (.$end > chr_strand_start_end_tibble[chr_strand_start_end_tibble$alternative_exon_coords == .x, "start"] %>% unique %>% paste %>% as.numeric), ]
...
tibble_queryisoforms_chr_strand_start_end_to_annotation_and_proteindomain <- function(tibble_queryisoforms_chr_strand_start_end, gtf_table, ensemblmart) {
  ...
  names(list_queryisoforms_overlapping_annotations_tibbles) <- tibble_queryisoforms_chr_strand_start_end$alternative_exon_coords
  ...
  tibble_transcript_level_results <- purrr::map2(.x = list_of_tibbles_transcript_level_results, .y = names(list_of_tibbles_transcript_level_results), .f = ~add_column(.x, "alternative_exon_coords" = .y, .before = 1)) %>% rbindlist %>% as_tibble
  ...
  list_of_tibbles_protein_level_results <- purrr::map2(.x = coinciding_domains_PTMs, .y = names(coinciding_domains_PTMs), .f = function(.x, .y) {
    ...
    # splice in the level 1 list name (in this case, it's alternative_exon_coords)
    list_L2_final <- purrr::map(.x = list_L2_processed, .f = ~purrr::splice(list("alternative_exon_coords" = L1_id), 
                                                                            .x))
...
list_of_psisigma_result_tibbles_raw <- furrr::future_map(.x = vector_OBseries_timepoints_raw, .f = ~read.delim(paste(psisigma_results_dir, "/BM_MSC_to_OB_", .x, "/2019_hmsc_to_ob_universal_r1_ir3.sorted.txt", sep = ""), sep = "\t") %>% as_tibble, .progress = TRUE)

# tibble_psisigma_dbfile <- data.table::fread(file = paste(psisigma_results_dir, "/2019_hmsc_to_ob_universal.db", sep = ""), sep = "\t", fill = TRUE) %>%
#   tibble::as_tibble() %>% 
#   .[!is.na(.$V2), ] %>%
#   setNames(nm = c("chr", "left_junc_start", "left_junc_end", "right_junc_start", "right_junc_end", "alternative_exon_start", "alternative_exon_end", "psisigma_nmd", "VSR_start", "VSR_end", "database_ID", "psisigma_gene_symbol")) %>% 
#   tibble::add_column("LIS_ID" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\1"), 
...
# 
# tibble_psisigma_dbfile_summarised <- tibble_psisigma_dbfile %>% 
#   dplyr::arrange(alternative_exon_start) %>%
#   dplyr::group_by(LIS_ID) %>%
#   dplyr::summarise("LIS_exon_coords" = alternative_exon_coords %>% paste(collapse = ";"))

```

## process column names, split the absolute PSI values and append comparison info

It is okay to not include ALL MES exons in a given sample because all partial variations of MES are represented by the same LIS ID and the same VSR coord.
Technically, PSI-Sigma does not in fact measure VSR coordinates - it indeed measures event coords because it doesn't make inferences over entire proper VSRs if they are very complex.
Therefore, we don't need the db file to generate all alternative exon coords.

```{r}

list_of_psisigma_result_tibbles_processed <- furrr::future_map(.x = list_of_psisigma_result_tibbles_raw, .f = 
                                                                 # rename all columns
                                                                 ~.x %>%
                                                                 dplyr::select(-`T`, -`T.Values`) %>%
                                                                 setNames(c("VSR_coords", "psisigma_gene_symbol", "alternative_exon_coords", "splicemode", "num.replicates", "psisigma_exon_type", "psisigma_reference.transcript", "dpsi", "pvalue", "FDR", "psi", "database_ID")) %>%
                                                                 # separate absolute PSI columns
                                                                 tidyr::separate(., col = psi, sep = "\\|", into = c("psi_1", "psi_2", "psi_3")) %>% 
                                                                 # separate the co-ordinates of the differential exon in preparation for matching
                                                                 add_column("chr" = gsub(x = .$alternative_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1"),
                                                                            
                                                                            "VSR_start" = purrr::map2(.x = gsub(x = .$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~min(.x, .y)) %>% unlist,
                                                                            "VSR_end" = purrr::map2(.x = gsub(x = .$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .y = gsub(x = .$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE), .f = ~max(.x, .y)) %>% unlist) %>% 
                                                                 dplyr::relocate(database_ID, .before = 1) %>%
                                                                 tibble::add_column("LIS_ID" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\1"), 
                                                                                    "exon_number" = gsub(x = .$database_ID, pattern = "(.*ENST.*)_(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE), .after = "database_ID"), .progress = TRUE)

# add column indicating what timepoint the tibble is from
list_of_psisigma_result_tibbles_processed <- purrr::map2(.x = list_of_psisigma_result_tibbles_processed, .y = vector_OBseries_timepoints_edited, .f = ~add_column(.x, "timepoint" = .y))

tibble_psisigma_result_tibbles_processed <- list_of_psisigma_result_tibbles_processed %>%
  data.table::rbindlist() %>%
  tibble::as_tibble() %>% 
  type_convert(na = c("", "NA", "na"))

```

## add numerator information

first, add numerator info so that we can filter out events with at least 3 reads in at least 2 samples

```{r}

list_of_psisigma_denominator_gct_tibbles <- furrr::future_map(.x = vector_OBseries_timepoints_raw, .f = ~read.delim(paste(psisigma_results_dir, "/BM_MSC_to_OB_", .x, "/2019_hmsc_to_ob_universal_r1_ir3.denominator.gct", sep = ""), skip = 2, sep = "\t") %>% tibble::as_tibble(), .progress = TRUE)

tibble_psisigma_denominator_gct_tibbles_processed <- purrr::map2(
  .x = list_of_psisigma_denominator_gct_tibbles, 
  .y = vector_OBseries_timepoints_edited,
  .f = function(a1, a2) {
    ...
    tibble_temp <- tibble_temp %>% 
      setNames(c("database_ID", "Annotation", "denominator_1", "denominator_2", "denominator_3", "comparison.2_denominator_1", "comparison.2_denominator_2")) %>% 
      
      add_column("timepoint" = a2) %>%
      dplyr::select(-Annotation, -contains("comparison.2"))
    
    return(tibble_temp)
    
  } ) %>% data.table::rbindlist() %>% tibble::as_tibble() %>% type_convert(na = c("", "na", "NA"))

# bind to main table

tibble_psisigma_result_tibbles_processed_with_denominator <- dplyr::left_join(tibble_psisigma_result_tibbles_processed, tibble_psisigma_denominator_gct_tibbles_processed, by = c("database_ID", "timepoint"))

tibble_psisigma_result_tibbles_processed_with_numerator <- tibble_psisigma_result_tibbles_processed_with_denominator %>% 
  
  # add in the numerator values
  dplyr::mutate("numerator_1" = psi_1 * denominator_1 * 0.01) %>%
  dplyr::mutate("numerator_2" = psi_2 * denominator_2 * 0.01) %>%
  dplyr::mutate("numerator_3" = psi_3 * denominator_3 * 0.01) %>%
  
  # add in the ER values
  dplyr::mutate("ER_1" = denominator_1 - numerator_1) %>%
  dplyr::mutate("ER_2" = denominator_2 - numerator_2) %>%
  dplyr::mutate("ER_3" = denominator_3 - numerator_3)

tibble_psisigma_result_tibbles_processed_with_numerator <- tibble_psisigma_result_tibbles_processed_with_numerator %>% 
  dplyr::mutate("id" = paste("id_", 1:nrow(tibble_psisigma_result_tibbles_processed_with_numerator), sep = ""), .before = 1)

# filter now

## filter out more than 1 NA PSI
tibble_psisigma_result_tibbles_processed_filtered <- tibble_psisigma_result_tibbles_processed_with_numerator[
  purrr::pmap(
  .l = list(
    "a1" = is.na(tibble_psisigma_result_tibbles_processed_with_numerator$psi_1),
    "a2" = is.na(tibble_psisigma_result_tibbles_processed_with_numerator$psi_2),
    "a3" = is.na(tibble_psisigma_result_tibbles_processed_with_numerator$psi_3)
  ),
  .f = function(a1, a2, a3) {
    return(
      length(which(c(a1, a2, a3) == TRUE)) < 2
      )
  } ) %>% unlist,
  
]

## filter out only exons with at least 3 exons in at least 2 replicates
tibble_psisigma_result_tibbles_processed <- tibble_psisigma_result_tibbles_processed_filtered[
  purrr::pmap(
  .l = list(
    "a1" = tibble_psisigma_result_tibbles_processed_filtered$numerator_1,
    "a2" = tibble_psisigma_result_tibbles_processed_filtered$numerator_2,
    "a3" = tibble_psisigma_result_tibbles_processed_filtered$numerator_3
  ),
  .f = function(a1, a2, a3) {
    return(
      length(which(na.omit(c(a1, a2, a3)) >=3)) >= 2
      )
  } ) %>% unlist,
  
]

## treat NA PSI values as 0
tibble_psisigma_result_tibbles_processed[is.na(tibble_psisigma_result_tibbles_processed$psi_1), "psi_1"] <- 0
tibble_psisigma_result_tibbles_processed[is.na(tibble_psisigma_result_tibbles_processed$psi_2), "psi_2"] <- 0
tibble_psisigma_result_tibbles_processed[is.na(tibble_psisigma_result_tibbles_processed$psi_3), "psi_3"] <- 0

```

## add in the LIS exon coords information

```{r}

# generate LIS exon coords per timepoint, per LIS ID.
tibble_psisigma_result_tibbles_processed_with_LIS_data <- tibble_psisigma_result_tibbles_processed %>% 
  dplyr::group_by(LIS_ID, timepoint) %>%
  dplyr::group_split() %>%
  furrr::future_map(
    .x = .,
    .f = function(a1) {
      
      tibble_result <- a1 %>% dplyr::mutate("LIS_exon_starts" = gsub(x = a1$alternative_exon_coords, pattern = "\\:(\\d+)\\-(\\d+)", replacement = "\\1") %>% paste(collapse = ";"),
                                            "LIS_exon_ends" = gsub(x = a1$alternative_exon_coords, pattern = "\\:(\\d+)\\-(\\d+)", replacement = "\\2") %>% paste(collapse = ";"))
...
tibble_psisigma_result_tibbles_processed_with_LIS_data[, "splicemode"] <- gsub(x = tibble_psisigma_result_tibbles_processed_with_LIS_data$splicemode, pattern = "(.*)overlapping(.*)", replacement = "IR_overlapping_region")

tibble_psisigma_result_tibbles_processed_with_LIS_data <- tibble_psisigma_result_tibbles_processed_with_LIS_data %>%
  dplyr::mutate(
    "alternative_exon_start" = gsub(x = alternative_exon_coords, pattern = "^.*\\:(\\d+)\\-(\\d+)", replacement = "\\1"),
    "alternative_exon_end" = gsub(x = alternative_exon_coords, pattern = "^.*\\:(\\d+)\\-(\\d+)", replacement = "\\2")
  )

# impute missing values of PSI

# tibble_psisigma_result_tibbles_processed_with_LIS_data <- tibble_psisigma_result_tibbles_processed_with_LIS_data %>% as.data.frame

# tibble_psisigma_result_tibbles_processed_with_LIS_data[, colnames(tibble_psisigma_result_tibbles_processed_with_LIS_data)[grep(x = colnames(tibble_psisigma_result_tibbles_processed_with_LIS_data), pattern = "psi_")]] <- future_map2(.x = tibble_psisigma_result_tibbles_processed_with_LIS_data %>% dplyr::select(contains("psi_")) %>% array_tree(margin = 1), .y = tibble_psisigma_result_tibbles_processed_with_LIS_data %>% dplyr::select(contains("psi_")) %>% rowMeans(na.rm = TRUE), .f = function(a1, a2) {a1[is.na(a1)] <- a2; return(a1 %>% t %>% as.data.frame)}, .progress = TRUE ) %>% rbindlist(use.names = TRUE) %>% tibble::as_tibble()

# tibble_psisigma_result_tibbles_processed_with_LIS_data <- tibble_psisigma_result_tibbles_processed_with_LIS_data %>% tibble::as_tibble()
...
wide_tibble_of_all_unique_VSR_and_exon_coords <- tibble_psisigma_result_tibbles_processed_with_LIS_data[, c("splicemode", "alternative_exon_coords", "VSR_coords", "chr", "alternative_exon_start", "alternative_exon_end", "VSR_start", "VSR_end")] %>% unique

# wide_tibble_of_all_unique_VSR_and_exon_coords <- tibble_psisigma_result_tibbles_processed_with_LIS_data[sample(1:nrow(tibble_psisigma_result_tibbles_processed_with_LIS_data), 1000), c("splicemode", "alternative_exon_coords", "VSR_coords", "chr", "alternative_exon_start", "alternative_exon_end", "VSR_start", "VSR_end")] %>% unique
...
list_ref_matching_exon_entries_IR <- furrr::future_imap(.x = wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_IR, .f = function(a1, a2) {
  
  # DEBUG ###
  # a1 <- wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_IR[[57100]]
  ###########
  
  cat(a2, "\n")
  
  # match exons to reference for IR events
  reference_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = a1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") %>% (function(x) {if (length(x) != 0) {x %>% rbindlist(use.names = TRUE) %>% as_tibble} } )
  
  reconstructed_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = a1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon") %>% (function(x) {if (length(x) != 0) {x %>% rbindlist(use.names = TRUE) %>% as_tibble} } )
...
list_ref_matching_exon_entries_not_IR <- future_map(.x = wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_not_IR, .f = function(a1, a2) {
  ...
  # DEBUG ###
  # a1 <- wide_tibble_of_all_unique_VSR_and_exon_coords_array.tree_not_IR[[110]]
  ###########
  ...
  raw_reference_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$VSR_start %>% type.convert(as.is = TRUE), query_end = a1$VSR_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = FALSE, return_type = "exon")
  
  raw_reconstructed_gtf_match <- extract_junction.flanking.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$VSR_start %>% type.convert(as.is = TRUE), query_end = a1$VSR_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = FALSE, return_type = "exon")
  
  # go to matching by exons only if VSR match failed
  if (length(raw_reference_gtf_match) == 0) {
    
    list_matched_exon_ref <- extract_matching.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = a1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_ref_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon")
    ...
    list_matched_exon_ref <- purrr::map(.x = list_ref_parent_transcript_entries, 
                                        .f = function(b1) {
                                          ...
                                          tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= a1$alternative_exon_end %>% type.convert(as.is = TRUE) & b1$end >= a1$alternative_exon_start %>% type.convert(as.is = TRUE)), ]
                                          # if that fails, then check for adjacency
                                          if (tibble_matched_overlapping_exon_entry %>% nrow == 0) {
                                            tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= (a1$alternative_exon_start %>% type.convert(as.is = TRUE)) - 1 & b1$end >= (a1$alternative_exon_start %>% type.convert(as.is = TRUE)) - 1),]
                                          }
                                          
                                          if (tibble_matched_overlapping_exon_entry %>% nrow == 0) {
                                            tibble_matched_overlapping_exon_entry <- b1[which(b1$start <= (a1$alternative_exon_end %>% type.convert(as.is = TRUE)) + 1 & b1$end >= (a1$alternative_exon_end %>% type.convert(as.is = TRUE)) + 1),]
                                          }
  ...
  if (length(raw_reconstructed_gtf_match) == 0) {
    ...
    list_matched_exon_recon <- extract_matching.exons(query_chr = a1$chr %>% type.convert(as.is = TRUE), query_start = a1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = a1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = "*", tibble_gtf_table = tibble_recon_gtf, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon")
...
list_ref_matching_exon_entries_not_IR_annotated <- future_imap(.x = list_ref_matching_exon_entries_not_IR, .f = ~annotate_differential_exon(.x) %>% as_tibble, .progress = TRUE)

tibble_ref_matching_exon_entries_all <- dplyr::bind_rows(list_ref_matching_exon_entries_IR_annotated %>% rbindlist, list_ref_matching_exon_entries_not_IR_annotated %>% data.table::rbindlist()) %>% tibble::as_tibble()
...
tibble_ref_matching_exon_entries_all_not.IR_summarised <- tibble_ref_matching_exon_entries_all[-row.indices_IR_events, ]
...
list_ref_matching_exon_entries_all_array.tree_IR_reannotated <- furrr::future_imap(.x = list_ref_matching_exon_entries_all_array.tree_IR, .f = function(.x, .y) {
  ...
  start <- .x$alternative_exon_start %>% paste %>% trimws %>% type.convert(as.is = TRUE)
  end <- .x$alternative_exon_end %>% paste %>% trimws %>% type.convert(as.is = TRUE)
  ...
  if (all(is.na(vec_ref_transcripts_matched_to_junction)) != TRUE) {
    ...
    tibble_ref_gtf_subset_junction_matching_entries <- dplyr::semi_join(tibble_ref_gtf[tibble_ref_gtf$type == "exon", ], vec_ref_transcripts_matched_to_junction, by = "transcript_id") %>% suppressMessages() %>% suppressWarnings()
    ...
    tibble_ref_gtf_subset_IR_overlap_start.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_start_coords) %>% suppressMessages() %>% suppressWarnings()
    ...
    tibble_ref_gtf_subset_IR_overlap_end.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_end_coords) %>% suppressMessages() %>% suppressWarnings()
    ...
    # find which IR matched entry has any same end coord
    tibble_ref_gtf_subset_IR_overlap_gene.name.matched <- dplyr::semi_join(tibble_ref_gtf_subset_IR_overlap, vector_junction_matched_gene_names) %>% suppressMessages() %>% suppressWarnings()
  ...
  if (all(is.na(vec_recon_transcripts_matched_to_junction)) != TRUE) {
    ...
    # get all the entries which matched to the junction
    tibble_recon_gtf_subset_junction_matching_entries <- dplyr::semi_join(tibble_recon_gtf[tibble_recon_gtf$type == "exon", ], vec_recon_transcripts_matched_to_junction, by = "transcript_id") %>% suppressMessages() %>% suppressWarnings()
    
    # extract all the start coords of the junction-matched entries
    vector_junction_matched_start_coords <- tibble_recon_gtf_subset_junction_matching_entries[, "start"]
    # find which IR matched entry has any same start coord
    tibble_recon_gtf_subset_IR_overlap_start.matched <- dplyr::semi_join(tibble_recon_gtf_subset_IR_overlap, vector_junction_matched_start_coords) %>% suppressMessages() %>% suppressWarnings()
    
    # extract all the end coords of the junction-matched entries
    vector_junction_matched_end_coords <- tibble_recon_gtf_subset_junction_matching_entries[, "end"]
    # find which IR matched entry has any same end coord
    tibble_recon_gtf_subset_IR_overlap_end.matched <- dplyr::semi_join(tibble_recon_gtf_subset_IR_overlap, vector_junction_matched_end_coords, by = "end") %>% suppressMessages() %>% suppressWarnings()
  ...
  global_tibble_all_confirmed_IR_entries_ref <<- tibble_all_confirmed_IR_entries_ref
  ...
  summarised_tibble_annotation[, "matched_ref_transcript_names"] <- if (tibble_all_confirmed_IR_entries_ref[[1]][1] != "uninit") {
    tibble_all_confirmed_IR_entries_ref$transcript_id %>% paste(collapse = ",")
  }
  
  summarised_tibble_annotation[, "matched_recon_transcript_names"] <- if (tibble_all_confirmed_IR_entries_recon[[1]][1] != "uninit") {
...
tibble_ref_matching_exon_entries_all_summarised <- dplyr::bind_rows(tibble_as.event.id_with_summarised_annotation_IR %>% readr::type_convert() %>% readr::type_convert(), 
                                                                    # look at this shit LOL
                                                                    tibble_ref_matching_exon_entries_all_not.IR_summarised %>% readr::type_convert() %>% readr::type_convert())

# left join of the annotated exon info onto the PSI-Sigma result tables
long_tibble_of_psisigma_DEXSeq_results <- dplyr::left_join(tibble_psisigma_result_tibbles_processed_with_LIS_data %>% readr::type_convert(), tibble_ref_matching_exon_entries_all_summarised %>% readr::type_convert(), by = c("VSR_coords" ,"alternative_exon_coords", "splicemode", "chr", "alternative_exon_start", "alternative_exon_end", "VSR_start", "VSR_end")) %>% dplyr::mutate_if(is.factor, as.character)

cat("There are", long_tibble_of_psisigma_DEXSeq_results[, c("VSR_coords", "alternative_exon_coords")] %>% unique %>% nrow, "exons detected in total (at least one observation across the entire time series)")
...
long_tibble_of_psisigma_DEXSeq_results[row.indices_na_matched_gene_name, "matched_gene_names"] <- long_tibble_of_psisigma_DEXSeq_results[row.indices_na_matched_gene_name, "psisigma_gene_symbol"]
...
list_missing_strand_info <- furrr::future_map(.x = row.indices_na_matched_strand, .f = ~tibble_ref_gtf[tibble_ref_gtf$gene_name == long_tibble_of_psisigma_DEXSeq_results[.x, "psisigma_gene_symbol"] %>% paste, "strand"] %>% unlist %>% unique %>% paste, .progress = TRUE)
...
long_tibble_of_psisigma_DEXSeq_results <- long_tibble_of_psisigma_DEXSeq_results %>%
  dplyr::relocate(matched_gene_names, matched_strand, .after = "psisigma_gene_symbol") %>%
  dplyr::select(-psisigma_gene_symbol, -matched_ref_transcript_names, -matched_recon_transcript_names)
...
## filter out only exons with at least 3 exons in at least 2 replicates
wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3 <- long_tibble_of_psisigma_DEXSeq_results[
  purrr::pmap(
  .l = list(
    "a1" = long_tibble_of_psisigma_DEXSeq_results$numerator_1,
    "a2" = long_tibble_of_psisigma_DEXSeq_results$numerator_2,
    "a3" = long_tibble_of_psisigma_DEXSeq_results$numerator_3
  ),
  .f = function(a1, a2, a3) {
    return(
      length(which(na.omit(c(a1, a2, a3)) >= 5)) >= 3
      )
  } ) %>% unlist,
...
We have to proceed with two different types of analyses - first having pairwise then second having just a general GLM.
...
### build GFF tibble and export (per comparison)
...
vector_of_comparisons_final <- combn(x = vector_OBseries_timepoints_edited, m = 2) %>% t %>% (function(x) {return(paste(x[, 1], "_minus_", x[, 2], sep = ""))})

# group_split by comparison
list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_comparison <- purrr::map(
  .x = combn(x = vector_OBseries_timepoints_edited, m = 2) %>% t %>% purrr::array_tree(),
  .f = function(a1) {
    
    # DEBUG ###
    # a1 <- list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_comparison[[1]]
    # a2 <- combn(x = vector_OBseries_timepoints_edited, m = 2) %>% t %>% purrr::array_tree() %>% .[[1]]
    ###########
    
    tibble_per_comparison <- wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3[wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3$timepoint %in% a1, ]
    
    tibble_bound_comparisons <- tibble_per_comparison %>% 
      dplyr::select(id, database_ID, VSR_coords, VSR_start, VSR_end, alternative_exon_coords, alternative_exon_start, alternative_exon_end, matched_strand, chr, splicemode, numerator_1, numerator_2, numerator_3, ER_1, ER_2, ER_3, timepoint) %>%
      dplyr::group_split(timepoint) %>%
      purrr::map(.f = function(b1) {
        tibble_per_timepoint <- b1
        timepoint <- tibble_per_timepoint$timepoint %>% unique
        names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "numerator\\_")] <- paste(names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "numerator\\_")], "_", timepoint, sep = "")
        names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "ER\\_")] <- paste(names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "ER\\_")], "_", timepoint, sep = "")
        # names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "id")] <- paste(names(tibble_per_timepoint)[grep(x = names(tibble_per_timepoint), pattern = "id")], "_", timepoint, sep = "")
        tibble_per_timepoint <- tibble_per_timepoint %>% dplyr::select(-timepoint)
        return(tibble_per_timepoint)
      } ) %>%
      purrr::reduce(dplyr::left_join, by = c("database_ID", "VSR_coords", "VSR_start", "VSR_end", "alternative_exon_coords", "alternative_exon_start", "alternative_exon_end", "matched_strand", "chr", "splicemode")) %>%
      dplyr::mutate("id_combined" = paste(id.x, "...", id.y, sep = "")) %>%
      dplyr::select(-id.x, -id.y) %>%
      .[dplyr::select(.data = ., contains("numerator_"), contains("ER_")) %>% apply(MARGIN = 1, FUN = function(X) {any(is.na(X)) == FALSE} ), ]
    
  } ) %>% purrr::set_names(nm = vector_of_comparisons_final)
...
furrr::future_map2(
  ...
  .f = function(a1, a2) {
    ...
    tibble_GFF_building_exons_info <- a1 %>% dplyr::distinct(id_combined, database_ID, VSR_coords, alternative_exon_coords, splicemode, .keep_all = TRUE)
    ...
    list_GFF_building_tibble <- furrr::future_map(
      .x = list_GFF_building_exons_info, 
      .f = ~tibble("seqnames" = .x$chr,
                   "start" = c(.x$VSR_start, .x$alternative_exon_start, .x$VSR_start),
                   "end" = c(.x$VSR_end, .x$alternative_exon_end, .x$VSR_end),
    ...
    rtracklayer::export(object = tibble_combined_GFF1, con = paste(DEXSeq_dir, a2, "_combined_dexseq_GFF_PSISigma_by_comparison.gff", sep = ""), format = "gff2")
    
  }, .progress = TRUE )

```

### build GFF tibble and export (per timepoint)

```{r}

# group_split
wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3 %>% (function(a1) {
    
    # DEBUG ###
    # a1 <- list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint[[1]]
    ###########
    
    # cat("now processing: ", a2)
    
    # only use distinct exons needed to build the GFF
    tibble_GFF_building_exons_info <- a1 %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, .keep_all = TRUE)
    ...
    rtracklayer::export(object = tibble_combined_GFF1, con = paste(DEXSeq_dir, "combined_dexseq_GFF_PSISigma_by_timepoint.gff", sep = ""), format = "gff2")
...
list_combined_count_tibbles_by_comparison <- purrr::map2(
  .x = list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_comparison,
  .y = combn(x = vector_OBseries_timepoints_edited, m = 2) %>% t %>% purrr::array_tree(),
  .f = function(a1, a2) {
    ...
    purrr::map(
      .x = purrr::cross2(a2, 1:3),
      .f = function(b1) {
        
        dplyr::bind_rows(
          tibble("gene:exon" = paste(a1$id_combined, ":001", sep = "") %>% unlist,
                 "count" = a1[, paste("numerator_", b1[[2]], "_", b1[[1]], sep = "")] %>% unlist),
          tibble("gene:exon" = paste(a1$id_combined, ":002", sep = "") %>% unlist,
                 "count" = a1[, paste("ER_", b1[[2]], "_", b1[[1]], sep = "")] %>% unlist)
        )
        
      } ) %>% 
      purrr::set_names(nm = purrr::cross2(a2, 1:3) %>% purrr::map(.f = ~paste(.x[[1]], "_replicate_", .x[[2]], sep = ""))) %>%
      return
...
# write tibbles
furrr::future_map2(
  .x = list_combined_count_tibbles_by_comparison, 
  .y = names(list_combined_count_tibbles_by_comparison),
  .f = function(a1, a2) {
    ...
    purrr::map2(
      ...
      .f = function(b1, b2) {
        ...
        write.table(x = b1 %>% .[mixedorder(.$`gene:exon`), ] %>% dplyr::mutate_at(.vars = "count", round), file = paste(DEXSeq_dir, a2, "_", b2, "_combined_count_by_comparison.txt", sep = ""), sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
# group_split
list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint <- purrr::map(
  .x = vector_OBseries_timepoints_edited,
  .f = function(a1) {
    return(
      wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3[wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3$timepoint == a1, ]
    )
  }
) %>% purrr::set_names(nm = vector_OBseries_timepoints_edited)

# map each timepoint >> map each replicate
list_combined_count_tibbles_by_timepoint <- purrr::map2(
  .x = list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint,
  .y = names(list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint),
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a1 <- list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint[[1]]
    # a2 <- names(list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_timepoint) %>% .[[1]]
    ###########
    ...
    purrr::map(
      ...
      .f = function(b1) {
        ...
        dplyr::bind_rows(
          tibble("gene:exon" = paste(a1$id, ":001", sep = "") %>% unlist,
                 "count" = a1[, paste("numerator_", b1, sep = "")] %>% unlist),
          tibble("gene:exon" = paste(a1$id, ":002", sep = "") %>% unlist,
                 "count" = a1[, paste("ER_", b1, sep = "")] %>% unlist)
        ) %>%
          return
        
      } ) %>% 
      purrr::set_names(nm = paste("replicate_", 1:3, sep = "")) %>%
      return
...
# write tibbles
furrr::future_map2(
  .x = list_combined_count_tibbles_by_timepoint, 
  .y = names(list_combined_count_tibbles_by_timepoint),
  .f = function(a1, a2) {
    ...
    purrr::map2(
      ...
      .f = function(b1, b2) {
        ...
        write.table(x = b1 %>% .[mixedorder(.$`gene:exon`), ] %>% dplyr::mutate_at(.vars = "count", round), file = paste(DEXSeq_dir, a2, "_", b2, "_combined_count_by_timepoint.txt", sep = ""), sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
plan(list(
  tweak(multicore, workers = 64),
  tweak(multicore, workers = 4)
          ))

list_DEXSeq_results_by_comparison <- furrr::future_map(
  .x = vector_of_comparisons_final,
  .f = function(a1) {
    
    cat("now processing: ", a1)
    
    DEXSeqDataSet_PSISigma <-  DEXSeqDataSetFromHTSeq(
      countfiles = list.files(path = DEXSeq_dir, pattern = paste(a1, ".*_combined_count_by_comparison.txt", sep = ""), full.names = TRUE),
      sampleData = data.frame("replicatename" = list.files(path = DEXSeq_dir, pattern = paste(a1, ".*_combined_count_by_comparison.txt", sep = ""), full.names = FALSE),
                              "condition" = list.files(path = DEXSeq_dir, pattern = paste(a1, ".*_combined_count_by_comparison.txt", sep = ""), full.names = FALSE) %>% gsub(pattern = paste(a1, "_(.*)_replicate.*", sep = ""), replacement = "\\1")),
      design = ~ sample + exon + condition:exon,
      flattenedfile = paste(DEXSeq_dir, a1, "_combined_dexseq_GFF_PSISigma_by_comparison.gff", sep = ""))
    
    DEXSeqDataSet_PSISigma = estimateSizeFactors(DEXSeqDataSet_PSISigma)
    DEXSeqDataSet_PSISigma = estimateDispersions(DEXSeqDataSet_PSISigma, BPPARAM = MulticoreParam(workers = 1, progressbar = TRUE))
    
    DEXSeqDataSet_PSISigma = testForDEU(DEXSeqDataSet_PSISigma, BPPARAM = MulticoreParam(workers = 1, progressbar = TRUE))
    DEXSeqDataSet_PSISigma = DEXSeq::estimateExonFoldChanges(DEXSeqDataSet_PSISigma, fitExpToVar = "condition", maxRowsMF = 5000, BPPARAM = MulticoreParam(workers = 1, progressbar = TRUE))
...
# DEXSeq results by timepoint

DEXSeqDataSet_PSISigma <-  DEXSeqDataSetFromHTSeq(
  countfiles = list.files(path = DEXSeq_dir, pattern = paste(".*_combined_count_by_timepoint.txt", sep = ""), full.names = TRUE),
  sampleData = data.frame("replicatename" = list.files(path = DEXSeq_dir, pattern = paste(".*_combined_count_by_timepoint.txt", sep = ""), full.names = FALSE),
                          "condition" = list.files(path = DEXSeq_dir, pattern = paste(".*_combined_count_by_timepoint.txt", sep = ""), full.names = FALSE) %>% gsub(pattern = paste("(.*)_replicate.*", sep = ""), replacement = "\\1") ),
  design = ~ sample + exon + condition:exon,
  flattenedfile = paste(DEXSeq_dir, "combined_dexseq_GFF_PSISigma_by_timepoint.gff", sep = "") )

DEXSeqDataSet_PSISigma = estimateSizeFactors(DEXSeqDataSet_PSISigma)
DEXSeqDataSet_PSISigma = estimateDispersions(DEXSeqDataSet_PSISigma, BPPARAM = MulticoreParam(workers = 4, progressbar = TRUE))

DEXSeqDataSet_PSISigma = testForDEU(DEXSeqDataSet_PSISigma, BPPARAM = MulticoreParam(workers = 4, progressbar = TRUE))
DEXSeqDataSet_PSISigma = DEXSeq::estimateExonFoldChanges(DEXSeqDataSet_PSISigma, fitExpToVar = "condition", maxRowsMF = 5000, BPPARAM = MulticoreParam(workers = 4, progressbar = TRUE))

tibble_dexseq_results <- DEXSeqResults(DEXSeqDataSet_PSISigma) %>% as_tibble %>% add_column("comparison" = a1)

list_DEXSeq_results_by_timepoint <- list(
  "DEXSeq_object" = DEXSeqDataSet_PSISigma,
  "tibble_results" = tibble_dexseq_results
)

###

names(list_DEXSeq_results_by_comparison) <- vector_of_comparisons_final

# rename the tibble column names
list_DEXSeq_results_by_comparison <- list_DEXSeq_results_by_comparison %>% purrr::map(~modify_at(.x = .x, .at = "tibble_results", .f = function(b1) {
...
save(list_DEXSeq_results_by_comparison, file = paste(DEXSeq_dir, "list_DEXSeq_results_by_comparison.Rlist", sep = ""))
# load(file = paste(DEXSeq_dir, "list_DEXSeq_results_by_comparison.Rlist", sep = ""))

# plot the statistics for each comparison
## this purrr returns nothing
future_map2(.x = list_DEXSeq_results_by_comparison,
            .y = names(list_DEXSeq_results_by_comparison),
           .f = function(a1, a2) {
             
             pdf(paste(DEXSeq_dir, "DEXSeq_dispersion_estimates_", a2, "_by_comparison.pdf", sep = ""))
             plotDispEsts(a1$DEXSeq_object)
             dev.off()
             
             pdf(paste(DEXSeq_dir, "DEXSeq_MAplot_", a2, "_by_comparison.pdf", sep = ""))
             plotMA(a1$DEXSeq_object, cex = 0.8)
             dev.off()
             
             pdf(paste(DEXSeq_dir, "DEXSeq_pvalue_distributions_", a2, "_by_comparison.pdf", sep = ""))
             plot(density(a1$tibble_results$DEXSeq_pvalue %>% na.omit))
             dev.off()
             
             # my own volcano plot
             plot <- ggplot(data = a1$tibble_results, mapping = aes(x = DEXSeq_log2FC_second_minus_first, y = -(DEXSeq_padj %>% log10))) +
               geom_point(size = 2) +
             ...
             ggsave(plot = plot, filename = paste(DEXSeq_dir, "DEXSeq_volcanoplot_", a2, "_by_comparison.pdf", sep = ""), device = "pdf", dpi = 600, width = 12, height = 15, units = "cm")
             ggsave(plot = plot, filename = paste(DEXSeq_dir, "DEXSeq_volcanoplot_", a2, "_by_comparison.svg", sep = ""), device = "svg", dpi = 600, width = 12, height = 15, units = "cm")
...
list_DEXSeq_results_tibble.only <- list_DEXSeq_results_by_comparison %>% purrr::map(~.x$tibble_results)
...
future_map2(.x = list_DEXSeq_results_tibble.only_with_PSI,
           ...
           .f = function(a1, a2) {
             ...
             pdf(paste(DEXSeq_dir, "DEXSeq_dpsi_density_distributions_", a2, "_by_comparison.pdf", sep = ""))
             plot(density(a1$DEXSeq_dpsi %>% na.omit))
             dev.off()
             
             pdf(paste(DEXSeq_dir, "DEXSeq_dpsi_cdf_distributions_", a2, "_by_comparison.pdf", sep = ""))
...
# select only cols that are informative
  dplyr::select(id, DEXSeq_exonBaseMean, DEXSeq_dispersion, DEXSeq_stat, DEXSeq_pvalue, DEXSeq_padj, comparison)
# , DEXSeq_IR_first, DEXSeq_IR_second, DEXSeq_pseudotranscript_IR_plus_ER_first, DEXSeq_pseudotranscript_IR_plus_ER_second, comparison.1_average.psi, comparison.2_average.psi
...
# add in PSI #1 and #2 columns
long_tibble_of_psisigma_DEXSeq_results <- dplyr::mutate(long_tibble_of_psisigma_DEXSeq_results, 
                                                        "comparison.1_average.psi" = rowMeans(long_tibble_of_psisigma_DEXSeq_results[, c("comparison.1_absolute.psi_1", "comparison.1_absolute.psi_2", "comparison.1_absolute.psi_3")]),
                                                        "comparison.2_average.psi" = rowMeans(long_tibble_of_psisigma_DEXSeq_results[, c("comparison.2_absolute.psi_1", "comparison.2_absolute.psi_2", "comparison.2_absolute.psi_3")]))
...
write.table(x = long_tibble_of_psisigma_DEXSeq_results[, c("VSR_coords", "alternative_exon_coords", "splicemode")], file = paste(R_processing_results_dir, "long_tibble_of_psisigma_DEXSeq_results_prepare_for_poison_exon_finder.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_poison_exon_finder_results_processed0 <- tibble_poison_exon_finder_results %>%
  dplyr::mutate("introduces_PTC" = grepl(x = PTC_exists_in_alternative_exon, pattern = "TRUE") == TRUE & grepl(x = alternative_exon_stop_codon_is_premature, pattern = "TRUE") == TRUE) %>%
...
tibble_poison_exon_finder_results_processed1 <- tibble_poison_exon_finder_results_processed0 %>%
  ...
  dplyr::mutate("VSR_coords" = paste(chr, ":", VSR_start, "-", VSR_end, sep = ""),
                "alternative_exon_coords" = paste(chr, ":", alternative_exon_start, "-", alternative_exon_end, sep = "")) %>%
  dplyr::ungroup() %>%
  dplyr::select(-VSR_start, -VSR_end, -alternative_exon_start, -alternative_exon_end, -chr)

# JOIN onto the main PSI-Sigma table
long_tibble_of_psisigma_DEXSeq_results <- dplyr::left_join(long_tibble_of_psisigma_DEXSeq_results, tibble_poison_exon_finder_results_processed1, by = c("VSR_coords", "alternative_exon_coords", "splicemode"))
...
## filter, return the co-ordinates of the differential LISs (Local Isoform Variants)

- PSI-Sigma approximates local isoform variants using its indexing method
- When we sum up all LISs in each Variable Splice Region (VSR), we should get the overall PSI for the exon in all isoforms of the gene.

```{r}

# filter dpsi, pvalue and also remove exons which have less than 2 observations in both comparisons
wide_tibble_of_psisigma_results_allcomparisons_differential <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::filter(., !!as.name(pvalue_or_FDR) <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & num.replicates >= 2 & num.replicates_comparison.2 >= 2)

number.of.LISs_differential_anysig_with_na <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode, chr, alternative_exon_start, alternative_exon_end, matched_strand) %>% nrow
number.of.exons_differential_anysig_with_na <- wide_tibble_of_psisigma_results_allcomparisons_differential$alternative_exon_coords %>% unique %>% length

message("There are ", number.of.LISs_differential_anysig_with_na, " differential LISs using the specified cutoff of ", pvalue_or_FDR, "<", pvalue_cutoff, " and dpsi > ", dpsi_cutoff)
...
## at the level of LISs
wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS <- dplyr::semi_join(long_tibble_of_psisigma_DEXSeq_results, wide_tibble_of_psisigma_results_allcomparisons_differential, by = c("database_ID", "VSR_coords", "alternative_exon_coords", "splicemode"))
## write table
write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS, file = paste(R_processing_results_dir, "long_tibble_of_psisigma_results_allcomparisons_differential_info_LIS", number.of.LISs_differential_anysig_with_na, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# export associated gene set
write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$matched_gene_names %>% unique %>% na.omit %>% mixedsort, file = paste(R_processing_results_dir, "geneset_of_differential_LIS_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_", wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$matched_gene_names %>% unique %>% na.omit %>% length, "genes_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)

# get table at the level of exons.
wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons <- dplyr::semi_join(long_tibble_of_psisigma_DEXSeq_results, wide_tibble_of_psisigma_results_allcomparisons_differential, by = c("alternative_exon_coords"))
## write
...
write.table(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons[, c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names")] %>% 
              dplyr::rename("VSR_coords" = "VSR_coords", "alternative_exon_coords" = "alternative_exon_coords", "gene_name" = "matched_gene_names") %>% 
...
# also get table of differential information for MSC, 6h and 1d ###
tibble_differential_results_MSC_6h_1d <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::filter(comparison %in% c("BM_MSC_to_OB_6h_vs_BM_MSC_to_OB_MSC", "BM_MSC_to_OB_1d_vs_BM_MSC_to_OB_MSC", "BM_MSC_to_OB_1d_vs_BM_MSC_to_OB_6h") & abs(dpsi) >= dpsi_cutoff)
# write table
write.table(x = tibble_differential_results_MSC_6h_1d, file = paste(R_processing_results_dir, "long_tibble_of_psisigma_results_allcomparisons_differential_info_LIS", tibble_differential_results_MSC_6h_1d %>% nrow, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_MSC_6h_1d_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
# export associated gene set
# write.table(x = tibble_differential_results_MSC_6h_1d$matched_gene_names %>% unique %>% na.omit %>% mixedsort, file = paste(R_processing_results_dir, "geneset_differential_LIS_", tibble_differential_results_12d_vs_ud$matched_gene_names %>% unique %>% length, "genes_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_12d.vs.ud_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE, col.names = FALSE)

# cat("There are", tibble_differential_results_MSC_6h_1d$matched_gene_names %>% unique %>% na.omit %>% length, "differential genes between MSC and 12d only, using the specified cutoff of", pvalue_or_FDR, "<", pvalue_cutoff, "and dpsi > ", dpsi_cutoff)
...
wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>%
  dplyr::filter(str_detect(string = `splicemode`, pattern = "IR"))

wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>%
  dplyr::filter(!str_detect(string = `splicemode`, pattern = "IR"))

tibble_cumulative_diff_changes_total <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d") & !str_detect(string = comparison, pattern = "12h")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff) %>% .$alternative_exon_coords %>% unique %>% length)) %>% 
...
tibble_cumulative_diff_changes_IR <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d") & !str_detect(string = comparison, pattern = "12h")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff) %>% .$alternative_exon_coords %>% unique %>% length)) %>% 
  add_column("cumulative_diff_changes_pct" = .$cumulative_diff_changes * 100 / (.$cumulative_diff_changes %>% max))

tibble_cumulative_diff_changes_non.IR <- tibble("timepoint" = vector_OBseries_timepoints_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d") & !str_detect(string = comparison, pattern = "12h")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d") & !str_detect(string = comparison, pattern = "1d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d") & !str_detect(string = comparison, pattern = "3d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d") & !str_detect(string = comparison, pattern = "6d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d") & !str_detect(string = comparison, pattern = "9d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff & !str_detect(string = comparison, pattern = "12d")) %>% .$alternative_exon_coords %>% unique %>% length,
                                             wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_non.IR %>% dplyr::filter(pvalue <= pvalue_cutoff & abs(dpsi) >= dpsi_cutoff) %>% .$alternative_exon_coords %>% unique %>% length)) %>% 
  add_column("cumulative_diff_changes_pct" = .$cumulative_diff_changes * 100 / (.$cumulative_diff_changes %>% max))
...
# all (LIS PSI)
pdf(paste(R_processing_results_dir, "plot_of_absolute_dpsi_changes_LIS_all.pdf", sep = ""), width = 5, height = 5)
plot(density(long_tibble_of_psisigma_DEXSeq_results$dpsi %>% abs, bw = 1), 
     main = "Distribution of absolute magnitude of change in dpsi, PSI-Sigma (LISs)", 
...
# LIS PSI - differential
pdf(paste(R_processing_results_dir, "plot_of_absolute_dpsi_changes_LIS_diff.pdf", sep = ""), width = 5, height = 5)
plot(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$dpsi %>% abs, bw = 1), 
     main = "Distribution of absolute magnitude of change in dpsi, PSI-Sigma (LISs)", 
     xlab = "Absolute dPSI in any direction (%)",
     ylab = "Frequency density",
     col = "firebrick4", 
     xaxt = "n",
     xlim = c(0, 100)) + 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$dpsi %>% abs, bw = 1),
          col = alpha("firebrick4", 0.33),
          border = FALSE,
          xaxt = "n",
          xlim = c(0, 100)) +
...
plot(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(alternative_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(dpsi, na.rm = TRUE)) %>% .$exon_dpsi %>% abs), 
  ...
  # xaxt = "n",
  # xlim = c(0, 100)) 
  polygon(density(wide_tibble_of_psisigma_results_allcomparisons_differential_info_exons %>% dplyr::group_by(alternative_exon_coords) %>% dplyr::summarise("exon_dpsi" = sum(dpsi, na.rm = TRUE)) %>% .$exon_dpsi %>% abs),
          col = alpha("firebrick4", 0.33),
          border = FALSE,
          xaxt = "n")
# ,
...
# to fetch the unique LISs which are constitutive:
## group all unique LISs together
wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::group_by(database_ID, VSR_coords, alternative_exon_coords, splicemode, chr, alternative_exon_start, alternative_exon_end, matched_strand)
## create logical of whether each LIS has all pvalue and dpsi cutoff AS WELL AS having all comparisons intact (and not missing)
## if return NA, then it's not constitutive.
wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS_constitutive.test <- wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS %>% dplyr::summarise("LIS_is_constitutive" = all(!!as.name(pvalue_or_FDR) > pvalue_cutoff & abs(dpsi) < dpsi_cutoff & num.replicates >= 2 & num.replicates_comparison.2 >= 2) & n() == vector_of_comparisons_final %>% length) %>% dplyr::mutate_at("LIS_is_constitutive", .funs = function(x) {
...
## subset table for only constitutive LISs and extract all unique LISs
wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS_constitutive.only <- wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS_constitutive.test[wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS_constitutive.test$LIS_is_constitutive == TRUE, ]
## extract unique constitutive LISs
tibble_unique_constitutive_LISs <- wide_tibble_of_psisigma_results_allcomparisons_final_grouped.by.LIS_constitutive.only %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode, chr, alternative_exon_start, alternative_exon_end, matched_strand, .keep_all = TRUE)
## rejoin onto main table
tibble_constitutive_LIS_info <- dplyr::semi_join(long_tibble_of_psisigma_DEXSeq_results, tibble_unique_constitutive_LISs)

# print numbers
number.of.LISs_constitutive <- nrow(tibble_unique_constitutive_LISs)
number.of.exons_constitutive <- tibble_unique_constitutive_LISs$alternative_exon_coords %>% unique %>% length

message("There are ", number.of.LISs_constitutive, " constitutive LISs using the specified cutoff of ", pvalue_or_FDR, "<", pvalue_cutoff, " and dpsi > ", dpsi_cutoff)
...
# write table of all constitutive LISs info
write.table(x = tibble_constitutive_LIS_info, file = paste(R_processing_results_dir, "long_table_of_all_constitutive_LIS", number.of.LISs_constitutive, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
# write coords for three-frame translation
write.table(x = tibble_constitutive_LIS_info[, c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names")] %>% 
              dplyr::rename("VSR_coords" = "VSR_coords", "alternative_exon_coords" = "alternative_exon_coords", "gene_name" = "matched_gene_names") %>% 
              add_column("organism" = "Homo sapiens", "custom_identifier" = NA), file = paste(R_processing_results_dir, "long_table_of_all_constitutive_LIS", number.of.LISs_constitutive, "_dpsi", dpsi_cutoff,  "_", pvalue_or_FDR, pvalue_cutoff, "_3FT.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 <- dplyr::filter(wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS, comparison %in% vector_of_ud_comparisons) %>% dplyr::select(database_ID, VSR_coords, alternative_exon_coords, splicemode, chr, alternative_exon_start, alternative_exon_end, matched_gene_names, matched_recon_transcript_names, matched_ref_transcript_names, matched_strand, NMD_biotype_reference, NMD_flagged_ref, NMD_flagged_recon, any_NMD, poison_exon_candidate_ref, poison_exon_candidate_recon, contains_PTC_ref, contains_PTC_recon, first_or_last_exon_reference, first_or_last_exon_recon, any_first_exon, any_last_exon, introduces_PTC_for_all_matched_transcripts, introduces_PTC_for_any_matched_transcripts, alters_frame_for_all_matched_transcripts, alters_frame_for_any_matched_transcripts, comparison, comparison.1_average.psi, comparison.2_average.psi) %>% dplyr::rename("comparison_ud" = "comparison.2_average.psi")
...
wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp1 <- dplyr::bind_rows(wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 %>% dplyr::select(-comparison_ud) %>% dplyr::rename("PSI" = "comparison.1_average.psi"),
                                                                                       wide_tibble_of_psisigma_results_allcomparisons_final_ud.only_temp0 %>% dplyr::select(-comparison.1_average.psi) %>% dplyr::rename("PSI" = "comparison_ud") %>% dplyr::mutate_at(.vars = "timepoint", .funs = function(x) {"MSC" %>% return}) %>% dplyr::group_by_at(.vars = setdiff(colnames(.), "PSI")) %>% dplyr::mutate_at(.vars = "PSI", .funs = mean )) %>% dplyr::distinct()
...
wide_tibble_of_psisigma_PSI_with_na_LIS <- wide_tibble_of_psisigma_results_allcomparisons_final_ud.only
...
wide_tibble_of_absolute_PSI_averages_sum_temp <- wide_tibble_of_psisigma_PSI_with_na_LIS %>% dplyr::select(., -id, -database_ID)

# create list of event region, splicemode
list_unique_exon_regions <- wide_tibble_of_absolute_PSI_averages_sum_temp[, c("VSR_coords", "alternative_exon_coords")] %>% unique %>% array_tree

# load the tibble of separated PSI averages into list
list_of_tibbles_absolute_PSI_average_subset_by_exon <- future_map(.x = list_unique_exon_regions, .f = ~wide_tibble_of_absolute_PSI_averages_sum_temp[wide_tibble_of_absolute_PSI_averages_sum_temp$VSR_coords == (.x[[1]] %>% paste) & wide_tibble_of_absolute_PSI_averages_sum_temp$alternative_exon_coords == (.x[[2]] %>% paste), ], .progress = TRUE, .options = future_options(globals = c("wide_tibble_of_absolute_PSI_averages_sum_temp")))
...
list_of_tibbles_absolute_PSI_levels_exon.switching <- future_map(.x = wide_tibble_of_psisigma_PSI_with_na_LIS$VSR_coords %>% unique, .f = ~wide_tibble_of_psisigma_PSI_with_na_LIS[wide_tibble_of_psisigma_PSI_with_na_LIS$VSR_coords == .x, ], .progress = TRUE, .options = future_options(globals = c("wide_tibble_of_psisigma_PSI_with_na_LIS")))
...
list_of_tibbles_absolute_PSI_levels_exon.switching <- future_map(.x = wide_tibble_of_psisigma_PSI_with_na_exon.sum$VSR_coords %>% unique, .f = ~wide_tibble_of_psisigma_PSI_with_na_exon.sum[wide_tibble_of_psisigma_PSI_with_na_exon.sum$VSR_coords == .x, ], .progress = TRUE, .options = future_options(globals = c("wide_tibble_of_psisigma_PSI_with_na_exon.sum")))
...
row.indices_with_na <- apply(X = wide_tibble_of_psisigma_PSI_with_na_LIS[, colnames(wide_tibble_of_psisigma_PSI_with_na_LIS) %in% vector_OBseries_timepoints_edited], MARGIN = 1, FUN = function(X) {any(is.na(X))})

# remove those rows that are true (that have any na)
wide_tibble_of_LIS_PSI_no_na <- wide_tibble_of_psisigma_PSI_with_na_LIS[which(row.indices_with_na == FALSE), ]

no.LIS.exons_differentially.spliced_complete.timeseries <- nrow(wide_tibble_of_LIS_PSI_no_na)

cat("There are", no.LIS.exons_differentially.spliced_complete.timeseries, "exons with a complete time profile of LIS-based PSI which are differentially spliced.")
...
wide_tibble_of_LIS_PSI_no_na_scaled <- wide_tibble_of_LIS_PSI_no_na

wide_tibble_of_LIS_PSI_no_na_scaled[, colnames(wide_tibble_of_LIS_PSI_no_na_scaled) %in% vector_OBseries_timepoints_edited] <- wide_tibble_of_LIS_PSI_no_na_scaled[, colnames(wide_tibble_of_LIS_PSI_no_na_scaled) %in% vector_OBseries_timepoints_edited] %>% genescale(m = ., axis = 1, method = "Z") %>% as_tibble

write.table(x = wide_tibble_of_LIS_PSI_no_na_scaled, file = paste(R_processing_results_dir, "wide_table_of_",  nrow(wide_tibble_of_LIS_PSI_no_na_scaled), "LISs_scaled.PSI_OB_diff.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# also write the scaled table for MSC_6h_1d only
wide_tibble_of_LIS_PSI_no_na_scaled_MSC_6h_1d <- dplyr::semi_join(wide_tibble_of_LIS_PSI_no_na_scaled, tibble_differential_results_MSC_6h_1d, by = c("database_ID", "VSR_coords", "alternative_exon_coords", "splicemode"))

write.table(x = wide_tibble_of_LIS_PSI_no_na_scaled_MSC_6h_1d, file = paste(R_processing_results_dir, "wide_table_of_",  nrow(wide_tibble_of_LIS_PSI_no_na_scaled_MSC_6h_1d), "LISs_scaled.PSI_OB_diff_MSC_6h_1d.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
somdata_5_by_5_OBseries <- som(wide_tibble_of_LIS_PSI_no_na_scaled[, vector_OBseries_timepoints_edited] %>% as.matrix, grid = somgrid(xdim = OB_xdim, ydim = OB_ydim, topo = "rectangular", toroidal = FALSE), rlen = 100, keep.data = TRUE)

wide_table_of_final_SOM_summary_5_by_5_OBseries_separate <- cbind(wide_tibble_of_LIS_PSI_no_na_scaled, "cluster" = somdata_5_by_5_OBseries[["unit.classif"]]) %>% as_tibble
...
ggplot() +
  geom_line(data = long_table_of_final_SOM_summary_5_by_5_OBseries_separate, aes(x = timepoint, y = scaled_PSI_value, group = paste(VSR_coords, alternative_exon_coords, splicemode)), alpha = 0.33) +
  ...
  ggtitle(paste(OB_xdim, "x", OB_ydim, "SOM of", nrow(wide_table_of_final_SOM_summary_5_by_5_OBseries_separate), "LIS PSI values during OB diff (any", pvalue_or_FDR, 
 ...
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_separate %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode) %>% nrow, "_exons_LIS.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_separate %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode) %>% nrow, "_exons_LIS.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".svg", sep = ""), device = "svg", dpi = 600, width = 33, height = 20, units = "cm")

write.table(x = long_table_of_final_SOM_summary_5_by_5_OBseries_separate, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_separate %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode) %>% nrow, "_exons_LIS.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_long.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

write.table(x = wide_table_of_final_SOM_summary_5_by_5_OBseries_separate, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_separate %>% dplyr::distinct(database_ID, VSR_coords, alternative_exon_coords, splicemode) %>% nrow, "_exons_LIS.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_wide.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
no.sum.exons_differentially.spliced_complete.timeseries <- length(wide_tibble_of_exon_PSI_no_na$alternative_exon_coords)
...
ggplot() +
  geom_line(data = long_table_of_final_SOM_summary_5_by_5_OBseries_sum, aes(x = timepoint, y = scaled_PSI_value, group = paste(alternative_exon_coords, splicemode)), alpha = 0.33) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_sum %>% dplyr::distinct(alternative_exon_coords, splicemode) %>% nrow, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_sum %>% dplyr::distinct(alternative_exon_coords, splicemode) %>% nrow, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, ".svg", sep = ""), device = "svg", dpi = 600, width = 33, height = 20, units = "cm")

write.table(x = long_table_of_final_SOM_summary_5_by_5_OBseries_sum, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_sum %>% dplyr::distinct(alternative_exon_coords, splicemode) %>% nrow, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_long.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

write.table(x = wide_table_of_final_SOM_summary_5_by_5_OBseries_sum, file = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", long_table_of_final_SOM_summary_5_by_5_OBseries_sum %>% dplyr::distinct(alternative_exon_coords, splicemode) %>% nrow, "_exons_exon.PSI_OB_diff_any_", pvalue_or_FDR, pvalue_cutoff, "_any_deltaPSI_greaterthan_", dpsi_cutoff, "_wide.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# just testing to see if certain splicemodes affect certain genes
# vector_differential_genes_anysig_with_na <- wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$splicemode, pattern = "IR"), ] %>% .$matched_gene_names %>% unique
...
logCPM_OBseries_RBP <- dplyr::semi_join(logCPM_OBseries_anysig_DEGs, column_RBP_genes_OBseries, by = "ensembl_gene_id") %>%
  add_column(., alternative_exon_coords = paste("RBP", .$ensembl_gene_id, sep = "_"), splicemode = "RNASeq", .after = 2)
...
logCPM_OBseries_lncRNA <- dplyr::semi_join(logCPM_OBseries_anysig_DEGs, column_lncRNA_genes_OBseries, by = "ensembl_gene_id") %>%
  add_column(., alternative_exon_coords = paste("lncRNA", .$ensembl_gene_id, sep = "_"), splicemode = "RNASeq", .after = 2)
...
logCPM_OBseries_snRNA <- dplyr::semi_join(logCPM_OBseries_anysig_DEGs, column_snRNA_genes_OBseries, by = "ensembl_gene_id") %>%
  add_column(., alternative_exon_coords = paste("snRNA", .$ensembl_gene_id, sep = "_"), splicemode = "RNASeq", .after = 2)
...
logCPM_OBseries_TFcomplex <- dplyr::semi_join(logCPM_OBseries_anysig_DEGs, column_TFcomplex_genes_OBseries, by = "ensembl_gene_id") %>%
  add_column(., alternative_exon_coords = paste("TF complex", .$ensembl_gene_id, sep = "_"), splicemode = "RNASeq", .after = 2)
...
avg_exons_per_gene_all <- (long_tibble_of_psisigma_DEXSeq_results$alternative_exon_coords %>% unique %>% length)/(long_tibble_of_psisigma_DEXSeq_results$matched_gene_names %>% unique %>% length)

print(paste("There are", avg_exons_per_gene_all, "unique exons/gene detected overall"))

tibble_all_exons_per_gene_freq <- long_tibble_of_psisigma_DEXSeq_results %>% dplyr::distinct(., alternative_exon_coords, .keep_all = TRUE) %>% group_by(matched_gene_names) %>% summarise(no_exons = n())
...
avg_exons_per_gene_constitutive <- (tibble_constitutive_LIS_info$alternative_exon_coords %>% unique %>% length)/(tibble_constitutive_LIS_info$matched_gene_names %>% unique %>% length)

print(paste("There are", avg_exons_per_gene_constitutive, "unique constitutive exons/gene detected"))

tibble_constitutive_exons_per_gene_freq <- tibble_constitutive_LIS_info %>% dplyr::distinct(., alternative_exon_coords, .keep_all = TRUE) %>% group_by(matched_gene_names) %>% summarise(no_exons = n())
...
avg_exons_per_gene_differential <- (wide_tibble_of_psisigma_results_allcomparisons_differential$alternative_exon_coords %>% unique %>% length)/(wide_tibble_of_psisigma_results_allcomparisons_differential$matched_gene_names %>% unique %>% length)

print(paste("There are", avg_exons_per_gene_differential, "unique differential exons/gene detected"))

tibble_differential_exons_per_gene_freq <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::distinct(., alternative_exon_coords, .keep_all = TRUE) %>% group_by(matched_gene_names) %>% summarise(no_exons = n())
...
tibble_constitutive_LIS_info_first.last.reference <- tibble_constitutive_LIS_info %>% 
  add_column("first_exon_reference" = grepl(x = tibble_constitutive_LIS_info$first_or_last_exon_reference, pattern = "first_exon"),
             "last_exon_reference" = grepl(x = tibble_constitutive_LIS_info$first_or_last_exon_reference, pattern = "last_exon"))

wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_first.last.reference <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS %>% 
  add_column("first_exon_reference" = grepl(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$first_or_last_exon_reference, pattern = "first_exon"),
             "last_exon_reference" = grepl(x = wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS$first_or_last_exon_reference, pattern = "last_exon"))

# category means splicemode + NMD status + first/last exon
# also clean up the splicemode column
tibble_exons_per_category_all <- long_tibble_of_psisigma_DEXSeq_results_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2", perl = TRUE)} ) %>% dplyr::distinct(., VSR_coords, alternative_exon_coords, .keep_all = TRUE) %>% 
  # , any_NMD
...
tibble_exons_per_category_differential <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., VSR_coords, alternative_exon_coords, .keep_all = TRUE) %>% group_by(splicemode, NMD_flagged_recon, introduces_PTC_for_all_matched_transcripts, alters_frame_for_all_matched_transcripts, first_exon_reference, last_exon_reference) %>% summarise(no_exons_differential = n())
...
# upset plot
message("all exons")
df <- long_tibble_of_psisigma_DEXSeq_results_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., VSR_coords, alternative_exon_coords, .keep_all = TRUE) %>% .[, c("VSR_coords", "alternative_exon_coords", "splicemode", "NMD_flagged_recon", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = VSR_coords + alternative_exon_coords + splicemode + NMD_flagged_recon + introduces_PTC_for_all_matched_transcripts + alters_frame_for_all_matched_transcripts + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 

df[is.na(df)] = FALSE

upset_combs <- data.frame(df %>% dplyr::select(., -VSR_coords, -alternative_exon_coords), row.names = paste(df$VSR_coords, df$alternative_exon_coords, sep = "_")) %>% make_comb_mat(mode = "distinct")
...
df <- tibble_constitutive_LIS_info_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., VSR_coords, alternative_exon_coords, .keep_all = TRUE) %>% .[, c("VSR_coords", "alternative_exon_coords", "splicemode", "NMD_flagged_recon", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = VSR_coords + alternative_exon_coords + splicemode + NMD_flagged_recon + introduces_PTC_for_all_matched_transcripts + alters_frame_for_all_matched_transcripts + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 
...
df <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_first.last.reference %>% modify_at(., .at = "splicemode", .f = function(x) {gsub(x = x, pattern = "^(TSS\\|)*([^_|\\|]+)(.*)*", replacement = "\\2")} ) %>% dplyr::distinct(., VSR_coords, alternative_exon_coords, .keep_all = TRUE) %>% .[, c("VSR_coords", "alternative_exon_coords", "splicemode", "NMD_flagged_recon", "introduces_PTC_for_all_matched_transcripts", "alters_frame_for_all_matched_transcripts", "first_exon_reference", "last_exon_reference")] %>% add_column("dummy" = TRUE) %>% reshape2::dcast(formula = VSR_coords + alternative_exon_coords + splicemode + NMD_flagged_recon + introduces_PTC_for_all_matched_transcripts + alters_frame_for_all_matched_transcripts + first_exon_reference + last_exon_reference ~ splicemode, value.var = "dummy") %>% as_tibble 
# , "any_NMD"
 # + any_NMD
...
exons_per_gene_constitutive <- tibble_constitutive_LIS_info_first.last.reference %>% 
  dplyr::distinct(., alternative_exon_coords, .keep_all = TRUE) %>% group_by(matched_gene_names) %>% summarise(no_exons_constitutive = n())

exons_per_gene_differential <- wide_tibble_of_psisigma_results_allcomparisons_differential_info_LIS_first.last.reference %>% 
  dplyr::distinct(., alternative_exon_coords, .keep_all = TRUE) %>% group_by(matched_gene_names) %>% summarise(no_exons_differential = n())
...
list_of_tibble_raw_counts_PSI_for_PCA_by_comparison <- purrr::map2(.x = list_of_tibble_raw_counts_PSI_for_PCA_by_comparison,
                                                                   ...
                                                                   .f = ~.x %>% dplyr::select(-comparison) %>% setNames(c("database_ID", "VSR_coords", "alternative_exon_coords", "splicemode", paste(.y, c("|r1", "|r2", "|r3"), sep = ""), paste("MSC", c("|r1", "|r2", "|r3"), sep = ""))) %>% na.omit %>% dplyr::distinct())

# reduce into a left join
wide_tibble_raw_counts_PSI_for_PCA <- list_of_tibble_raw_counts_PSI_for_PCA_by_comparison %>% purrr::reduce(.f = inner_join , by = c("database_ID", "VSR_coords", "alternative_exon_coords", "splicemode", paste("MSC", c("|r1", "|r2", "|r3"), sep = ""))) %>% dplyr::distinct()
...
plot_PCA_for_timepoint_and_replicate(wide_tibble_raw_counts_PSI_for_PCA %>% dplyr::select(-database_ID, -VSR_coords, -alternative_exon_coords, -splicemode), 
...
# identifier table
tibble_alternative_exons_identifier <- wide_tibble_of_psisigma_results_allcomparisons_differential %>% dplyr::select(VSR_coords, alternative_exon_coords, splicemode, matched_gene_names) %>% setNames(c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names")) %>% unique
# chr start end strand table
vector_event_region_chr <- gsub(x = tibble_alternative_exons_identifier$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1")
vector_VSR_start <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                .y = gsub(x = tibble_alternative_exons_identifier$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                .f = ~min(.x, .y)) %>% unlist
vector_VSR_end <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                              .y = gsub(x = tibble_alternative_exons_identifier$VSR_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                              .f = ~max(.x, .y)) %>% unlist

vector_alternative_exon_start <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$alternative_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                             .y = gsub(x = tibble_alternative_exons_identifier$alternative_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                             .f = ~min(.x, .y)) %>% unlist
vector_alternative_exon_end <- purrr::map2(.x = gsub(x = tibble_alternative_exons_identifier$alternative_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                           .y = gsub(x = tibble_alternative_exons_identifier$alternative_exon_coords, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                           .f = ~max(.x, .y)) %>% unlist

tibble_alternative_exons_chr_start_end_strand <- tibble(
  "VSR_coords" = tibble_alternative_exons_identifier$VSR_coords,
  "alternative_exon_coords" = tibble_alternative_exons_identifier$alternative_exon_coords,
  ...
  .f = function(a1, a2) {
    ...
    list_per_chromosome <- future_imap(
      ...
      .f = function(b1, b2) {
        ...
        vector_VSR_exon_coords <- b1[c("VSR_start", "VSR_end", "alternative_exon_start", "alternative_exon_end")] %>% unlist %>% type.convert(as.is = TRUE)
        ...
        if (grepl(x = b1$splicemode, pattern = intron_retention_string) == TRUE) {
          ...
          list_matched_GTF_exon_entries <- extract_matching.exons(query_chr = b1$chr, query_start = b1$VSR_start %>% type.convert(as.is = TRUE), query_end = b1$VSR_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") %>% dplyr::group_split(transcript_id) %>% set_names(nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
          ...
          if (list_matched_GTF_exon_entries %>% length == 0) {
            list_GTF_entries_matched_to_IR_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = b1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
            ...
            list_matched_GTF_exon_entries <- 
              tibble_all_candidate_overlapping_full_entries[tibble_all_candidate_overlapping_full_entries$type == "exon" &
                                                         tibble_all_candidate_overlapping_full_entries$start <= b1$alternative_exon_start %>% type.convert(as.is = TRUE) &
                                                         tibble_all_candidate_overlapping_full_entries$end >= b1$alternative_exon_end %>% type.convert(as.is = TRUE), ] %>% 
          ...
          # exact exon match
          list_matched_GTF_exon_entries <- extract_matching.exons(query_chr = b1$chr, query_start = b1$alternative_exon_start %>% type.convert(as.is = TRUE), query_end = b1$alternative_exon_end %>% type.convert(as.is = TRUE), query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, return_type = "exon") %>% dplyr::group_split(transcript_id) %>% set_names(nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
          # get parent transcript entries
          list_parent_GTF_transcript_entries <- purrr::map(.x = list_matched_GTF_exon_entries %>% names, .f = ~a2[which(a2$transcript_id == .x), ]) %>% set_names(list_matched_GTF_exon_entries %>% names)
          ...
          common_coord <- intersect(c(b1$VSR_start %>% type.convert(as.is = TRUE), b1$VSR_end %>% type.convert(as.is = TRUE)), 
                                    c(b1$alternative_exon_start %>% type.convert(as.is = TRUE), b1$alternative_exon_end %>% type.convert(as.is = TRUE)))
          event_region_only_coord <- setdiff(c(b1$VSR_start %>% type.convert(as.is = TRUE), b1$VSR_end %>% type.convert(as.is = TRUE)), common_coord)
          exon_only_coord <- setdiff(c(b1$alternative_exon_start %>% type.convert(as.is = TRUE), b1$alternative_exon_end %>% type.convert(as.is = TRUE)), common_coord)
...
tibble_ref_entries_matched_to_exons_unmatched <- list_ref_entries_matched_to_exons %>% purrr::map(~.x$unmatched_list) %>% flatten %>% purrr::map(~.x[c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names", "chr", "strand", "VSR_start", "VSR_end", "alternative_exon_start", "alternative_exon_end")] %>% as_tibble) %>% rbindlist %>% as_tibble
# write
...
list_genome_relative_exonic.and.CDS_coords <- future_imap(.x = list_ref_entries_matched_to_exons.and.CDS, .f = function(a1, a2) {
  ...
  # cat("\nprocessing entry number: ", a2)
    ...
    vector_genomic_coords_of_all_differential_exon_positions <- (a1$alternative_exon_start %>% type.convert(as.is = TRUE)):(a1$alternative_exon_end %>% type.convert(as.is = TRUE))
    ...
    tibble_overlapping_parent_transcript_features <- tibble_parent_transcript_entries[which(tibble_parent_transcript_entries$start <= a1$alternative_exon_end %>% type.convert(as.is = TRUE) & tibble_parent_transcript_entries$end >= a1$alternative_exon_start %>% type.convert(as.is = TRUE)), ]
...
suppressMessages(list_matched_annotation <- future_map(.x = list_protein_relative_differential_coords, .f = function(a1) {
...
tibble_final_transcript_feature_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "alternative_exon_coords", "matched_strand", "splicemode", "vector_matched_transcript_ids", "tibble_overlapping_parent_transcript_features")] %>% as_tibble) %>%
  rbindlist %>% 
  as_tibble %>%
  setNames(c("VSR_coords", "alternative_exon_coords", "matched_strand", "splicemode", "vector_matched_transcript_ids", "tibble_overlapping_parent_transcript_features")) %>% 
  ## add in gene symbols
  dplyr::right_join(tibble_ref_matching_exon_entries_all[, c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names")] %>% na.omit %>% unique, ., by = c("VSR_coords", "alternative_exon_coords", "splicemode"))
# write table
...
tibble_final_biomart_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "alternative_exon_coords", "matched_strand", "splicemode", "biomart_annotations")] ) %>%
...
tibble_final_PTM_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("VSR_coords", "alternative_exon_coords", "matched_strand", "splicemode", "PTMs")] ) %>%
...
tibble_final_annotation_supp <- list_tidied_annotation %>% 
  purrr::map(~.x[c("VSR_coords", "alternative_exon_coords", "matched_strand", "splicemode", "vector_genomic_coords_of_all_differential_exon_positions", "list_differential_CDS_positions", "vector_matched_transcript_ids", "protein_relative_differential_coords")]) %>%
...
## special case: match intron junctions to the reference to see where they are occurring
...
# get IR events only
tibble_differential_IR_events <- wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$splicemode, pattern = "IR"), ] %>% dplyr::select(VSR_coords, alternative_exon_coords, splicemode, matched_gene_names) %>% setNames(c("VSR_coords", "alternative_exon_coords", "splicemode", "matched_gene_names")) %>% unique

# extract chr start end strand
tibble_differential_IR_events <- tibble_differential_IR_events %>% 
  dplyr::mutate(
    "chr" = gsub(x = `VSR_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\1"),
    "strand" = "*",
    "VSR_start" = purrr::map2(.x = gsub(x = `VSR_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                .y = gsub(x = `VSR_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                .f = ~min(.x, .y)) %>% unlist,
    "VSR_end" = purrr::map2(.x = gsub(x = `VSR_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                              .y = gsub(x = `VSR_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                              .f = ~max(.x, .y)) %>% unlist,
    "alternative_exon_start" = purrr::map2(.x = gsub(x = `alternative_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                             .y = gsub(x = `alternative_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                             .f = ~min(.x, .y)) %>% unlist,
    "alternative_exon_end" = purrr::map2(.x = gsub(x = `alternative_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\2") %>% type.convert(as.is = TRUE),
                                           .y = gsub(x = `alternative_exon_coords`, pattern = "(.*):(\\d+)-(.*)", replacement = "\\3") %>% type.convert(as.is = TRUE),
                                           .f = ~max(.x, .y)) %>% unlist
  )

# list-ify by chromosome 
list_differential_IR_events_by_chr <- tibble_differential_IR_events %>% 
  dplyr::group_split(chr) %>% 
  set_names(x = ., nm = purrr::map(.x = ., .f = ~.x$chr %>% unique) %>% unlist) %>%
  purrr::map(~.x %>% purrr::array_tree())

# list-ify tibble_ref_gtf
list_tibble_ref_gtf_by_chr <- tibble_ref_gtf_original %>% 
  dplyr::group_split(seqnames) %>% 
  set_names(x = ., nm = purrr::map(.x = ., .f = ~.x$seqnames %>% unique) %>% unlist)

# get chr in common
vector_chr_in_commmon <- intersect(list_differential_IR_events_by_chr %>% names,
                                   list_tibble_ref_gtf_by_chr %>% names)

# match IR junctions to reference and annotate with transcript ontology
list_IR_junctions_matched_to_reference_transcript_ontology <- furrr::future_pmap(
  .l = list(
    "a1" = list_differential_IR_events_by_chr[vector_chr_in_commmon],
    "a2" = list_tibble_ref_gtf_by_chr[vector_chr_in_commmon],
    "a3" = vector_chr_in_commmon
  ), 
  .f = function(a1, a2, a3) {
    
    # DEBUG ###
    # a1 <- list_differential_IR_events_by_chr[[1]]
    # a2 <- list_tibble_ref_gtf_by_chr[[1]]
    ###########
    
    # cat(a3, "\n")
    
    list_L2_matched_ref_tibble_entries <- furrr::future_imap(
      .x = a1,
      .f = function(b1, b2) {
        ...
        # get list of tibbles of ref exon pairs matched to the IR junction
        list_tibbles_ref_matched_to_junction <- extract_junction.flanking.exons(query_chr = b1$chr, query_start = b1$alternative_exon_start, query_end = b1$alternative_exon_end, query_strand = b1$strand, tibble_gtf_table = a2, tolerance_left = 0, tolerance_right = 0, tolerance_inside = 0, tolerance_outside = 0, match_consecutive = TRUE, return_type = "exon")
        
        # get the ENST IDs which contain the IR event region
        list_ref_entries_containing_IR_event_region <- a2[a2$type == "exon", ] %>% .[which(.$start == b1$VSR_start %>% type.convert(as.is = TRUE) | .$end == b1$VSR_end %>% type.convert(as.is = TRUE)),] %>% dplyr::group_split(transcript_id) %>% set_names(x = ., nm = purrr::map(.x = ., .f = ~.x$transcript_id %>% unique) %>% unlist)
        
        vector_transcript_ids_matched_to_event_region <- list_ref_entries_containing_IR_event_region %>% purrr::keep(.p = ~.x %>% nrow == 2) %>% names

        if (intersect(vector_transcript_ids_matched_to_event_region, list_tibbles_ref_matched_to_junction %>% names) %>% length > 0) {

          list_tibbles_ref_matched_to_junction <- list_tibbles_ref_matched_to_junction[intersect(vector_transcript_ids_matched_to_event_region, list_tibbles_ref_matched_to_junction %>% names)]

        }
        
        # now extract features overlapping with the IR region
        tibble_overlapping_features_with_IR_region <- extract_overlapping_features(query_chr = b1$chr, query_start = b1$alternative_exon_start, query_end = b1$alternative_exon_end, query_strand = b1$strand, tibble_gtf_table = a2, left_query_shift = 0, right_query_shift = 0, left_tolerance = 1, right_tolerance = 1, return_type = ".*") 
        
        # filter overlapping features for the transcript_ids with matched exactly to the IR junction
        tibble_overlapping_features_final <- tibble_overlapping_features_with_IR_region[tibble_overlapping_features_with_IR_region$transcript_id %in% (list_tibbles_ref_matched_to_junction %>% names), ]
        
        return(splice(
          b1,
          "matched_transcript_type" = tibble_overlapping_features_final$type %>% unique %>% paste(collapse = ","),
          "matched_transcript_biotype" = tibble_overlapping_features_final$transcript_biotype %>% unique %>% paste(collapse = ",")
        ))
        
      } )
    
    return(list_L2_matched_ref_tibble_entries)
    
  }, .progress = TRUE)

# rbind and tibblise
tibble_IR_transcript_ontology <- list_IR_junctions_matched_to_reference_transcript_ontology %>% flatten %>% rbindlist %>% as_tibble

tibble_IR_transcript_ontology$matched_transcript_type %>% strsplit(split = ",") %>% unlist %>% tibble::enframe(value = "transcript_type") %>% dplyr::group_by(transcript_type) %>% dplyr::summarise("tally" = n()) %>% dplyr::arrange(desc(tally))

tibble_IR_transcript_ontology$matched_transcript_biotype %>% strsplit(split = ",") %>% unlist %>% tibble::enframe(value = "transcript_biotype") %>% dplyr::group_by(transcript_biotype) %>% dplyr::summarise("tally" = n()) %>% dplyr::arrange(desc(tally))
...
# transcript-level ###
message("total number of unique differential exons with any sort of transcript-level annotation: ", tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length)

message("percent of differential regions matched to protein coding transcript: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "protein_coding") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions matched to an lncRNA transcript: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "lncRNA") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions matched to a miRNA transcript: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "miRNA") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))


message("percent of differential regions overlapping a start codon: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "start_codon") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions overlapping a stop codon: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "stop_codon") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions matched to a 3' UTR region: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "three_prime_utr") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions matched to a 5' UTR region: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "five_prime_utr") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))
message("percent of differential regions matched to a CDS region: ", 100 * grep(x = tibble_final_transcript_feature_annotation$tibble_overlapping_parent_transcript_features, pattern = "CDS") %>% length/(tibble_final_transcript_feature_annotation$alternative_exon_coords %>% length))

# create table of transcript-level summary
tibble_transcript_level_summary_count <- tibble_final_transcript_feature_annotation %>%
  dplyr::mutate(
    # "protein_coding" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "protein_coding"),
    # "lncRNA" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "lncRNA"),
    # "miRNA" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "miRNA"),
    
    "start_codon" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "start_codon"),
    "stop_codon" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "stop_codon"),
    "three_prime_utr" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "three_prime_utr"),
    "five_prime_utr" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "five_prime_utr"),
    "CDS" = grepl(x = tibble_overlapping_parent_transcript_features, pattern = "CDS"))
tibble_transcript_level_summary_count[, "splicemode"] <- gsub(x = tibble_transcript_level_summary_count$splicemode, pattern = "IR_overlapping_region", replacement = "IR")
tibble_transcript_level_summary_count[, "splicemode"] <- gsub(x = tibble_transcript_level_summary_count$splicemode, pattern = "TSS\\|", replacement = "")

tibble_transcript_level_summary_count <- tibble_transcript_level_summary_count %>% dplyr::select(-matched_strand, -matched_gene_names, -vector_matched_transcript_ids, -tibble_overlapping_parent_transcript_features, )

# summarise per splicemode for a truth matrix
# tibble_transcript_level_summary_count <- tibble_transcript_level_summary_count %>% 
#   dplyr::group_by(splicemode, 
#                   protein_coding, lncRNA, miRNA, 
#                   start_codon, stop_codon, three_prime_utr, five_prime_utr, CDS) %>%
#   dplyr::summarise("tally" = n())

# change splicemode to a logical
list_transcript_level_summary_count <- tibble_transcript_level_summary_count %>% 
  dplyr::ungroup() %>% 
  dplyr::group_split(splicemode) %>%
  setNames(nm = purrr::map(.x = ., .f = ~.x$splicemode %>% unique) %>% unlist) %>%
  purrr::map2(
    .x = .,
    .y = names(.),
    .f = function(a1, a2) {
      ...
      tibble_output[, "splicemode"] <- TRUE
      
      tibble_output <- tibble_output %>% dplyr::rename(!!a2 := "splicemode")
...
# rbind and tibblise
tibble_transcript_level_summary_count <- list_transcript_level_summary_count %>% rbindlist(fill = TRUE, use.names = TRUE) %>% as_tibble

# fill NA with FALSE
tibble_transcript_level_summary_count[is.na(tibble_transcript_level_summary_count)] <- FALSE

# upset plot
message("transcript-level plot by splicemode")

upset_combs <- data.frame(tibble_transcript_level_summary_count, row.names = paste(1:nrow(tibble_transcript_level_summary_count), tibble_transcript_level_summary_count$VSR_coords, tibble_transcript_level_summary_count$alternative_exon_coords, sep = "|")) %>% make_comb_mat(mode = "distinct")

print(set_size(upset_combs))

pdf(file = paste(R_processing_results_dir, "upset_plot_transcript_ontology_x_splicemode_", pvalue_or_FDR, pvalue_cutoff, "_dpsi", dpsi_cutoff, "_differential.pdf", sep = ""), height = 4, width = 12)

ht <- ComplexHeatmap::UpSet(upset_combs, 
                            comb_order = order(-comb_size(upset_combs)), 
                            left_annotation = rowAnnotation("Set Type" = c(
                              "A3SS" = "Splicemode",
                              # "protein_coding" = "Transcript type",
                              # "lncRNA" = "Transcript type",
                              # "miRNA" = "Transcript type",
                              
                              "start_codon" = "Transcript region",
                              "stop_codon" = "Transcript region",
                              "three_prime_utr" = "Transcript region",
                              "five_prime_utr" = "Transcript region",
                              "CDS" = "Transcript region",
                              ...
                              "MXS" = "Splicemode",
                              "SES" = "Splicemode")[set_name(upset_combs)], show_annotation_name = FALSE, 
                              col = list("Set Type" = c("Transcript region" = "sandybrown", "Splicemode" = "red4"))
                            ),
                            row_labels =  c("A3SS", "Start codon", "Stop codon", "3' UTR", "5' UTR", "CDS", "A5SS", "IR", "MES", "MXS", "SES")
)
...
tibble_protein_level_summary_count <- tibble_final_biomart_annotation %>% dplyr::group_by(alternative_exon_coords) %>% 
...
# create tally of each unique instance of interpro ID per alternative_exon_coords
...
tibble_interpro_ID_to_matched_alternative_exon_coords_mapping <- tibble_final_biomart_annotation %>% dplyr::group_by(interpro) %>% 
  dplyr::summarise("alternative_exon_coords" = paste(alternative_exon_coords %>% unique %>% mixedsort, collapse = ",")) %>% setNames(c("interpro_id", "alternative_exon_coords"))
...
tibble_interpro_id_tally_per_diff.exon <- dplyr::right_join(tibble_interpro_ID_to_matched_alternative_exon_coords_mapping, tibble_interpro_id_tally_per_diff.exon, by = "interpro_id")
# plot
...
message("total number of unique exons with any sort of protein-level annotation: ", tibble_final_biomart_annotation$alternative_exon_coords %>% unique %>% length, " (", tibble_final_biomart_annotation$alternative_exon_coords %>% unique %>% length *100/nrow(tibble_final_transcript_feature_annotation), "% of those which have any transcript annotation)")
...
tibble_PTM_tally <- tibble_final_PTM_annotation %>% dplyr::distinct(alternative_exon_coords, splicemode, modified_residue_position, PTM_type) %>% na.omit %>% dplyr::group_by(PTM_type) %>% dplyr::summarise("tally" = n())
