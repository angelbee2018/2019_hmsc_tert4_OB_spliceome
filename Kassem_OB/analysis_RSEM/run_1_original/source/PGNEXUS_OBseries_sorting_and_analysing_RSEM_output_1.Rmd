...
OBseries_rsem_results_dir <- "Y:/PGNEXUS_OBseries/results_RSEM_quant/RSEM_results/"

OBseries_R_processing_results_dir <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB_fastqc/analysis_RSEM_local/run_1_original/R_processing_results/"
...
nsYg <- RUVg(OBseries_gene_expectedcount_raw_highpass_upperQN %>% as.matrix, cIdx = empiricalcontrolgenes_row_positions, k = 2)
 
OBseries_gene_expectedcount_RUVg <- bind_cols(OBseries_gene_expectedcount_raw_highpass[, 1], nsYg$normalizedCounts %>% as_tibble)
  
  pdf(paste(OBseries_qualitycheck_results_dir, "RLE_boxplot_RUVg_gene_expectedcount_originalconfig_k2.pdf", sep = "")) 
  
  par(mar = c(20, 4.1, 4.1, 2.1))
  
  plotRLE(OBseries_gene_expectedcount_RUVg[, 2:ncol(OBseries_gene_expectedcount_RUVg)] %>% as.matrix, outline = FALSE, ylim = c(-1.5, 1.5), col = timepoints_OB, las = 2, main = "RLE plot of RUVg expected count, k = 2", )
  
  dev.off()
  
# RUVs

nsYs <- RUVs(OBseries_gene_expectedcount_raw_highpass_upperQN %>% as.matrix, scIdx = replicate_rownumbers, k = 3)
 
OBseries_gene_expectedcount_RUVs <- bind_cols(OBseries_gene_expectedcount_raw_highpass[, 1], nsYs$normalizedCounts %>% as_tibble)
  
  pdf(paste(OBseries_qualitycheck_results_dir, "RLE_boxplot_RUVs_gene_expectedcount_originalconfig_k3.pdf", sep = "")) 
  
  par(mar = c(20, 4.1, 4.1, 2.1))
  
  plotRLE(OBseries_gene_expectedcount_RUVs[, 2:ncol(OBseries_gene_expectedcount_RUVs)] %>% as.matrix, outline = FALSE, ylim = c(-1.5, 1.5), col = timepoints_OB, las = 2, main = "RLE plot of RUVs expected count, k = 3", )
...
nsYr_upperonly <- RUVr(OBseries_gene_expectedcount_raw_highpass_upperQN %>% as.matrix, cIdx = 1:length(OBseries_gene_expectedcount_raw_highpass$ensembl_gene_id), k = 3, residuals = residuals_upperonly)
...
OBseries_gene_expectedcount_RUVr <- bind_cols(OBseries_gene_expectedcount_raw_highpass[, 1], nsYr_upperonly$normalizedCounts %>% as_tibble)
  ...
  pdf(paste(OBseries_qualitycheck_results_dir, "RLE_boxplot_RUVr_gene_expectedcount_originalconfig_k3.pdf", sep = "")) 
  
  par(mar = c(20, 4.1, 4.1, 2.1))
  
  plotRLE(OBseries_gene_expectedcount_RUVr[, 2:ncol(OBseries_gene_expectedcount_RUVr)] %>% as.matrix, outline = FALSE, ylim = c(-1.5, 1.5), col = timepoints_OB, las = 2, main = "RLE plot of RUVr expected count, k = 3", )
...
# estimate dispersion
OBseries_dge_genecounts_disp <- estimateDisp(OBseries_dge_genecounts_normfactors)
...
column_anysig_DEgenes_OBseries <- purrr::map(.x = list_edgeR_pairwise_topTags_OBseries, .f = ~.x$table %>% .[.$FDR <= 0.01 & abs(.$logFC) > 1.2, "genes"]) %>% purrr::reduce(union) %>% as_tibble %>% setNames("ensembl_gene_id")
...
tibble_GLM_OBseries_DEGrownums <- which(apply(tibble_GLM_OBseries, 1, FUN = function(X) {(any(abs(as.numeric(X[logFC_colnums])) > 1.2)) & (X[FDR_colnum] %>% as.numeric < 0.01)}))
...
purrr::map(.x = list_of_OBseries_DEGs_hyperGOresult_allGO_tables, .f = ~print(head(.x, n = 10)))
