...
list_scRNAseq_inclusion_exclusion_biomarkers <- list(
  "unique_inclusion" = read.delim("Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_unique_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "unique_exclusion" = read.delim("Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_unique_exclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "inclusion" = read.delim("Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "exclusion" = read.delim("Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_exclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree
) 
...
# make average tibble for each group
...
tibble_inclusion_exclusion_biomarkers_expression[, "scRNAseq_cluster"] <- factor(tibble_inclusion_exclusion_biomarkers_expression$scRNAseq_cluster %>% as.character, levels = c(11, 10, 1, 3, 6, 7, 8, 0, 2, 9, 5, 4) %>% as.character)

tibble_inclusion_exclusion_biomarkers_expression_average.line[, "scRNAseq_cluster"] <- factor(tibble_inclusion_exclusion_biomarkers_expression_average.line$scRNAseq_cluster %>% as.character, levels = c(11, 10, 1, 3, 6, 7, 8, 0, 2, 9, 5, 4) %>% as.character)
...
vector_goterms_to_plot <- c("response to unfolded protein", "positive regulation of mitotic cell cycle", "negative regulation of mitotic cell cycle", "ossification", "response to glucocorticoid", "osteoblast differentiation", "extracellular matrix organization", "actin cytoskeleton organization", "p38MAPK cascade", "Wnt signaling pathway", "regulation of BMP signaling pathway", "negative regulation of cell death", "integrin-mediated signaling pathway", "cell migration")
...
tibble_scRNAseq_clusterwise_biomarker_hyperGO <- read.delim("Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/GSEA_hyperGO_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble
...
vector_scRNAseq_cluster_order <- c(1, 3, 6, 10, 7, 8, 0, 2, 4, 5, 9) %>% as.character
...
ggplot(tibble_scRNAseq_clusterwise_biomarker_hyperGO, aes(fill = -Padj)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, axis.text.y = element_text(lineheight = 0.75, colour = "black"), axis.title.y = element_text(), text = element_text(family = "Helvetica")) +
  ggsave(filename = paste(OBseries_R_processing_results_dir, "heatmap_BP_GOterms_per_scRNAseq_cluster_select.pdf", sep = ""), device = "pdf", dpi = 600, width = 14, height = 10, units = "cm", limitsize = FALSE)
