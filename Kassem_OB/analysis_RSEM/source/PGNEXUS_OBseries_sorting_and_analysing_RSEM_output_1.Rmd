...
OBseries_sampleandaccessionlists_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/"

OBseries_rsem_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_RSEM/run_1_original/RSEM_quant_results/"

OBseries_R_processing_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_RSEM/run_1_original/R_processing_results/"
...
ggplot(PCA_gene_expectedcount_RUVr_loadings) + 
  geom_point(aes(y = PC2, x = PC1, shape = replicatenumber, color = timepoint, size = 2)) +
  scale_color_brewer(palette = "Spectral", breaks = order_of_timepoints_OB, limits = order_of_timepoints_OB) +
  ggtitle(paste("PCA standard deviations based on gene expected count\nRSEM/EdgeR/RUVr", sep = "")) +
  ...
  theme_bw() +
  theme(text = element_text(family = "Helvetica")) +
  ggsave(filename = paste(OBseries_qualitycheck_results_dir, "PCA_plot_PC2_vs_PC1_based_on_RUVr_gene_expectedcount_originalconfig.pdf", sep = ""), device = "pdf", dpi = 600, width = 14, height = 12, units = "cm") +
  ggsave(filename = paste(OBseries_qualitycheck_results_dir, "PCA_plot_PC2_vs_PC1_based_on_RUVr_gene_expectedcount_originalconfig.svg", sep = ""), device = "svg", dpi = 600, width = 14, height = 12, units = "cm")
...
timepoint_comparisons_OBseries <- combn(timepoints_OB %>% unique, 2) %>% t
...
tibble_all_pairwise_results <- purrr::map2(.x = list_edgeR_pairwise_topTags_OBseries, .y = paste(timepoint_comparisons_OBseries[, 1], timepoint_comparisons_OBseries[, 2], sep = "_vs_"), .f = ~.x$table %>% add_column(comparison = .y)) %>% rbindlist %>% as_tibble
# filter by significant gene ids
tibble_all_pairwise_results_significant_only <- dplyr::semi_join(tibble_all_pairwise_results, 
                                                                 column_anysig_DEgenes_OBseries %>% dplyr::rename("genes" = "ensembl_gene_id"),
                                                                 by = "genes")

tibble_cumulative_diff_changes <- tibble("timepoint" = vec_timepoint_order_OBseries_short_edited, 
                                         "cumulative_diff_changes" = 
                                           c(0, 
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & !str_detect(comparison, "9d") & !str_detect(comparison, "6d") & !str_detect(comparison, "3d") & !str_detect(comparison, "24h") & !str_detect(comparison, "12h") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & !str_detect(comparison, "9d") & !str_detect(comparison, "6d") & !str_detect(comparison, "3d") & !str_detect(comparison, "24h") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & !str_detect(comparison, "9d") & !str_detect(comparison, "6d") & !str_detect(comparison, "3d") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & !str_detect(comparison, "9d") & !str_detect(comparison, "6d") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & !str_detect(comparison, "9d") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
                                             tibble_all_pairwise_results_significant_only %>% dplyr::filter(!str_detect(comparison, "12d") & FDR <= 0.01 & abs(logFC) >= 1) %>% nrow,
...
# rbind and tibblise
tibble_DEGs_hyperGOresult_allGO_tables <- list_of_OBseries_DEGs_hyperGOresult_allGO_tables %>% rbindlist %>% as_tibble

# write table of all GO categories combined
write.table(x = tibble_DEGs_hyperGOresult_allGO_tables, file = paste(OBseries_R_processing_results_dir, "table_hypergoresult_allDEGs_allGO.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

# write tables of each separate GO category
# purrr::map2(.x = list_of_OBseries_DEGs_hyperGOresult_allGO_tables,
#             .y = list_of_OBseries_DEGs_hyperGOresult_allGO_tables %>% names,
#             .f = ~write.table(x = .x, file = paste(OBseries_R_processing_results_dir, "table_hypergoresult_allDEGs_", .y, ".txt", sep = "")))
...
vec_timepoint_order_OBseries_short_edited <- gsub(x = vec_timepoint_order_OBseries_short_edited, pattern = "ud", replacement = "MSC")
...
ggplot() +
 ...
 ggsave(filename = paste(OBseries_R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(wide_tibble_of_final_SOM_summary_5_by_5_OBseries$ensembl_gene_id %>% unique), "_genes_expr_OB_diff_qvalue0.01_fc2.pdf", sep = ""), device = "pdf", dpi = 600, width = 28, height = 20, units = "cm") +
 ggsave(filename = paste(OBseries_R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(wide_tibble_of_final_SOM_summary_5_by_5_OBseries$ensembl_gene_id %>% unique), "_genes_expr_OB_diff_qvalue0.01_fc2.svg", sep = ""), device = "svg", dpi = 600, width = 28, height = 20, units = "cm")
...
#### Co-plotting of gene expr. profiles for each cluster in scRNA-Seq

```{r}

# import tibble of scRNA-Seq cluster inclusion and exclusion biomarkers
# process them by unlisting the concatenated gene names.
list_scRNAseq_inclusion_exclusion_biomarkers <- list(
  "unique_inclusion" = read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_unique_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "unique_exclusion" = read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_unique_exclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "inclusion" = read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree,
  "exclusion" = read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/geneset_exclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble %>% setNames(c("cluster", "genes")) %>% array_tree
) 

# strsplit
list_scRNAseq_inclusion_exclusion_biomarkers <- purrr::map2(.x = list_scRNAseq_inclusion_exclusion_biomarkers,
                                                            ...
                                                            .f = function(a1, a2) {
                                                              ...
                                                              # name the L2 lists
                                                              vec_cluster_names <- a1 %>% purrr::map(.f = ~.x$cluster %>% trimws) %>% unlist
                                                              
                                                              names(a1) <- vec_cluster_names
                                                              ...
                                                              purrr::map2(.x = a1,
                                                                          ...
                                                                          .f = function(b1, b2) {
                                                                            ...
                                                                            b1$genes <- strsplit(b1$genes, split = ",") %>% unlist
                                                                            
                                                                            return(b1)
                                                                            
                                                                          } ) %>% return # L2 - scRNA-Seq clusters
                                                              
                                                            } ) %>% return # L1 - exclusion/inclusion

# semi join the long gene expression SOM table
list_scRNAseq_inclusion_exclusion_biomarkers <- purrr::map2(.x = list_scRNAseq_inclusion_exclusion_biomarkers,
                                                            .y = names(list_scRNAseq_inclusion_exclusion_biomarkers),
                                                            .f = function(a1, a2) {
                                                              ...
                                                              purrr::map2(.x = a1,
                                                                          ...
                                                                          .f = function(b1, b2) {
                                                                            ...
                                                                            # tibblise the geneset for joining
                                                                            tibble_biomarker_geneset <- b1$genes %>% 
                                                                              tibble::enframe(value = "external_gene_name")
                                                                            
                                                                            tibble_joined <- dplyr::semi_join(long_tibble_of_final_SOM_summary_5_by_5_OBseries, 
                                                                                                              tibble_biomarker_geneset,
                                                                                                              by = "external_gene_name") %>%
                                                                              # now add the above level infos
                                                                              add_column("scRNAseq_cluster" = b2,
                                                                                         "inclusion_or_exclusion" = a2) %>% return
                                                                            
                                                                          } ) %>% return # L2 - scRNA-Seq clusters
                                                              
                                                            } ) %>% return # L1 - exclusion/inclusion

# flatten, rbind and tibblise.
tibble_inclusion_exclusion_biomarkers_expression <- list_scRNAseq_inclusion_exclusion_biomarkers %>% flatten %>% rbindlist %>% as_tibble %>% type_convert

# make average and (avg +/-1sd) tibble for each group ###
tibble_inclusion_exclusion_biomarkers_expression_average_sd <- tibble_inclusion_exclusion_biomarkers_expression %>% dplyr::group_by(inclusion_or_exclusion, scRNAseq_cluster, timepoint) %>% 
  dplyr::summarise("avg_expr" = mean(scaled_expr),
                   "avg_expr_plus_1sd" = mean(scaled_expr) + sd(scaled_expr),
                   "avg_expr_minus_1sd" = mean(scaled_expr) - sd(scaled_expr))
# order the cluster order factors
tibble_inclusion_exclusion_biomarkers_expression[, "scRNAseq_cluster"] <- factor(tibble_inclusion_exclusion_biomarkers_expression$scRNAseq_cluster %>% as.character, levels = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4) %>% as.character)
tibble_inclusion_exclusion_biomarkers_expression_average_sd[, "scRNAseq_cluster"] <- factor(tibble_inclusion_exclusion_biomarkers_expression_average_sd$scRNAseq_cluster %>% as.character, levels = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4) %>% as.character)

# THE GGPLOT ###
## line plot
ggplot() +
  geom_line(data = tibble_inclusion_exclusion_biomarkers_expression, aes(x = timepoint, y = scaled_expr, group = ensembl_gene_id), alpha = 0.1, size = 0.5) +
  geom_line(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, y = avg_expr, group = scRNAseq_cluster), colour = "white", size = 0.7) +
  scale_colour_manual(values = c("black")) +
  facet_grid(scRNAseq_cluster ~ inclusion_or_exclusion) +
  ggtitle(paste("Gene expression profiles\nDifferential inclusion/exclusion biomarkers per scRNA-Seq cluster", sep = "")) +
  scale_x_discrete(limits = vec_timepoint_order_OBseries_short, labels = vec_timepoint_order_OBseries_short_edited) +
  xlab("Time-point") +
  ylab("Scaled expr Level") +
  # guides(colour = FALSE) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica")) +
 ggsave(filename = paste(OBseries_R_processing_results_dir, "gene_expression_profiles_biomarkers_per_scRNAseq_cluster_OBseries_qvalue0.01_fc2.pdf", sep = ""), device = "pdf", dpi = 600, width = 14, height = 25, units = "cm") +
 ggsave(filename = paste(OBseries_R_processing_results_dir, "gene_expression_profiles_biomarkers_per_scRNAseq_cluster_OBseries_qvalue0.01_fc2.svg", sep = ""), device = "svg", dpi = 600, width = 14, height = 25, units = "cm")

## bar plot
ggplot() +
  geom_col(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, y = avg_expr, group = scRNAseq_cluster, fill = inclusion_or_exclusion), colour = "black", size = 0.2) +
  geom_errorbar(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, ymax = avg_expr_plus_1sd, ymin = avg_expr_minus_1sd, group = scRNAseq_cluster), colour = "black", size = 0.2) +
  scale_colour_manual(values = c("black")) +
 ...
 ggsave(filename = paste(OBseries_R_processing_results_dir, "gene_expression_errorbars_biomarkers_per_scRNAseq_cluster_OBseries_qvalue0.01_fc2.pdf", sep = ""), device = "pdf", dpi = 600, width = 17, height = 25, units = "cm") +
 ggsave(filename = paste(OBseries_R_processing_results_dir, "gene_expression_errorbars_biomarkers_per_scRNAseq_cluster_OBseries_qvalue0.01_fc2.svg", sep = ""), device = "svg", dpi = 600, width = 17, height = 25, units = "cm")

write.table(x = tibble_inclusion_exclusion_biomarkers_expression, file = paste(OBseries_R_processing_results_dir, "gene_expression_profiles_biomarkers_per_scRNAseq_cluster_OBseries_qvalue0.01_fc2.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
wide_tibble_of_final_SOM_summary_5_by_5_OBseries_edited <- wide_tibble_of_final_SOM_summary_5_by_5_OBseries %>% add_column("1_3d" = (.$`24h` + .$`3d`)/2)

vec_timepoints_for_bootstrap <- c(vec_timepoint_order_OBseries_short, "1_3d")

library(MASS)

list_of_probability_cdf_bootstrap_tibbles <- furrr::future_map2(
  ...
  .f = function(a1, a2) {
    ...
    furrr::future_map2(
      ...
      .f = function(b1, b2) {
        ...
        tibble_subset_by_cluster <- dplyr::semi_join(wide_tibble_of_final_SOM_summary_5_by_5_OBseries_edited, b1 %>% .[, "external_gene_name"], by = "external_gene_name")
        
        # extract only the timepoint data to get a "matrix", calculate sum of squares
        tibble_subset_by_cluster_matrix <- tibble_subset_by_cluster[, vec_timepoints_for_bootstrap] %>%
        ...
        tibble_timepointwise_bootstrap_result <- future_map(.x = 1:10000, .f = function(.x) {
          ...
          tibble_bootstrap_sample <- wide_tibble_of_final_SOM_summary_5_by_5_OBseries_edited %>% 
            dplyr::sample_n(size = sample_size) %>% 
            .[, vec_timepoints_for_bootstrap] %>%
            dplyr::mutate_all(.funs = function(x) {((x - mean(x))^2) %>% sum %>% return} ) %>% unique
          
          return(tibble_bootstrap_sample)
          
        }, .options = future_options(globals = c("wide_tibble_of_final_SOM_summary_5_by_5_OBseries_edited", "sample_size", "dplyr::sample_n", "vec_timepoints_for_bootstrap", "dplyr::mutate_all")) ) %>% rbindlist %>% as_tibble
        ...
        purrr::map(.x = tibble_timepointwise_bootstrap_result %>% colnames,
                   .f = function(c1) {
                     
                     pdf(file = paste(OBseries_R_processing_results_dir, "QQplot_significance_of_coherence_expression_", inclusion_or_exclusion, "_cluster", scRNAseq_cluster, "_timepoint", c1, ".pdf", sep = ""))
                     
                     qqnorm(tibble_timepointwise_bootstrap_result[, c1] %>% unlist, main = "", xlab = "", ylab = "")
...
# save the list
save(list_of_probability_cdf_bootstrap_tibbles, file = paste(OBseries_R_processing_results_dir, "list_of_probability_cdf_bootstrap_tibbles.Rlist", sep = ""))
...
wide_tibble_hyperGO_per_timepoint <- reshape2::dcast(data = tibble_hyperGO_per_timepoint %>% dplyr::filter(Ont == "BP" & highest_or_lowest == "highest"),
...
ggplot(long_tibble_reordered, aes(fill = -Padj)) +
  geom_tile(aes(y = timepoint, x = Term)) +
...
vector_goterms_to_plot_bulk <- c("response to unfolded protein", "positive regulation of mitotic cell cycle", "ossification", "response to glucocorticoid", "osteoblast differentiation", "extracellular matrix organization", "actin cytoskeleton organization", "p38MAPK cascade", "Wnt signaling pathway", "regulation of BMP signaling pathway", "negative regulation of cell death", "integrin-mediated signaling pathway", "cell migration", "canonical glycolysis")

ggplot(long_tibble_reordered, aes(fill = -Padj)) +
  geom_tile(aes(x = timepoint, y = Term)) + 
  ggtitle(paste("Heatmap of select BP GOterms per timepoint", sep = "")) +
  scale_x_discrete(breaks = vec_timepoint_order_OBseries_short, labels = vec_timepoint_order_OBseries_short_edited, limits = vec_timepoint_order_OBseries_short) +
  scale_y_discrete(breaks = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_bulk)], labels = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_bulk)], limits = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_bulk)]) +
  scale_fill_gradientn(colours = c("white", "yellow", "red", "black"), values = c(0, 0.5, 0.95, 1)) + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, axis.text.y = element_text(lineheight = 0.75, colour = "black"), axis.title.y = element_text(), text = element_text(family = "Helvetica")) +
  ggsave(filename = paste(OBseries_R_processing_results_dir, "heatmap_BP_GOterms_per_timepoint_select.pdf", sep = ""), device = "pdf", dpi = 600, width = 12, height = 10, units = "cm", limitsize = FALSE)

# import tibble of scRNA-Seq cluster GO terms
tibble_scRNAseq_clusterwise_biomarker_hyperGO <- read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/seurat/GSEA_hyperGO_inclusion_biomarkers_per_cluster.txt", sep = "\t", header = TRUE, row.names = NULL, stringsAsFactors = FALSE) %>% as_tibble

tibble_scRNAseq_clusterwise_biomarker_hyperGO <- tibble_scRNAseq_clusterwise_biomarker_hyperGO %>% dplyr::mutate_at(.vars = "cluster", as.character)

vector_scRNAseq_cluster_order <- c(1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4) %>% as.character

# get GO Term order
## dcast
wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO <- tibble_scRNAseq_clusterwise_biomarker_hyperGO %>% 
  dplyr::select(-Phyper) %>% 
  reshape2::dcast(formula = Term ~ paste("cluster_", cluster, sep = ""), value.var = "Padj") %>%
  as_tibble

wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO[is.na(wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO)] <- 1

dist_result <- dist(wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO[, paste("cluster_", vector_scRNAseq_cluster_order, sep = "")], method = "euclidean")
hclust_result <- hclust(dist_result, method = "ward.D2")
dendrogram <- as.dendrogram(hclust_result)

matrix_PCA1_rowwise <- prcomp(wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO[, paste("cluster_", vector_scRNAseq_cluster_order, sep = "")] %>% t) %>% .[["rotation"]] %>% .[, 1]
dendrogram_reordered <- reorder(dendrogram, matrix_PCA1_rowwise)

tibble_reordered_scRNAseq <- wide_tibble_scRNAseq_clusterwise_biomarker_hyperGO[dendrogram_reordered %>% unlist, ]
# get the order of goterms due to the clustering
vector_goterms_ordered_scRNAseq <- tibble_reordered_scRNAseq$Term

# plot select GO terms
vector_goterms_to_plot_scRNAseq <- c("response to unfolded protein", "mitotic cell cycle", "negative regulation of G2/M transition of mitotic cell cycle", "ossification", "mRNA splicing, via spliceosome", "NIK/NF-kappaB signaling", "SRP-dependent cotranslational protein targeting to membrane", "p38MAPK cascade", "response to stress", "response to glucocorticoid", "osteoblast differentiation", "negative regulation of cell death", "actin cytoskeleton organization", "regulated exocytosis", "stem cell population maintenance", "negative regulation of Wnt signaling pathway")

# same order as bulk
# ggplot(tibble_scRNAseq_clusterwise_biomarker_hyperGO, aes(fill = -Padj)) +
#   geom_tile(aes(x = cluster, y = Term)) + 
#   ggtitle(paste("Heatmap of select BP GOterms per cluster", sep = "")) +
#   scale_x_discrete(breaks = vector_scRNAseq_cluster_order, labels = vector_scRNAseq_cluster_order, limits = vector_scRNAseq_cluster_order) +
#   scale_y_discrete(breaks = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_scRNAseq)], labels = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_scRNAseq)], limits = vector_goterms_ordered[which(vector_goterms_ordered %in% vector_goterms_to_plot_scRNAseq)]) +
#   scale_fill_gradientn(colours = c("black"), na.value = "white") +
#   theme_bw() +
#   theme(axis.text.x = element_text(angle = 90, hjust = 1, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, axis.text.y = element_text(lineheight = 0.75, colour = "black"), axis.title.y = element_text(), text = element_text(family = "Helvetica")) +
#   ggsave(filename = paste(OBseries_R_processing_results_dir, "heatmap_BP_GOterms_per_scRNAseq_cluster_bulk_order.pdf", sep = ""), device = "pdf", dpi = 600, width = 14, height = 10, units = "cm", limitsize = FALSE)

# independent order for scRNA-Seq
ggplot(tibble_scRNAseq_clusterwise_biomarker_hyperGO, aes(fill = -Padj)) +
  geom_tile(aes(x = cluster, y = Term)) + 
  ggtitle(paste("Heatmap of select BP GOterms per cluster", sep = "")) +
  scale_x_discrete(breaks = vector_scRNAseq_cluster_order, labels = vector_scRNAseq_cluster_order, limits = vector_scRNAseq_cluster_order) +
  scale_y_discrete(breaks = vector_goterms_ordered_scRNAseq[which(vector_goterms_ordered_scRNAseq %in% vector_goterms_to_plot_scRNAseq)], labels = vector_goterms_ordered_scRNAseq[which(vector_goterms_ordered_scRNAseq %in% vector_goterms_to_plot_scRNAseq)], limits = vector_goterms_ordered_scRNAseq[which(vector_goterms_ordered_scRNAseq %in% vector_goterms_to_plot_scRNAseq)]) +
  scale_fill_gradientn(colours = c("black"), na.value = "white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, axis.text.y = element_text(lineheight = 0.75, colour = "black"), axis.title.y = element_text(), text = element_text(family = "Helvetica")) +
  ggsave(filename = paste(OBseries_R_processing_results_dir, "heatmap_BP_GOterms_per_scRNAseq_cluster_scRNAseq_order.pdf", sep = ""), device = "pdf", dpi = 600, width = 16, height = 10, units = "cm", limitsize = FALSE)
