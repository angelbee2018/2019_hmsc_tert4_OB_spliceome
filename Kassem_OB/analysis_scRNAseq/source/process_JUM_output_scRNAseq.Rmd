---
title: "Sorting JUM output files and analysing them for 2019 Rauch scRNA-Seq"
author: "Angel Liang"
date: "10/21/2019"
output: pdf_document
---
...
R_processing_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/JUM/"
...
JUM_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/results_JUM/run_2_clusters_and_anticlusters/"
...
vector_of_timepoint_comparisons_final <- read.delim(paste(JUM_results_dir, "list_of_timepoint_comparisons_final.txt", sep = ""), stringsAsFactors = FALSE, sep = "\t", header = FALSE, row.names = NULL)

print(head(vector_of_timepoint_comparisons_final))

# cross the AS events with the comparisons for importing.
vector_splicemodes <- c("A3SS_events", "A5SS_events", "cassette_exon_events", "composite_events", "intron_retention", "MXE_events")

list_comparisons_x_splicemodes <- purrr::cross2(vector_of_timepoint_comparisons_final, vector_splicemodes) %>% 
  set_names(vector_splicemodes) %>%
  purrr::map(.f = ~cross2(.x[[1]], .x[[2]]) %>% set_names(.x[[1]]))

# L2[[1]]: comparison. L2[[2]]: splicemode
list_JUM_simplified_tibbles_raw <- purrr::map_depth(.x = list_comparisons_x_splicemodes, .depth = 2,
                                             .f = ~read.delim(file = paste(JUM_results_dir, "final_JUM_output_", .x[[1]], "/", list.files(path = paste(JUM_results_dir, "final_JUM_output_", .x[[1]], "/", sep = ""), pattern = paste("(.)", .x[[2]], "(.*)simplified.txt", sep = "")), sep = ""), sep = "\t", header = TRUE, stringsAsFactors = FALSE, row.names = NULL, na.strings = c("NONE", "NA"), colClasses = "character") %>% as_tibble)
# .progress = TRUE, .options = future_options(globals = c("as_tibble"))

```

# read all the detailed tables into environment

```{r}

# L2[[1]]: comparison. L2[[2]]: splicemode
list_JUM_detailed_tibbles_raw <- purrr::map_depth(.x = list_comparisons_x_splicemodes, .depth = 2,
                                             .f = ~read.delim(file = paste(JUM_results_dir, "final_JUM_output_", .x[[1]], "/", list.files(path = paste(JUM_results_dir, "final_JUM_output_", .x[[1]], "/", sep = ""), pattern = paste("(.)", .x[[2]], "(.*)detailed.txt", sep = "")), sep = ""), sep = "\t", header = TRUE, stringsAsFactors = FALSE, row.names = NULL, na.strings = c("NONE", "NA"), colClasses = "character") %>% as_tibble)
# .progress = TRUE, .options = future_options(globals = c("as_tibble"))
...
long_tibble_AS.event.ID_to_composite_coords_mapping <- purrr::map2(
  .x = list_JUM_detailed_tibbles_raw,
  .y = names(list_JUM_detailed_tibbles_raw), 
  .f = function(a1, a2) {
    ...
    .f = function(b1, b2) {
      ...
      temp <- temp %>% add_column("splicemode" = a2, .after = "AS_event_ID")
      temp <- temp %>% add_column("comparison" = b2, .after = "splicemode")
...
# chopping simplified tables into a net summary table
...
list_JUM_simplified_tibbles_processed_wide <- future_map2(
  .x = list_JUM_simplified_tibbles_raw,
  .y = names(list_JUM_simplified_tibbles_raw), 
  .f = function(a1, a2) {
    ...
    .f = function(b1, b2) {
      ...
      colnames(temp)[which(colnames(temp) == "pvalue")] <- paste("pvalue_", b2, sep = "")
      colnames(temp)[which(colnames(temp) == "qvalue")] <- paste("qvalue_", b2, sep = "")
      ...
      temp <- split_delimited_column_in_table(temp, target_colname = colnames(temp)[grep(x = colnames(temp), pattern = "deltaPSI")], split = ";", columns_to_deduplicate = "AS_event_ID")
...
wide_tibble_all_splicemodes_split_with_na <- list_JUM_simplified_tibbles_processed_wide %>% purrr::map(.f = ~purrr::reduce(.x, dplyr::full_join)) %>% rbindlist(fill = TRUE) %>% as_tibble %>% filter(Gene != "Gene") %>% type_convert
...
long_tibble_all_splicemodes_split_with_na <- list_JUM_simplified_tibbles_processed_long %>% flatten %>% rbindlist %>% as_tibble %>% dplyr::filter(Gene != "Gene") %>% type_convert
...
# Filter for differential events

```{r}

long_tibble_all_splicemodes_split_with_na_differential <- long_tibble_all_splicemodes_split_with_na %>% dplyr::filter(qvalue <= 0.05 & (deltaPSI >= 0.15 | deltaPSI <= -0.15) & deltaPSI != -Inf)

# write the table
write.table(long_tibble_all_splicemodes_split_with_na_differential, file = paste(R_processing_results_dir, "long_tibble_all_splicemodes_split_with_na_differential.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

```


# plot magnitude of dpsi

## all

```{r}

vector_all_dpsi <- long_tibble_all_splicemodes_split_with_na %>% dplyr::filter(is.infinite(deltaPSI) != TRUE) %>% .$deltaPSI %>% unlist %>% na.omit

pdf(paste(R_processing_results_dir, "plot_of_absolute_dpsi_changes_all.pdf", sep = ""), width = 5, height = 5)
plot(density((vector_all_dpsi * 100) %>% abs, bw = 1), 
     ...
     xaxt = "n",
     xlim = c(0, 100)) + 
  polygon(density((vector_all_dpsi * 100) %>% abs, bw = 1),
...
vector_differential_dpsi <- long_tibble_all_splicemodes_split_with_na_differential %>% dplyr::filter(is.infinite(deltaPSI) != TRUE) %>% .$deltaPSI %>% unlist %>% na.omit
...
# Determine LSV biomarkers for each cluster

```{r}

long_tibble_LSV_biomarkers_per_cluster <- long_tibble_all_splicemodes_split_with_na_differential %>% 
  dplyr::filter(str_detect(string = comparison, pattern = "anti.") & qvalue <= 0.05 & abs(deltaPSI) >= 0.15 & is.infinite(deltaPSI) != TRUE)

# get table which tallies number of LSV biomarkers per cluster
tibble_number_of_LSV_biomarkers_per_cluster <- long_tibble_LSV_biomarkers_per_cluster %>%
  dplyr::group_by(comparison) %>%
  dplyr::distinct(AS_event_ID, splicemode, .keep_all = TRUE) %>%
  dplyr::summarise("tally" = n())
# write table
write.table(tibble_number_of_LSV_biomarkers_per_cluster, file = paste(R_processing_results_dir, "tibble_number_of_LSV_biomarkers_per_cluster.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

# import bulk JUM differential table 
...
# filter for only genes which were in the bulk dataset.
long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk <- dplyr::inner_join(long_tibble_LSV_biomarkers_per_cluster, tibble_JUM_differential_dpsi0.15_qvalue0.01_with_na %>% dplyr::select(Gene, AS_event_ID, sub_junction_start_coor, sub_junction_end_coor, splicemode) %>% setNames(c("Gene", "AS_event_ID_bulk", "sub_junction_start_coor_bulk", "sub_junction_end_coor_bulk", "splicemode_bulk")), by = c("Gene"))
# write table
write.table(long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk, file = paste(R_processing_results_dir, "long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
...
### melt table into a long form

```{r}

# list-ify
long_tibble_all_splicemodes_split_with_na <- purrr::map(.x = vector_of_timepoint_comparisons_final %>% unlist,
                                                        .f = ~wide_table_all_splicemodes_split_with_na %>% 
                                                          dplyr::select(Gene, AS_event_ID, isoform_number, splicemode, ends_with(.x), ends_with(gsub(x = .x, pattern = "_vs_", replacement = "."))) %>%
                                                          setNames(c("Gene", "AS_event_ID", "isoform_number", "splicemode", "pvalue", "qvalue", "dpsi_1.2")) %>% 
                                                          add_column("comparison" = .x)) %>% 
  rbindlist %>% as_tibble

# write table 
write.table(x = long_tibble_all_splicemodes_split_with_na, file = paste(R_processing_results_dir, "long_tibble_all_splicemodes_split_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
PSI_levels_timeseries_OB_wide_with_na <- wide_table_all_splicemodes_split_with_na %>% as.data.frame
...
ggplot(OB_anysig_GOtest_all_topten, aes(x = reorder(Term, Padj), y = SampleMatch)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented GO terms for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries, "_genes_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented GO terms for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries, "_genes_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 15, units = "cm") 
...
# import dbPTM info
# /mnt/Tertiary/sharedfolder/dbPTM_download/extract/allPTM_human.txt
...
# dbPTM_allhuman_annotations <- read.delim("/mnt/Tertiary/sharedfolder/dbPTM_download/extract/allPTM_human.txt", col.names = c("uniprotkb_entryname", "uniprotkb_entry", "modified_residue_position", "PTM_type", "pubmed_accessions", "context_sequence"), na = c("NA", ""), sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

# import PhosphoSitePlus info
# /mnt/Tertiary/sharedfolder/phosphositeplus_phosphosites_human.tab
...
tibble_final_transcript_feature_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("AS_event_ID", "splicemode", "vector_matched_transcript_ids", "transcript_features")] %>% as_tibble) %>%
  ...
  ## add in gene symbols
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "Gene")], ., by = "AS_event_ID")
# write table
write.table(x = tibble_final_transcript_feature_annotation, file = paste(results_dir, "table_junction_ontology_matched_transcript_features_", query_name, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_final_biomart_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("AS_event_ID", "splicemode", "biomart_annotations")] ) %>%
  ...
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "Gene")], ., by = "AS_event_ID")
# write table
write.table(x = tibble_final_biomart_annotation, file = paste(results_dir, "table_junction_ontology_matched_biomart_domains_", query_name, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_final_PTM_annotation <- list_tidied_annotation %>% purrr::map(~.x[c("AS_event_ID", "splicemode", "PTMs")] ) %>%
  ...
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "Gene")], ., by = "AS_event_ID")
# write table
write.table(x = tibble_final_PTM_annotation, file = paste(results_dir, "table_junction_ontology_matched_PTMs_", query_name, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
tibble_final_annotation_supp <- list_tidied_annotation %>% 
  ...
  dplyr::right_join(wide_table_all_splicemodes_split_with_na[, c("AS_event_ID", "Gene")], ., by = "AS_event_ID")
# write table
write.table(x = tibble_final_annotation_supp, file = paste(results_dir, "table_junction_ontology_supporting_info_", query_name, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# write table
write.table(x = tibble_final_annotation_supp_with_frameshift_test, file = paste(results_dir, "table_junction_ontology_supporting_info_frameshift_test_", query_name, ".txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(tibble_interpro_id_tally_per_ASeventID %>% head(90), aes(x = reorder(interpro_identifier, -tally), y = tally)) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 15, units = "cm") 

write.table(x = tibble_interpro_id_tally_per_ASeventID, file = paste(R_processing_results_dir, "tally_overlapping_interpro_domains_differential_junctions_", dPSI_cutoff,  "_", p_or_q_value, qpvalue_cutoff, "_", nrow(tibble_protein_level_summary_count), "_junctions_anysig_with_na.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(OB_anysig_GOtest_all_topten_AS_plausible, aes(x = reorder(Term, Padj), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented GO terms for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 80, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented GO terms for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 80, height = 15, units = "cm") 


write.table(x = OB_anysig_GOtest_all_topten_AS_plausible, file = paste(R_processing_results_dir, "Top 10 significantly over-represented GO terms for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_with_na_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(OB_anysig_family_enrichment_topten_AS_plausible, aes(x = reorder(TermID, padj), y = genes)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented families for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 45, height = 25, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "Top 10 significantly over-represented families for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 45, height = 25, units = "cm")

write.table(x = OB_anysig_family_enrichment_topten_AS_plausible, file = paste(R_processing_results_dir, "Top 10 significantly over-represented families for OB series dPSI_", dPSI_cutoff,  "_any", p_or_q_value, qpvalue_cutoff, "_", numberofgenes_anysig_OBseries_AS_plausible, "_genes_anysig_with_na_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_protein_kinases, by = "Gene"), aes(x = timepoint, y = scaled_PSI_value)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_protein_kinases, by = "Gene") %>% .$isoform_ID %>% unique), "_junctions_PSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength_kinases.pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
...
ggplot(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_nuclear_speck_proteins, by = "Gene"), aes(x = timepoint, y = scaled_PSI_value)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_nuclear_speck_proteins, by = "Gene") %>% .$isoform_ID %>% unique), "_junctions_PSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength_nuclear_speck_proteins.pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
...
ggplot(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_chromatin_remodeling_proteins, by = "Gene"), aes(x = timepoint, y = scaled_PSI_value)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_chromatin_remodeling_proteins, by = "Gene") %>% .$isoform_ID %>% unique), "_junctions_PSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength_chromatin_remodeling_proteins.pdf", sep = ""), device = "pdf", dpi = 600, width = 33, height = 20, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, OB_xdim, "x", OB_ydim, "_SOM_", length(dplyr::semi_join(long_table_of_final_SOM_summary_5_by_5_OBseries_AS_plausible, vector_chromatin_remodeling_proteins, by = "Gene") %>% .$isoform_ID %>% unique), "_junctions_PSI_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength_chromatin_remodeling_proteins.svg", sep = ""), device = "svg", dpi = 600, width = 33, height = 20, units = "cm")
...
ggplot(wide_table_of_timepointwise_highestPSI_results_MF_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "MF_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "MF_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")
...
ggplot(wide_table_of_timepointwise_highestPSI_results_BP_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_highestPSI_results_BP_OBseries_AS_plausible_processed, file = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_highestPSI_results_CC_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_highestPSI_results_CC_OBseries_AS_plausible_processed, file = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_lowestPSI_results_MF_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "MF_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "MF_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_lowestPSI_results_MF_OBseries_AS_plausible_processed, file = paste(R_processing_results_dir, "MF_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_lowestPSI_results_BP_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_lowestPSI_results_BP_OBseries_AS_plausible_processed, file = paste(R_processing_results_dir, "BP_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_lowestPSI_results_CC_OBseries_AS_plausible_processed, aes(x = reorder(Term, -SampleMatch), y = SampleMatch)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_lowestPSI_results_CC_OBseries_AS_plausible_processed, file = paste(R_processing_results_dir, "CC_GOterms_top10_", high_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_highestPSI_families_OBseries_AS_plausible, aes(x = reorder(TermID, padj), y = genes)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "Enrichedfamilies_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "Enrichedfamilies_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_highestPSI_families_OBseries_AS_plausible, file = paste(R_processing_results_dir, "Enrichedfamilies_top10_", high_scaledPSIcutoff * 100, "_percent_highestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot(wide_table_of_timepointwise_lowestPSI_families_OBseries_AS_plausible, aes(x = reorder(TermID, padj), y = genes)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5, lineheight = 0.75, colour = "black"), legend.title.align = 0.5, legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_text(margin = margin(r = 20)), text = element_text(family="Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "Enrichedfamilies_top10_", low_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.pdf", sep = ""), device = "pdf", dpi = 600, width = 25, height = 15, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "Enrichedfamilies_top10_", low_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.svg", sep = ""), device = "svg", dpi = 600, width = 25, height = 15, units = "cm")

write.table(x = wide_table_of_timepointwise_lowestPSI_families_OBseries_AS_plausible, file = paste(R_processing_results_dir, "Enrichedfamilies_top10_", low_scaledPSIcutoff * 100, "_percent_lowestPSI_eachtimepoint_OB_diff_any_", p_or_q_value, qpvalue_cutoff, "_any_deltaPSI_greaterthan_", dPSI_cutoff, "_plausibleASlength.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
ggplot() + 
  ...
  theme(text = element_text(family = "Helvetica")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, colour = "black"), text = element_text(family = "Helvetica")) +
  ggsave(filename = paste(R_processing_results_dir, "frequency_distribution_of_junctions_per_NMD.first.last.exon_", p_or_q_value, qpvalue_cutoff, "_dpsi", dPSI_cutoff, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 20, height = 30, units = "cm") +
  ggsave(filename = paste(R_processing_results_dir, "frequency_distribution_of_junctions_per_NMD.first.last.exon_", p_or_q_value, qpvalue_cutoff, "_dpsi", dPSI_cutoff, ".svg", sep = ""), device = "svg", dpi = 600, width = 20, height = 30, units = "cm") 
...
logCPM_OBseries_anysig_DEGs_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_RSEM/run_2_eRNA/R_processing_results/edgeR_GLM_DEGs_logCPM.txt"
