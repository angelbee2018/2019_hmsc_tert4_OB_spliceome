---
title: "Downstream processing of PSI-Sigma analysis results (2019 Rauch, MSC 0d and OB 7d)"
author: "Angel Liang"
date: "02/03/2020"
...
psisigma_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/results_PSIsigma/"

R_processing_results_dir <- "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/R_processing_results/PSIsigma/"
...
vector_of_comparisons_final <- read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_scRNAseq/results_PSIsigma/list_of_timepoint_comparisons_final.txt", sep = "\t", header = FALSE, stringsAsFactors = FALSE) %>% unlist %>% as.character
...
# catdb
load(file = paste(shared_dir, "polyA_RNAseq_GO_background_GOTERM.catdb", sep = ""))
...
list_of_psisigma_result_tibbles_processed <- future_imap(.x = list_of_psisigma_result_tibbles_raw, 
                                                        .f = function(a1, a2) {
                                                          
                                                          # DEBUG ###
                                                          # a1 <- list_of_psisigma_result_tibbles_raw[[12]]
                                                          ###########
                                                          
                                                          # message(a2)
                                                          
                                                          # rename all columns
                                                          setNames(a1, c("event_region_coords", "psisigma_gene.symbol", "diff_exon_coords", "splicemode", "num.replicates_comparison.1", "num.replicates_comparison.2", "psisigma_exon_type", "psisigma_reference.transcript", "dpsi", "pvalue", "FDR", "psi_comparison.1", "psi_comparison.2", "database_ID")) %>%
                                                            # separate absolute PSI columns
                                                            tidyr::separate(., col = psi_comparison.1, sep = "\\|", into = c("comparison.1_absolute.psi_1", "comparison.1_absolute.psi_2", "comparison.1_absolute.psi_3", "comparison.1_absolute.psi_4", "comparison.1_absolute.psi_5", "comparison.1_absolute.psi_6")) %>% 
                                                            # separate absolute PSI columns
                                                            tidyr::separate(., col = psi_comparison.2, sep = "\\|", into = c("comparison.2_absolute.psi_1", "comparison.2_absolute.psi_2", "comparison.2_absolute.psi_3", "comparison.2_absolute.psi_4", "comparison.2_absolute.psi_5", "comparison.2_absolute.psi_6")) %>%
...
list_of_psisigma_result_tibbles_processed <- map2(.x = list_of_psisigma_result_tibbles_processed, .y = names(list_of_psisigma_result_tibbles_processed), .f = ~add_column(.x, "comparison" = .y))
...
suppressWarnings(suppressMessages(list_ref_matching_exon_entries_all_array.tree_IR_reannotated <- future_imap(.x = list_ref_matching_exon_entries_all_array.tree_IR, .f = function(.x, .y) {
...
# save.image(paste(R_processing_results_dir, "16092020_psisigma_dexseq_processed.RData", sep = ""))
...
list_of_psisigma_denominator_gct_tibbles_processed <- purrr::map2(.x = list_of_psisigma_denominator_gct_tibbles, 
                                                                 ...
                                                                 .f = function(a1, a2) {
                                                                   ...
                                                                   tibble_temp <- tibble_temp %>% 
                                                                     ...
                                                                     setNames(c("database_ID", "comparison.1_denominator_1", "comparison.1_denominator_2", "comparison.1_denominator_3", "comparison.1_denominator_4", "comparison.1_denominator_5", "comparison.1_denominator_6", "comparison.2_denominator_1", "comparison.2_denominator_2", "comparison.2_denominator_3", "comparison.2_denominator_4", "comparison.2_denominator_5", "comparison.2_denominator_6", "comparison"))
...
wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator_numerator <- wide_tibble_of_psisigma_results_allcomparisons_final_with_denominator %>% 
  ...
  dplyr::mutate("comparison.1_numerator_4" = comparison.1_absolute.psi_4 * comparison.1_denominator_4 * 0.01) %>%
  dplyr::mutate("comparison.1_numerator_5" = comparison.1_absolute.psi_5 * comparison.1_denominator_5 * 0.01) %>%
  dplyr::mutate("comparison.1_numerator_6" = comparison.1_absolute.psi_6 * comparison.1_denominator_6 * 0.01) %>%
  ...
  dplyr::mutate("comparison.2_numerator_4" = comparison.2_absolute.psi_4 * comparison.2_denominator_4 * 0.01) %>%
  dplyr::mutate("comparison.2_numerator_5" = comparison.2_absolute.psi_5 * comparison.2_denominator_5 * 0.01) %>%
  dplyr::mutate("comparison.2_numerator_6" = comparison.2_absolute.psi_6 * comparison.2_denominator_6 * 0.01) %>%
  ...
  dplyr::mutate("comparison.1_ER_4" = comparison.1_denominator_4 - comparison.1_numerator_4) %>%
  dplyr::mutate("comparison.1_ER_5" = comparison.1_denominator_5 - comparison.1_numerator_5) %>%
  dplyr::mutate("comparison.1_ER_6" = comparison.1_denominator_6 - comparison.1_numerator_6) %>%
  ...
  dplyr::mutate("comparison.2_ER_4" = comparison.2_denominator_4 - comparison.2_numerator_4) %>%
  dplyr::mutate("comparison.2_ER_5" = comparison.2_denominator_5 - comparison.2_numerator_5) %>%
  dplyr::mutate("comparison.2_ER_6" = comparison.2_denominator_6 - comparison.2_numerator_6)
...
plan(list(tweak(multisession, workers = 12),
          tweak(multisession, workers = 6)))

future_map2(.x = list_of_psisigma_results_allcomparisons_final_with_denominator_numerator_filtered_5_in_at_least_3_by_comparison, 
...
list_combined_count_tibbles <- purrr::map(
  ...
  .f = function(a1) {
    ...
    list(
      ...
      "comparison.1_replicate_4" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.1_numerator_4), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.1_denominator_4) - (a1$comparison.1_numerator_4))),
      "comparison.1_replicate_5" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.1_numerator_5), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.1_denominator_5) - (a1$comparison.1_numerator_5))),
      "comparison.1_replicate_6" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.1_numerator_6), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.1_denominator_6) - (a1$comparison.1_numerator_6))),
      ...
      "comparison.2_replicate_4" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.2_numerator_4), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.2_denominator_4) - (a1$comparison.2_numerator_4))),
      "comparison.2_replicate_5" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.2_numerator_5), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.2_denominator_5) - (a1$comparison.2_numerator_5))),
      "comparison.2_replicate_6" = dplyr::bind_rows(tibble("gene:exon" = paste(a1$id, ":001", sep = ""),
                                                           "count" = a1$comparison.2_numerator_6), 
                                                    tibble("gene:exon" = paste(a1$id, ":002", sep = ""),
                                                           "count" = (a1$comparison.2_denominator_6) - (a1$comparison.2_numerator_6)))
    )
...
list_DEXSeq_results <- future_map(.x = list_of_psisigma_result_tibbles_processed %>% names,
           .f = function(a1) {
             
             # DEBUG ###
             # a1 <- list_of_psisigma_result_tibbles_processed %>% names %>% .[[12]]
             ###########
             
             cat("now processing: ", a1)
             
             DEXSeqDataSet_PSISigma <-  DEXSeqDataSetFromHTSeq(
               countfiles = list.files(path = DEXSeq_dir, pattern = paste(a1, "_.*.txt", sep = ""), full.names = TRUE),
               sampleData = data.frame("replicatename" = paste(a1, c("_comparison1r1_combined_count.txt", "_comparison1r2_combined_count.txt", "_comparison1r3_combined_count.txt", "_comparison1r4_combined_count.txt", "_comparison1r5_combined_count.txt", "_comparison1r6_combined_count.txt", "_comparison2r1_combined_count.txt", "_comparison2r2_combined_count.txt", "_comparison2r3_combined_count.txt", "_comparison2r4_combined_count.txt", "_comparison2r5_combined_count.txt", "_comparison2r6_combined_count.txt"), sep = ""),
             ...
             DEXSeqDataSet_PSISigma = estimateDispersions(DEXSeqDataSet_PSISigma, maxit	= 1000, niter = 100, BPPARAM = MulticoreParam(workers = 4, progressbar = TRUE))
...
names(list_DEXSeq_results) <- list_of_psisigma_result_tibbles_processed %>% names
...
# Determine LSV biomarkers and timepoint identity for each cluster

## import the bulk data and extract the LSVs or exons in common.

NOTE: dPSI is comparison 2 - comparison 1. a.k.a. 2nd minus the 1st.

```{r}

# get table which tallies number of LSV biomarkers per cluster
tibble_number_of_LSV_biomarkers_per_cluster <- wide_tibble_of_psisigma_results_allcomparisons_differential %>%
  dplyr::group_by(comparison) %>%
  dplyr::distinct(database_ID, event_region_coords, diff_exon_coords, splicemode, .keep_all = TRUE) %>%
...
# import bulk PSI-Sigma differential table (all differential)
tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_with_na <- read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/R_processing_results/long_tibble_of_psisigma_results_allcomparisons_differential_info_exons1810_dpsi15_DEXSeq_padj0.01_anysig_with_na.txt", sep = "\t", row.names = NULL, header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>% as_tibble
# import bulk PSI-Sigma time series profile table 
tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles <- read.delim("/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_1.9g_in_parallel_with_denominator_sorted_GTF/R_processing_results/wide_table_of_1368LIVs_scaled.PSI_OB_diff.txt", sep = "\t", row.names = NULL, header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>% as_tibble

# filter for only LSVs which were in the bulk dataset.
long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs <- dplyr::inner_join(wide_tibble_of_psisigma_results_allcomparisons_differential %>% 
                                                                               setNames(nm = paste(colnames(.), "_sc", sep = "")) %>% 
                                                                               dplyr::rename("database_ID" = "database_ID_sc", "event_region_coords" = "event_region_coords_sc", "diff_exon_coords" = "diff_exon_coords_sc", "splicemode" = "splicemode_sc"),
                                                                             tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles %>% 
                                                                               setNames(nm = paste(colnames(.), "_bulk", sep = "")) %>% 
                                                                               dplyr::rename("database_ID" = "database_ID_bulk", "event_region_coords" = "event_region_coords_bulk", "diff_exon_coords" = "diff_exon_coords_bulk", "splicemode" = "splicemode_bulk"), 
                                                                             by = c("database_ID", "event_region_coords", "diff_exon_coords"))
# filter for only diff_exons which were in the bulk dataset.
long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_exons <- dplyr::inner_join(wide_tibble_of_psisigma_results_allcomparisons_differential %>% 
                                                                               setNames(nm = paste(colnames(.), "_sc", sep = "")) %>% 
                                                                               dplyr::rename("diff_exon_coords" = "diff_exon_coords_sc"),
                                                                             tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles %>% 
                                                                               setNames(nm = paste(colnames(.), "_bulk", sep = "")) %>% 
                                                                               dplyr::rename("diff_exon_coords" = "diff_exon_coords_bulk"), 
                                                                             by = c("diff_exon_coords"))
# write tables
write.table(long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs, file = paste(R_processing_results_dir, "long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
write.table(long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_exons, file = paste(R_processing_results_dir, "long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_exons.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

```

## Determine timepoint identity for each single cell cluster

- Use the cluster vs. anti-cluster comparisons.
- Most negative dPSI = inclusion gene markers. 
- Most positive dPSI = exclusion gene markers.
- Could use the filtering by bulk exons to cover for noise effects. Interestingly, there's not much difference between exons and LSVs, which indicates robustness.
- However, because exons can have multiple time profiles as they belong to multiple LSVs, we have to match by LSV.

### Plot graph superimposing the bulk time series PSI values.

```{r}

# we have to fetch the inclusion and exclusion entries, both unique and non-unique. To do that, we will have to get the common LSVs first, then ggplot.
## unique LSVs for inclusion
tibble_non_unique_LSVs_inclusion_filtered_by_bulk_LSVs <- long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` < 0) %>% .[, c("database_ID", "event_region_coords", "diff_exon_coords", "comparison_sc")] %>% unique
tibble_non_unique_LSVs_inclusion_filtered_by_bulk_LSVs <- tibble_non_unique_LSVs_inclusion_filtered_by_bulk_LSVs %>% dplyr::group_by(database_ID, event_region_coords, diff_exon_coords) %>% dplyr::summarise("duplicate_tally" = n()) %>% dplyr::filter(duplicate_tally > 1) %>% dplyr::select(-duplicate_tally)

## unique LSVs for exclusion
tibble_non_unique_LSVs_exclusion_filtered_by_bulk_LSVs <- long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` > 0) %>% .[, c("database_ID", "event_region_coords", "diff_exon_coords", "comparison_sc")] %>% unique
tibble_non_unique_LSVs_exclusion_filtered_by_bulk_LSVs <- tibble_non_unique_LSVs_exclusion_filtered_by_bulk_LSVs %>% dplyr::group_by(database_ID, event_region_coords, diff_exon_coords) %>% dplyr::summarise("duplicate_tally" = n()) %>% dplyr::filter(duplicate_tally > 1) %>% dplyr::select(-duplicate_tally)

# create tibble merging everything together
tibble_LSV_inclusion_exclusion_biomarkers_filtered_by_bulk_LSVs <- dplyr::bind_rows(
  long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` < 0) %>% add_column(inclusion_or_exclusion = "inclusion"),
  long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` > 0) %>% add_column(inclusion_or_exclusion = "exclusion"),
  long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` < 0) %>% dplyr::anti_join(., tibble_non_unique_LSVs_inclusion_filtered_by_bulk_LSVs) %>% add_column(inclusion_or_exclusion = "unique_inclusion"),
  long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs[grep(x = long_tibble_LSV_biomarkers_per_cluster_filtered_by_bulk_LSVs$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi_sc` > 0) %>% dplyr::anti_join(., tibble_non_unique_LSVs_exclusion_filtered_by_bulk_LSVs) %>% add_column(inclusion_or_exclusion = "unique_exclusion")
)

# melt tibble to make long by bulk timepoint. This is for ggplot.
long_tibble_LSV_inclusion_exclusion_biomarkers <- tibble_LSV_inclusion_exclusion_biomarkers_filtered_by_bulk_LSVs %>% 
  reshape2::melt(measure.vars = paste(vector_OBseries_timepoints_edited, "_bulk", sep = ""), id.vars = colnames(tibble_LSV_inclusion_exclusion_biomarkers_filtered_by_bulk_LSVs)[!colnames(tibble_LSV_inclusion_exclusion_biomarkers_filtered_by_bulk_LSVs) %in% paste(vector_OBseries_timepoints_edited, "_bulk", sep = "")], value.name = "scaled_bulk_PSI", variable.name = "timepoint") %>%
  dplyr::mutate_at(.vars = "timepoint", .funs = function(x) {gsub(x = x, pattern = "_bulk", replacement = "")}) %>%
  dplyr::mutate("scRNAseq_cluster" = gsub(x = `comparison_sc`, pattern = "cluster_([^_]+)_vs_anti.cluster_.*", replacement = "\\1")) %>%
  as_tibble

# make average and (avg +/-1sd) tibble for each group (inclusion_or_exclusion + scRNAseq_cluster + timepoint) ###
tibble_inclusion_exclusion_biomarkers_expression_average_sd <- long_tibble_LSV_inclusion_exclusion_biomarkers %>% dplyr::group_by(inclusion_or_exclusion, scRNAseq_cluster, timepoint) %>% 
  dplyr::summarise("avg_PSI" = mean(scaled_bulk_PSI),
                   "avg_PSI_plus_1sd" = mean(scaled_bulk_PSI) + sd(scaled_bulk_PSI),
                   "avg_PSI_minus_1sd" = mean(scaled_bulk_PSI) - sd(scaled_bulk_PSI))
# order the cluster order factors
long_tibble_LSV_inclusion_exclusion_biomarkers[, "scRNAseq_cluster"] <- factor(long_tibble_LSV_inclusion_exclusion_biomarkers$scRNAseq_cluster %>% as.character, levels = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4)[purrr::map(.x = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4), .f = ~.x %in% tibble_inclusion_exclusion_biomarkers_expression_average_sd$scRNAseq_cluster %>% unique) %>% unlist] %>% as.character)
tibble_inclusion_exclusion_biomarkers_expression_average_sd[, "scRNAseq_cluster"] <- factor(tibble_inclusion_exclusion_biomarkers_expression_average_sd$scRNAseq_cluster %>% as.character, levels = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4)[purrr::map(.x = c(11, 1, 3, 6, 10, 7, 8, 0, 2, 9, 5, 4), .f = ~.x %in% tibble_inclusion_exclusion_biomarkers_expression_average_sd$scRNAseq_cluster %>% unique) %>% unlist] %>% as.character)

# tally up the number of LSVs per cluster
tibble_tally_LSV_markers_joined_to_bulk <- long_tibble_LSV_inclusion_exclusion_biomarkers %>% 
  dplyr::distinct(id_sc, inclusion_or_exclusion, scRNAseq_cluster, .keep_all = TRUE) %>%
  dplyr::group_by(inclusion_or_exclusion, scRNAseq_cluster) %>% 
  dplyr::summarise("tally" = n())
# write table
write.table(x = tibble_tally_LSV_markers_joined_to_bulk, file = paste(R_processing_results_dir, "tibble_tally_LSV_markers_joined_to_bulk_sparseness.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

# THE GGPLOT ###
## line plot
ggplot() +
  geom_line(data = long_tibble_LSV_inclusion_exclusion_biomarkers, aes(x = timepoint, y = scaled_bulk_PSI, group = id_sc), alpha = 0.2, size = 0.5) +
  geom_line(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, y = avg_PSI, group = scRNAseq_cluster), colour = "white", size = 0.7) +
  scale_colour_manual(values = c("black")) +
  facet_grid(scRNAseq_cluster ~ inclusion_or_exclusion) +
  ggtitle(paste("Splicing PSI profiles\nDifferential inclusion/exclusion biomarkers per scRNA-Seq cluster", sep = "")) +
 ...
 ggsave(filename = paste(R_processing_results_dir, "splicing_PSI_profiles_biomarkers_per_scRNAseq_cluster_matched_to_bulk.pdf", sep = ""), device = "pdf", dpi = 600, width = 14, height = 25, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "splicing_PSI_profiles_biomarkers_per_scRNAseq_cluster_matched_to_bulk.svg", sep = ""), device = "svg", dpi = 600, width = 14, height = 25, units = "cm")

## bar plot
ggplot() +
  geom_col(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, y = avg_PSI, group = scRNAseq_cluster, fill = inclusion_or_exclusion), colour = "black", size = 0.2) +
  geom_errorbar(data = tibble_inclusion_exclusion_biomarkers_expression_average_sd, aes(x = timepoint, ymax = avg_PSI_plus_1sd, ymin = avg_PSI_minus_1sd, group = scRNAseq_cluster), colour = "black", size = 0.2) +
  scale_colour_manual(values = c("black")) +
  scale_fill_manual(name = NULL, limits = c("exclusion", "inclusion", "unique_exclusion", "unique_inclusion"), values = c("red", "forestgreen", "red", "forestgreen")) + 
  facet_grid(scRNAseq_cluster ~ inclusion_or_exclusion) +
  ggtitle(paste("Splicing PSI profiles\nDifferential inclusion/exclusion biomarkers per scRNA-Seq cluster", sep = "")) +
  scale_x_discrete(limits = vector_OBseries_timepoints_edited, labels = vector_OBseries_timepoints_edited) +
  xlab("Time-point") +
  ...
  # guides(colour = FALSE) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica"), panel.spacing = unit(0, "lines")) +
 ggsave(filename = paste(R_processing_results_dir, "splicing_PSI_errorbars_biomarkers_per_scRNAseq_cluster_matched_to_bulk.pdf", sep = ""), device = "pdf", dpi = 600, width = 17, height = 25, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "splicing_PSI_errorbars_biomarkers_per_scRNAseq_cluster_matched_to_bulk.svg", sep = ""), device = "svg", dpi = 600, width = 17, height = 25, units = "cm")

write.table(x = long_tibble_LSV_inclusion_exclusion_biomarkers, file = paste(R_processing_results_dir, "splicing_PSI_profiles_biomarkers_per_scRNAseq_cluster_matched_to_bulk.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
write.table(x = tibble_inclusion_exclusion_biomarkers_expression_average_sd, file = paste(R_processing_results_dir, "splicing_PSI_errorbars_biomarkers_per_scRNAseq_cluster_matched_to_bulk.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)

```

### calculate P-values of lines grouped by scRNA-Seq cluster by bootstrap sampling.

- Calculate the obs. sum of squares at each timepoint
- Get a distribution of randomly sampled sum of squares by bootstrapping and calculate sigma value for each timepoint.

```{r}

# create a 4-element list of the inclusion and exclusion, unique/non-unique biomarkers except without being pre-joined to the bulk data.

# we have to fetch the inclusion and exclusion entries, both unique and non-unique. To do that, we will have to get the common LSVs first, then ggplot.
## unique LSVs for inclusion
tibble_non_unique_LSVs_inclusion <- wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` < 0) %>% .[, c("database_ID", "event_region_coords", "diff_exon_coords", "comparison")] %>% unique
tibble_non_unique_LSVs_inclusion <- tibble_non_unique_LSVs_inclusion %>% dplyr::group_by(database_ID, event_region_coords, diff_exon_coords) %>% dplyr::summarise("duplicate_tally" = n()) %>% dplyr::filter(duplicate_tally > 1) %>% dplyr::select(-duplicate_tally)

## unique LSVs for exclusion
tibble_non_unique_LSVs_exclusion <- wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison_sc, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` > 0) %>% .[, c("database_ID", "event_region_coords", "diff_exon_coords", "comparison")] %>% unique
tibble_non_unique_LSVs_exclusion <- tibble_non_unique_LSVs_exclusion %>% dplyr::group_by(database_ID, event_region_coords, diff_exon_coords) %>% dplyr::summarise("duplicate_tally" = n()) %>% dplyr::filter(duplicate_tally > 1) %>% dplyr::select(-duplicate_tally)

# create tibble merging everything together
tibble_LSV_inclusion_exclusion_biomarkers <- dplyr::bind_rows(
  wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` < 0) %>% add_column(inclusion_or_exclusion = "inclusion"),
  wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` > 0) %>% add_column(inclusion_or_exclusion = "exclusion"),
  wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` < 0) %>% dplyr::anti_join(., tibble_non_unique_LSVs_inclusion) %>% add_column(inclusion_or_exclusion = "unique_inclusion"),
  wide_tibble_of_psisigma_results_allcomparisons_differential[grep(x = wide_tibble_of_psisigma_results_allcomparisons_differential$comparison, pattern = "anti.cluster"), ] %>% dplyr::filter(`dpsi` > 0) %>% dplyr::anti_join(., tibble_non_unique_LSVs_exclusion) %>% add_column(inclusion_or_exclusion = "unique_exclusion")
)

# list-ify the tibble of inclusion and exclusion biomarkers
## L1: inclusion or exclusion
## L2: scRNA-Seq cluster
list_scRNAseq_inclusion_exclusion_biomarkers <- tibble_LSV_inclusion_exclusion_biomarkers %>%
  dplyr::mutate("scRNAseq_cluster" = gsub(x = `comparison`, pattern = "cluster_([^_]+)_vs_anti.cluster_.*", replacement = "\\1")) %>%
  dplyr::group_split(inclusion_or_exclusion) %>%
  set_names(nm = purrr::map(.x = ., .f = ~.x$inclusion_or_exclusion %>% unique) %>% unlist) %>%
  # L2 split
  purrr::map(.f = function(a1) {
    a1 %>%
      dplyr::group_split(scRNAseq_cluster) %>%
      set_names(nm = purrr::map(.x = ., .f = ~.x$scRNAseq_cluster %>% unique) %>% unlist) %>%
      return
  } )
...
list_of_probability_cdf_bootstrap_tibbles <- future_map2(
  .x = list_scRNAseq_inclusion_exclusion_biomarkers,
  .y = names(list_scRNAseq_inclusion_exclusion_biomarkers),
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a1 <- list_scRNAseq_inclusion_exclusion_biomarkers[[1]]
    # a2 <- names(list_scRNAseq_inclusion_exclusion_biomarkers) %>% .[[1]]
    ###########
    ...
    future_map2(
      ...
      .f = function(b1, b2) {
        ...
        inclusion_or_exclusion <- a2
        scRNAseq_cluster <- b2
        
        # filter the tibble for only genes in the scRNA-Seq cluster
        tibble_subset_by_cluster <- dplyr::semi_join(tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles, 
                                                     b1[, c("database_ID", "event_region_coords", "diff_exon_coords")] %>% unique, 
                                                     by = c("database_ID", "event_region_coords", "diff_exon_coords"))
        
        if (tibble_subset_by_cluster %>% nrow == 0) {
          return(NULL)
        } else {
            
          # extract only the timepoint data to get a "matrix", calculate sum of squares
        tibble_subset_by_cluster_matrix <- tibble_subset_by_cluster[, vector_OBseries_timepoints_edited] %>%
          dplyr::mutate_all(.funs = function(x) {((x - mean(x))^2) %>% sum %>% return} ) %>% unique
        
        # how many lines are in the group? we will sample this same amount when we bootstrap.
        sample_size <- tibble_subset_by_cluster %>% nrow
        
        tibble_timepointwise_bootstrap_result <- future_map(.x = 1:10000, .f = function(.x) {
          
          # bootstrap
          set.seed(.x)
          
          ## sample random rows based on sample size, calculate sum of squares.
          tibble_bootstrap_sample <- tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles %>% 
            dplyr::sample_n(size = sample_size) %>% 
            .[, vector_OBseries_timepoints_edited] %>%
            dplyr::mutate_all(.funs = function(x) {((x - mean(x))^2) %>% sum %>% return} ) %>% unique
          
          return(tibble_bootstrap_sample)
          
        }, .progress = TRUE, .options = future_options(globals = c("tibble_PSIsigma_differential_dpsi0.15_qvalue0.01_timeseries_profiles", "sample_size", "dplyr::sample_n", "vector_OBseries_timepoints_edited", "dplyr::mutate_all")) ) %>% rbindlist %>% as_tibble
        
        # fit normal distribution to each column
        tibble_normal_distribution_fit <- tibble_timepointwise_bootstrap_result %>% 
          array_tree(margin = 2) %>%
          purrr::map(.f = function(.x) {
            
            fit <- MASS::fitdistr(x = .x, densfun = "normal") 
            
            col <- c(fit$estimate %>% .[[1]], # mean
                     fit$estimate %>% .[[2]]) # sd
        ...
        # calculate sigma value for all timepoints.
        ## first bind rows so that: [1, ]: obs. values, [2, ]: mean, [3, ]: sd.
        tibble_group_info <- dplyr::bind_rows(tibble_subset_by_cluster_matrix, tibble_normal_distribution_fit)
        ## calculate sigma value for each column.
        tibble_probability_cdf <- tibble_group_info %>% 
          dplyr::mutate_all(.funs = function(x) {
            pnorm(q = x[1] %>% paste %>% as.numeric, mean = x[2] %>% paste %>% as.numeric, sd = x[3] %>% paste %>% as.numeric) %>% return
          } ) %>% unique
        
        # test normality of the bootstrapping for goodness of fit - all good :D
        # qqnorm(tibble_timepointwise_bootstrap_result$ud)
        
        # plot the bootstrap frequency density and show where the observation is.
        # plot(density(rnorm(n = 10000, mean = fit_mean, sd = fit_sd)), main = probability_cdf, xlim = c(0, max(c(tibble_timepointwise_bootstrap_result %>% .[[query_timepoint]], tibble_subset_by_cluster_matrix %>% .[[query_timepoint]])))) +
        #   abline(v = tibble_subset_by_cluster_matrix %>% .[[query_timepoint]] %>% paste %>% as.numeric, lty = 2) +
        #   abline(v = (fit_mean - 3*fit_sd), lty = 1) +
        #   abline(v = (fit_mean + 3*fit_sd), lty = 1)
        
        return(tibble_probability_cdf)
          
        }
        
      }, .progress = TRUE ) %>%  # L2 - scRNA-Seq clusters
    # prune
    
      purrr::discard(.p = ~.x %>% is.null == TRUE) %>%
      return
      
  }, .progress = TRUE ) %>% return # L1 - exclusion/inclusion

# save the list
save(list_of_probability_cdf_bootstrap_tibbles, file = paste(R_processing_results_dir, "list_of_probability_cdf_bootstrap_tibbles.Rlist", sep = ""))
