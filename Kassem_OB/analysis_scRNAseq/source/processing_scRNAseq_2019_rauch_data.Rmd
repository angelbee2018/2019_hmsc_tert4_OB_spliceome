...
FeaturePlot(seurat_object_combined_filtered, 
            features = c("TEAD1", "TEAD2", "TEAD3", "TEAD4", "SNAI1", "SNAI2", "MEF2C", "ELK4", "HIF1A", "JUNB", "NKX3-1", "SMAD3", "KLF4", "SOX9", "HES1", "SFRP4", "VEGFA", "ATP2B1", "FOXP1", "CHD3", "PRUNE2", "HNRNPH1", "HNRNPH3", "SVIL", "LMO4"), 
            pt.size = 0.05, ncol = 5, reduction = "tsne") +
  ...
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_SC.maintenance.factors.pdf", sep = ""), device = "pdf", dpi = 600, width = 40, height = 40, units = "cm") +
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_SC.maintenance.factors.svg", sep = ""), device = "svg", dpi = 600, width = 40, height = 40, units = "cm")

FeaturePlot(seurat_object_combined_filtered, 
            features = c("CDK1", "WEE1", "EZH2", "SRSF2", "TRA2B", "SRRM2", "TSEN2", "PRMT2", "HDAC7", "HDAC11", "MYO5A", "DNMT1", "TET1", "TET2", "MBD6"), 
            pt.size = 0.05, ncol = 5, reduction = "tsne") +
  # scale_alpha_manual(values = 0.5) +
  # theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica"), legend.position = "none") +
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_major.cell.factors.pdf", sep = ""), device = "pdf", dpi = 600, width = 40, height = 30, units = "cm") +
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_major.cell.factors.svg", sep = ""), device = "svg", dpi = 600, width = 40, height = 30, units = "cm")

FeaturePlot(seurat_object_combined_filtered, 
            features = c("RUNX2", "ALPL", "ZBTB16", "ZBTB38", "ZNF25", "TIRAP", "JAK1", "STAT3", "NFKB1", "IL6", "WNT5A", "TCF4", "TCF7L1", "NFATC4", "SFRP1", "SFRP2", "CCT2", "KCNE4", "KCNK6", "KCNMA1", "KCTD6", "ATF4", "CEBPB", "HES4", "NFE2L2", "WNK1", "ILK", "CTNNA1", "DDX5", "HIF1A"), 
            pt.size = 0.05, ncol = 5, reduction = "tsne") +
  # scale_alpha_manual(values = 0.5) +
  # theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica"), legend.position = "none") +
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_osteoblastic.factors.pdf", sep = ""), device = "pdf", dpi = 600, width = 40, height = 40, units = "cm") +
  ggsave(filename = paste(seurat_output_dir, "tSNE_seurat_SCT_filtered_annotation_set_osteoblastic.factors.svg", sep = ""), device = "svg", dpi = 600, width = 40, height = 40, units = "cm")

```

## Save the Seurat objet for use in Sierra
...
tibble_DTU_7d_vs_ud_all <- DUTest(seurat_object_combined_filtered_peak.level, 
                          ...
                          fc.thresh = 0,
                          adj.pval.thresh = 1) %>% as_tibble(rownames = "gene_peak_id")

# volcano plot
ggplot(data = tibble_DTU_7d_vs_ud_all, mapping = aes(x = Log2_fold_change, y = -log10(pvalue))) +
  geom_point(size = 2) +
...
list_pairwise_cluster_DTU <- future_map2(.x = tibble_pairwise_cluster_comparisons$comparison_1,
                                         ...
                                         .f = function(a1, a2){
                                           ...
                                           DUTest(seurat_object_combined_filtered_peak.level, 
                                                  population.1 = a1, 
                                                  population.2 = a2, 
                                                  exp.thresh = 0.1, 
                                                  feature.type = c("UTR3", "exon"),
  ...
  purrr::map2(.x = ., .y = names(.), .f = ~add_column(.x, "comparison" = .y))

# rbind and tibblise
tibble_pairwise_cluster_DTU_all <- list_pairwise_cluster_DTU %>% rbindlist %>% as_tibble

# save(list_pairwise_cluster_DTU, file = paste(sierra_output_dir, "list_pairwise_cluster_DTU.Rlist", sep = ""))

# write the table of results
write.table(tibble_pairwise_cluster_DTU_all, file = paste(sierra_output_dir, "table_DTU_pairwise_comparison_clusters_all.txt", sep = ""), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
#### Filter the pairwise result for logFC and qvalue

```{r}

tibble_pairwise_cluster_DTU_filtered <- tibble_pairwise_cluster_DTU_all %>% 
  dplyr::group_by(comparison) %>% 
  dplyr::filter(padj < pvalue_cutoff & abs(Log2_fold_change) >= log2(fold_change_cutoff))

# tally for differential peaks in common between each cluster comparison
tibble_pairwise_cluster_DTU_tally_common_peaks_with_9 <- tibble_pairwise_cluster_DTU_filtered %>%
  dplyr::filter(comparison %in% c("9_vs_8", "9_vs_7")) %>%
  dplyr::group_by(gene_peak_id) %>%
  dplyr::summarise("tally" = n()) %>% 
  dplyr::filter(tally > 1)
# tibble join to get info on the common peaks
tibble_pairwise_cluster_DTU_common_peaks_with_9 <- dplyr::semi_join(tibble_pairwise_cluster_DTU_filtered %>% dplyr::filter(comparison %in% c("9_vs_8", "9_vs_7")), 
                                                                    tibble_pairwise_cluster_DTU_tally_common_peaks_with_9, 
                                                                    by = "gene_peak_id")
