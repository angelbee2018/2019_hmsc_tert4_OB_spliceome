...
tibble_fasta_supp_info_exons <- read.delim(file = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_exons/exon_table_OB_diff_dpsi10_anypvalue0.01_3272_exons_anysig_with_na.txt", sep = "\t", row.names = NULL, header = TRUE) %>% as_tibble
...
tibble_fasta_supp_info_junctions <- read.delim(file = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_junctions/junction_table_OB_diff_any_qvalue0.01_any_deltaPSI_greaterthan_0.15.txt", sep = "\t", row.names = NULL, header = TRUE) %>% as_tibble
...
list_tibble_ebi_gwas_catalog_by_chr <- tibble_ebi_gwas_catalog %>% 
...
JUM_constitutive_dpsi0.15_qvalue0.01_with_na_vs_PSIsigma_constitutive_dpsi0.10_pvalue0.01_with_na <-
  calculate_intersections_from_chr_strand_start_end(tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na %>% JUM_AS.event.ID_to_chr_strand_start_end,
...
JUM_constitutive_dpsi0.15_qvalue0.01_with_na_vs_PSIsigma_constitutive_dpsi0.10_pvalue0.01_with_na_common.genes <-
  calculate_intersections_from_chr_strand_start_end(dplyr::semi_join(tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na, tibble_PSIsigma_constitutive_dpsi0.10_pvalue0.01_with_na %>% dplyr::rename("Gene" = "matched_gene_names"), by = "Gene") %>% JUM_AS.event.ID_to_chr_strand_start_end,
                                                    dplyr::semi_join(tibble_PSIsigma_constitutive_dpsi0.10_pvalue0.01_with_na, tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na %>% dplyr::select(-matched_gene_names) %>% dplyr::rename("matched_gene_names" = "Gene"), by = "matched_gene_names") %>% PSIsigma_diff.exon.coords_to_chr_strand_start_end, tibble_1_tolerance_left = 2, tibble_1_tolerance_right = 2)
...
### determine if there's any coordinate level overlap between differential regions/exons and Ago2 iCLIP
...
### Reference preprocessing: define and map topologies, cast reference regions onto exon/CDS-relative coords.
...
tibble_non_exonic_GTF_entries <- tibble_ref_gtf_original[tibble_ref_gtf_original$type %in% c("CDS", "start_codon", "stop_codon", "five_prime_utr", "three_prime_utr"), ]
...
# save(tibble_non_exonic_GTF_entries_exon_relative_coords, file = "/media/angel/seagate_500/tibble_non_exonic_GTF_entries_exon_relative_coords.Rlist")

# load(file = "/media/angel/seagate_500/tibble_non_exonic_GTF_entries_exon_relative_coords.Rlist")

# create granges of transcript "type" 
list_granges_ensembl_transcript_types <- tibble_non_exonic_GTF_entries_exon_relative_coords %>%
...
### Preprocessing: convert differential positions into UNIQUE Granges and convert to exon/CDS-relative coords.
...
#### Cast biomart regions onto the genome, convert to CDS-relative topology, then convert to granges.
...
## convert vector of discontinuous sites into chr, start, end.
list_of_tibbles_biomart_regions_cast_onto_genome_simple_ranges <- purrr::map(
...
list_of_tibbles_biomart_regions_cast_onto_CDS_simple_ranges <- future_map(
  ...
  .f = function(a1) {
    ...
    chr_in_common <- intersect(
      ...
      names(list_CDS_GTF_entries_genome_to_CDS_relative_mapping)
    )
    
    ## map2 over exonic and non-exonic GTF entries, per chromosome
    list_of_tibbles_biomart_regions_CDS_relative_coords_single_annotation <- future_map2(
      .x = list_CDS_GTF_entries_genome_to_CDS_relative_mapping[chr_in_common],
      .y = list_tibbles_biomart_regions_split_by_chromosome[chr_in_common],
      .f = function(b1, b2) {
        
        # DEBUG ###
        # b1 <- list_CDS_GTF_entries_genome_to_CDS_relative_mapping[chr_in_common] %>% .[[1]]
        ...
        # loop thru each element, find overlapping exonic interval, subtract 
        list_biomart_subset_CDS_relative_coords <- future_map(
          ...
          .f = function(c1) {
            ...
            shift <- tibble_matching_CDS_entry$start - tibble_matching_CDS_entry$CDS_relative_start
            CDS_relative_start <- (c1$start %>% type.convert) - shift
            CDS_relative_end <- (c1$end %>% type.convert) - shift
        ...
        tibble_biomart_subset_CDS_relative_coords <- list_biomart_subset_CDS_relative_coords %>% rbindlist %>% as_tibble
        
        return(tibble_biomart_subset_CDS_relative_coords)
        
      }, .progress = TRUE)
    
    # rbind and tibblise
    tibble_biomart_regions_CDS_relative_coords_single_annotation <- list_of_tibbles_biomart_regions_CDS_relative_coords_single_annotation %>% 
      rbindlist %>% as_tibble %>% type_convert
    
    return(tibble_biomart_regions_CDS_relative_coords_single_annotation)
    
  }, .progress = TRUE )

# finally convert the CDS relative biomart regions into granges
list_granges_biomart_regions_CDS_relative <- future_map(
  .x = list_of_tibbles_biomart_regions_cast_onto_CDS_simple_ranges,
  .f = function(a1) {
    
    granges_biomart_region <- toGRanges(a1[, c("chr", "start", "end")] %>% 
                                          # turn chr column into chr1 chr2 etc...
                                          dplyr::mutate_at(.vars = "chr", .funs = function(x) {paste("chr", x, sep = "")} ) %>%
                                          type_convert %>% 
                                          # data frame is absolutele necessary for regioneR.
...
# L1: different biomart annotations
# L2: split by chromosome
list_of_tibbles_PTM_sites_cast_onto_CDS_simple_ranges <- future_map(
  ...
  .f = function(a1) {
    ...
    ## map2 over exonic and non-exonic GTF entries, per chromosome
    list_of_tibbles_PTM_sites_CDS_relative_coords_single_annotation <- future_map2(
      .x = list_CDS_GTF_entries_genome_to_CDS_relative_mapping[chr_in_common],
      ...
      .f = function(b1, b2) {
        ...
        list_PTM_subset_CDS_relative_coords <- future_map(
          .x = list_subset,
          .f = function(c1) {
        ...
        # rbindlist and tibblise
        tibble_PTM_subset_CDS_relative_coords <- list_PTM_subset_CDS_relative_coords %>% rbindlist %>% as_tibble
        
        return(tibble_PTM_subset_CDS_relative_coords)
        
      }, .progress = TRUE)
    
    # rbind and tibblise
    tibble_PTM_sites_CDS_relative_coords_single_annotation <- list_of_tibbles_PTM_sites_CDS_relative_coords_single_annotation %>% 
      rbindlist %>% as_tibble %>% type_convert
    
    return(tibble_PTM_sites_CDS_relative_coords_single_annotation)
    
  }, .progress = TRUE )

# finally convert the CDS relative PTM sites into granges
list_granges_PTM_sites_CDS_relative <- future_map(
  .x = list_of_tibbles_PTM_sites_cast_onto_CDS_simple_ranges,
  .f = function(a1) {
    
    granges_PTM_sites <- toGRanges(a1[, c("chr", "start", "end")] %>% 
                                          # turn chr column into chr1 chr2 etc...
                                          dplyr::mutate_at(.vars = "chr", .funs = function(x) {paste("chr", x, sep = "")} ) %>%
                                          type_convert %>% 
                                          # data frame is absolutely necessary for regioneR.
...
tibble_JUM_differential_AS.event.ID_snps_summary_by_trait <- tibble_JUM_differential_AS.event.ID_all_overlapping_snps_filtered %>% dplyr::distinct(CHR_ID, CHR_POS, .keep_all = TRUE) %>% .[grep(x = .$`INITIAL SAMPLE SIZE`, pattern = "europ", ignore.case = TRUE), ] %>% dplyr::group_by(`DISEASE/TRAIT`) %>% dplyr::summarise("trait_tally" = n()) %>% dplyr::arrange(desc(trait_tally))
...
list_permtest_results_JUM_differential <- purrr::map(.x = tibble_JUM_differential_AS.event.ID_snps_summary_by_trait[1:8, ] %>% .$`DISEASE/TRAIT`, .f = function(.x) {
...
save(list_permtest_results_JUM_differential, file = paste(results_dir, "list_SNP_permtest_results_JUM_differential.Rlist", sep = ""))
...
tibble_AS.event.IDs_chr_start_end_constitutive <- tibble(
  "AS_event_ID" = tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na$AS_event_ID,
  "chr" = gsub(x = tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na$AS_event_ID, pattern = "^([^_]+)_(\\+|\\-)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10}$)", replacement = "\\1"),
  "start" = gsub(x = tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na$AS_event_ID, pattern = "^([^_]+)_(\\+|\\-)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10}$)", replacement = "\\3"),
  "end" = gsub(x = tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na$AS_event_ID, pattern = "^([^_]+)_(\\+|\\-)_([0-9]{1,10})(.*{0,1})_([0-9]{1,10}$)", replacement = "\\5"),
  "splicemode" = tibble_JUM_constitutive_dpsi0.15_qvalue0.01_with_na$splicemode) %>%
  unique
...
tibble_JUM_constitutive_AS.event.ID_snps_summary_by_trait <- tibble_JUM_constitutive_AS.event.ID_all_overlapping_snps_filtered %>% dplyr::distinct(CHR_ID, CHR_POS, .keep_all = TRUE) %>% .[grep(x = .$`INITIAL SAMPLE SIZE`, pattern = "europ", ignore.case = TRUE), ] %>% dplyr::group_by(`DISEASE/TRAIT`) %>% dplyr::summarise("trait_tally" = n()) %>% dplyr::arrange(desc(trait_tally))
...
# use purrr to test significance of all traits simultaneously
list_permtest_results_JUM_constitutive <- purrr::map(.x = tibble_JUM_constitutive_AS.event.ID_snps_summary_by_trait[1:8, ] %>% .$`DISEASE/TRAIT`, .f = function(.x) {
...
save(list_permtest_results_JUM_constitutive, file = paste(results_dir, "list_SNP_permtest_results_JUM_constitutive.Rlist", sep = ""))
...
tibble_psisigma_differential_snps_summary_by_trait <- tibble_psisigma_differential_all_overlapping_snps_filtered %>% dplyr::distinct(CHR_ID, CHR_POS, .keep_all = TRUE) %>% .[grep(x = .$`INITIAL SAMPLE SIZE`, pattern = "europ", ignore.case = TRUE), ] %>% dplyr::group_by(`DISEASE/TRAIT`) %>% dplyr::summarise("trait_tally" = n()) %>% dplyr::arrange(desc(trait_tally))
...
# use purrr to test significance of all traits simultaneously
list_permtest_results_PSIsigma_differential <- purrr::map(.x = tibble_psisigma_differential_snps_summary_by_trait[1:8, ] %>% .$`DISEASE/TRAIT`, .f = function(.x) {
...
save(list_permtest_results_PSIsigma_differential, file = paste(results_dir, "list_SNP_permtest_results_PSIsigma_differential.Rlist", sep = ""))
...
tibble_psisigma_constitutive_snps_summary_by_trait <- tibble_psisigma_constitutive_all_overlapping_snps_filtered %>% dplyr::summarise("trait_tally" = n()) %>% dplyr::arrange(desc(trait_tally))
...
# use purrr to test significance of all traits simultaneously
list_permtest_results_PSIsigma_constitutive <- purrr::map(.x = tibble_psisigma_constitutive_snps_summary_by_trait[1:8, ] %>% .$`DISEASE/TRAIT`, .f = function(.x) {
...
save(list_permtest_results_PSIsigma_constitutive, file = paste(results_dir, "list_SNP_permtest_results_PSIsigma_constitutive.Rlist", sep = ""))
