...
polyA_RNAseq_background_pfam <- read.delim(file = paste(results_dir, "polyA_RNAseq_GO_background_PROTEINFAMILY.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = FALSE, row.names = NULL, check.names = FALSE) %>% setNames(c("external_gene_name", "family_accession", "family_description"))
...
tibble_sp.trembl_fasta_allhuman_processed <- dplyr::bind_rows(tibble_sp.canonical_fasta_allhuman) %>% 
...
list_overlaps_clipseq_peakfiles_x_jum_psisigma <- furrr::future_map2(
  ...
  .f = function(a1, a2) {
    ...
    return(list(tibble_VSRs_with_overlap) %>% set_names(paste(a2[[2]], a2[[1]], sep = "|")))
...
# table join back onto the JUM/PSIsigma diff/const tables.
list_JUM_diff_VSR_overlapping_clip_seq <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "differential")] %>% purrr::map(~dplyr::inner_join(tibble_JUM_differential_dpsi0.15_qvalue0.01_with_na, .x, by = c("AS_event_ID", "splicemode")))
list_JUM_const_VSR_overlapping_clip_seq <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "constitutive")] %>% purrr::map(~dplyr::inner_join(tibble_JUM_constitutive_dpsi0.15_qvalue0.01_no_na, .x, by = c("AS_event_ID", "splicemode")))

list_PSIsigma_diff_VSR_overlapping_clip_seq <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "differential")] %>% purrr::map(~dplyr::inner_join(tibble_PSIsigma_differential_dpsi0.10_pvalue0.01_with_na, .x, by = c("event_region_coords", "splicemode")))
list_PSIsigma_const_VSR_overlapping_clip_seq <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "constitutive")] %>% purrr::map(~dplyr::inner_join(tibble_PSIsigma_constitutive_dpsi0.10_pvalue0.01_with_na, .x, by = c("event_region_coords", "splicemode")))

# extract genesets
list_geneset_JUM_diff_VSR_overlapping_clip_seq <- list_JUM_diff_VSR_overlapping_clip_seq %>% purrr::map(~.x$Gene %>% unique %>% mixedsort)

list_geneset_PSIsigma_diff_VSR_overlapping_clip_seq <- list_PSIsigma_diff_VSR_overlapping_clip_seq %>% purrr::map(~.x$matched_gene_names %>% unique %>% mixedsort)
...
# map GO test
list_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms <- future_map2(
  .x = list_geneset_JUM_diff_VSR_overlapping_clip_seq, 
  .y = list_geneset_PSIsigma_diff_VSR_overlapping_clip_seq,
  .f = function(a1, a2) {
  
  future_map(.x = c("MF", "BP", "CC"),
             .f = function(b1) {
               
               systemPipeR::GOHyperGAll(catdb = catdb, gocat = b1, sample = c(a1, a2) %>% unique %>% mixedsort, Nannot = 2) %>% GOHyperGAll_benjamini_correction(FDR_cutoff = 1) %>% type_convert %>% as_tibble
               
             } ) %>% set_names(c("MF", "BP", "CC")) %>% return
  
}, .progress = TRUE)

# plot the GO terms into a a cute rectangular heatmap
## rbind the GO enrichmnent tables
long_tibble_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms <- purrr::map2(
  .x = list_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms,
  .y = names(list_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms),
  .f = function(a1, a2) {
...
long_tibble_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms <- long_tibble_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms %>% 
  dplyr::filter(!grepl(x = `sample`, pattern = "(SRSF2 HITS-CLIP GSE111900)|(SRSF4)|(SRSF7)|(SRSF9)")) %>%
  dplyr::mutate_at(.vars = "sample", .funs = function(x) {gsub(x = x, pattern = "^(.*)\\|(.*)", replacement = "\\2")} ) %>%
  dplyr::mutate_at(.vars = "sample", .funs = function(x) {gsub(x = x, pattern = "\\_", replacement = " ")} )

## ggplot
### set the order of sample
vector_clipseq_sample_order_for_ggplot <- long_tibble_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms$sample %>% unique %>% mixedsort
### set the order of GO terms
vector_clipseq_goterm_order_for_ggplot <- c(
  # BP
  "mRNA splicing, via spliceosome", "histone modification", "Golgi vesicle transport",
  # MF
  "chromatin binding", "integrin binding", "transcription coregulator activity", "protein kinase regulator activity",
  # CC
  "focal adhesion", "actin cytoskeleton"
  ) %>% rev

ggplot(long_tibble_JUM_PSIsigma_diff_coordinate_level_clip_seq_GOterms) +
  geom_tile(aes(x = sample, y = Term, fill = Padj)) +
  facet_wrap(facets = ~Ont, scales = "free", ncol = 1) +
  ggtitle(paste("Heatmap of GO Terms for public CLIP-Seq targets (JUM)", sep = "")) +
  ...
  scale_fill_gradientn(colours = c("white", "yellow", "red", "black"), values = c(0, 0.5, 0.95, 1), trans = "reverse") + 
  theme_bw() +
  ...
  ggsave(filename = paste(results_dir, "heatmap_JUM_PSisigma_combined_public_clipseq_GOterms.pdf", sep = ""), device = "pdf", dpi = 600, width = 18, height = 25, units = "cm", limitsize = FALSE) +
  ggsave(filename = paste(results_dir, "heatmap_JUM_PSisigma_combined_public_clipseq_GOterms.svg", sep = ""), device = "svg", dpi = 600, width = 18, height = 25, units = "cm", limitsize = FALSE)
...
# table join back onto the JUM/PSIsigma timeseries tables.
list_JUM_diff_VSR_overlapping_clip_seq_matched_to_timeseries_table <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "differential")] %>% purrr::map(~dplyr::inner_join(tibble_JUM_scaled_PSI_timeseries_wide, .x, by = c("AS_event_ID", "splicemode")))

list_PSIsigma_diff_VSR_overlapping_clip_seq_matched_to_timeseries_table <- list_overlaps_clipseq_peakfiles_x_jum_psisigma[grep(x = names(list_overlaps_clipseq_peakfiles_x_jum_psisigma), pattern = "differential")] %>% purrr::map(~dplyr::inner_join(tibble_PSIsigma_scaled_PSI_timeseries_wide, .x, by = c("event_region_coords", "splicemode")))
...
tibble_ENSG_to_GOterm_and_biotype_mapping <- read.delim(file = paste(shared_dir, "table_ENSG_to_GOterm_and_biotype_mapping.txt", sep = ""), sep = "\t", stringsAsFactors = FALSE, header = TRUE, row.names = NULL, check.names = FALSE) %>% as_tibble %>%
...
list_of_tibbles_consensus.SOM_family.enrichment_clusterwise <- future_map2(
  .x = list_of_genesets_by_cluster, 
  .y = names(list_of_genesets_by_cluster), 
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a1 <- list_of_genesets_by_cluster[[1]]
    # a2 <- names(list_of_genesets_by_cluster) %>% .[[1]]
    ###########
  ...
  .f = function(a1, a2) {
    ...
    list_non_exonic_subset_transcript_relative_coords <- future_map(
      ...
      .f = function(b1) {
        ...
        tibble_matching_transcriptomic_entry <- a1[which(a1$start <= b1$start %>% type.convert & b1$end %>% type.convert <= a1$end), ]
...
# save(tibble_ENSP_to_uniprotkb, file = paste(shared_dir, "tibble_ENSP_to_uniprotkb.tibble", sep = ""))
...
list_permtest_exonic_transcript_types <- future_map(
  .x = list_granges_transcriptomic_VSR_positions, 
  .f = function(a1) {
