...
future_map2(.x = list_imported_maxquant_tibbles_processed, .y = list_of_nested_table_names, .f = function(.x, .y) {
  ...
  X <- .x[c("phosphosites", "protein_groups")]
  Y <- .y[c("phosphosites", "protein_groups")]
  ...
  "L4" = names(list_of_nested_table_names)), .f = function(L1, L2, L3, L4) {
    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        # define tables #####
        data_cols <- l1 %>% zero_to_NA %>% transform_non.na_to_log2
        untransformed_cols <- l2
        names <- l3
        
        # calculate PI value
        # this doesn't seem to work if you have rows which are totally complete
        pdf(paste(R_processing_results_dir, "eigenMS_PI_", names[[1]], "_", names[[2]], ".pdf", sep = ""), width = 10, height = 6)
        ...
        design_factor_info <- factor(x = gsub(x = colnames(data_cols), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE), levels = c("ud_1_24", "ud_30_6", "30m", "1h", "6h", "1d"))
        
        # first normalisation
        pdf(paste(R_processing_results_dir, "eigenMS_first_normalisation_", names[[1]], "_", names[[2]], ".pdf", sep = ""), width = 10, height = 6)
        ...
        future_map(.x = h.c_values_to_run, .f = function(.x) {
          ...
          pdf(paste(R_processing_results_dir, "eigenMS_second_normalisation_", names[[1]], "_", names[[2]], "_hc", .x, ".pdf", sep = ""), width = 10, height = 6)
          ...
          cat("PLOT RLE\n")
          # plot raw
          pdf(paste(R_processing_results_dir, "RLE_plot_raw_", names[[1]], "_", names[[2]], ".pdf", sep = ""), width = 10, height = 6)
            plotRLE(data_cols %>% as.matrix, main = paste("raw", names[[1]], names[[2]], sep = "\n"))
          dev.off()
          # plot normalised
          pdf(paste(R_processing_results_dir, "RLE_plot_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".pdf", sep = ""), width = 10, height = 6)
          ...
          cat("PLOT MDS\n")
          # plot normalised
          pdf(paste(R_processing_results_dir, "MDSplot_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".pdf", sep = ""), width = 10, height = 6)
            par(oma = c(0, 0, 0, 5))
          ...
          dev.off()
          # plot raw
          pdf(paste(R_processing_results_dir, "MDSplot_raw_", names[[1]], "_", names[[2]], ".pdf", sep = ""), width = 10, height = 6)
            par(oma = c(0, 0, 0, 5))
          ...
          ggplot(tibble_pairwise_comparisons_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
            ...
            theme_bw() +
            theme(text = element_text(family = "Helvetica")) +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_intra.timepoint_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 35, height = 20, units = "cm") +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_intra.timepoint_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".svg", sep = ""), device = "svg", dpi = 600, width = 35, height = 20, units = "cm")
            ################
          ...
          ggplot(tibble_of_values_per_timepoint_raw, aes(x = 2^mean, y = CV)) +
            ...
            theme_bw() +
            theme(text = element_text(family = "Helvetica")) +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_raw_", names[[1]], "_", names[[2]], ".pdf", sep = ""), device = "pdf", dpi = 600, width = 35, height = 20, units = "cm") +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_raw_", names[[1]], "_", names[[2]], ".svg", sep = ""), device = "svg", dpi = 600, width = 35, height = 20, units = "cm")
          ...
          ggplot(tibble_of_values_per_timepoint, aes(x = 2^mean, y = CV)) +
            ...
            theme_bw() +
            theme(text = element_text(family = "Helvetica")) +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 35, height = 20, units = "cm") +
            ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_eigenMS_normalised_", names[[1]], "_", names[[2]], "_hc", .x, ".svg", sep = ""), device = "svg", dpi = 600, width = 35, height = 20, units = "cm")
