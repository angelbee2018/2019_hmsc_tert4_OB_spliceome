...
list_of_dfs_phosphosites.proteingroups_intensities <- purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], 
                                                                  .y = list_imported_maxquant_tibbles_processed[vector_proteingroups.txt_list.indices],
                                                                  .f = ~list("phosphosites" = .x,
                                                                             "protein groups" = .y))

list_of_dfs_phosphosites_intensities <- purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .f = function(.x) {
...
list_of_dfs_proteingroups_intensities <- purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_proteingroups.txt_list.indices], .f = function(.x) {
...
# list-ify
list_of_tibble_intensities_for_pca <- purrr::map2(.x = list_of_dfs_phosphosites_intensities, .y = list_of_dfs_proteingroups_intensities, 
                                                  .f = ~list("phosphosites" = .x,
                                                             "protein groups" = .y))
...
list_of_dfs_phosphosites_ratios <- purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .f = function(.x) {
...
list_of_dfs_proteingroups_ratios <- purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_proteingroups.txt_list.indices], .f = function(.x) {
...
# list-ify
list_of_tibble_ratios_for_pca <- purrr::map2(.x = list_of_dfs_phosphosites_ratios, .y = list_of_dfs_proteingroups_ratios, 
...
purrr::map(.x = list_of_tibble_intensities_for_processing, .f = function(.x) {
  
  # define tables #####
  data_cols <- list_of_tibble_intensities_for_processing[[1]]
  annotation_cols <- list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices][[1]][, c("id", "Proteins")]
  ...
  design_factor_info <- gsub(x = colnames(list_of_tibble_intensities_for_processing[[2]]), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE) %>% as.factor
  ...
  pdf(paste(R_processing_results_dir, "test.pdf", sep = ""), width = 10, height = 6)
...
plotRLE(list_of_tibble_intensities_for_processing[[2]] %>% as.matrix, main = paste("raw"))

plotRLE(tibble_normalised_result[, 3:ncol(tibble_normalised_result)] %>% as.matrix, main = paste("normalised"))


pdf(paste(R_processing_results_dir, "MDSplot_eigen_test.pdf", sep = ""))

par(oma = c(0, 0, 0, 5))

plotMDS(tibble_normalised_result[, 3:ncol(tibble_normalised_result)] %>% as.matrix, col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info))), main = paste("normalised"), cex = 2, pch = 1:length(levels(design_factor_info)))
