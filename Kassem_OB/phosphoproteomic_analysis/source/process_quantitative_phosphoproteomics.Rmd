...
list_of_nested_table_names <- purrr::map(.x = names(list_maxquant_run_dirs), .f = ~purrr::cross2(.x, names(list_output.table_categories)))
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .y = list_of_nested_table_names[vector_phosphosites.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(df2) +
    ...
    ggtitle(paste("Distribution of intensities for each mass channel", .y[[1]], .y[[2]], sep = ", ")) +
    ...
    ggsave(filename = paste(R_processing_results_dir, "qualityplot_intensity_distribution_per_mass_channel_", .y[[1]], "_phosphosites.pdf", sep = ""), device = "pdf", dpi = 600, width = 45, height = 25, units = "cm") +
    ggsave(filename = paste(R_processing_results_dir, "qualityplot_intensity_distribution_per_mass_channel_", .y[[1]], "_phosphosites.svg", sep = ""), device = "svg", dpi = 600, width = 45, height = 25, units = "cm")
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .y = list_of_nested_table_names[vector_phosphosites.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(df2) +
    ...
    ggtitle(paste("Distribution of ratios for each replicate + phosphostate", .y[[1]], .y[[2]], sep = ", ")) +
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .y = list_of_nested_table_names[vector_phosphosites.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(tibble_pairwise_comparisons_intensity_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
    ...
    ggtitle(paste("Correlation of mass channel intensities between biological replicates of the same time point", .y[[1]], .y[[2]], sep = ", ")) +
  ...
  ggplot(tibble_pairwise_comparisons_intensity_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value %>% log2, y = comparison_2_value %>% log2)) +
    ...
    ggtitle(paste("Correlation of log2 mass channel intensities between biological replicates of the same time point", .y[[1]], .y[[2]], sep = ", ")) +
    facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
purrr::map2(.x = list_imported_maxquant_tibbles[vector_phosphosites.txt_list.indices], .y = list_of_nested_table_names[vector_phosphosites.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(tibble_pairwise_comparisons_ratio_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
    ...
    ggtitle(paste("Correlation of ratios (un-normalised) between replicates of the same time point", .y[[1]], .y[[2]], sep = ", ")) +
    facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free")
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .y = list_of_nested_table_names[vector_phosphosites.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(tibble_pairwise_comparisons_ratio_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
    ...
    xlab("Comparison 1 ratio") + 
    ylab("Comparison 2 ratio") + 
    ggtitle(paste("Correlation of mass channel ratios between replicates of the same time point", .y[[1]], .y[[2]], sep = ", ")) +
    facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
  ...
  ggplot(tibble_pairwise_comparisons_ratio_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value %>% log2, y = comparison_2_value %>% log2)) +
    ...
    xlab("Comparison 1 ratio (log2)") + 
    ylab("Comparison 2 ratio (log2)") + 
    ggtitle(paste("Correlation of log2 mass channel ratios between replicates of the same time point", .y[[1]], .y[[2]], sep = ", ")) +
    facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
purrr::map2(.x = list_imported_maxquant_tibbles[vector_peptides.txt_list.indices], .y = list_of_nested_table_names[vector_peptides.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(long_tibble_MA.plot, aes(x = avg_intensity, y = log2(MQ_normalized_ratio))) +
    ...
    ggtitle(paste("M-A plot of log non-normalised MQ ratios vs. avg. intensities for phosphopeptide", .y[[1]], .y[[2]], sep = ", ")) +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_peptides.txt_list.indices], .y = list_of_nested_table_names[vector_peptides.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(long_tibble_MA.plot, aes(x = avg_intensity, y = log2(MQ_normalized_ratio))) +
    ...
    xlab("average of log2 intensities") +
    facet_grid(comparison_name_ratio ~ comparison_name_sample, scales = "free") +
    ggtitle(paste("M-A plot of log normalised MQ ratios vs. avg. intensities for phosphopeptide", .y[[1]], .y[[2]], sep = ", ")) +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_modificationSpecificPeptides.txt_list.indices], .y = list_of_nested_table_names[vector_modificationSpecificPeptides.txt_list.indices], .f = function(.x, .y) {
  ...
  ggplot(df3 %>% dplyr::filter(., intensity > 0), aes(x = intensity, colour = phospho_or_not)) +
    ...
    ggtitle(bquote("Comparison of intensity distributions of phosphopeptide vs. nonphospho per mass channel, "~.(.y[[1]])~", "~.(.y[[2]]))) +
...
list_of_dfs_phosphosites.proteingroups <- purrr::map2(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], 
                                                                  .y = list_imported_maxquant_tibbles_processed[vector_proteingroups.txt_list.indices],
                                                                  .f = ~list("phosphosites" = .x,
                                                                             "protein groups" = .y))

list_of_tibble_intensities_for_pca <- purrr::map_depth(.x = list_of_dfs_phosphosites.proteingroups, .depth = 2, .ragged = FALSE, .f = function(.x) {
...
purrr::map2(.x = list_of_tibble_intensities_for_pca, .y = list_of_nested_table_names[vector_proteingroups.txt_list.indices], .f = function(.x, .y) {
  
  Y <- .y
  
  purrr::map2(.x = .x, .y = names(.x), .f = ~missmap(.x, main = paste("Missingness heatmap for raw intensities\n", Y[[1]], .y, sep = ", ")))
...
purrr::map2(.x = list_of_tibble_intensities_for_pca, .y = list_of_nested_table_names[vector_proteingroups.txt_list.indices], .f = function(.x, .y) {
  ...
  purrr::map2(.x = .x, .y = names(.x), .f = function(.x, .y) {
    ...
    ggplot(PCA_variance) + 
      ...
      ggtitle(paste("PCA variance distribution", Y[[1]], .y, sep = ", ")) +
      guides(size = FALSE) + 
      xlab("PC") +
    ...
    ggplot(PCA_loadings) + 
      ...
      ggtitle(paste("PCA loadings", Y[[1]], .y, sep = ", ")) +
      guides(size = FALSE) + 
...
list_of_tibble_ratios_for_pca <- purrr::map_depth(.x = list_of_dfs_phosphosites.proteingroups, .depth = 2, .ragged = FALSE, .f = function(.x) {
...
purrr::map2(.x = list_of_tibble_ratios_for_pca, .y = list_of_nested_table_names[vector_proteingroups.txt_list.indices], .f = function(.x, .y) {
  
  Y <- .y
  
  purrr::map2(.x = .x, .y = names(.x), .f = ~missmap(.x, main = paste("Missingness heatmap for raw ratios\n", Y[[1]], .y, sep = ", ")))
...
EigenMS input intensities MUST be log transformed and not zero - not even potentially real zeroes.
```{r}

list_of_tibble_intensities_for_processing <- list_of_tibble_intensities_for_pca %>% purrr::map_depth(.depth = 2, .ragged = FALSE, .f = ~mutate_all(., .funs = function(x) {
...
## prepare annotation column so that each row is traceable

```{r}

list_of_tibble_intensities_for_processing_ids <- list_of_dfs_phosphosites.proteingroups %>% purrr::map_depth(.depth = 2, .ragged = FALSE, .f = ~.x[, "id"])

```

## run eigenMS normalisation

(Karpievitch et al. Bioinformatics 2009)
```{r}

# SECOND-PASS NORMALISATION #####
# data_cols <- tibble_normalised_result[, 3:ncol(tibble_normalised_result)]
# annotation_cols <- tibble_normalised_result[, c("id", "Protein.IDs")]

list_of_tibbles_eigenMS_normalisation_result <- future_pmap(.l = list(
  
  "L1" = list_of_tibble_intensities_for_processing,
  "L2" = list_of_tibble_intensities_for_processing_ids,
  "L3" = list_of_nested_table_names[vector_phosphosites.txt_list.indices]
  
), .f = function(L1, L2, L3) {
  
  # DEBUG ###
  # list_data_tibbles <- list_of_tibble_intensities_for_processing[[1]]
  # list_id_tibbles <- list_of_tibble_intensities_for_processing_ids[[1]]
  # TABLE_NAMES <- list_of_nested_table_names[vector_phosphosites.txt_list.indices][[1]]
  ###########
  
  list_data_tibbles <- L1
  list_id_tibbles <- L2
  TABLE_NAMES <- L3
  
  eigenMS_result_of_single_table <- purrr::pmap(.l = list(
    
    "l1" = list_data_tibbles, 
    "l2" = list_id_tibbles,
    "l3" = names(list_data_tibbles)
    
  ), .f = function(l1, l2, l3) {
    
    # DEBUG ###
    # data_cols <- list_of_tibble_intensities_for_processing[[1]][[1]]
    # annotation_cols <- list_of_tibble_intensities_for_processing_ids[[1]][[1]]
    # name_phospho_or_protein <- names(list_of_tibble_intensities_for_processing[[1]])[[1]]
    ###########
    
    # define tables #####
    data_cols <- l1
    annotation_cols <- l2
    name_phospho_or_protein <- l3
    
    # calculate PI value
    # this doesn't seem to work if you have rows which are totally complete
    pdf(paste(R_processing_results_dir, "eigenMS_PI_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
    ...
    # first normalisation
    pdf(paste(R_processing_results_dir, "eigenMS_first_normalisation_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
    eigen_ms_first_normalisation_result <- ProteoMM::eig_norm1(data_cols %>% as.matrix, treatment = design_factor_info, prot.info = annotation_cols %>% as.matrix)
    dev.off()
    
    # set the number of bias trends manually
    # eigen_ms_first_normalisation_result$h.c <- 4
    
    pdf(paste(R_processing_results_dir, "eigenMS_second_normalisation_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
  ...
  return(eigenMS_result_of_single_table)
  
}, .progress = TRUE)

```

## plot RLE, MDS, intra-timepoint correlation scatterplot and CV vs. mean before and after normalisation
...
purrr::pmap(.l = list(
  ...
  "L1" = list_of_tibbles_eigenMS_normalisation_result,
  "L2" = list_of_tibble_intensities_for_pca,
  "L3" = list_of_nested_table_names[vector_phosphosites.txt_list.indices]
  
  ), .f = function(L1, L2, L3) {
    
    # DEBUG ###
    # list_eigenMS_normalised_tibbles <- list_of_tibbles_eigenMS_normalisation_result[[1]]
    # list_raw_tibbles <- list_of_tibble_intensities_for_pca[[1]]
    # TABLE_NAMES <- list_of_nested_table_names[vector_phosphosites.txt_list.indices][[1]]
    ###########
  
    list_eigenMS_normalised_tibbles <- L1
    list_raw_tibbles <- L2
    TABLE_NAMES <- L3
    ...
    purrr::pmap(.l = list(
      ...
      "l1" = list_eigenMS_normalised_tibbles, 
      "l2" = list_raw_tibbles,
      "l3" = names(list_eigenMS_normalised_tibbles)
      
    ), .f = function(l1, l2, l3) {
      
      # DEBUG ###
      # normalised_tibble_and_hc <- list_of_tibbles_eigenMS_normalisation_result[[1]][[1]]
      # raw_tibble <- list_of_tibble_intensities_for_pca[[1]][[1]]
      # name_phospho_or_protein <- names(list_of_tibbles_eigenMS_normalisation_result[[1]])[[1]]
      ###########
    
      # define tables #####
      normalised_tibble_and_hc <- l1
      raw_tibble <- l2
      name_phospho_or_protein <- l3
      
      # PLOT RLE #####
      # plot raw
      pdf(paste(R_processing_results_dir, "RLE_plot_raw_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      plotRLE(raw_tibble %>% as.matrix, main = paste("raw", TABLE_NAMES[[1]], name_phospho_or_protein, sep = ", "))
      dev.off()
      # plot normalised
      pdf(paste(R_processing_results_dir, "RLE_plot_normalised_eigenMS_hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      plotRLE(normalised_tibble_and_hc$result_table %>% .[, -1] %>% as.matrix, main = paste("normalised_eigenMS, hc=", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = ""))
      dev.off()
      ################
      
      # PLOT MDS #####
      # design factor info for legend
      design_factor_info_normalised <- gsub(x = colnames(normalised_tibble_and_hc$result_table %>% .[, -1]), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE) %>% as.factor
      design_factor_info_raw <- gsub(x = colnames(raw_tibble), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE) %>% as.factor
      # plot normalised
      pdf(paste(R_processing_results_dir, "MDSplot_eigenMS_normalised, hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      par(oma = c(0, 0, 0, 5))
      plotMDS(normalised_tibble_and_hc$result_table %>% .[, -1] %>% as.matrix, col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info_normalised))), main = paste("normalised, hc=", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = ""), cex = 2, pch = 1:length(levels(design_factor_info_normalised)))
      legend(par('usr')[2], par('usr')[4], xpd = NA, bty = "n", legend = levels(design_factor_info_normalised), col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info_normalised))), pch = 1:length(levels(design_factor_info_normalised)))
      dev.off()
      # plot raw
      pdf(paste(R_processing_results_dir, "MDSplot_raw_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      par(oma = c(0, 0, 0, 5))
      plotMDS(raw_tibble %>% as.matrix, col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info_raw))), main = paste("raw", TABLE_NAMES[[1]], name_phospho_or_protein, sep = ", "), cex = 2, pch = 1:length(levels(design_factor_info_raw)))
      legend(par('usr')[2], par('usr')[4], xpd = NA, bty = "n", legend = levels(design_factor_info_raw), col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info_raw))), pch = 1:length(levels(design_factor_info_raw)))
      dev.off()
      ################
      
      # SCATTERPLOT: the intra-timepoint correlation #####
      df <- normalised_tibble_and_hc$result_table
      combs <- combn(x = colnames(df[, -1]), m = 2) %>% t
      ...
      list_of_comparisons <- purrr::map2(.x = combs2[, 1] %>% array_tree, .y = combs2[, 2] %>% array_tree, .f = function(.x, .y) {
        ...
        colnames_id.vars <- colnames(df[, 1])
        
        pairwise_subset_table <- df[, colnames_id.vars] %>% add_column("comparison_1_value" = df[, .x %>% paste] %>% unlist, "comparison_2_value" = df[, .y %>% paste] %>% unlist, "comparison_1_name" = .x %>% paste, "comparison_2_name" = .y %>% paste)
      ...
      ggplot(tibble_pairwise_comparisons_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
        ...
        ggtitle(paste("Intra-timepoint (between replicates) correlation after EigenMS normalisation,
                hc = ", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = "")) +
        # facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
        theme_bw() +
        theme(text = element_text(family = "Helvetica")) +
        ggsave(filename = paste(R_processing_results_dir, "scatterplot_intra.timepoint_eigenMS_normalised_hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 35, height = 20, units = "cm") +
        ggsave(filename = paste(R_processing_results_dir, "scatterplot_intra.timepoint_eigenMS_normalised_hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".svg", sep = ""), device = "svg", dpi = 600, width = 35, height = 20, units = "cm")
        ################
      
        # plot the CV vs. mean log2 intensity for each item-per-timepoint #####
        # melt by subsetting table by each timepoint.
        list_of_values_per_timepoint <- purrr::map(.x = design_factor_info_normalised, .f = function(.x) {
          ...
          col.indices_data.vars <- grep(x = colnames(df), pattern = .x)
          
          # extract the data cols for all reps of 1 timepoint and rename the colnames so that we can rbind properly
          data.cols <- df[, col.indices_data.vars] %>% setNames(., c("intensity_IB", "intensity_IB2", "intensity_KR")) %>% add_column(., "timepoint" = .x)
          
          timepointwise_subset_table <- dplyr::bind_cols(id.cols, data.cols)
        ...
        ggplot(tibble_of_values_per_timepoint, aes(x = 2^mean, y = CV)) +
          geom_point() +
          geom_smooth(formula = y ~ x, method = "loess", span = normalised_tibble_and_hc$PI_value) +
          # scale_x_continuous(trans = "log2") +
          # scale_y_continuous(trans = "log2") +
          ...
          ylab("CV") + 
          ggtitle(paste("CV vs. mean after EigenMS normalisation,
                hc = ", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = "")) +
          # facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
          theme_bw() +
          theme(text = element_text(family = "Helvetica")) +
          ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_eigenMS_normalised_hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), device = "pdf", dpi = 600, width = 35, height = 20, units = "cm") +
          ggsave(filename = paste(R_processing_results_dir, "scatterplot_CV_vs_mean_eigenMS_normalised_hc", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".svg", sep = ""), device = "svg", dpi = 600, width = 35, height = 20, units = "cm")
...
# purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .f = function(.x) {
  ...
  ggplot(tibble_of_values_per_timepoint, aes(x = 2^mean, y = CV)) +
    geom_point() +
    geom_smooth(formula = y ~ x, method = "dpois") +
    # scale_x_continuous(trans = "log2") +
    # scale_y_continuous(trans = "log2") +
    coord_cartesian(xlim = c(0, 7.5e8)) +
    xlab("Mean Intensity (normalised)") + 
    ylab("CV") + 
    ggtitle("CV vs. mean for EigenMS") +
    # facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
Normalyzer input intensities should NOT be log transformed, since it's going to be done somewhere along the track
```{r}
...
list_of_tibble_intensities_for_processing_with_id <- purrr::map2(.x = list_of_tibble_intensities_for_processing, 
                                                                .y = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], 
                                                                .f = ~dplyr::bind_cols(.y[, c("id", "Proteins")], .x))

# Proteins: phosphosites.txt
# Protein.IDs: proteingroups.txt
...
normalyzer_design_matrix <- tibble("sample" = colnames(list_of_tibble_intensities_for_processing_with_id[[1]])[-c(1, 2)],
                        "group" = 
                          # create sample groupings for each timepoint
                          # to treat all MSC time points as the same: ^([^_|\\|]+).* 
                          design_factor_info <- gsub(x = colnames(list_of_tibble_intensities_for_processing_with_id[[1]])[-c(1, 2)], pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE) %>% as.factor
                        )
...
sumExpObj <- SummarizedExperiment::SummarizedExperiment(
  assays = as.matrix(list_of_tibble_intensities_for_processing_with_id[[1]][-c(1, 2)]), 
  colData = normalyzer_design_matrix, 
  rowData = list_of_tibble_intensities_for_processing_with_id[[1]][c(1, 2)])

# setupRawDataObject(sumExpObj, inputFormat = "default", zeroToNA = TRUE, sampleColName = "sample", groupColName = "group")

# normObj <- SummarizedExperiment::verifySummarizedExperiment(experimentObj = sumExpObj, sampleColName = "sample")
...
# normResults <- normMethods(sumExpObj)

# normalyzer(jobName = "test_run", 
#            designPath = designFp, 
#            dataPath = dataFp, 
#            outputDir = R_processing_results_dir)

normalyzer(jobName = "test", 
