...
list_of_nested_table_names <- purrr::cross2(names(list_maxquant_run_dirs), names(list_output.table_categories))

list_imported_maxquant_tibbles <- future_map(.x = list_for_maxquant_table_import, .f = ~read.delim(paste(.x[[1]], .x[[2]], sep = ""), stringsAsFactors = FALSE, row.names = NULL, sep = "\t") %>% as_tibble, .progress = TRUE)
...
## plot RLE and MDS before and after normalisation
...
purrr::pmap(.l = list(
  ...
  "L3" = list_of_nested_table_names[vector_phosphosites.txt_list.indices]
    ...
    purrr::pmap(.l = list(
      ...
      dev.off()
      # plot normalised
      pdf(paste(R_processing_results_dir, "RLE_plot_normalised_eigenMS_k", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      plotRLE(normalised_tibble_and_hc$result_table %>% .[, -1] %>% as.matrix, main = paste("normalised_eigenMS, k=", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = ""))
      dev.off()
      ################
      ...
      # plot normalised
      pdf(paste(R_processing_results_dir, "MDSplot_eigenMS_normalised, k", normalised_tibble_and_hc$hc_value, "_", TABLE_NAMES[[1]], "_", name_phospho_or_protein, ".pdf", sep = ""), width = 10, height = 6)
      par(oma = c(0, 0, 0, 5))
      plotMDS(normalised_tibble_and_hc$result_table %>% .[, -1] %>% as.matrix, col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info_normalised))), main = paste("normalised, k=", normalised_tibble_and_hc$hc_value, ", ", TABLE_NAMES[[1]], ", ", name_phospho_or_protein, sep = ""), cex = 2, pch = 1:length(levels(design_factor_info_normalised)))
...
## plot MDS before and after normalisation
...
plot the intra-timepoint correlation
...
# purrr::map(.x = list_imported_maxquant_tibbles_processed[vector_phosphosites.txt_list.indices], .f = function(.x) {
  ...
  combs <- combn(x = colnames(df[, 3:ncol(df)]), m = 2) %>% t
  ...
  list_of_comparisons <- purrr::map2(.x = combs2[, 1] %>% array_tree, .y = combs2[, 2] %>% array_tree, .f = function(.x, .y) {
    ...
    colnames_id.vars <- colnames(df[, 1:2])
  ...
  ggplot(tibble_pairwise_comparisons_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value, y = comparison_2_value)) +
    ...
    ggtitle("Intra-timepoint (between replicates) correlation after EigenMS normalisation") +
    # facet_wrap(paste("comparison 2:", comparison_2_name)~paste("comparison 1:", comparison_1_name), scales = "free") +
    theme_bw() +
    theme(text = element_text(family = "Helvetica"))
...
list_of_tibble_intensities_for_processing <- list_of_dfs_phosphosites_intensities %>% purrr::map(.f = ~mutate_all(., .funs = function(x) {
...
normalyzer(jobName = "test_run_phosphosites_junc.exons.diff", 
