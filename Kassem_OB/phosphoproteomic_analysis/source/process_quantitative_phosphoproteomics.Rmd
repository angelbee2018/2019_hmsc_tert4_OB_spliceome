...
### create cryptic function to re-calculate the (now normalised) ratio (logFC) w.r.t MSC timepoint

```{r}

recalculate_silac_ratio_from_intensity_tibble <- function(input_table) {
  ...
  output_table_temp <- input_table %>% 
    dplyr::mutate_at(.vars = "1h|IB", .funs = function(x) {x - input_table$`ud_1_24|IB`} ) %>%
    dplyr::mutate_at(.vars = "1d|IB", .funs = function(x) {x - input_table$`ud_1_24|IB`} ) %>%
    
    dplyr::mutate_at(.vars = "1h|IB2", .funs = function(x) {x - input_table$`ud_1_24|IB2`} ) %>%
    dplyr::mutate_at(.vars = "1d|IB2", .funs = function(x) {x - input_table$`ud_1_24|IB2`} ) %>%
    
    dplyr::mutate_at(.vars = "1h|KR", .funs = function(x) {x - input_table$`ud_1_24|KR`} ) %>%
    dplyr::mutate_at(.vars = "1d|KR", .funs = function(x) {x - input_table$`ud_1_24|KR`} ) %>%
    
    dplyr::mutate_at(.vars = "30m|IB", .funs = function(x) {x - input_table$`ud_30_6|IB`} ) %>%
    dplyr::mutate_at(.vars = "6h|IB", .funs = function(x) {x - input_table$`ud_30_6|IB`} ) %>%
    
    dplyr::mutate_at(.vars = "30m|IB2", .funs = function(x) {x - input_table$`ud_30_6|IB2`} ) %>%
    dplyr::mutate_at(.vars = "6h|IB2", .funs = function(x) {x - input_table$`ud_30_6|IB2`} ) %>%
    
    dplyr::mutate_at(.vars = "30m|KR", .funs = function(x) {x - input_table$`ud_30_6|KR`} ) %>%
    dplyr::mutate_at(.vars = "6h|KR", .funs = function(x) {x - input_table$`ud_30_6|KR`} )
  
  # get rid of ud columns
  output_table <- output_table_temp %>% dplyr::select(!contains("ud"))
...
list_of_reimported_cycloess_normalised_tibbles_logFC <- list_of_reimported_cycloess_normalised_tibbles %>% future_map(.f = ~purrr::map(.x = .x, .f = ~recalculate_silac_ratio_from_intensity_tibble(.x)), .progress = TRUE)

list_of_eigenMS_results_logFC <- list_of_eigenMS_results_tibble.only %>% future_map(.f = ~purrr::map(.x = .x, .f = ~recalculate_silac_ratio_from_intensity_tibble(.x)), .progress = TRUE)
