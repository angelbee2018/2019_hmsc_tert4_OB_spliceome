...
# shared_dir <- "Z:/sharedfolder/"
shared_dir <- "/mnt/Tertiary/sharedfolder/"

R_processing_results_dir <- paste("/mnt/LTS/projects/2019_hmsc_spliceome/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/R_processing_results/", sep = "")
# R_processing_results_dir <- paste("Z:/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/R_processing_results/", sep = "")
...
list_maxquant_run_dirs <- list("junc.exons" = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/2020_phosphoproteome_OBseries_junc.exons_deskside/txt/",
                               "sp.hsa.canonical.isoforms_junc.exons" = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/2020_phosphoproteome_OBseries_sp.hsa.canonical.isoforms_junc.exons_deskside/txt/",
                               # "con_sp.hsa.canonical.isoforms_junc.exons" = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/2020_phosphoproteome_OBseries_con_sp.hsa.canonical.isoforms_junc.exons/txt/",
                               
                               "con_sp.hsa.canonical.isoforms" = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/2020_phosphoproteome_OBseries_con_sp.hsa.canonical.isoforms/txt/",
                               "con_sp.hsa.canonical.isoforms_trembl" = "/mnt/Tertiary/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/phosphoproteomic_analysis/analysis_maxquant/results/2020_phosphoproteome_OBseries_con_sp.hsa.canonical.isoforms_trembl/txt/"
                               )
...
list_imported_maxquant_tibbles_processed <- list_imported_maxquant_tibbles %>% 
  ...
  purrr::map(~modify_at(.x = .x, .at = c("peptides", "protein_groups", "phosphosites"), .f = function(.x) {
...
save(list_imported_maxquant_tibbles_processed, file = paste(R_processing_results_dir, "list_imported_maxquant_tibbles_processed.Rlist", sep = ""))
...
}, .progress = TRUE , .options = future_options(globals = c("ggplot2", "tidyverse", "tibble")))
...
list_of_eigenMS_results <- furrr::future_pmap(.l = list(
  ...
  "L4" = names(list_of_nested_table_names)), .f = function(L1, L2, L3, L4) {
    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        list_lvl.2_result <- purrr::map(.x = h.c_values_to_run, .f = function(.x) {
...
names(list_of_eigenMS_results) <- names(list_of_nested_table_names)

save(list_of_eigenMS_results, file = paste(R_processing_results_dir, "list_of_eigenMS_results.Rlist", sep = ""))
    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        normalyzer(jobName = paste(names[[1]], "_", names[[2]], sep = ""),
                   ...
                   outputDir = paste("/mnt/Tertiary/sharedfolder/", sep = ""))
...
list_of_eigenMS_results_filtered_less_than_1_na_all_timepoints <- future_map2(.x = list_of_eigenMS_results, .y = names(list_of_eigenMS_results), .f = function(a1, a2) {
  
  # DEBUG ###
  # a1 <- list_of_eigenMS_results[[1]]
  # a2 <- names(list_of_eigenMS_results) %>% .[[1]]
  ###########
...
}, .progress = TRUE, .options = future_options(globals = c("filter_less_than_x_na_for_all_timepoints", "dplyr", "as_tibble", "maximum_na_observations_per_timepoint")) )
...
list_of_eigenMS_results_DreamAI_imputed_sp <- purrr::map2(.x = list_of_eigenMS_results_filtered_less_than_1_na_all_timepoints[c("sp.hsa.canonical.isoforms_junc.exons", "con_sp.hsa.canonical.isoforms", "con_sp.hsa.canonical.isoforms_trembl")], 
                                                          .y = names(list_of_eigenMS_results_filtered_less_than_1_na_all_timepoints[c("sp.hsa.canonical.isoforms_junc.exons", "con_sp.hsa.canonical.isoforms", "con_sp.hsa.canonical.isoforms_trembl")]), 
                                                          .f = function(a1, a2) {
                                                            
                                                            # DEBUG ###
                                                            # a1 <- list_of_eigenMS_results_filtered_less_than_1_na_all_timepoints$sp.hsa.canonical.isoforms_junc.exons
                                                            # a2 <- names(list_of_eigenMS_results_filtered_less_than_1_na_all_timepoints["sp.hsa.canonical.isoforms_junc.exons"])
                                                            ###########
                                                            ...
                                                            L2_list <- purrr::map2(.x = a1, .y = names(a1), .f = function(b1, b2) {
                                                              ...
                                                              list_imputed_dataframe <- DreamAI::DreamAI(b1$result_table %>% dplyr::select(-id) %>% as.matrix, k = 10, maxiter_MF = 10, ntree = 100,
                                                              ...
                                                              imputed_tibble <- imputed_dataframe %>% as_tibble %>% add_column("id" = b1$result_table$id, .before = 1)
                                                              
                                                              return(imputed_tibble)
...
# , .progress = TRUE, .options = future_options(globals = c("DreamAI", "KNN", "MissForest", "ADMIN", "Brinn", "SpectroFM", "RegImpute", "R_processing_results_dir", "plot_PCA_for_timepoint_and_replicate", "vector_replicate_names", "as_tibble", "add_column", "impute.KNN"))

save.image(file = paste(R_processing_results_dir, "02112020_windows_dreamAI.Rdata"))
...
list_of_eigenMS_results_imputed.incomplete_sp_raw <- 
  list_of_eigenMS_results_DreamAI_imputed_sp <- purrr::pmap(.l = list(
    "a1" = list_of_eigenMS_results_DreamAI_imputed_sp,
    "a2" = list_of_eigenMS_results_filtered_less_than_1_na_for_2_timepoints[names(list_of_eigenMS_results_DreamAI_imputed_sp)],
    "a3" = names(list_of_eigenMS_results_DreamAI_imputed_sp)
  ), .f = function(a1, a2, a3) {
    
    # DEBUG ###
    # a1 <- list_of_eigenMS_results_DreamAI_imputed_sp[[1]]
    # a2 <- list_of_eigenMS_results_filtered_less_than_1_na_for_2_timepoints[names(list_of_eigenMS_results_DreamAI_imputed_sp)] %>% .[[1]]
    # a3 <- names(list_of_eigenMS_results_DreamAI_imputed_sp) %>% .[[1]]
    ###########
...
suppressWarnings(suppressMessages(list_of_eigenMS_results_quantitative_cleaned <- purrr::pmap(
...
### create ud = 1 timepoint

```{r}

list_of_eigenMS_results_quantitative_cleaned <- list_of_eigenMS_results_quantitative_cleaned %>% purrr::map_depth(.depth = 2, .f = ~.x %>% add_column("ud_vs_ud|IB" = 0, "ud_vs_ud|IB2" = 0, "ud_vs_ud|KR" = 0))
...
ratio_colnames <- c("1h_vs_ud|IB", "1d_vs_ud|IB", "30m_vs_ud|IB", "6h_vs_ud|IB", "1h_vs_ud|IB2", "1d_vs_ud|IB2", "30m_vs_ud|IB2", "6h_vs_ud|IB2", "1h_vs_ud|KR", "1d_vs_ud|KR", "30m_vs_ud|KR", "6h_vs_ud|KR",
                    "ud_vs_ud|IB", "ud_vs_ud|IB2", "ud_vs_ud|KR")

ratio_names <- gsub(x = ratio_colnames, 
                    pattern = "(.*)\\|(.*)", 
                    replacement = "\\1") %>% 
  factor(x = ., levels = c("ud_vs_ud", "30m_vs_ud", "1h_vs_ud", "6h_vs_ud", "1d_vs_ud"))
...
pairwise_contrasts <- makeContrasts(
  "ratio_names30m_vs_ud - ratio_namesud_vs_ud",
  "ratio_names1h_vs_ud - ratio_namesud_vs_ud",
  "ratio_names6h_vs_ud - ratio_namesud_vs_ud",
  "ratio_names1d_vs_ud - ratio_namesud_vs_ud",
  
  "ratio_names1h_vs_ud - ratio_names30m_vs_ud", 
...
list_of_eigenMS_limma_results <- purrr::map2(.x = list_of_eigenMS_results_quantitative_cleaned, .y = names(list_of_eigenMS_results_quantitative_cleaned), .f = function(.x, .y) {
  ...
  L2_list <- purrr::map2(.x = a1[c("phosphosites", "protein_groups")], .y = names(a1[c("phosphosites", "protein_groups")]), .f = function(.x, .y) {
    ...
    pdf(file = paste("./qualityplot_limma_pvalue_distribution_missing.obs.per.ratio_", a2, "_", b2, ".pdf", sep = ""))
...
We will check if this works better than limma because there's a chance that the noise made it unsuitable for limma
...
do_t.test_per_row_of_table <- function(table, column_regexpr_1, column_regexpr_2, no_more_than_x_na = 1) {
  ...
  # DEBUG ###
  # table <- list_of_eigenMS_results_filtered_less_than_1_na_for_2_timepoints %>% purrr::map(.f = ~purrr::map(.x = .x, .f = ~.x$result_table)) %>% .[[1]] %>% .[[1]] %>% add_column("ud_vs_ud|IB" = 0, "ud_vs_ud|IB2" = 0, "ud_vs_ud|KR" = 0)
  # column_regexpr_1 <- "30m"
  # column_regexpr_2 <- "ud_vs_ud"
  # no_more_than_x_na <- 1
  ###########
  ...
  purrr::pmap(
    .l = list(
      "a1" = table %>% dplyr::select(contains(column_regexpr_1)) %>% purrr::array_tree(margin = 1),
      "a2" = table %>% dplyr::select(contains(column_regexpr_2)) %>% purrr::array_tree(margin = 1),
      "a3" = 1:length(table %>% dplyr::select(contains(column_regexpr_1)) %>% purrr::array_tree(margin = 1))
    ), 
    .f = function(a1, a2, a3) {
      
      # DEBUG ###
      # a1 <- table %>% dplyr::select(contains(column_regexpr_1)) %>% purrr::array_tree(margin = 1) %>% .[[1]]
      # a2 <- table %>% dplyr::select(contains(column_regexpr_2)) %>% purrr::array_tree(margin = 1) %>% .[[1]]
      # a3 <- 1:length(table %>% dplyr::select(contains(column_regexpr_1)) %>% purrr::array_tree(margin = 1)) %>% .[[1]]
      ###########
      
      # cat(a3, "\n")
      
      if (which(is.na(a1) | is.nan(a1)) %>% length > 1 | which(is.na(a2) | is.nan(a1)) %>% length > 1) {
        result <- NA
      } else {
...
list_of_eigenMS_t.test_results <- list_of_eigenMS_results_quantitative_cleaned %>%
  purrr::modify_at(.at = "junc.exons", .f = ~.x[c("peptides", "protein_groups")]) %>% 
  purrr::map_depth(.depth = 2, .f = ~.x %>% 
                     add_column(
                       ...
                       "logFC_30m_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist,
                       "logFC_1h_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist,
                       "logFC_6h_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist,
                       "logFC_1d_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist,
                       "logFC_1h_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist, 
                       "logFC_6h_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist, 
                       "logFC_1d_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist,
                       "logFC_6h_vs_ud_over_1h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))))} else {return(NA)}}) %>% unlist, 
                       "logFC_1d_vs_ud_over_1h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist, 
                       "logFC_1d_vs_ud_over_6h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return((mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert))) %>% log2)} else {return(NA)}}) %>% unlist
                       ) )

# purrr::pmap(.l = list("c1" = test %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), 
#                       "c2" = test %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), 
#                       "c3" = 1:length(test %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1))
# ),
# .f = function(c1, c2, c3) {
#   
#   cat(c3, "\n")
#   
#   if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {
#     return(mean(2^(.x %>% na.omit %>% type.convert))/mean(2^(.y %>% na.omit %>% type.convert)))
#   } else {
#     return(NA)
#   }
#   
...
list_of_eigenMS_t.test_results_significant <- purrr::map_depth(
  ...
  .f = function(b1) {
    ...
    # DEBUG ###
    # b1 <- list_of_eigenMS_t.test_results$`con_sp.hsa.canonical.isoforms`$protein_groups %>% .[7, ]
    ###########
    
    vector_logical_significant_rowwise <- purrr::map2(.x = b1 %>% dplyr::select(contains("pvalue")) %>% array_tree(margin = 1), .y = b1 %>% dplyr::select(contains("FC_")) %>% array_tree(margin = 1), .f = ~any(.x %>% na.omit < pvalue_cutoff) & any(.y %>% na.omit %>% abs > log2(fold_change_cutoff))) %>% unlist
...
list_eigenMS_DE_results_geneset <- purrr::pmap(
  .l = list("a1" = list_of_eigenMS_t.test_results_significant,
            "a2" = list_imported_maxquant_tibbles_processed[names(list_of_eigenMS_t.test_results_significant)],
            "a3" = names(list_of_eigenMS_t.test_results_significant)), 
  .f = function(a1, a2, a3) {
    ...
    purrr::pmap(
      .l = list("b1" = a1[c("phosphosites", "protein_groups")], 
                ...
                "b3" = names(a1[c("phosphosites", "protein_groups")])), .f = function(b1, b2, b3) {
                  ...
                  # DEBUG ###
                  # b1 <- list_of_eigenMS_limma_results[["con_sp.hsa.canonical.isoforms_trembl"]][["phosphosites"]]
                  # b2 <- list_imported_maxquant_tibbles_processed[["con_sp.hsa.canonical.isoforms_trembl"]][["phosphosites"]]
                  # b3 <- list_of_eigenMS_limma_results[["con_sp.hsa.canonical.isoforms_trembl"]]["phosphosites"] %>% names
                  ###########
                  ...
                  vec_ids_of_limma_differential_items <- b1$id
                  ...
                  tibble_raw_gene_names <- tibble_differential_item_info[, colnames(tibble_differential_item_info) %in% c("Gene.names", "Fasta.headers")] %>%
                    dplyr::mutate_all(.funs = function(x) {return(gsub(x = x, pattern = "([^\\ ]+).*{0,1}", replacement = "\\1"))} ) %>% unlist
                  # remove blanks
...
list_GOhyper_enrichment <- purrr::pmap(.l = list(
  "a1" = list_eigenMS_DE_results_geneset,
  "a2" = names(list_eigenMS_DE_results_geneset)), .f = function(a1, a2) {
...
list_of_significant_items_averaged <- purrr::map2(
  ...
  .f = function(a1, a2) {
    ...
    L2_list <- purrr::map2(.x = a1, .y = names(a1), .f = function(b1, b2) {
      ...
      design_factor_info <- factor(x = gsub(x = colnames(raw_tibble %>% dplyr::select(-id)), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE), levels = c("ud_vs_ud", "30m_vs_ud", "1h_vs_ud", "6h_vs_ud", "1d_vs_ud"))
...
# peptides have a few tens of thousands of entries which we won't process
list_of_significant_items_averaged <- list_of_significant_items_averaged %>% purrr::map(~.x[c("phosphosites", "protein_groups")])
...
list_scaled_MQ_ratios <- purrr::map2(
...
list_MQ_scaled_tibbles_matched_to_geneset_per_cluster <- purrr::pmap(
  .l = list("a1" = list_clustered_MQ_ratios,
            "a2" = list_imported_maxquant_tibbles_processed[names(list_clustered_MQ_ratios)],
            "a3" = names(list_clustered_MQ_ratios)), 
  .f = function(a1, a2, a3) {
    ...
    purrr::pmap(
      .l = list("b1" = a1[c("phosphosites", "protein_groups")], 
                ...
                "b3" = names(a1[c("phosphosites", "protein_groups")])), .f = function(b1, b2, b3) {
                  ...
                  # DEBUG ###
                  # b1 <- list_clustered_MQ_ratios[["con_sp.hsa.canonical.isoforms"]][["phosphosites"]]
                  # b2 <- list_imported_maxquant_tibbles_processed[["con_sp.hsa.canonical.isoforms"]][["phosphosites"]]
                  # b3 <- list_clustered_MQ_ratios[["con_sp.hsa.canonical.isoforms"]]["phosphosites"] %>% names
                  ###########
                  ...
                  # table join by id
                  tibble_scaled_ratios_joined_to_information <- dplyr::left_join(b1 %>% dplyr::mutate_at(.vars = "id", .funs = function(x) {gsub(x = x, pattern = "(.*)_.*", replacement = "\\1") %>% type.convert %>% return} ), b2[, c("id", "Gene.names", "Fasta.headers")], by = "id") %>%
                    dplyr::mutate_at(.vars = c("Fasta.headers"), .funs = function(x) {return(gsub(x = x, pattern = "^([^\\ ]+).*{0,1}", replacement = "\\1"))} ) 
                  
                  return(tibble_scaled_ratios_joined_to_information)
...
tibble_MQ_scaled_tibbles_matched_to_geneset_per_cluster <- purrr::map2(
  .x = list_MQ_scaled_tibbles_matched_to_geneset_per_cluster,
  .y = names(list_MQ_scaled_tibbles_matched_to_geneset_per_cluster),
  .f = function(a1, a2) {
    ...
    purrr::map2(
      ...
      .f = function(b1, b2) {
        ...
        return(b1 %>% tibble::add_column("type" = b2))
        
      } ) %>% data.table::rbindlist() %>% tibble::as_tibble() %>% tibble::add_column("experiment" = a2) %>% return
    
    } ) %>% data.table::rbindlist() %>% tibble::as_tibble()

write.table(x = tibble_MQ_scaled_tibbles_matched_to_geneset_per_cluster, file = paste(R_processing_results_dir, "tibble_MQ_scaled_tibbles_matched_to_geneset_per_cluster.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
plan(list(tweak(multiprocess, workers = 2),
          ...
          tweak(multiprocess, workers = 30),
          tweak(multiprocess, workers = 3)))

# let's try to do all enrichment at once using purrr
list_gohyper_enrichment_per_cluster <- future_map2(
  .x = list_MQ_scaled_tibbles_matched_to_geneset_per_cluster,
  .y = names(list_MQ_scaled_tibbles_matched_to_geneset_per_cluster),
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a1 <- list_MQ_scaled_tibbles_matched_to_geneset_per_cluster[[1]]
    # a2 <- names(list_MQ_scaled_tibbles_matched_to_geneset_per_cluster) %>% .[[1]]
    ###########
    ...
    future_map2(
      ...
      .f = function(b1, b2) {
        ...
        # group_split by cluster
        list_scaled_ratios_by_cluster <- b1 %>% 
          dplyr::group_split(cluster) %>%
          set_names(nm = purrr::map(.x = ., .f = ~.x$cluster %>% unique) %>% unlist)
        
        future_map2(
          .x = list_scaled_ratios_by_cluster,
          .y = names(list_scaled_ratios_by_cluster),
          .f = function(c1, c2) {
            
            # DEBUG ###
            # c1 <- list_scaled_ratios_by_cluster[[1]]
            # c2 <- names(list_scaled_ratios_by_cluster) %>% .[[1]]
            ###########
            ...
            future_map(
              ...
              .f = function(d1) {
                ...
                # extract geneset
                vector_geneset <- c1[, c("Gene.names", "Fasta.headers")] %>% unlist %>% strsplit(split = ";") %>% unlist %>% unique
                
                tibble_gohyper_enrichment <- GOHyperGAll(catdb = catdb, gocat = d1, Nannot = 2, sample = vector_geneset) %>% GOHyperGAll_benjamini_correction
                
              }, .options = future_options(globals = c("GOHyperGAll", "GOHyperGAll_benjamini_correction", "catdb", "rbindlist", "as_tibble", "add_column")) ) %>% rbindlist %>% as_tibble %>% add_column("cluster" = c2) %>% return # L4: GO categories
            
          }, .options = future_options(globals = c("GOHyperGAll", "GOHyperGAll_benjamini_correction", "catdb", "rbindlist", "as_tibble", "add_column")) ) %>% rbindlist %>% as_tibble %>% add_column("data_type" = b2) %>% return # L3: each consensus cluster
        
      }, .options = future_options(globals = c("GOHyperGAll", "GOHyperGAll_benjamini_correction", "catdb", "rbindlist", "as_tibble", "add_column")) ) %>% rbindlist %>% as_tibble %>% add_column("dbrun" = a2) %>% return # L2: phosphosites/protein_groups
    
  }, .progress = TRUE, .options = future_options(globals = c("GOHyperGAll", "GOHyperGAll_benjamini_correction", "catdb", "rbindlist", "as_tibble", "add_column")) ) # L1: con/sp/canonical/junc.exons

# write tables
furrr::future_map2(
  .x = list_gohyper_enrichment_per_cluster,
  .y = names(list_gohyper_enrichment_per_cluster),
  .f = function(a1, a2) {
    
    write.table(x = a1, file = paste(R_processing_results_dir, "kinase_substrate_analysis/som_consensus_matrix_combinedexpr_pvalue", pvalue_cutoff, "_FC", fold_change_cutoff, "_sweep_from_", min(xdim_range_combinedexpr), "x", min(ydim_range_combinedexpr), "_to_", max(xdim_range_combinedexpr), "x", max(ydim_range_combinedexpr), "_", a2, "_GO.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
...
# Export only the MSC, 6h and 1d timepoints for multiomic clustering

## re-do significance filtering with only MSC, 6h and 1d

```{r}

list_of_eigenMS_t.test_results_significant_MSC_6h_1d <- purrr::map_depth(
  .x = list_of_eigenMS_t.test_results_significant,
  .depth = 2, 
  .f = function(b1) {
    ...
    vector_logical_significant_rowwise <- purrr::map2(
      .x = b1 %>% dplyr::select(-contains("30m"), -contains("1h")) %>% dplyr::select(contains("pvalue")) %>% array_tree(margin = 1), 
      .y = b1 %>% dplyr::select(-contains("30m"), -contains("1h")) %>% dplyr::select(contains("logFC")) %>% array_tree(margin = 1), 
      .f = ~any(.y %>% na.omit %>% abs > log2(fold_change_cutoff))) %>% unlist
    # any(.x %>% na.omit < pvalue_cutoff) & 
    
    return(b1[which(vector_logical_significant_rowwise == TRUE), ] %>% dplyr::select(-contains("30m"), -contains("1h")))
    
  } )

```

## average the ratios for each timepoint

```{r}

list_of_significant_items_averaged_MSC_6h_1d <- purrr::map2(
  .x = list_of_eigenMS_t.test_results_significant_MSC_6h_1d, 
  .y = names(list_of_eigenMS_t.test_results_significant_MSC_6h_1d), 
  .f = function(a1, a2) {
    
    # DEBUG ###
    # .x <- list_of_eigenMS_t.test_results_significant_MSC_6h_1d[[1]]
    # .y <- names(list_of_eigenMS_t.test_results_significant_MSC_6h_1d) %>% .[[1]]
    ###########
    ...
    L2_list <- purrr::map2(.x = a1, .y = names(a1), .f = function(b1, b2) {
      ...
      # DEBUG ###
      # b1 <- list_of_eigenMS_t.test_results_significant_MSC_6h_1d$con_sp.hsa.canonical.isoforms$peptides
      # b2 <-list_of_eigenMS_t.test_results_significant_MSC_6h_1d$con_sp.hsa.canonical.isoforms %>% .["peptides"] %>% names
      ###########
      ...
      design_factor_info <- factor(x = gsub(x = colnames(raw_tibble %>% dplyr::select(-id)), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE), levels = c("ud_vs_ud", "6h_vs_ud", "1d_vs_ud"))
...
## match the scaled ratios to the original information 

```{r}

# first match significant entries to the processed info tables 

list_significant_MQ_scaled_tibbles_matched_to_geneset_per_cluster_MSC_6h_1d <- purrr::pmap(
  .l = list("a1" = list_of_significant_items_averaged_MSC_6h_1d,
            "a2" = list_imported_maxquant_tibbles_processed[names(list_of_significant_items_averaged_MSC_6h_1d)],
            "a3" = names(list_of_significant_items_averaged_MSC_6h_1d)), 
  .f = function(a1, a2, a3) {
    
    cat("\nnow processing", a3, "at level 1")
    
    purrr::pmap(
      .l = list("b1" = a1[names(a1) %>% .[. != "peptides"]], 
                "b2" = a2[names(a1) %>% .[. != "peptides"]],
                "b3" = names(a1) %>% .[. != "peptides"]), .f = function(b1, b2, b3) {
                  ...
                  # table join by id
                  tibble_scaled_ratios_joined_to_information <- dplyr::left_join(b1 %>% dplyr::mutate_at(.vars = "id", .funs = function(x) {gsub(x = x, pattern = "(.*)_.*", replacement = "\\1") %>% type.convert %>% return} ), b2, by = "id") %>%
                    dplyr::mutate_at(.vars = c("Fasta.headers"), .funs = function(x) {return(gsub(x = x, pattern = "^([^\\ ]+).*{0,1}", replacement = "\\1"))} ) %>% 
                    dplyr::rename("MSC" = "mean|ud_vs_ud", "6h" = "mean|6h_vs_ud", "1d" = "mean|1d_vs_ud")
...
save(list_significant_MQ_scaled_tibbles_matched_to_geneset_per_cluster_MSC_6h_1d, file = paste(R_processing_results_dir, "list_significant_MQ_scaled_tibbles_matched_to_geneset_per_cluster_MSC_6h_1d.Rlist", sep = ""))
