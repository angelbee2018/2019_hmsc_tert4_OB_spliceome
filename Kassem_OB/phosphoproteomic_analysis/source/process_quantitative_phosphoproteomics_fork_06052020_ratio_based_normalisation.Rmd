...
list_of_eigenMS_t.test_results <- list_of_eigenMS_results_quantitative_cleaned %>%
  ...
  purrr::map_depth(.depth = 2, .f = ~.x %>% 
                     add_column(
                       ...
                       "FC_30m_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist,
                       "FC_1h_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist,
                       "FC_6h_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist,
                       "FC_1d_vs_ud_over_ud_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("ud_vs_ud")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist,
                       "FC_1h_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist, 
                       "FC_6h_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist, 
                       "FC_1d_vs_ud_over_30m_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("30m")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist,
                       "FC_6h_vs_ud_over_1h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist, 
                       "FC_1d_vs_ud_over_1h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("1h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist, 
                       "FC_1d_vs_ud_over_6h_vs_ud" = purrr::map2(.x = .x %>% dplyr::select(contains("1d")) %>% array_tree(margin = 1), .y = .x %>% dplyr::select(contains("6h")) %>% array_tree(margin = 1), .f = function(c1, c2) {if (is.na(c1) %>% which %>% length <= 2 & is.na(c1) %>% which %>% length <= 2) {return(mean(2^(c1 %>% na.omit %>% type.convert))/mean(2^(c2 %>% na.omit %>% type.convert)))} else {return(NA)}}) %>% unlist
                       ) )
...
list_of_eigenMS_t.test_results_significant <- purrr::map_depth(
  ...
  .f = function(b1) {
    ...
    vector_logical_significant_rowwise <- purrr::map2(.x = b1 %>% dplyr::select(contains("pvalue")) %>% array_tree(margin = 1), .y = b1 %>% dplyr::select(contains("FC_")) %>% array_tree(margin = 1), .f = ~any(.x %>% na.omit < pvalue_cutoff) & any(.y %>% na.omit %>% abs > fold_change_cutoff)) %>% unlist
...
list_of_eigenMS_t.test_results_significant_MSC_6h_1d <- purrr::map_depth(
  ...
  .f = function(b1) {
    ...
    vector_logical_significant_rowwise <- purrr::map2(
      ...
      .f = ~any(.x %>% na.omit < pvalue_cutoff & .y %>% na.omit %>% abs > log2(fold_change_cutoff))) %>% unlist
