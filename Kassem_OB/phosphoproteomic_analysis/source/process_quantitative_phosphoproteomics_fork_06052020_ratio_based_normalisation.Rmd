    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        future_map(.x = h.c_values_to_run, .f = function(.x) {
          ...
          ggplot(tibble_of_values_per_timepoint, aes(x = mean, y = CV)) +
            geom_point() +
            # geom_smooth(formula = y ~ x, method = "loess", span = PI_value) +
            # scale_x_continuous(trans = "log2") +
            # scale_y_continuous(trans = "log2") +
  ...
  "L4" = names(list_of_nested_table_names)), .f = function(L1, L2, L3, L4) {
    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        # DEBUG ###
        # l1 <- list_of_peptide_phosphopeptide_proteingroups_normalised_ratio_matrixtibbles[["con_sp.hsa.canonical.isoforms_junc.exons"]][["protein_groups"]]
        ...
        # define tables #####
        # fuck you, normalyzer.
        data_cols <- l1 %>% mutate_all(.funs = function(x) {return(2^x)} ) %>% mutate_all(.funs = function(x) {return(na_if(x, Inf) %>% na_if(., -Inf))} ) %>% mutate_all(.funs = function(X) {return(tidyr::replace_na(X, replace = NA))} ) %>% as_tibble
        untransformed_cols <- l2
        names <- l3
