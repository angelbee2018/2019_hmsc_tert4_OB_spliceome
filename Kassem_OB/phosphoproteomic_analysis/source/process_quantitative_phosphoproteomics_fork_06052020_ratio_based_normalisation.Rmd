...
split_delimited_column_in_table <- function(input_table, target_colname, split) {
  ...
  split_table <- input_table_repeated_by_split %>% type_convert
...
filter_less_than_x_na_for_y_timepoints <- function(unfiltered_tibble, variable_levels, max_na_per_replicates_x, minimum_number_of_timepoints_considered_y) {
  ...
  # DEBUG ###
  # unfiltered_tibble <- list_of_eigenMS_results$sp.hsa.canonical.isoforms_junc.exons$phosphosites$result_table
  # variable_levels <- c("30m_vs_ud", "1h_vs_ud", "6h_vs_ud", "1d_vs_ud")
  ...
  vector_of_logical_has.min.replicates.min.timepoint <- tibble_tibble_subset_by_ratio_NaN_count %>% future_apply(MARGIN = 1, FUN = function(X) {
...
list_of_eigenMS_results_quantitative_cleaned <- purrr::pmap(
  .l = list("a1" = list_of_eigenMS_results_imputed.incomplete_sp_raw,
            "a2" = list_imported_maxquant_tibbles_processed[names(list_of_eigenMS_results_imputed.incomplete_sp_raw)],
            "a3" = names(list_of_eigenMS_results_imputed.incomplete_sp_raw)),
  .f = function(a1, a2, a3) {
    
    # DEBUG ###
    # a1 <- list_of_eigenMS_results_imputed.incomplete_sp_raw[["sp.hsa.canonical.isoforms_junc.exons"]]
    # a2 <- list_imported_maxquant_tibbles_processed[["sp.hsa.canonical.isoforms_junc.exons"]]
    # a3 <- names(list_of_eigenMS_results_imputed.incomplete_sp_raw["sp.hsa.canonical.isoforms_junc.exons"])
    ###########
    ...
    tibble_phosphosites_subtracted_by_protein.groups <- list_phosphosites_matched_to_protein.groups_array.tree %>% 
      purrr::map(.f = function(.x) {
        ...
        tibble_protein_row_matching_ID <- tibble_protein.groups_subsetted_to_phosphosite[tibble_protein.groups_subsetted_to_phosphosite$Protein.group.IDs == .x$Protein.group.IDs, ] %>% 
          dplyr::select(-Protein.group.IDs)
        
        tibble_phosphosite_normalised_row <- .x$phosphosites - tibble_protein_row_matching_ID
...
list_of_eigenMS_limma_results <- purrr::map2(.x = list_of_eigenMS_results_quantitative_cleaned, .y = names(list_of_eigenMS_results_quantitative_cleaned), .f = function(.x, .y) {
  ...
  L2_list <- purrr::map2(.x = a1, .y = names(a1), .f = function(.x, .y) {
    ...
    tibble_values_only <- b1 %>% dplyr::select(-id) 
    
    # create DESIGN MATRIX
    ratio_colnames <- colnames(tibble_values_only)
    ...
    vector_pairwise_ratio_combos_edited <- paste("ratio_names", tibble_pairwise_ratio_combos$V1, " - ", "ratio_names", tibble_pairwise_ratio_combos$V2, sep = "")
    
    # loop thru each pairwise contrast
    list_pairwise_contrasts <- list()
    
    for (combo in vector_pairwise_ratio_combos_edited) {
      list_pairwise_contrasts <- purrr::splice(list_pairwise_contrasts, limma::makeContrasts(combo, levels = limma_design_matrix))
    }
    
    list_limma_fit <- purrr::map2(.x = list_pairwise_contrasts, .y = vector_pairwise_ratio_combos_edited, .f = function(.x, .y) {
      
      # DEBUG ###
      .x <- list_pairwise_contrasts[[1]]
      .y <- vector_pairwise_ratio_combos_edited[[1]]
      ###########
      
      pairwise_contrast <- .x
    
      fit <- lmFit(tibble_values_only, limma_design_matrix)
      
      fit.contrasts <- contrasts.fit(fit, pairwise_contrast)
      ...
      tibble_fitted_coefficients <- tibble_fitted_coefficients %>% add_column("id" = b1$id, .before = 1)
      
      toptable <- topTable(ebayes, adjust.method = "BH", number = nrow(tibble_values_only)) %>% as.matrix %>% as_tibble(rownames = "row_number")
      # rename the toptable column names in preparation for dplyr table join
      colnames(toptable)[!colnames(toptable) %in% "row_number"] <- paste(.y, "_", colnames(toptable)[!colnames(toptable) %in% "row_number"], sep = "")
      
      # get tibble of significant items
      # adj.P.Val
      # P.Value
      tibble_sig_items <- b1[toptable %>% dplyr::filter(!!as.name(paste(.y, "_", "P.Value", sep = "")) < pvalue_cutoff & abs(!!as.name(paste(.y, "_", "logFC", sep = ""))) > log2(fold_change_cutoff)) %>% .$row_number %>% as.numeric, ] %>% 
        dplyr::bind_cols(., toptable %>% dplyr::filter(!!as.name(paste(.y, "_", "P.Value", sep = "")) < pvalue_cutoff & abs(!!as.name(paste(.y, "_", "logFC", sep = ""))) > log2(fold_change_cutoff)) %>% dplyr::select(-row_number))
      
      # PLOTS #####
    ## p-value distribution
    pdf(file = paste("./qualityplot_limma_pvalue_distribution_", a2, "_", b2, "_", .y %>% str_remove_all("ratio_names") %>% str_remove_all(" ") %>% str_replace(pattern = "\\-", replacement = "_vs_"), ".txt", sep = ""))
    plot(density(toptable[, paste(.y, "_", "P.Value", sep = "")] %>% unlist %>% na.omit), main = paste("LIMMA p-value distribution\n", .y, sep = ""))
    dev.off()
    ...
    # my own volcano plot
    ggplot(data = toptable %>% na.omit, mapping = aes(x = !!as.name(paste(.y, "_", "logFC", sep = "")), y = -(!!as.name(paste(.y, "_", "P.Value", sep = "")) %>% log10))) +
      geom_point(size = 2) +
      ...
      theme_bw() +
      theme(text = element_text(family = "Helvetica")) +
      ggsave(filename = paste("./volcano_plot_limma_anysig_", a2, "_", b2, "_pvalue", pvalue_cutoff, "_fc", fold_change_cutoff, "_", .y %>% str_remove_all("ratio_names") %>% str_remove_all(" ") %>% str_replace(pattern = "\\-", replacement = "_vs_"), ".pdf", sep = ""), device = "pdf", dpi = 600, width = 30, height = 25, units = "cm") +
      ggsave(filename = paste("./volcano_plot_limma_anysig_", a2, "_", b2, "_pvalue", pvalue_cutoff, "_fc", fold_change_cutoff, "_", .y %>% str_remove_all("ratio_names") %>% str_remove_all(" ") %>% str_replace(pattern = "\\-", replacement = "_vs_"), ".svg", sep = ""), device = "svg", dpi = 600, width = 30, height = 25, units = "cm")
    ...
    } ) %>% set_names(vector_pairwise_ratio_combos_edited)
    
    ## write table of fitted coefficients
    tibble_fitted_coefficients <- list_limma_fit %>% purrr::map(~.x$tibble_fitted_coefficients) %>% purrr::reduce(dplyr::full_join)
    write.table(x = tibble_fitted_coefficients, file = paste("./table_limma_fitted_SILAC_ratios_", a2, "_", b2, ".txt", sep = ""), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
    
    ## write table of significant items with limma toptable info tagged on.
    tibble_sig_items <- list_limma_fit %>% purrr::map(~.x$tibble_sig_items) %>% purrr::reduce(dplyr::full_join)
    write.table(x = tibble_sig_items, file = paste("./table_limma_anysig_entries_", a2, "_", b2, "_pvalue", pvalue_cutoff, "_fc", fold_change_cutoff, ".txt", sep = ""), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
  ...
  "a3" = names(list_of_eigenMS_limma_results)), .f = function(a1, a2, a3) {
    ...
    L2_list <- purrr::pmap(.l = list(
      ...
      "b3" = names(a1[c("phosphosites", "protein_groups")])), .f = function(b1, b2, b3) {
        ...
        vec_differential_gene_names <- tibble_differential_item_info$Gene.names %>% strsplit(split = ";") %>% unlist %>% unique %>% mixedsort
        ...
        return(list("GOhyper_result_MF" = tibble_GOhyper_result_MF,
                    ...
                    "tibble_differential_items" = tibble_differential_item_info))
