...
list_of_eigenMS_results <- purrr::pmap(.l = list(
  "L1" = list_of_peptide_phosphopeptide_proteingroups_encode.intensity_matrixtibbles,
  ...
  "L4" = names(list_of_nested_table_names)), .f = function(L1, L2, L3, L4) {
    ...
    # DEBUG ###
    # L1 <- list_of_peptide_phosphopeptide_proteingroups_encode.intensity_matrixtibbles[["junc.exons"]]
    ...
    L2_list_of_tibbles <- purrr::pmap(.l = list(
      ...
      "l4" = c("peptides", "phosphosites", "protein_groups")), .f = function(l1, l2, l3, l4) {
        ...
        # DEBUG ###
        # l1 <- list_of_peptide_phosphopeptide_proteingroups_encode.intensity_matrixtibbles[["junc.exons"]][["peptides"]]
        ...
        # define tables #####
        data_cols <- l1 %>% dplyr::select(-id, -mass_channel) %>% zero_to_NA %>% transform_non.na_to_log2 %>% as_tibble
        ...
        # have to concatenate the columns of the untransformed_cols to avoid "duplicate row.names" issue
        eigen_ms_first_normalisation_result <- ProteoMM::eig_norm1(data_cols %>% as.matrix, treatment = design_factor_info, prot.info = paste(untransformed_cols$id, untransformed_cols$mass_channel, sep = "|") %>% as.matrix)
        dev.off()
        ...
        list_lvl.2_result <- future_map(.x = h.c_values_to_run, .f = function(.x) {
          ...
          # extract the normalised values and put them into a tibble, then process into the form amenable to plotting
          ## with extra annotation columns
          tibble_normalised_result_raw <- eigen_ms_second_normalisation_result$normalized %>% as_tibble
          
          ## re-split the "present" column back into the same form as the untransformed_cols to avoid "duplicate row.names" issue
          tibble_normalised_result_raw_2 <- dplyr::bind_cols(str_split_fixed(string = tibble_normalised_result_raw$present, pattern = "\\|", n = 2) %>% as_tibble %>% setNames(colnames(untransformed_cols)), tibble_normalised_result_raw %>% dplyr::select(-present))
          
          ## then we disassemble the encoding
          tibble_normalised_result <- tibble_normalised_result_raw_2 %>% disassemble_encode.grouped.df
          
          # rename columns because eigenMS screws up our names
          ...
          tibble_normalised_result_values.only <- tibble_normalised_result %>% dplyr::select(-id)
          ...
          # process the raw tibble once so we don't have to do it ever again
          raw_tibble_with_id <- dplyr::bind_cols(l1 %>% disassemble_encode.grouped.df %>% .[, "id"], l1 %>% disassemble_encode.grouped.df %>% dplyr::select(-id) %>% zero_to_NA %>% as_tibble)
          
          raw_tibble <- raw_tibble_with_id %>% dplyr::select(-id) 
          
          # new design factor info based on disassembled df
          design_factor_info <- factor(x = gsub(x = colnames(raw_tibble), pattern = "^([^\\|]+).*", replacement = "\\1", perl = TRUE), levels = c("ud_1_24", "ud_30_6", "30m", "1h", "6h", "1d"))
          
          cat("PLOT RLE\n")
          # plot raw
          pdf(paste("./RLE_plot_raw_", names[[1]], "_", names[[2]], ".pdf", sep = ""), width = 10, height = 6)
          plotRLE(raw_tibble %>% as.matrix, main = paste("raw", names[[1]], names[[2]], sep = "\n"))
          dev.off()
          # plot normalised
          ...
          par(oma = c(0, 0, 0, 5))
          plotMDS(raw_tibble %>% as.matrix, col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info))), main = paste("raw", names[[1]], names[[2]], sep = "\n"), cex = 2, pch = 1:length(levels(design_factor_info)))
          legend(par('usr')[2], par('usr')[4], xpd = NA, bty = "n", legend = levels(design_factor_info), col = brewer.pal(n = 11, name = "Spectral") %>% .[-(5:7)] %>% tail(length(levels(design_factor_info))), pch = 1:length(levels(design_factor_info)))
          dev.off()
          ################
          ...
          plot_PCA_for_timepoint_and_replicate(tibble_normalised_result_values.only %>% na.omit %>% as.matrix, 
                                               ...
                                               PCA_depths_y = c(2, 3, 4),
                                               PCA_depths_x = c(1, 2, 3),
                                               timepoint_order = levels(design_factor_info),
                                               replicate_order = vector_replicate_names,
                                               save_dir = paste("./", sep = ""),
                                               save_name = paste("eigenMS_normalised.per.encode_", names[[1]], "_", names[[2]], "_hc", .x, sep = ""))
          
          cat("do SCATTERPLOT: the intra-timepoint correlation\n")
          
          cat("raw\n")
          combs <- combn(x = colnames(raw_tibble), m = 2) %>% t
          ...
          list_of_comparisons <- purrr::map2(.x = combs2[, 1] %>% array_tree, .y = combs2[, 2] %>% array_tree, .f = function(.x, .y) {
            ...
            pairwise_subset_table <- raw_tibble_with_id[, "id"] %>% 
              add_column("comparison_1_value" = raw_tibble[, .x %>% paste] %>% unlist, 
                         "comparison_2_value" = raw_tibble[, .y %>% paste] %>% unlist, "comparison_1_name" = .x %>% paste, "comparison_2_name" = .y %>% paste)
          ...
          ggplot(tibble_pairwise_comparisons_per_replicate %>% dplyr::filter(., comparison_1_value > 0 & comparison_2_value > 0), aes(x = comparison_1_value %>% log2, y = comparison_2_value %>% log2)) +
            ...
            # scale_x_continuous(trans = "log2") +
            # scale_y_continuous(trans = "log2") +
            xlab("Comparison 1 raw intensity") + 
            ylab("Comparison 2 raw intensity") + 
            ggtitle(paste("Intra-timepoint (between replicates) correlation, raw intensity", names[[1]], names[[2]], sep = "\n")) +
          ...
          list_of_comparisons <- purrr::map2(.x = combs2[, 1] %>% array_tree, .y = combs2[, 2] %>% array_tree, .f = function(.x, .y) {
            ...
            pairwise_subset_table <- tibble_normalised_result[, "id"] %>% 
          ...
          cat("raw\n")
          list_of_values_per_timepoint_raw <- purrr::map(.x = levels(design_factor_info), .f = function(.x) {
            
            # DEBUG #####
            # .x <- design_factor_info[1]
            #############
            
            id.cols <- raw_tibble_with_id[, "id"]
            
            col.indices_data.vars <- grep(x = colnames(raw_tibble %>% as_tibble), pattern = .x)
            
            # extract the data cols for all reps of 1 timepoint and rename the colnames so that we can rbind properly
            obs.cols <- raw_tibble[, col.indices_data.vars] %>% as_tibble %>% setNames(., nm = c("intensity_IB", "intensity_IB2", "intensity_KR")) %>% add_column(., "timepoint" = .x)
          ...
          ggplot(tibble_of_values_per_timepoint, aes(x = 2^mean, y = CV)) +
            ...
            # scale_y_continuous(trans = "log2") +
            # cut the axis at the 90% percentile highest value
            coord_cartesian(xlim = c(0, 2^quantile(tibble_of_values_per_timepoint$mean, probs = c(0.92), na.rm = TRUE))) +
