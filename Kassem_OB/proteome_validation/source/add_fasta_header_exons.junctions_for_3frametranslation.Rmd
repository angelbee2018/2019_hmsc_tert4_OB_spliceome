...
fasta_header <- paste(diff.splice.tool_name, 
                      "_", 
...
extract_junction.flanking.exons_JUM <- function(spliceregion_list, tibble_gtf_table, index) {
...
# FUNCTION TO RETURN FASTA HEADER FROM MATCHED REF/RECONSTRUCTED GTF ROWS - !!!!!!!!DEPRECATED!!!!!!!!
# NOTE: to be used with purrr
...
JUM_results_dir <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_JUM/run_1_PGNEXUS_OBseries_pvalue1/results/"

# differential_junction_table_path <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/analysis_JUM/run_1_PGNEXUS_OBseries_pvalue1/results/figures/wide_table_of_all_JUM_results_36279_entries_with_na.txt"
differential_junction_table_path <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_JUM/run_1_PGNEXUS_OBseries_pvalue1/results/figures/wide_table_of_all_JUM_results_36279_entries_with_na.txt"

# 5x5_SOM_1663_junctions_PSI_OB_diff_any_qvalue0.01_any_deltaPSI_greaterthan_0.2.txt
...
# output_dir_JUM <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_junctions/"
output_dir_JUM <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_junctions/"
output_file_name_JUM <- "junction_table_OBseries_all"
...
AS_junction.IDs <- tibble("junction_ID" = wide_table_of_all_detailed_tables_diffonly$AS_structure_ID %>% gsub(x = ., pattern = "^(5_|3_|.*-)(.*)", replacement = "\\2"))
...
tibble_junction_table_array.tree <- tibble_junction_table %>% unique %>% array_tree
...
tibble_ref_gtf <- rtracklayer::import("/media/Ubuntu/sharedfolder/hg38_ensembl_reference/gtf/Homo_sapiens.GRCh38.98.gtf") %>% as_tibble %>% dplyr::mutate_if(is.factor, as.character)
...
alltimepoints_reconstructed_gtf_path <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_strawberry/results_assemblyonly/merged/alltimepoints_denovo_reconstructed_stringtiemerged.gtf"
...
list_ref_matching_junction_entries <-future_imap(.x = tibble_junction_table_array.tree, .f = ~list(
  ...
  "end" = .x$end,
  "strand" = .x$strand,
  "reference_gtf_match" = extract_junction.flanking.exons_JUM(.x, tibble_ref_gtf, .y) %>% bind_rows, 
  "reconstructed_gtf_match" = extract_junction.flanking.exons_JUM(.x, tibble_alltimepoints_reconstructed_gtf, .y) %>% bind_rows), 
  .progress = TRUE, .options = future_options(globals = c("tibble_ref_gtf", "tibble_alltimepoints_reconstructed_gtf", "extract_junction.flanking.exons_JUM", "data.table", "tibble", "bind_rows")))
...
tibble_ref_matching_junction_entries_all_translatable <- tibble_ref_matching_junction_entries_all_translatable %>% add_column("identifier" = paste("junction_", tibble_ref_matching_junction_entries_all_translatable$chr, ":", tibble_ref_matching_junction_entries_all_translatable$start, "-", tibble_ref_matching_junction_entries_all_translatable$end, ":", tibble_ref_matching_junction_entries_all_translatable$strand, sep = ""))
...
write.table(tibble_junction_table_matched_with_fasta_header, file = paste(output_dir_JUM, output_file_name_JUM, ".txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# write a test version consisting of 20 rows selected at random
write.table(tibble_junction_table_matched_with_fasta_header[sample(1:nrow(tibble_junction_table_matched_with_fasta_header), 20), ], file = paste(output_dir_JUM, output_file_name_JUM, ".txt_test", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# output the discarded entries
write.table(dplyr::anti_join(tibble_junction_table, tibble_junction_table_matched_with_fasta_header), file = paste(output_dir_JUM, output_file_name_JUM, "_discarded.entries.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
write.table(junc_bed_table, file = paste(output_dir_JUM, output_file_name_JUM, ".bed", sep = ""), sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE)
...
output_dir_psisigma <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_exons/"

if(! dir.exists(output_dir_psisigma) ) {
     dir.create(output_dir_psisigma, recursive = TRUE)}

output_file_name_PSI.Sigma <- "exon_table_OBseries_all"


table_of_exon_coords_path <- "/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/analysis_PSIsigma/results/run_2_psi.sigma.1.9_in_parallel/R_processing_results/table_of_all_psi.sigma_results.txt"

ref_gtf_path <- "/media/Ubuntu/sharedfolder/hg38_ensembl_reference/gtf/Homo_sapiens.GRCh38.98.gtf"
...
extract_junction.flanking.exons_psisigma <- function(spliceregion_list, tibble_gtf_table, index) {
...
tibble_exon_coords <- tibble_exon_coords %>% 
  
  dplyr::rename_at(.vars = c("diff_exon_chr", "diff_exon_start", "diff_exon_end", "matched_strand"), .funs = function(x) {c("chr", "start", "end", "strand") %>% return} ) %>% 
...
list_ref_matching_exon_entries_IR <- future_imap(.x = tibble_exon_coords_array.tree_IR, .f = ~list(
  ...
  "end" = .x$end,
  "strand" = .x$strand,
  "reference_gtf_match" = extract_junction.flanking.exons_psisigma(.x, tibble_ref_gtf, .y) %>% bind_rows, 
  "reconstructed_gtf_match" = extract_junction.flanking.exons_psisigma(.x, tibble_alltimepoints_reconstructed_gtf, .y) %>% bind_rows), 
  .progress = TRUE, .options = future_options(globals = c("tibble_ref_gtf", "tibble_alltimepoints_reconstructed_gtf", "extract_junction.flanking.exons_psisigma", "data.table", "tibble", "bind_rows")))
...
list_ref_matching_exon_entries_not_IR <- future_imap(.x = tibble_exon_coords_array.tree_not_IR, .f = ~list(
  ...
  "end" = .x$end,
  "strand" = .x$strand,
  "reference_gtf_match" = extract_overlapping.exons(.x, tibble_ref_gtf, .y) %>% bind_rows, 
  "reconstructed_gtf_match" = extract_overlapping.exons(.x, tibble_alltimepoints_reconstructed_gtf, .y) %>% bind_rows), 
  .progress = TRUE, .options = future_options(globals = c("tibble_ref_gtf", "tibble_alltimepoints_reconstructed_gtf", "extract_overlapping.exons", "data.table", "tibble", "bind_rows")))
...
# add identifier column
tibble_ref_matching_exon_entries_all_translatable <- tibble_ref_matching_exon_entries_all_translatable %>% add_column("identifier" = paste("exon_", tibble_ref_matching_exon_entries_all_translatable$chr, ":", tibble_ref_matching_exon_entries_all_translatable$start, "-", tibble_ref_matching_exon_entries_all_translatable$end, ":", tibble_ref_matching_exon_entries_all_translatable$strand, sep = ""))
...
# write the final fasta info table
write.table(tibble_exon_coords_matched_with_fasta_header, file = paste(output_dir_psisigma, output_file_name_PSI.Sigma, ".txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# write a test version consisting of 20 rows selected at random
write.table(tibble_exon_coords_matched_with_fasta_header[sample(1:nrow(tibble_exon_coords_matched_with_fasta_header), 20), ], file = paste(output_dir_psisigma, output_file_name_PSI.Sigma, ".txt_test", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# output the discarded entries
write.table(dplyr::anti_join(tibble_exon_coords, tibble_exon_coords_matched_with_fasta_header), file = paste(output_dir_psisigma, output_file_name_PSI.Sigma, "_discarded.entries.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
# cat(paste("track name=\"Alternative exons\" description=\"", output_file_name_PSI.Sigma, "\" graphType=junctions\n", sep = ""), file = paste(output_dir_psisigma, output_file_name, ".bed", sep = ""))
write.table(exon_bed_table, file = paste(output_dir_psisigma, output_file_name_PSI.Sigma, ".bed", sep = ""), sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE, append = FALSE)

```


# FEATURES TO BE ADDED TO THE 3FT TOOL: 

- deduplication of sub-string peptides
- output of the average number of frames, total database size statistics etc...

```{r}

junctions_3FT_info <- read.delim("/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/three_frame_translation_results/threeframetranslate_junctions_qvalue0.01_dpsi0.15_diff_3FT.summary.info.txt", row.names = NULL, header = TRUE, sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

exons_3FT_info <- read.delim("/media/Ubuntu/sharedfolder/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/three_frame_translation_results/threeframetranslate_exons_pvalue0.01_dpsi10_diff_3FT.summary.info.txt", row.names = NULL, header = TRUE, sep = "\t", stringsAsFactors = FALSE) %>% as_tibble

junctions_3FT_info_unique_peptide_info <- junctions_3FT_info %>% add_column("substring_or_not" = future_imap(.x = .$ORF_valid, .f = ~grepl(x = junctions_3FT_info$ORF_valid %>% .[-.y], pattern = .x) %>% any == TRUE, .progress = TRUE) %>% unlist)
exons_3FT_info_unique_peptide_info <- exons_3FT_info %>% add_column("substring_or_not" = future_imap(.x = .$ORF_valid, .f = ~grepl(x = exons_3FT_info$ORF_valid %>% .[-.y], pattern = .x) %>% any == TRUE, .progress = TRUE) %>% unlist)

junctions_3FT_info_unique_peptide_info_valid_and_unique <- junctions_3FT_info_unique_peptide_info %>% dplyr::filter(ORF_valid != "NONE_VALID" & substring_or_not != TRUE)
exons_3FT_info_unique_peptide_info_valid_and_unique <- exons_3FT_info_unique_peptide_info %>% dplyr::filter(ORF_valid != "NONE_VALID" & substring_or_not != TRUE)

tibble_exons_frame_tally <- exons_3FT_info_unique_peptide_info_valid_and_unique %>% dplyr::distinct(translation_frame, fasta_header) %>% dplyr::group_by(fasta_header) %>% dplyr::summarise("tally" = n())
tibble_junctions_frame_tally <- junctions_3FT_info_unique_peptide_info_valid_and_unique %>% dplyr::distinct(translation_frame, fasta_header) %>% dplyr::group_by(fasta_header) %>% dplyr::summarise("tally" = n())

cat("\naverage number of translation frames for UNIQUE exonic sequences: ", mean(tibble_exons_frame_tally$tally), "\n")
