...
tibble_ENSP_to_uniprotkb <- read.delim(file = paste(shared_dir, "table_ENSP_to_uniprot_entry_mapping_38.98.txt", sep = ""), check.names = FALSE, stringsAsFactors = FALSE, sep = "\t") %>% as_tibble
...
tibble_frequency_of_confirmed_peptides_per_splicemode_checked_against_sp <- tibble_confirmed_junc.exons_checked_against_sp %>% 
  dplyr::mutate_at(.vars = "splicemode", .funs = function(x) {gsub(x = x, pattern = "IR_overlapping_region", replacement = "IR") %>% gsub(x = ., pattern = "TSS\\|", replacement = "")} ) %>%
  dplyr::mutate("tool" = gsub(x = `Leading.razor.protein`, pattern = "^([^_]+).*", replacement = "\\1")) %>%
  dplyr::distinct(Leading.razor.protein, tool, .keep_all = TRUE) %>% 
  dplyr::group_by(splicemode, tool) %>% 
...
ggplot(tibble_frequency_of_confirmed_peptides_per_splicemode_checked_against_sp, aes(x = reorder(splicemode, -splicemode_tally), y = splicemode_tally)) +
  facet_wrap(~tool, scales = "free", ncol = 1) +
  geom_col(mapping = aes(fill = tool)) +
  geom_text(stat = "identity", nudge_y = 1, aes(label = splicemode_tally), angle = 90, hjust = 0) +
  ggtitle(paste("Tally of splicemodes for validated splice events")) +
  ...
  scale_x_discrete(labels = function(x) {str_wrap(x, width = 50)}) +
  ylab("Frequency") +
  ylim(c(0, max(tibble_frequency_of_confirmed_peptides_per_splicemode_checked_against_sp$splicemode_tally) + 50)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title.y = element_text(margin = margin(r = 10)), legend.title.align = 0.5, legend.background = element_rect(size = 0.5, linetype = "solid", colour = "black"), text = element_text(family = "Helvetica")) +
 ggsave(filename = paste(R_processing_results_dir, "tally_validated_peptides_by_splicemode_checked_against_sp.pdf", sep = ""), device = "pdf", dpi = 600, width = 8, height = 14, units = "cm") +
 ggsave(filename = paste(R_processing_results_dir, "tally_validated_peptides_by_splicemode_checked_against_sp.svg", sep = ""), device = "svg", dpi = 600, width = 8, height = 14, units = "cm") 
