...
list_imported_maxquant_tibbles_processed <- list_imported_maxquant_tibbles %>% 
  ...
  purrr::map(.f = ~modify_at(.x = .x, .at = c("peptides", "protein_groups", "phosphosites"), .f = ~dplyr::filter(.x, MS.MS.count != 0) %>% dplyr::arrange(., PEP)) )
...
tibble_junc.exons_filtered <- list_imported_maxquant_tibbles_processed$junc.exons$peptides %>%
...
# CONDUCT ENRICHMENT
tibble_hyperGO_confirmed_junc.exons <- dplyr::bind_rows(GOHyperGAll(catdb = catdb, gocat = "MF", sample = tibble_confirmed_junc.exons$gene_name %>% strsplit(split = ","), Nannot = 2) %>% GOHyperGAll_benjamini_correction,
                                                        GOHyperGAll(catdb = catdb, gocat = "BP", sample = tibble_confirmed_junc.exons$gene_name %>% strsplit(split = ","), Nannot = 2) %>% GOHyperGAll_benjamini_correction,
                                                        GOHyperGAll(catdb = catdb, gocat = "CC", sample = tibble_confirmed_junc.exons$gene_name %>% strsplit(split = ","), Nannot = 2) %>% GOHyperGAll_benjamini_correction) %>% as_tibble
