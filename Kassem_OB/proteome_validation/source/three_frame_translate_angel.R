...
# requires at least 80 GB of RAM for ~2500 junctions. 120 GB to be safe.

# arg1: path to table containing junctions/exons of interest. for example, UNION_junc_coor type files. MUST contain columns start, end, chr, strand. one row per junction.
...
reference_genome_fasta_path <- args[3]
output_dir <- args[6]
...
a <- purrr::map_depth(.x = list_three_frame_translate_result, .depth = 4, .ragged = FALSE, .f = ~.x %>% unlist %>% paste(collapse = ""))

b <- purrr::map_depth(.x = a, .depth = 4, .ragged = FALSE, .f = ~.x %>% paste %>% strsplit(., split = "\\*") %>% .[[1]] %>% .[1])

c <- b %>% set_names(UNION_junc_coor_differential$fasta_header)

d <- purrr::map_depth(.x = c, .depth = 2, .ragged = FALSE, .f = ~set_names(.x, c("three_frame_translate", "fasta_header")))

e <- purrr::map_depth(.x = d, .depth = 2, .ragged = FALSE, .f = ~.x$three_frame_translate)

f <- purrr::map_depth(.x = e, .depth = 2, .ragged = FALSE, .f = ~purrr::discard(.x, .p = ~.x %>% paste %>% nchar < 6))

g <- purrr::map_depth(.x = f, .depth = 2, .ragged = FALSE, .f = ~.x %>% unlist %>% setNames(., NULL))

h <- purrr::map(.x = g, .f = ~.x %>% unlist %>% setNames(., NULL) %>% unique) %>% compact

j <- stack(h) %>% as_tibble %>% setNames(c("peptide_sequence", "fasta_header"))

# FINALLY! WE SAVE THE TIBBLE!

write.table(x = j, file = paste(output_dir, "three_frame_translation_angel_table.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# FINALLY! WE WRITE THE FASTA!

# since our window was 100 nt, that means max. AA length of peptide = 33

write.fasta(sequences = test9$peptide_sequence %>% array_tree %>% flatten, names = test9$fasta_header, file.out = paste(output_dir, "three_frame_translation_angel.fasta", sep = ""), open = "w", nbchar = 40, as.string = TRUE)
