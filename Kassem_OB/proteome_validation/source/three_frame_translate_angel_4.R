...
1. the number of junctions processed by the 3-frame translate function 2. the number of junctions actually translated.
  ...
  h <- dplyr::left_join(g, tibble_junction_table, by = "fasta_header")
  # write a table
  write.table(x = h, file = paste(output_dir, output_database_name, "_peptide.summary.info.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
