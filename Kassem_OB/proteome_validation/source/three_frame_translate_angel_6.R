...
# DEBUG ################
# reference_genome_fasta <- seqinr::read.fasta(file = "Z:/hg38_ensembl_reference/raw_genome_fasta/genome_fasta_extract2/Homo_sapiens.GRCh38.dna.chromosome.all.fa", forceDNAtolower = FALSE)

reference_genome_fasta <- seqinr::read.fasta(file = "Z:/hg38_ensembl_reference/raw_genome_fasta/Homo_sapiens.GRCh38.dna.primary_assembly.fa", forceDNAtolower = FALSE)
...
junction_table_path <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB_fastqc/proteome_validation/results_database_generation/angel_3FT_junctions/junction_table_OB_diff_any_qvalue0.01_any_deltaPSI_greaterthan_0.2.txt_test"
tibble_junction_table <- read.delim(junction_table_path, sep = "\t", stringsAsFactors = FALSE) %>% .[.$chr == 12, ] %>% as_tibble
upstream_window_size <- 50
downstream_window_size <- 50
########################
...
upstream_window_size <- args[3] %>% as.numeric
downstream_window_size <- args[4] %>% as.numeric

total_peptide_window_size <- upstream_window_size + downstream_window_size

print(paste("total translation window size:", total_peptide_window_size))
...
for (i in 1:nrow(file_information_table)) {
  ...
  list_three_frame_translate_result <- purrr::imap(.x = list_junction_table_array.tree, .f = ~three_frame_translate_splicejunctions(.x, .y, reconstructed_gtf_table = reconstructed_gtf, mode = NULL, transcript.upstream_window = upstream_window_size, transcript.downstream_window = downstream_window_size))
...
1. the number of junctions processed by the 3-frame translate function 2. the number of junctions actually translated.
  ...
  print("use FASTA headers as the top level list names")
  ...
  print("complex filtering of virtual peptides using trim_virtual_peptides()")
  b <- purrr::map_depth(.x = a, .depth = 2, .ragged = FALSE, .f = ~purrr::modify_at(.x = .x, .at = "threeframetranslate", .f = ~purrr::map(.x = .x, .f = ~.x %>% trim_virtual_peptides)))
  # trim off resulting virtual AAs which are "none"
  print("trim off resulting virtual AAs which are \"none\"")
  ...
  print("remove transcript-level elements that do not have any more extant three frame translate results ")
  ...
  print("remove junction entries that do not have any extant transcript-level results after filtering")
  e <- purrr::keep(.x = d, .p = ~length(.x) >= 1)
  
  # COLLAPSING TIME
  # first collapse the 3FT virtual peptides into its own tibble
  print("first collapse the 3FT virtual peptides into its own tibble")
  ...
  print("collapse inside every junction")
  g <- purrr::map_depth(.x = f, .depth = 2, .ragged = FALSE, .f = ~.x %>% as_tibble)
  # ALL COLLAPSE, remove duplicated peptide sequences + header
  print("ALL COLLAPSE, remove duplicated peptide sequences + header")
  h <- g %>% flatten %>% rbindlist %>% as_tibble %>% dplyr::distinct(., virtual_peptide_sequence, fasta_header, .keep_all = TRUE)
  # tidy up the translation frame column
  print("tidy up the translation frame column")
