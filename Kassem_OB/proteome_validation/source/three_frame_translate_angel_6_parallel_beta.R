...
# arg2: path to the directory containing the genome FASTA file. Ideally from Ensembl... you need separate files by chromosomes, not the primary assembly. FORMATTING IMPORTANT!!!! MUST BE IN THE FORMAT: <_anything_>chromosome.<chr>.fa
...
#BiocManager::install(c("seqinr", "tidyverse", "purrr", "dplyr", "rtracklayer", "data.table", "furrr", "RhpcBLASctl", "gtools"))
...
reference_genome_fasta_dir <- args[2]
output_dir <- args[5]
...
vector_ref_genome_paths_by_chr <- paste(reference_genome_fasta_dir, list.files(reference_genome_fasta_dir)[list.files(reference_genome_fasta_dir) %>% grep(., pattern = "chromosome.*.fa$")], sep = "")
...
for (i in 1:nrow(file_information_table)) {
  ...
  for (chr in chr_to_run) {
    ...
    print(paste("now running chromosome", chr))
    
    # get positions of the vector where the path of the ref. genome fasta file is
    vector_ref_genome_paths_by_chr_position <- grep(x = vector_ref_genome_paths_by_chr, pattern = paste("chromosome.", chr, ".fa", sep = ""))
  ...
  # SANITY CHECK
  print("CHECK: If the junction table used in translation was prefiltered by the same GTFs, then the following two numbers MUST be equal:")
  print("Number of junctions to be translated:")
  tibble_junction_table %>% nrow %>% print
  print("Number of junctions actually translated:")
  list_three_frame_translate_result %>% length %>% print
  print("Additionally, the number of junctions which didn't have any valid sequences due to filtering criteria:")
  ((list_three_frame_translate_result %>% length) - (e %>% length)) %>% print
