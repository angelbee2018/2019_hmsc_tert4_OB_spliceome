...
Recommended system requirements: 16 cores for ~30,000 junctions. 8 cores for ~3000 junctions. 4 cores for ~1000 junctions. 
...
RAM usage: scales by the number of cores you use. 8 cores: 32GB is usually enough. I do not recommend going lower than 32GB. 16 cores: 64GB."
...
# check if input arguments seem O.K
cat("Arguments input:", commandArgs())
args = 
...
for (i in 1:nrow(file_information_table)) {
  ...
  for (chr in chr_to_run) {
    ...
    vector_ref_genome_paths_by_chr_position <- grep(x = vector_ref_genome_paths_by_chr, pattern = paste(chr, ".fa$", sep = ""))
  ...
  cat("Number of junctions to be translated:")
  tibble_junction_table %>% nrow %>% cat
  cat("Number of junctions which could not be translated:")
  list_translated_unsuccessfully <- purrr::map_depth(.x = list_three_frame_translate_result[1:20], .depth = 2, .ragged = FALSE, .f = ~any(length(.x[["threeframetranslate"]]) != 0) == TRUE) %>% purrr::map(~any(.x) == TRUE) %>% unlist
  which(list_translated_unsuccessfully == TRUE) %>% length %>% cat
...
- If you didn't filter using the same GTF, then this message may not mean there a problem. It just means that these junctions simply did not find any flanking exons.")
  ...
  cat("Additionally, the number of junctions which didn't have any valid sequences due to filtering criteria:")
  ((list_three_frame_translate_result %>% length) - (e %>% length)) %>% cat
