...
Recommended CPU/RAM connsumption: 6 cores/32GB for ~30,000 junctions. 4 cores/24GB for ~4000 junctions. 2 cores/16GB for ~1000 junctions. 
...
# arg3: how many nucleotides to translate upstream W.R.T. the transcript. 
# arg4: how many nt to translate downstream W.R.T. the transcript. 
# args 3 and 4: 
# - for junction-based type, the total nt. length to be translated will be arg3 + arg4. default for both is 50. 
# - for exon-based type, this will by default translate symmetrically past both sides of the exon such that the total window is made up to 100 nt (arg3 = 50, arg4 = 50). the total arg3 + arg4 will indicate how many nt to translate upstream/downstream starting from the centre of the exon. NOTE: for even number windows, the total window will be 1 less than specified.
...
list_input_arg_info = list(
  ...
  "9" = make_option(c("-L", "--lookaheadwindow"), type = "character", default = NULL, 
                    help = _, metavar = "character")
...
generate_all.genomic.positions_for_translation_junctions <- function(list_junction.adjacent_exon_start.end_coords_one_transcript, vector_all.genomic.positions_one_transcript, transcript.upstream_window = 50, transcript.downstream_window = 50) {
  ...
  if (abs(transcript.upstream_flanking.nt.pos - transcript.downstream_flanking.nt.pos) != 1) {
    ...
    stop("ERROR IN FUNCTION: \"generate_all.genomic.positions_for_translation_junctions\": junction-flanking nucleotide positions are not adjacent in the mature spliced transcript")
...
# END generate_all.genomic.positions_for_translation_junctions
...
# MAIN FUNCTION TO DO THREE FRAME TRANSLATION - JUNCTIONS
...
# type: can be one of the following: "junction" OR "exon"
# junction type e.g. JUM: spliceregion_table must contain columns: chr, start, end and strand of the splice junction region. one row per junction.
# exon type e.g. Whippet, PSI-Sigma: spliceregion_table must contain columns: chr, start, end and strand of the differentially included exons. one row per exon.
# loop_number: used to print out the progress via purrr::imap (prints which junction it's currently looping through)

three_frame_translate_junctions <- function(spliceregion_list, loop_number, reconstructed_gtf_table, reference_genome_fasta = reference_genome_fasta, transcript.upstream_window = 50, transcript.downstream_window = 50) {
  ...
  list_genome.coords_for_translation <- purrr::map2(.x = list_of_junction.adjacent_exon_start.end_coords_per_transcript, .y = list_all.genomic.positions_per_transcript, .f = ~generate_all.genomic.positions_for_translation_junctions(.x, .y, transcript.upstream_window = transcript.upstream_window, transcript.downstream_window = transcript.downstream_window))
...
# END three_frame_translate_junctions

# MAIN FUNCTION TO DO 3FT OF EXONS
# Behaviour: for each exon, define a larger lookup window (ideally the total read length e.g. paired end 75bp = 150bp.). Within this lookup window, test a few conditions.
# There are exons which didn't match to ref/recon transcriptome. For those, we only count them in if they have at least length 150bp. Because we cannot define a lookup window for these exons.
# The smallest protein in humans is 44 AA so the smalles valid translatable nucleotide length is 132 nt. 
# For short exons, if:
...
#   - 1+ stop codon inside exon: middle not translated, left translated only if more than 6 AA belongs to the exon. Right translated only if more than 6 AA belongs to the exon downstream of the downstream methionine.
# Exons at least 150bp long will not have to be matched. Only exons below 150bp will be matched.
# For long exons, if:
#   - left of stop codon always translated if length > 6 AA. Right translated only if followed by a methionine

three_frame_translate_exons <- function(list_exon_coord, loop_number, reconstructed_gtf_table, reference_genome_fasta = reference_genome_fasta, transcript.upstream_window = 50, transcript.downstream_window = 50, lookahead_window = 150) {
  
  cat("Now processing exon number:", loop_number, "\n")
  
  
  
}

# FUNCTION TO TRIM VIRTUAL PEPTIDES - JUNCTIONS
...
trim_virtual_peptides_junctions <- function(vector_raw_virtual_peptide) {
...
# END trim_virtual_peptides_junctions ###
...
# junction_table_path <- "Z:/PGNEXUS_kassem_MSC/Kassem_OB/proteome_validation/results_database_generation/angel_3FT_junctions/junction_table_OB_diff_any_qvalue0.01_any_deltaPSI_greaterthan_0.2.txt"
...
for (i in 1:nrow(file_information_table)) {
  ...
  for (chr in chr_to_run) {
    ...
    assign(x = paste("list_three_frame_translate_result_temp_chr_", chr, sep = ""), 
           value = future_imap(.x = list_junction_table_array.tree_temp, .f = ~three_frame_translate_junctions(.x, .y, reconstructed_gtf_table = reconstructed_gtf_temp, reference_genome_fasta = reference_genome_fasta_chr_temp, type = NULL, transcript.upstream_window = upstream_window_size, transcript.downstream_window = downstream_window_size), .progress = TRUE, .options = future_options(globals = c("reconstructed_gtf_temp", "list_junction_table_array.tree_temp", "reference_genome_fasta_chr_temp", "three_frame_translate_junctions", "generate_all.genomic.positions_for_translation", "nt.sequence_strand_threeframetranslate", "trim_virtual_peptides_junctions", "upstream_window_size", "downstream_window_size"))))
  ...
  # test <- three_frame_translate_junctions(list_junction_table_array.tree[[11]], reconstructed_gtf_table = reconstructed_gtf, type = NULL, transcript.upstream_window = 50, transcript.downstream_window = 50)
  # 
  # list_three_frame_translate_result_chr12 <- purrr::map(.x = list_junction_table_array.tree, .f = ~three_frame_translate_junctions(.x, reconstructed_gtf_table = reconstructed_gtf, type = NULL, transcript.upstream_window = 50, transcript.downstream_window = 50))
  # 
  ...
  # complex filtering of virtual peptides using trim_virtual_peptides_junctions()
  cat("complex filtering of virtual peptides using trim_virtual_peptides_junctions()\n")
  # b <- purrr::map_depth(.x = list_three_frame_translate_result, .depth = 2, .ragged = FALSE, .f = ~purrr::modify_at(.x = .x, .at = "threeframetranslate", .f = ~purrr::map(.x = .x, .f = ~.x %>% trim_virtual_peptides_junctions)))
  b <- future_map(.x = list_three_frame_translate_result, 
                  .f = ~purrr::map(.x = .x, 
                                   .f = ~purrr::modify_at(.x = .x, .at = "threeframetranslate", 
                                                          .f = ~purrr::map(.x = .x, .f = ~.x %>% trim_virtual_peptides_junctions)
                                                          )
                                   ), .progress = TRUE, .options = future_options(globals = c("list_three_frame_translate_result", "trim_virtual_peptides_junctions", "upstream_window_size", "downstream_window_size", "last", "purrr::map", "purrr::modify_at"))
                  )
...
for (chr in chr_to_run) {
  ...
  list_GTF_matching_exon_entries_IR <- future_imap(.x = tibble_exon_table_temp_IR_array.tree, .f = ~list(
    ...
    .progress = TRUE, .options = future_options(globals = c("reconstructed_gtf_temp", "extract_junction.flanking.exons", "splicemode_column_name", "data.table")))
  ...
  list_GTF_matching_exon_entries_not_IR <- future_imap(.x = tibble_exon_table_temp_not_IR_array.tree, .f = ~list(
    ...
    "strand" = .x$strand %>% trimws,
    "splicemode" = .x[[splicemode_column_name]] %>% trimws,
    "matching_GTF_entries" = extract_overlapping.exons(.x, reconstructed_gtf_temp, .y)), 
    .progress = TRUE, .options = future_options(globals = c("reconstructed_gtf_temp", "extract_overlapping.exons", "splicemode_column_name", "data.table")))
  ...
  # subset the junction table for the current chromosome and array_tree it for furrr
  tibble_exon_table_long.exons_temp <- tibble_exon_table_long.exons[tibble_exon_table_long.exons$chr == chr, ]
  
  # generate all the genomic coordinates for translation
  col.index_start <- which(colnames(tibble_exon_table_long.exons) == "start")
  col.index_end <- which(colnames(tibble_exon_table_long.exons) == "end")
  
  list_of_long.exon_coords_for_translation <- apply(X = tibble_exon_table_long.exons_temp, MARGIN = 1, FUN = function(X) {X[col.index_start]:X[col.index_end]})
  
  # convert the genomic coordinates into forward nucleotides
  list_of_long.exon_nucleotides_for_translation <- purrr::map2(.x = list_of_long.exon_coords_for_translation, .y = tibble_exon_table_long.exons_temp$strand, .f = ~list(
    "forward_nucleotides" = reference_genome_fasta_chr_temp[[chr %>% paste]] %>% .[.x],
    "strand" = .y %>% trimws))
  
  # 3FT each of the forward nucleotides according to strand info
  list_of_long.exon_nucleotides_3FT_raw <- purrr::map(.x = list_of_long.exon_nucleotides_for_translation, .f = ~nt.sequence_strand_threeframetranslate(.x))
  
  
  
  
  assign(x = paste("list_three_frame_translate_result_temp_chr_", chr, sep = ""), 
         value = future_imap(.x = list_junction_table_array.tree_temp, .f = ~three_frame_translate_junctions(.x, .y, reconstructed_gtf_table = reconstructed_gtf_temp, reference_genome_fasta = reference_genome_fasta_chr_temp, type = NULL, transcript.upstream_window = upstream_window_size, transcript.downstream_window = downstream_window_size), .progress = TRUE, .options = future_options(globals = c("reconstructed_gtf_temp", "list_junction_table_array.tree_temp", "reference_genome_fasta_chr_temp", "three_frame_translate_junctions", "generate_all.genomic.positions_for_translation_junctions", "nt.sequence_strand_threeframetranslate", "trim_virtual_peptides_junctions", "upstream_window_size", "downstream_window_size"))))
