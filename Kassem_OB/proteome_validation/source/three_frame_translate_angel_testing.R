...
three_frame_translate_splicejunctions <- function(spliceregion_list, loop_number, reconstructed_gtf_table, mode = NULL, transcript.upstream_window = 50, transcript.downstream_window = 50) {
  ...
  list_threeframetranslate <- purrr::map(.x = list_forward_nucleotides_from_coords, .f = ~nt.sequence_strand_threeframetranslate(.x) %>% purrr::map(.x = ., .f = ~.x %>% trim_virtual_peptides) %>% purrr::discard(.x = ., .p = ~.x %>% paste == "none")) %>% purrr::keep(.x = ., .p = ~length(.x) >= 1)
  ...
  tibble_threeframetranslate_fwd.nucleotides.genome.coords <- purrr::map(.x = list_threeframetranslate_fwd.nucleotides.genome.coords, .f = ~purrr::modify_at(.x = .x, .at = "threeframetranslate", .f = ~.x %>% as_tibble %>% t %>% as_tibble(rownames = "translation_frame") %>% setNames(c("translation_frame", "virtual_peptide_sequence")))) %>% purrr::map(~flatten(.x) %>% as_tibble) %>% rbindlist %>% as_tibble %>% dplyr::distinct(., virtual_peptide_sequence, fasta_header, .keep_all = TRUE)
  
  tibble_threeframetranslate_fwd.nucleotides.genome.coords[, "translation_frame"] <- gsub(x = tibble_threeframetranslate_fwd.nucleotides.genome.coords$translation_frame, pattern = "translation_frame_", replacement = "")
  
  print("...Success!!")
  
  return(tibble_threeframetranslate_fwd.nucleotides.genome.coords)
...
for (i in 1:nrow(file_information_table)) {
  ...
  write.fasta(sequences = tibble_three_frame_translate_result$virtual_peptide_sequence %>% array_tree %>% flatten, names = tibble_three_frame_translate_result$fasta_header, file.out = paste(output_dir, output_database_name, ".fasta", sep = ""), open = "w", nbchar = 40, as.string = TRUE)
  
  # ADDITIONALLY! WE WRITE A TIBBLE CONTAINING THE GENOME COORD-PEPTIDE MAPPING
  summary_info_3FT <- dplyr::left_join(tibble_three_frame_translate_result, tibble_junction_table, by = "fasta_header")
  # write a table
  write.table(x = summary_info_3FT, file = paste(output_dir, output_database_name, "_3FT.summary.info.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
