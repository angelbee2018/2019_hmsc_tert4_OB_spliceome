...
three_frame_translate_junctions <- function(spliceregion_list, loop_number, reconstructed_gtf_table, reference_genome_fasta = reference_genome_fasta, transcript.upstream_window = 50, transcript.downstream_window = 50) {
  ...
  list_threeframetranslate <- purrr::map(.x = list_forward_nucleotides_from_coords, .f = ~nt.sequence_strand_threeframetranslate(.x$forward_nucleotides, .x$strand))
...
.# FUNCTION to test if the left/right side of stop codons are translatable or not.
